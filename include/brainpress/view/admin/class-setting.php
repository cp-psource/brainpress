...
class BrainPress_View_Admin_Setting {

	private static $slug = 'brainpress_settings';
	...
	public static function init() {
		...
		self::$title = __( 'Einstellungen/BrainPress', 'brainpress' );
		self::$menu_title = __( 'Einstellungen', 'brainpress' );
		...
		add_action( 'brainpress_admin_' . self::$slug, array( __CLASS__, 'process_form' ), 1 );
	...
	public static function add_page( $pages ) {
		$pages[ self::$slug ] = array(
			...
			'cap' => apply_filters( 'brainpress_capabilities', 'brainpress_settings_cap' ),
		);
	...
	public static function render_page() {
		...
		$output .= BrainPress_Helper_UI::get_admin_page_title( self::$menu_title );
		$output .= BrainPress_Helper_Tabs::render_tabs( $tabs, $content, $hidden_args, self::$slug, $tab, false );
		$output .= '</div>';
