...
+(function($){
	...
	BrainPress.Models.Units = BrainPress.Models.Post.extend({
		url: _brainpress._ajax_url + '?action=brainpress_assessments',
	...
	BrainPress.UnitsPost = new BrainPress.Models.Units();
	...
	updateModuleGrade = function() {
		...
		var progress = BrainPress.ProgressIndicator(),
			param = {
		...
		BrainPress.UnitsPost.save( param );
		BrainPress.UnitsPost.off( 'brainpress:update_success' );
		BrainPress.UnitsPost.on( 'brainpress:update_success', function(data){
			BrainPress.Events.on( 'brainpress:progress:success', function() {
				...
				if ( is_pass ) {
					cpCheck.removeClass( 'red' ).addClass( 'green' ).html( _brainpress.assessment_labels.pass );
				} else {
					cpCheck.removeClass( 'green' ).addClass( 'red' ).html( _brainpress.assessment_labels.fail );
				}
				withFeedbackButton.html( _brainpress.assessment_labels.edit_with_feedback );
				noFeedbackButton.html( _brainpress.assessment_labels.edit_no_feedback );
				...
				if ( with_feedback && '' != param.feedback_content.trim() ) {
					...
					$( 'cite', feedback_editor ).html( '- ' + _brainpress.instructor_name );
				}
		...
		BrainPress.UnitsPost.on( 'brainpress:update_error', function(){
			progress.error( _brainpress.server_error );
		});
	},
	enableFeedbackEditor = function( editor_id, editor_container, container ) {
		...
		if ( ! has_editor ) {
			BrainPress.Events.off( 'editor:keyup' );
			BrainPress.Events.on( 'editor:keyup', function( ed ) {
			...
			BrainPress.editor.create( editor_container, editor_id, editor_id, old_content );
		}
	},
	...
	saveFeedbackAsDraft = function() {
		...
		BrainPress.UnitsPost.save( param );
		BrainPress.UnitsPost.off( 'brainpress:save_draft_feedback_success' );
		BrainPress.UnitsPost.on( 'brainpress:save_draft_feedback_success', function() {
			...
			progress.success( _brainpress.assessment_labels.sucess );
		});
		BrainPress.UnitsPost.off( 'brainpress:save_draft_feedback_error' );
		BrainPress.UnitsPost.on( 'brainpress:save_draft_feedback_error', function() {
			progress.error( _brainpress.server_error );
		});
	};
	...
	var filterStudentRows = function() {
		...
		$( '.cp-edit-grade-box' ).attr( 'data-title', _brainpress.assessment_labels.help_tooltip );
	};
	...
	var updateGradeView = function( unit_id, student_id ) {
		var unitGrade = calculateUnitGrade( unit_id, student_id ),
			...
			pass_label = $( '<span class="cp-check green">' ).html( _brainpress.assessment_labels.pass ),
			fail_label = $( '<span class="cp-check red">' ).html( _brainpress.assessment_labels.fail )
		;
	...
	var loadStudentTable = function() {
		...
		BrainPress.UnitsPost.save( data );
		BrainPress.UnitsPost.on( 'brainpress:table_success', function( data ) {
	...
	var toggleStudentUnits = function() {
		...
		if ( 0 == template_script.length ) {
			...
			BrainPress.UnitsPost.save( param );
			BrainPress.UnitsPost.off( 'brainpress:get_student_modules_success' );
			BrainPress.UnitsPost.on( 'brainpress:get_student_modules_success', function( data ) {
	...
	BrainPress.Events.on( 'brainpress:assessment_loaded', function() {
	...
	$( document )
		.ready(function() {
			...
			BrainPress.Events.trigger( 'brainpress:assessment_loaded' );
		})
