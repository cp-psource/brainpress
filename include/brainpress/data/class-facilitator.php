 ...
 * @since 2.0
 **/
class BrainPress_Data_Facilitator {
	...
	public static function add_course_facilitator( $course_id, $user_id = 0 ) {
		...
		// Set capabilities
		BrainPress_Data_Capabilities::assign_facilitator_capabilities( $user_id );

		do_action( 'brainpress_facilitator_added', $course_id, $user_id );
	...
	public static function remove_course_facilitator( $course_id, $user_id = 0 ) {
		...
		do_action( 'brainpress_facilitator_removed', $course_id, $user_id );
	}
	...
	public static function send_invitation( $course_id, $email, $first_name, $last_name ) {
		...
		$invite_data = BrainPress_Data_Instructor::create_invite_code_hash( $email_args );
		...
		$sent = BrainPress_Helper_Email::send_email(
			BrainPress_Helper_Email::FACILITATOR_INVITATION,
			$email_args
		);
		...
		if ( ! isset( $return_data['message']['exists'] ) ) {
			$return_data['message']['exists'] = __( 'Einladung existiert bereits.', 'brainpress' );
		}
	...
	public static function add_from_invitation( $course_id, $instructor_id, $invitation_code ) {
		...
		if ( ! empty( $invite_data['email'] ) && $invite_data['email'] == $userdata->user_email ) {
			 ...
			 *
			 */
			do_action( 'brainpress_facilitator_invite_confirmed', $course_id, $instructor_id );
		 ...
		 *
		 */
		do_action( 'brainpress_facilitator_invite_confirm_fail', $course_id, $instructor_id );
