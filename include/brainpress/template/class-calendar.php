...
class BrainPress_Template_Calendar {
	...
	public function create_calendar( $pre = '&laquo;', $next = '&raquo;' ) {
		...
		$calendar .= '</table>';
		$calendar .= '</div>';
		$calendar .= '<script type="text/javascript">var wpajaxurl = "' . BrainPress_Helper_Utility::get_ajax_url() . '";</script>';

		/**
		 * Add course-calendar.js script and front.css to ensure it works when use outside of brainpress pages
	...
	public function enqueue_calendar_script() {
		...
		$calendar_js = BrainPress::$url . 'asset/js/brainpress-calendar.js';
		wp_enqueue_script( 'brainpress-calendar', $calendar_js, array(
			'jquery'
		), BrainPress::$version );
