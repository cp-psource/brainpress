...
class BrainPress_Helper_Table_CourseAssessments extends BrainPress_Helper_Table_CourseStudent {
		...
		switch ( $column_name ) {
			...
			case 'grade':
				...
				if ( 'all' == $this->the_unit ) {
					$course_grade = BrainPress_Data_Student::average_course_responses( $item_id, $this->course_id );
				} else {
					$student_progress = BrainPress_Data_Student::get_completion_data( $item_id, $this->course_id );
					$course_grade = BrainPress_Helper_Utility::get_array_val( $student_progress, 'completion/' . $this->the_unit. '/average' );
				}
	...
	public function get_columns() {
		...
		$columns = array(
			'username'     => esc_html__( 'Benutzername', 'brainpress' ),
		    'display_name' => esc_html__( 'Anzeigename', 'brainpress' ),
			'last_active'  => esc_html__( 'Letzte Aktivität', 'brainpress' ),
			'grade'        => esc_html__( 'Bewertung', 'brainpress' ),
			'certificates' => esc_html__( 'Zertifiziert', 'brainpress' ),
			'modules'      => esc_html__( 'Module', 'brainpress' ),
			'view_all'     => esc_html__( 'Alle ansehen', 'brainpress' ),
		);
	...
	public function prepare_items() {
		...
		if ( ! empty( $this->search ) ) {
			$this->student_ids = BrainPress_Admin_Assessment::search_students( $this->course_id, $this->search );
			if ( ! empty( $this->student_ids ) ) {
				$this->results = BrainPress_Admin_Assessment::filter_students( $this->course_id, $this->the_unit, $this->type, $this->student_ids );
			}
		} else {
			$this->results = BrainPress_Admin_Assessment::filter_students( $this->course_id, $this->the_unit, $this->type );
		}
	...
	public function no_items() {
		esc_html_e( 'Es wurden keine Studenten gefunden.', 'brainpress' );
	}
	...
	public function extra_tablenav( $which ) {
		if ( 'top' == $which ) {
			 ...
			 * Grading system
			 */
			$grading_system = __( 'Gesamtzahl der erworbenen Noten % Gesamtzahl der benotbaren Module', 'brainpress' );
			if ( 'all' != $this->the_unit ) {
				$grading_system = __( 'insgesamt erworbene bewertbare Note % Gesamtzahl der bewertbaren Module', 'brainpress' );
			}
			/**
			 * table
			 */
			...
			echo '<tr>
        <td>' . __( 'Studenten gefunden:', 'brainpress' ) . ' ' . $this->_pagination_args['total_items'] . '</td>
        <td>' . __( 'Module:', 'brainpress' ) . ' <span class="cp-total-assessable">' . $this->results['assessable'] . '</span></td>
        <td>' . __( 'Benötigte Bewertung: ', 'brainpress' ) . ' <span class="cp-pasing-grade">' . $this->results['passing_grade'] . '%</span></td>
        <td>'. __( 'Bewertungssystem: ', 'brainpress' ) . '<em>'. $grading_system . '</em></td>
    </tr>
</table>';
		}
		if ( 'bottom' == $which ) {
