<?php
class BrainPress_Template_Discussion {
	public static function get_comments( $post_id, $paged = 0 ) {
		...
		$is_focus = 'focus' == BrainPress_Data_Course::get_setting( $course_id, 'course_view' );
		 ...
		 */
		$use_scripts = apply_filters( 'brainpress_discussion_use_scripts', true );
	...
	public static function show_all_comments( $comments_array ) {
		 ...
		 */
		$comments_array->query_vars['number'] = apply_filters( 'brainpress_show_all_comments', null );
	...
	public static function is_discussion_subscriber() {
		...
		$post_id = get_the_ID();
		return BrainPress_Data_Discussion::is_discussion_subscriber( $user_id, $post_id );
	}

	public static function add_subscribe_button( $submit_button ) {
		global $post;
		$user_subscribe = BrainPress_Helper_Discussion::get_subscription_status( $post->ID );
		$options = BrainPress_Helper_Discussion::get_subscription_statuses_array();
		$subscribe = sprintf(
			'<span class="comment-notification"><select name="%s">',
			esc_attr( BrainPress_Helper_Discussion::get_field_name() )
		);
	...
	public static function discussion_url( $post_id ) {
		...
		if ( BrainPress_Data_Discussion::is_comment_in_discussion( $post_id ) ) {
			$post_type = get_post_type( $post_id );

			if ( BrainPress_Data_Discussion::get_post_type_name() == $post_type ) {
				$unit_id = get_post_field( 'post_parent', $post_id );
				$course_id = get_post_field( 'post_parent', $unit_id );

				$url = BrainPress_Core::get_slug( 'courses/', true ) . get_post_field( 'post_name', $course_id );
				$url .= '/' . BrainPress_Core::get_slug( 'unit/' ) . get_post_field( 'post_name', $unit_id );
				$url .= '#module-' . $post_id;
				return $url;
			}
		}
	...
	public static function comment_url( $comment_id, $comment_post_id ) {
		return BrainPress_Helper_Discussion::get_comment_url( $comment_id, $comment_post_id );
	}
	...
	public static function get_comments_nav( $post_id, $course_id, $per_page = 5 ) {
		...
		$navs = apply_filters( 'brainpress_discussion_navs', $navs );
