...
class BrainPress_Module {
	...
	public static function is_valid() {
		return ( ! empty( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'brainpress_submit_modules' ) );
	}
	...
	public static function validate_course( $input ) {
		...
		if ( empty( $input['course_id'] ) ) {
			$has_error = true;
			self::$error_message = __( 'Ungültige Kurs-ID!', 'brainpress' );
		} elseif ( false === BrainPress_Data_Course::student_enrolled( $input['student_id'], $input['course_id'] ) ) {
			$has_error = true;
			self::$error_message = __( 'Du bist derzeit nicht für diesen Kurs angemeldet!', 'brainpress' );
		} elseif ( 'closed' == ( $course_status = BrainPress_Data_Course::get_course_status( $input['course_id'] ) ) ) {
			$has_error = true;
			self::$error_message = __( 'Dieser Kurs ist abgeschlossen, Du kannst keine Antworten mehr einreichen.', 'brainpress' );
		} elseif ( empty( $input['unit_id'] ) ) {
			$has_error = true;
			self::$error_message = __( 'Ungültige Einheit!', 'brainpress' );
		}
	...
	public static function validate_module( $module_id, $response = '', $student_id = 0 ) {
		...
		if ( true === $mandatory ) {
			if ( '' == $response ) {
				$has_error = true;
				self::$error_message = $is_focus ? __( 'Du mussst dieses Modul ausfüllen!', 'brainpress' ) : __( 'Du musst die erforderlichen Module vervollständigen!', 'brainpress' );
			} else {
				...
				if ( true === $is_assessable && ! in_array( $module_type, $excluded_modules ) ) {
					...
					if ( false === $pass ) {
						$has_error = true;
						self::$error_message = __( 'Du hast die erforderliche Mindestnote nicht bestanden!', 'brainpress' );
					}
				}
			}
		}
	...
	public static function add_comment( $comments, $student_id ) {
		...
		$is_module = BrainPress_Data_Module::is_module( $module_id );
		if ( ! $is_module ) {
			return new WP_Error( 'error', __( 'Modul existiert nicht.', 'brainpress' ) );
		}
		 ...
		 * @since 2.0
		 **/
		do_action( 'brainpress_before_add_comment', $module_id, $course_id );
		 ...
		 **/
		do_action( 'brainpress_after_add_comment', $comment_id, $student_id, $module_id, $course_id );
	...
	public static function add_single_comment( $input ) {
		...
		if ( 0 < $comment_id ) {
			...
			if ( empty( $value ) ) {
				$value = BrainPress_Helper_Discussion::get_default_key();
			}
			BrainPress_Data_Discussion::update_user_subscription( $student_id, $module_id, $value );

			// Add comment filters
			add_filter( 'comment_reply_link', array( 'BrainPress_Template_Module', 'comment_reply_link' ), 10, 4 );

			$json_data = array(
				'success' => true,
				'html' => BrainPress_Template_Discussion::get_single_comment( $comment_id ),
				'comment_parent' => $comments['comment_parent'],
				'comment_id' => $comment_id,
			);

			// Remove comment filters, etc
			remove_filter( 'comment_reply_link', array( 'BrainPress_Template_Module', 'comment_reply_link' ), 10, 4 );
	...
	public static function submit( $input ) {
		...
		// Validate the course
		$error = BrainPress_Data_Course::can_access( $course_id, $unit_id );
		...
		if ( ! empty( $error ) ) {
				...
				foreach ( $module_ids as $module_id ) {
					...
					if ( 'input-upload' == $module_type ) {
						if ( false == $can_update_course && empty( $_FILES ) ) {
							if ( $validate ) {
								self::$error_message = __( 'Du musst das erforderliche Modul ausfüllen!', 'brainpress' );
								$has_error = true;
							}
						}
					...
					if ( 'discussion' == $module_type ) {
						if ( empty( $input['comment'] ) ) {
							if ( $is_mandatory && false == $can_update_course ) {
								...
								if ( BrainPress_Data_Discussion::have_comments( $student_id, $module_id ) ) {
									continue;
								} else {
									$has_error = true;
									self::$error_message = __( 'Deine Teilnahme an der Diskussion ist erforderlich!', 'brainpress' );
									continue;
								}
							}
						} else {
							...
							$value = isset( $input[ $field_name ] ) ? $input[ $field_name ] : BrainPress_Helper_Discussion::get_default_key();
							BrainPress_Data_Discussion::update_user_subscription( $student_id, $module_id, $value );
					...
					if ( $is_answerable ) {
						if ( ! isset( $module[ $module_id ] ) || '' === ( $module[ $module_id ] ) ) {
							...
							// logic from BrainPress_Data_Module::get_form_results() is that Form will have a grade of 100 if not required, otherwise check if empty for all submodules
							...
							if ( true === $is_assessable && ! in_array( $module_type, $excluded_modules ) ) {
								...
								if ( false === $pass && false == $can_update_course && $validate ) {
									...
									self::$error_message = ( $module_type == 'input-form' ) ?
										__( 'Du hast das Formular nicht ausgefüllt!', 'brainpress' )
										: __( 'Du hast die erforderliche Mindestnote nicht bestanden!', 'brainpress' );
								}
							}
						}
					}
			...
			if ( ! empty( $_FILES['module'] ) ) {
				...
				foreach ( $files['name'] as $_module_id => $filename ) {
					...
					$response = BrainPress_Data_Student::get_response( $student_id, $course_id, $unit_id, $_module_id );
					...
					if ( ! empty( $response['error'] ) ) {
						...
						BrainPress_Data_Student::module_response( $student_id, $course_id, $unit_id, $_module_id, $response );
					}
				}
			}
		}
		...
		if ( $has_error ) {
			if ( $via_ajax ) {
				if ( $is_focus ) {
					$html = '[brainpress_focus_item course="%s" unit="%s" type="%s" item_id="%s"]';
					$html = do_shortcode( sprintf( $html, $course_id, $unit_id, 'module', $module_id ) );
				} else {
					$html = BrainPress_Template_Unit::unit_with_modules( $course_id, $unit_id, $page, $student_id );
				}
				...
				add_action( 'brainpress_before_unit_modules', array( __CLASS__, 'show_error_message' ) );
			}
		} else {
			...
			//BrainPress_Data_Student::get_calculated_completion_data( $student_id, $course_id );
			$next = BrainPress_Data_Course::get_next_accessible_module( $course_id, $unit_id, $page, $module_id );
			...
			if ( $via_ajax ) {
				...
				if ( $is_focus ) {
					...
					if ( 'completion_page' != $next['id'] ) {
						$html = BrainPress_Template_Unit::unit_with_modules( $course_id, $unit_id, $item_id, $student_id );
					}
				}
				...
				if ( isset( $input['save_progress_and_exit'] ) ) {
					...
					$next['url'] = BrainPress_Data_Course::get_course_url( $course_id );
				}
				...
				$json_data = apply_filters( 'brainpress_module_submit_atts',  array( 'success' => true, 'data' => $json_data ) );
