...
class BrainPress_Data_Discussion {
	...
	public static function get_format() {
		...
		return array(
			...
			'post_args' => array(
				'labels' => array(
					'name' => __( 'Diskussionen', 'brainpress' ),
					'singular_name' => __( 'Diskussion', 'brainpress' ),
					'add_new' => __( 'Neue Diskussion', 'brainpress' ),
					'add_new_item' => __( 'Neues Thema erstellen', 'brainpress' ),
					'edit_item' => __( 'Thema bearbeiten', 'brainpress' ),
					'edit' => __( 'Bearbeiten', 'brainpress' ),
					'new_item' => __( 'Neues Thema', 'brainpress' ),
					'view_item' => __( 'Thema anzeigen', 'brainpress' ),
					'search_items' => __( 'Thema suchen', 'brainpress' ),
					'not_found' => __( 'Keine Themen gefunden', 'brainpress' ),
					'not_found_in_trash' => __( 'Keine Themen im Papierkorb gefunden', 'brainpress' ),
					'view' => __( 'Thema anzeigen', 'brainpress' ),
				),
				'public' => false,
				'show_ui' => false,
				...
				'capabilities' => array(
					'read' => 'read',
					'edit_published_posts' => 'brainpress_update_discussion_cap',
					'edit_posts' => 'brainpress_update_discussion_cap',
				),
				'map_meta_cap' => true,
				'query_var' => true,
				// 'rewrite' => array(
				// 'slug' => trailingslashit( BrainPress_Core::get_slug( 'course' ) ) . '%course%/' . BrainPress_Core::get_slug( 'discussion' )
				// )
			),
		);
	...
	public static function attributes( $n_id ) {
		...
		$unit_title = ! empty( $unit_id ) ? get_the_title( $unit_id ) : __( 'Alle Einheiten', 'brainpress' );
	...
	// Hook from BrainPress_View_Front
	...
	public static function update_discussion( $discussion_title = '', $discussion_description = '', $course_id = '', $unit_id = '' ) {
		...
		$post = array(
			...
			'post_content' => BrainPress_Helper_Utility::filter_content( ! $discussion_description ? $_POST['discussion_description'] : $discussion_description ),
			'post_status'  => $post_status,
			'post_title'   => BrainPress_Helper_Utility::filter_content( ( ! $discussion_title ? $_POST['discussion_name'] : $discussion_title ), true ),
		...
		if ( $post_id ) {
			...
			if ( ! isset( $_POST['discussion_id'] ) ) {//new discussion added
				$instructors = BrainPress_Data_Course::get_setting( $course_id, 'instructors', false );
				do_action( 'new_discussion_added_instructor_notification', $user_id, $course_id, $instructors );

				$students = BrainPress_Data_Course::get_student_ids( $course_id );
			...
			foreach ( $_POST as $key => $value ) {
				if ( preg_match( '/meta_/i', $key ) ) {//every field name with prefix "meta_" will be saved as post meta automatically
					update_post_meta( $post_id, str_replace( 'meta_', '', $key ), BrainPress_Helper_Utility::filter_content( $value ) );
				}
			}
		}
	...
	public static function before_add_comment( $comment_post_ID, $course_id ) {
		...
		$post_format = BrainPress_Data_Module::get_format();
	...
	public static function after_add_comment( $comment_id, $student_id, $comment_post_ID, $course_id ) {
		...
		if ( ! isset( $student_progress['units'] ) && ! isset( $student_progress['units'][ $comment_id ] ) ) {
			$student_progress = BrainPress_Helper_Utility::set_array_value( $student_progress, 'units/' . $comment_id, array() );
			BrainPress_Data_Student::update_completion_data( $student_id, $course_id, $student_progress );
		}

		/** notify users */
		BrainPress_Data_Discussion_Cron::add_comment_id( $comment_id );
	...
	public static function init() {
		...
		add_action( 'brainpress_before_add_comment', array( __CLASS__, 'before_add_comment' ), 10, 2 );
		// Trigger hooks after adding comment
		add_action( 'brainpress_after_add_comment', array( __CLASS__, 'after_add_comment' ), 10, 4 );
		...
		add_filter( 'brainpress_no_access_redirect_url', array( __CLASS__, 'is_unsubscribe_link' ), 10, 2 );
		 ...
		 * add capabilities
		 */
		add_filter( 'user_has_cap', array( 'BrainPress_Data_Discussion', 'fix_comments_capabilites' ), 10, 4 );
	}
	...
	public static function approved_discussion_comment( $is_approved, $commentdata ) {
		if ( self::is_comment_in_discussion( $commentdata['comment_post_ID'] ) ) {
			 ...
			 **/
			$is_approved = apply_filters( 'brainpress_discussion_comment_status', 1, $commentdata['comment_post_ID'], $commentdata );
		}
	...
	public static function redirect_back( $location, $comment ) {
		...
		if ( self::is_comment_in_discussion( $post_id ) ) {
			$location = BrainPress_Template_Discussion::discussion_url( $post_id );
		}
		return $location;
	}

	public static function is_discussion_subscriber( $user_id, $discussion_id ) {
		$key = BrainPress_Helper_Discussion::get_user_meta_name( $discussion_id );
		$value = get_user_meta( $user_id, $key, true );
		$value = BrainPress_Helper_Discussion::sanitize_cp_subscribe_to_key( $value );
		...
		if ( isset( $_GET['unsubscribe'] ) && isset( $_GET['uid'] ) ) {
			...
			if ( self::is_subscriber( $user_id, $post_id ) ) {
				...
				if ( in_array( $post_type, $discussion_types ) ) {
					...
					$message = sprintf( '<h3 class="cp-unsubscribe-title">%s</h3>', __( 'Erfolgreich Deabonniert', 'brainpress' ) );
					$message .= '<p>' . sprintf( __( 'Du wurdest aus der Diskussion "%s" entfernt.', 'brainpress' ), get_the_title( $post_id ) ) . '</p>';
					 ...
					 * @param (int) $user_id
					 **/
					$message = apply_filters( 'brainpress_unsubscribe_message', $message, $post_id, $user_id );
	...
	public static function update_user_subscription( $user_id, $discussion_id, $new_value = false ) {
		if ( empty( $new_value ) ) {
			$new_value = BrainPress_Helper_Discussion::get_value_from_post();
		}
		$user_meta_key = BrainPress_Helper_Discussion::get_user_meta_name( $discussion_id );
	...
	public static function comment_add_new( $data, $json_data ) {
		...
		$check_nonce = BrainPress_Helper_Discussion::check_nonce_add( $data->nonce );
		 ...
		 */
		$field_name = BrainPress_Helper_Discussion::get_field_name();
		$value = isset( $data->$field_name ) ? $data->$field_name : BrainPress_Helper_Discussion::get_default_key();
		self::update_user_subscription( $user_id, $data->comment_post_ID, $value );
		/**
		 * set course_id
		 */
		$course_id = BrainPress_Data_Module::get_course_id_by_module( $data->comment_post_ID );
		BrainPress_Data_Course::set_last_course_id( $course_id );
		 ...
		 */
		$single_comment_output = apply_filters( 'brainpress_discussion_single_comment', false, $comment_id, $data );
		...
		if ( ! empty( $single_comment_output ) ) {
			...
			$json_data['data']['html'] = BrainPress_Template_Discussion::get_comments( $data->comment_post_ID );
		}

		// Update course progress
		$student_data = BrainPress_Data_Student::get_completion_data( $user_id, $course_id );
		if ( ! isset( $student_data['units'] ) && ! isset( $student_data['units'][ $data->comment_post_ID ] ) ) {
			$student_data = BrainPress_Helper_Utility::set_array_value( $student_data, 'units/' . $data->comment_post_ID, array() );
			BrainPress_Data_Student::update_completion_data( $user_id, $course_id, $student_data );
		}
		BrainPress_Data_Student::get_calculated_completion_data( $user_id, $course_id );

		$module = get_post( $data->comment_post_ID );
		$unit_id = $module->post_parent;
		$page = BrainPress_Data_Shortcode_Template::get_module_page( $course_id, $unit_id, $module->ID );
		...
		$labels = array(
			'pre_text' => __( '&laquo; Bisherige', 'brainpress' ),
			'next_text' => __( 'Weitere &raquo;', 'brainpress' ),
		...
		$json_data['data']['next_nav'] = BrainPress_Data_Shortcode_Template::show_nav_button(
			$next,
			$text,
			$next_module_class,
			$title
	...
	public static function ajax_update() {
		...
		if ( empty( $data->action ) ) {
			$json_data['message'] = __( 'Diskussions-Update: Keine Aktion.', 'brainpress' );
			...
			case 'toggle':
				...
				if ( wp_verify_nonce( $data->data->nonce, $nounce_name ) ) {
					if ( self::is_correct_post_type( $discussion_id ) ) {
						...
						$json_data['message'] = __( 'Diskussionsaktualisierung fehlgeschlagen: Post-Type-Mismatch.', 'brainpress' );
					}
				} else {
					$json_data['message'] = __( 'Diskussionsaktualisierung fehlgeschlagen: falsche Ankündigung.', 'brainpress' );
				}
				break;
				...
				if ( wp_verify_nonce( $data->data->nonce, 'bulk_action_nonce' ) ) {
					...
					foreach ( $ids as $id ) {
						...
						if ( 'bulk_delete' === $action ) {
							if ( BrainPress_Data_Capabilities::can_delete_discussion( $id ) ) {
								wp_delete_post( $id );
							}
						}
					}
	...
	public static function comments_template_query_args( $args ) {
		...
		$add_new = BrainPress_Core::get_setting( 'slugs/discussions_new', 'add_new_discussion' );
	...
	public static function get_url( $discussion ) {
		...
		$url = BrainPress_Core::get_slug( 'courses/', true ) . $course->post_name . '/';
		$url = $url . BrainPress_Core::get_slug( 'discussion/' ) . $discussion->post_name;
		return $url;
	}
