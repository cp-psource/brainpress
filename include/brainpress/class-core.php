...
/**
 * Plugin initialization for the BrainPress core plugin.
 */
class BrainPress_Core {
	public static $is_cp_page = false;

	/**
	 * Initialize BrainPress Core.
	...
	public static function init() {
		 ...
		 */
		BrainPress_Data_PostFormat::init();
		add_filter( 'brainpress_post_formats', array( __CLASS__, 'register_formats' ) );
		...
		BrainPress_Hooks::init();

		// Initialize Plugin Integrations.
		BrainPress_Helper_Integration::init();

		// Initialize unsubscriber
		BrainPress_Data_Unsubscribe::init();

		// Initialize TemplateTag Object Helper.
		BrainPress_Helper_TemplateTag::init();

		// Initialize Legacy Object Helper.
		BrainPress_Helper_Legacy::init();

		// Initialize Email alerts
		BrainPress_Helper_EmailAlert::init();
		BrainPress_Data_Discussion_Cron::init();

		// Init shortcodes.
		BrainPress_Data_Shortcode::init();

		// Init WooCommerce
		BrainPress_Helper_Integration_WooCommerce::init();

		if ( is_admin() ) {
			// Initialize Admin Settings.
			BrainPress_Helper_Setting::init();

			// Initialize Admin Views.
			//BrainPress_View_Admin_BrainPress::init();
			//BrainPress_View_Admin_Instructor::init();
			//BrainPress_View_Admin_Student::init();

			//BrainPress_View_Admin_Course_Export::init();
			BrainPress_Helper_PDF::init();

			new BrainPress_Admin_Students;
			new BrainPress_Admin_Instructors;
			new BrainPress_Admin_Assessment;
			new BrainPress_Admin_Reports;
			new BrainPress_Admin_Notifications;
			new BrainPress_Admin_Forums;
			new BrainPress_Admin_Comments;
			//new BrainPress_Admin_Certificate;
			new BrainPress_Admin_Import;
			new BrainPress_Admin_Export;
			new BrainPress_Admin_Settings;
			//	BrainPress_View_Admin_Setting::init();
			new BrainPress_Admin_GDPR();
		} else {
			// Now we're in the front.
			BrainPress_View_Front_General::init();
			BrainPress_View_Front_Instructor::init();
			BrainPress_View_Front_Facilitator::init();
			BrainPress_View_Front_Dashboard::init();
			BrainPress_View_Front_Settings::init();
			BrainPress_View_Front_Student::init();
			BrainPress_View_Front_Login::init();
			BrainPress_View_Front_Signup::init();
			/**
			 * add schema.org microdata
			 */
			BrainPress_Helper_Schema::init();
			/**
			 * BrainPress messages
			 */
			BrainPress_Helper_Message::init();
		}

		// Always initialize the Front-End; needed in is_admin() for ajax calls!
		BrainPress_View_Front_Course::init();

		// Initialize Utility actions.
		BrainPress_Helper_Utility::init();

		// Init Unit/Module hooks.
		BrainPress_Data_Unit::init_hooks();
		BrainPress_Data_Module::module_init_hooks();

		// Initialize Calendar actions
		BrainPress_View_Front_Calendar::init();

		// Init categories widget
		BrainPress_Widget_Categories::init();

		// Init Course Structure widget
		BrainPress_Widget_Structure::init();

		// Init Course Calendar widget
		BrainPress_Widget_Calendar::init();

		// Init Latest Course widget
		BrainPress_Widget_LatestCourse::init();

		// Init Featured Course widget
		BrainPress_Widget_FeaturedCourse::init();
		 ...
		 */
		do_action( 'brainpress_initialized' );
	}
	...
	/**
	 * Return a single BrainPress setting.
	 *
	 * @since  2.0.0
	 * @param  string $key Setting key.
	 * @param  mixed  $default Optional. Default value.
	...
	public static function get_setting( $key, $default = null, $network = false ) {
		if ( ! $network ) {
			$cp_settings = get_option( 'brainpress_settings' );
		} else {
			$cp_settings = get_site_option( 'brainpress_settings' );
		}
		...
		if ( empty( $key ) ) {
			...
			$setting = BrainPress_Helper_Utility::get_array_val(
				$cp_settings,
				$key
			);
	...
	public static function merge_settings( $settings_old, $settings_new ) {
		...
		return BrainPress_Helper_Utility::merge_distinct( $settings_old, $settings_new );
	}
	...
	public static function update_setting( $key, $value, $network = false ) {
		...
		if ( false === $network ) {
			...
			$settings = get_site_option( 'brainpress_settings' );
		}
		...
		if ( false === $network ) {
			return update_option( 'brainpress_settings', $settings );
		} else {
			return update_site_option( 'brainpress_settings', $settings );
		}
	}

	public static function get_slug_array() {
		return apply_filters(
			'brainpress_slug_array',
			array(
				'course' => array(
					'default' => 'kurse',
					'option' => 'slugs/course',
				...
				'inbox' => array(
					'default' => 'student-postfach',
					'option' => 'slugs/inbox',
				),
				'messages_sent' => array(
					'default' => 'student-gesendete-nachrichten',
					'option' => 'slugs/sent_messages',
				),
				'messages_new' => array(
					'default' => 'student-neue-nachricht',
					'option' => 'slugs/new_messages',
				),
				'completion_page' => array(
					'default' => 'kursabschluss',
	...
	/**
	 * Returns the slug or URL to the specified BrainPress page.
	 *
	 * Examples:
	 *   BrainPress_Core::get_slug( 'course/', true );
	 *   -> http://example.com/courses/
	 *
	 *   BrainPress_Core::get_slug( 'course', true );
	 *   -> http://example.com/courses/
	 *
	 *   BrainPress_Core::get_slug( 'course/' );
	 *   -> courses/
	 *
	 *   BrainPress_Core::get_slug( 'course' );
	 *   -> courses
	...
	public static function get_slug( $context, $full_url = false ) {
		...
		*/
		...
		if ( ! $full_url ) {
			$return_value = BrainPress_Core::get_setting(
				$option_key,
				$default
			);
		} else {
			...
			if ( $page_option ) {
				$page_id = BrainPress_Core::get_setting( $page_option, 0 );
			}

			if ( $page_id ) {
				$return_value = get_permalink( (int) $page_id );
			} else {
				$path = BrainPress_Core::get_setting(
					$option_key,
					$default
				);
	...
	/**
	 * Returns a list of classes that register BrainPress specific post-types.
	 *
	 ...
	 * custom taxonomy:
	 *   BrainPress_Data_X::get_format()   // Details about post-type.
	 *   BrainPress_Data_X::get_taxonomy() // Details about taxonomy.
	 *
	 * @since  2.0.0
	...
	public static function register_formats( $classes ) {
		return array_merge(
			$classes,
			array(
				'BrainPress_Data_Course',
				'BrainPress_Data_Unit',
				'BrainPress_Data_Module',
				'BrainPress_Data_Discussion',
				'BrainPress_Data_Notification',
				'BrainPress_Data_Certificate',
			)
		);
	}
	...
	public static function add_rewrite_rules( $rules ) {
		...
		// Special Rules for BrainPress Focus mode
		$new_rules['^brainpress_focus/([^/]*)/([^/]*)/([^/]*)/([^/]*)/?$'] = 'index.php?brainpress_focus=1&course=$matches[1]&unit=$matches[2]&type=$matches[3]&item=$matches[4]'; // Matches item
		$new_rules['^brainpress_focus/([^/]*)/([^/]*)/([^/]*)/?$'] = 'index.php?brainpress_focus=1&course=$matches[1]&unit=$matches[2]&type=$matches[3]'; // Matches type
		$new_rules['^brainpress_focus/([^/]*)/([^/]*)/?$'] = 'index.php?brainpress_focus=1&course=$matches[1]&unit=$matches[2]'; // Matches unit
		$new_rules['^brainpress_focus/([^/]*)/?$'] = 'index.php?brainpress_focus=1&course=$matches[1]'; // Matches course
		$new_rules['^brainpress_focus/.*?$'] = 'index.php?brainpress_focus=1';  // Not useful practically
		...
		foreach ( $pages as $page ) {
			$page_id = intval( BrainPress_Core::get_setting( 'pages/'.$page, 0 ) );
			if ( 0 === $page_id ) {
	 ...
	 *
	 * Redirect to Guide page after activate BrainPress plugin, only once and
	 * only when we do not have courses in database.
	 *
	 * @since 2.0.0
	 */
	public static function redirect_to_guide_page() {
		$maybe_redirect = get_option( 'brainpress_maybe_redirect', false );

		if ( false === $maybe_redirect ) { return; }

		// Remove redirect marker
		delete_option( 'brainpress_maybe_redirect' );
