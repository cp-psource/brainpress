 ...
 */
class BrainPress_Template_Unit {
	/**
	 * Render the complete contents of a single course unit.
	 *
	 * This function is called by:
	 * BrainPress_View_Front_Course::render_course_unit()
	 *
	...
	public static function unit_with_modules( $course_id = 0, $unit_id = 0, $page = 0, $student_id = 0 ) {
		...
		if ( empty( $unit_id ) ) {
			$unit = BrainPress_Helper_Utility::the_post();
			if ( is_object( $unit ) ) {
				$unit_id = $unit->ID;
			}
		} else {
		...
		if ( empty( $page ) ) {
			$page = (int) BrainPress_Helper_Utility::the_pagination();
		}
		if ( empty( $student_id ) ) {
		...
		if ( ! $enrolled && ! $can_preview_page && ! $is_instructor ) {
			return __( 'Es tut uns leid. Du darfst diesen Teil des Kurses nicht ansehen.', 'brainpress' );
		}
		...
		if ( 'focus' == $view_mode ) {
			...
			$format = '<div class="brainpress-focus-view">[brainpress_focus_item course="%s" unit="%s" type="%s" item_id="%s"]</div>';
		 ...
		 * @since 2.0
		 **/
		$show_submenu = apply_filters( 'brainpress_show_submenu', true );
		...
		$availability = BrainPress_Data_Unit::get_unit_availability_status( $course_id, $unit_id, $previous_unit_id );

		if ( false === $can_update_course && empty( $availability['available'] ) ) {
			$message = BrainPress_Data_Shortcode_Unit::module_status(
				array(
					'course_id' => $course_id,
					'unit_id' => $unit_id,
			...
			if ( $previous_unit_id ) {
				$unit_url = BrainPress_Data_Unit::get_unit_url( $previous_unit_id );
				$format = '<span class="next-button unit unit-%s"><a href="%s"><button>%s</button></a></span> ';
				$content .= sprintf( $format, $previous_unit_id, esc_url_raw( $unit_url ), __( 'Vorherige Einheit', 'brainpress' ) );
			}
		...
		$modules = BrainPress_Data_Course::get_unit_modules(
			$unit->ID,
			array( 'publish' ),
			false,
			false,
		 ...
		 **/
		do_action( 'brainpress_normal_items_loaded', $course_id, $unit->ID, $page );

		$before_html = apply_filters( 'brainpress_before_unit_modules', '' );
		...
		if ( $show_page_title && isset( $page_titles[ 'page_' . $page ] ) ) {
			$page_title = BrainPress_Helper_Utility::filter_content(
		...
		// Modules.
		$module_template = wp_nonce_field( 'brainpress_submit_modules', '_wpnonce', true, false );
		...
		if ( $error && ! ( $is_instructor || $can_update_course ) ) {
			...
			foreach ( $modules as $module ) {
				...
				if ( $enrolled || $is_instructor || $can_update_course || 'output' == $attributes['mode'] ) {
					$module_template .= BrainPress_Template_Module::template( $module->ID );
		...
		$url_path = BrainPress_Data_Unit::get_unit_url( $unit->ID );
		$url_path .= trailingslashit( 'page' );
		$has_submit_button = false;
		$next_text = __( 'Weiter &raquo;', 'brainpress' );
		$prev_text = __( '&laquo; Zurück', 'brainpress' );
		$previous_page = false;
		...
		for ( $i = $unit_index; $i < count( $units ); $i++ ) {
			...
			if ( $is_instructor || $enrolled || $can_update_course ) {
				...
				$unit_available = BrainPress_Data_Unit::is_unit_available(
					$course_id,
					$units[ $i ],
					$units[ $unit_index ]
				);
		...
		if ( ! empty( $next_unit ) && empty( $next_page ) ) {
			$unit_url = BrainPress_Data_Unit::get_unit_url( $next_unit );
		...
		if ( 'normal' == $view_mode && $enrolled && $has_submit_button && ! $error ) {
			$save_progress_link = sprintf(
				'<div class="button" name="save-progress-and-exit-container"><a href="#" class="save-progress-and-exit">%s</a></div>',
				__( 'Fortschritt speichern und beenden', 'brainpress' )
			);
		}
	...
	public static function previous_pages_required_modules_incomplete( $unit_id, $student_id, $page ) {
		...
		while ( 1 !== $page ) {
			$previous_page    = $page - 1;
			$previous_modules = BrainPress_Data_Course::get_unit_modules(
				$unit_id,
				array( 'publish' ),
				false,
				false,
			...
			$previous_modules = array_map( array( 'BrainPress_Data_Course', 'get_course_id' ), $previous_modules );
	 ...
	 *
	 * This function is called by:
	 * BrainPress_View_Front_Course::render_course_unit_archive()
	 *
	 * It renders the contents of a VirtualPage.
	 *
	 * @since  2.0.0
