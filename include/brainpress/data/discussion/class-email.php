 ...
 * @since 2.0.0
 */
class BrainPress_Data_Discussion_Email {
	...
	public static function notify_all_only_reactions_subscribers( $post, $message, $current_user, $comment_id ) {
		...
		$sql = $wpdb->prepare(
			...
			AND m.meta_value = %s',
			$current_user,
			BrainPress_Helper_Discussion::get_user_meta_name( $post_id ),
			'subscribe-reactions'
		);
	...
	private static function _send( $post, $message, $current_user, $comment_id, $receipients ) {
		...
		$subject = __( 'Neuer Kommentar: ', 'brainpress' );
		 ...
		 **/
		$subject = apply_filters( 'brainpress_discussion_subject', $subject, $post->ID );
		...
		$args = array(
			...
			'comment' => $message,
			'discussion_link' => BrainPress_Helper_Discussion::get_comment_url( $comment_id, $post->ID ),
		);
		...
		if ( ! empty( $receipients ) ) {
			foreach ( $receipients as $user_id => $user_email ) {
				...
				$args['unsubscribe_link'] = add_query_arg(
					...
					BrainPress_Template_Discussion::discussion_url( $post->ID )
				);
				BrainPress_Helper_Email::send_email(
					BrainPress_Helper_Email::DISCUSSION_NOTIFICATION,
					$args
				);
			}
		}
	...
	public static function notify_all_full_subscribers( $post, $message, $current_user, $comment_id ) {
		$post_id = self::_get_post_id( $post );
		$meta_key = BrainPress_Helper_Discussion::get_user_meta_name( $post_id );
		/**
		 * get users
		 */
		global $wpdb;
		 ...
		 * get facilitators
		 */
		$facilitators = BrainPress_Data_Course::get_facilitators( $course_id, true );
