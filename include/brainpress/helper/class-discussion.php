<?php
class BrainPress_Helper_Discussion {

	static private $default_key = 'subscribe-reactions';
	static private $field_name = 'brainpress_subscribe';
	...
	public static function add_nonce_to_comment_form() {
		...
		wp_nonce_field( $nonce_action, 'brainpress-add-commment-nonce' );
	}
	...
	public static function get_subscription_statuses_array() {
		return array(
			'subscribe-reactions' => __( 'Benachrichtige mich über neue Beiträge in diesem Thread', 'brainpress' ),
			'do-not-subscribe' => __( 'Benachrichtige mich nicht über neue Beiträge', 'brainpress' ),
			'subscribe-all' => __( 'Benachrichtige mich über alle neuen Beiträge', 'brainpress' ),
		);
	}
	...
	public static function get_comment_url( $comment_id, $comment_post_id ) {
		...
		$url = null;
		if ( BrainPress_Data_Module::get_post_type_name() == $comment_post->post_type ) {
			$unit = get_post( $comment_post->post_parent );
			$course = get_post( $unit->post_parent );
			$base_url = BrainPress_Core::get_slug( 'courses/', true ) . $course->post_name;
			$url = $base_url . '/' . BrainPress_Core::get_slug( 'unit/' ) . $unit->post_name;
