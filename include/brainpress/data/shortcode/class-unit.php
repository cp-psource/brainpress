 ...
 */
class BrainPress_Data_Shortcode_Unit {
	...
	public static function course_unit_details( $atts ) {
		...
		extract( shortcode_atts(
			apply_filters( 'shortcode_atts_course_unit_details',
				array(
					...
					'last_visited' => 'false',
					'parent_course_preceding_content' => __( 'Kurse: ', 'brainpress' ),
		...
		if ( 'permalink' == $field ) {
			...
			$content = get_permalink( $course_id ) . BrainPress_Core::get_slug( 'unit/' ) . $unit->post_name;
		}
		return $content;
	}
	...
	public static function course_unit_archive_submenu( $atts ) {
		...
		$subpage = BrainPress_Helper_Utility::the_course_subpage();
		...
		<div class="submenu-main-container cp-submenu">
			<ul id="submenu-main" class="submenu nav-submenu">
				<li class="submenu-item submenu-units ' . ( 'units' == $subpage ? 'submenu-active' : '' ) . '"><a href="' . esc_url_raw( $course_base_url . BrainPress_Core::get_slug( 'unit/' ) ) . '" class="course-units-link">' . esc_html__( 'Einheiten', 'brainpress' ) . '</a></li>
		';
		...
		if ( $enrolled || $is_instructor ) {
			$content .= '
				<li class="submenu-item submenu-notifications ' . ( 'notifications' == $subpage ? 'submenu-active' : '' ) . '"><a href="' . esc_url_raw( $course_base_url . BrainPress_Core::get_slug( 'notification' ) ) . '">' . esc_html__( 'Benachrichtigungen', 'brainpress' ) . '</a></li>
			';
		}

		$pages = BrainPress_Data_Course::allow_pages( $course_id );

		if ( $pages['course_discussion'] && ( $enrolled || $is_instructor ) ) {
			$content .= '<li class="submenu-item submenu-discussions ' . ( 'discussions' == $subpage ? 'submenu-active' : '' ) . '"><a href="' . esc_url_raw( $course_base_url . BrainPress_Core::get_slug( 'discussion' ) ) . '">' . esc_html__( 'Diskussionen', 'brainpress' ) . '</a></li>';
		}

		if ( $pages['workbook'] && $enrolled ) {
			$content .= '<li class="submenu-item submenu-workbook ' . ( 'workbook' == $subpage ? 'submenu-active' : '' ) . '"><a href="' . esc_url_raw( $course_base_url . BrainPress_Core::get_slug( 'workbook' ) ) . '">' . esc_html__( 'Arbeitsmappe', 'brainpress' ) . '</a></li>';
		}

		if ( $pages['grades'] && $enrolled ) {
			$content .= '<li class="submenu-item submenu-grades ' . ( 'grades' == $subpage ? 'submenu-active' : '' ) . '"><a href="' . esc_url_raw( $course_base_url . BrainPress_Core::get_slug( 'grades' ) ) . '">' . esc_html__( 'Bewertungen', 'brainpress' ) . '</a></li>';
		}

		$content .= '<li class="submenu-item submenu-info"><a href="' . esc_url_raw( $course_base_url ) . '">' . esc_html__( 'Kursdetails', 'brainpress' ) . '</a></li>';

		$show_link = false;

		if ( CP_IS_PREMIUM ) {
			// CERTIFICATE CLASS.
			$show_link = BrainPress_Data_Certificate::is_enabled() && BrainPress_Data_Student::is_enrolled_in_course( $student_id, $course_id );
		}

		if ( is_user_logged_in() && $show_link ) {
			// COMPLETION LOGIC.
			if ( BrainPress_Data_Student::is_course_complete( get_current_user_id(), $course_id ) ) {
				$certificate = BrainPress_Data_Certificate::get_certificate_link( get_current_user_id(), $course_id, __( 'Zertifikat', 'brainpress' ) );
	...
	public static function module_status( $atts ) {
		...
		extract( shortcode_atts(
			array(
				...
				'previous_unit' => false,
				'message' => __( '%d von %d erforderliche Elemente abgeschlossen.', 'brainpress' ),
				'format' => 'true',
			),
			$atts,
			'module_status'
		...
		$unit_status = BrainPress_Data_Unit::get_unit_availability_status( $course_id, $unit_id, $previous_unit );
		...
		if ( $unit_available ) {
			...
			if ( $unit_status['mandatory_required']['enabled'] && ! $unit_status['mandatory_required']['result'] && ! $unit_status['completion_required']['enabled'] ) {
				...
				$content .= BrainPress_Helper_UI::get_message_required_modules( $first_line );
				 ...
				 */
				$first_line = __( 'Du mussen alle ERFORDERLICHEN Module vor dieser Einheit erledigen.', 'brainpress' );
				$content .= BrainPress_Helper_UI::get_message_required_modules( $first_line );
			}
			if ( ! empty( $unit_status['date_restriction'] ) && ! $unit_status['date_restriction']['result'] ) {
				...
				if ( ! empty( $unit_availability_date ) ) {
					$available_on = date_i18n( get_option( 'date_format' ), BrainPress_Data_Course::strtotime( $unit_availability_date ) );
					$content .= esc_html__( 'Diese Einheit ist verfügbar ab', 'brainpress' ) . ' ' . $available_on;
				}
			}
		}
	...
	public static function unit_discussion( $atts ) {
		...
		if ( array_key_exists( 'unitname', $wp->query_vars ) ) {
			$unit_id = BrainPress_Data_Unit::by_name( $wp->query_vars['unitname'] );
		} else {
			$unit_id = 0;
		}
		...
		$comments_args = array(
			...
			'title_reply' => 'Schreibe eine Antwort oder einen Kommentar',
	...
	public static function course_unit_title( $atts ) {
		...
		$the_permalink = BrainPress_Data_Unit::get_url( $unit_id );
