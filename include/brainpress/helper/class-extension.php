...
class BrainPress_Helper_Extension {
	...
	private static function get_plugins() {
		return apply_filters( 'brainpress_extensions_plugins', self::$plugins );
	}
	...
	public static function plugins_table() {
		...
		$content .= '<h3>' . esc_html__( 'Plugins', 'brainpress' ) . '</h3>';
			...
			if ( $installed && $activated ) {
				...
				if ( current_user_can( 'manage_options' ) ) {
					...
					$action = '<form method="post">
						...
						<input type="submit" class="button" value="' . esc_attr__( 'De-Aktivieren', 'brainpress' ) . '" />
					</form>
					';
				}
			} elseif ( $installed ) {
				...
				if ( current_user_can( 'activate_plugins' ) ) {
					$action = '<form method="post">
						...
						<input type="submit" class="button" value="' . esc_attr__( 'Aktivieren', 'brainpress' ) . '" />
					</form>
					';
				}
			} else {
				...
				if ( current_user_can( 'install_plugins' ) ) {
					...
					if ( empty( $plugin['is_link'] ) ) {
						$action = '<form method="post">
							...
							<input type="submit" class="button" value="' . esc_attr__( 'Installieren', 'brainpress' ) . '" />
						</form>
						';
					} else {
						$action = sprintf( '<a href="%1$s" class="button">%2$s</a>', esc_url_raw( $plugin['source'] ), __( 'Installieren', 'brainpress') );
					}
				}
			}
