 ...
 * @since 2.0
 **/
class BrainPress_Data_Unsubscribe {
	...
	public static function hook_unsubscribe_link( $message ) {
		...
		$unsubscribe_link = add_query_arg(
			...
			BrainPress_Core::get_slug( 'courses/', true )
		);

		$message = BrainPress_Helper_Utility::replace_vars(
			$message,
			array(
		...
		// Remove the filter
		remove_filter( 'brainpress_email_message', array( __CLASS__, 'hook_unsubscribe_link' ) );
		...
		if ( ! empty( $mail_to ) ) {
			...
			if ( in_array( $email_type, $unsubscribable ) ) {
				if ( $is_unsubscriber ) {
					...
					add_filter( 'brainpress_email_message', array( __CLASS__, 'hook_unsubscribe_link' ), 10, 2 );
				}
			}
		}
	...
	public static function unsubscribe_message() {
		$msg = sprintf( '<h3 class="bbm-modal__title cp-unsubscribe-message">%s</h3>', __( 'Erfolgreich Deabonniert!', 'brainpress' ) );
		$msg .= sprintf( '<div class="bbm-modal__section"><p>%s</p></div>', __( 'Du wurdest von unserer Abonnentenliste entfernt.', 'brainpress' ) );
		$msg .= sprintf( '<div class="bbm-modal__bottombar"><a class="cancel-link">%s</a></div>', __( 'Erledigt', 'brainpress' ) );
		/**
		 * Filter the unsubscribe message.
		 *
		 * @since 2.0
		 **/
		$msg = apply_filters( 'brainpress_unsubscribe_message', $msg );
	...
	public static function unsubscribe( $unsubscribe_id ) {
		if ( (int) $unsubscribe_id > 0 ) {
			 ...
			 * @param (int) $user_id
			 **/
			do_action( 'brainpress_remove_subscriber', $unsubscribe_id );
			 ...
			 **/
			do_action( 'brainpress_removed_subscriber', $unsubscribe_id );
		}
	}
