...
(function( $ ) {
	...
	BrainPress.Course.multiple_elements = function( items, needle ) {
		var item_count = 0;
	...
	BrainPress.Course.add_array_to_data = function( data, items ) {
		var item_count = 0;
		var last_item = '';
		...
		$.each( items, function( index, element ) {
			...
			if ( name.match( /\/$/g ) || BrainPress.Course.multiple_elements( items, element.name ) ) {
				if ( name.match( /\/$/g ) ) {
					BrainPress.utility.update_object_by_path( data, name + item_count, element.value );
				} else {
					BrainPress.utility.update_object_by_path( data, name + '/' + item_count, element.value );
				}
			} else {
				BrainPress.utility.update_object_by_path( data, name, element.value );
			}
	...
	BrainPress.Course.fix_step_checkboxes = function( items, step, false_value ) {
		return BrainPress.utility.fix_checkboxes( items, '.step-content.step-' + step, false_value );
	};

	BrainPress.Course.hasError = false;
	BrainPress.Course.get_step = function( step, action_type ) {
		...
		if ( 1 <= step ) {
			...
			meta_items = BrainPress.Course.fix_step_checkboxes( meta_items, step );
			BrainPress.Course.add_array_to_data( data, meta_items );
		}
		...
		if ( 2 <= step ) {
			...
			meta_items = BrainPress.Course.fix_step_checkboxes( meta_items, step, '0' );
			BrainPress.Course.add_array_to_data( data, meta_items );
		}
		...
		if ( 0 < checklist.length ) {
			...
			BrainPress.Course.add_array_to_data( data, meta_category );
		}
		...
		BrainPress.Course.set( 'action', 'update_course' );
		BrainPress.Course.set( 'next_step', next_step );
	};
	
	BrainPress.Course.show_message = function( message, notice_class ) {
	...
	function setup_UI() {
		 ...
		 * course title
		 */
		if ( _brainpress.course_title ) {
			$('.brainpress_settings_wrapper h1').append( ': <small>'+_brainpress.course_title+'</small>');
			$('#course_name').on('change', function() {
				$('.brainpress_settings_wrapper h1 small').html($(this).val());
			});
		}
	...
	function bind_buttons() {
		...
		BrainPress.Events.on('editor:keyup', function( e ) {
		...
		$( '.cp-box-content .button.step.prev, .cp-box-content .button.step.next, .cp-box-content .button.step.update, .cp-box-content .button.step.finish' ).on( 'click', function( e ) {
			...
			if ( required_fields.length > 0 ) {
				...
				if ( found > 0 ) {
					BrainPress.Course.hasError = true;
					// Alert
					// @todo: Make this message info nicer!
					alert( _brainpress.labels.required_fields );
			...
			if ( null !== step ) {
				BrainPress.Course.show_message( _brainpress.unit_builder_form.messages.setup.saving, 'info' );
				...
				BrainPress.Course.get_step( step, action_type );
		...
		$( '.button.instructor-assign' ).on( 'click', function() {
			...
			dropdown.html( _brainpress.labels.user_dropdown_placeholder );
			...
			BrainPress.Course.set( 'action', 'add_instructor' );

			var data = {
				instructor_id: instructor_id,
				course_id: _brainpress.course_id,
		...
		$( '#invite-instructor-trigger' ).on( 'click', function() {
			...
			var email_valid = email.match( _brainpress.email_validation_pattern ) !== null;

			if ( email_valid ) {
				BrainPress.Course.set( 'action', 'invite_instructor' );
				var data = {
				...
				BrainPress.Course.set( 'data', data );
				BrainPress.Course.on( 'brainpress:invite_instructor_success', function() {
		...
		$( '.add-new-student-button' ).on( 'click', function( e ) {
			...
			BrainPress.Course.set( 'action', 'enroll_student' );
			var data = {
				nonce: nonce,
				course_id: _brainpress.course_id,
		...
		$( 'a.withdraw-student' ).on( 'click', function() {
			if ( window.confirm( _brainpress.student_delete_confirm ) ) {
				BrainPress.Course.set( 'action', 'withdraw_student' );
				var data = {
				...
				BrainPress.Course.set( 'data', data );
				BrainPress.Course.save();
				BrainPress.Course.on( 'brainpress:withdraw_student_success', function(){
		...
		$( 'a.withdraw-all-students' ).on( 'click', function() {
			if ( window.confirm( _brainpress.student_delete_all_confirm ) ) {
				BrainPress.Course.set( 'action', 'withdraw_all_students' );
				var data = {
					course_id: _brainpress.course_id,
					nonce: $( this ).attr( 'data-nonce' )
				};
				BrainPress.Course.set( 'data', data );
				BrainPress.Course.save();
				BrainPress.Course.on( 'brainpress:withdraw_all_students_success', function() {
					window.location.reload();
				});
				return false;
			}
		...
		$( '.brainpress_course_invite_student_wrapper input' ).keypress( function( ev ) {
			if ( ev.which === 13 ) {
				switch ( $( this ).attr( 'name' ) ) {
					...
					case 'invite-email':
					case '.brainpress_course_invite_student_wrapper .invite-submit':
						$( '.brainpress_course_invite_student_wrapper .invite-submit' ).trigger( 'click' );
		...
		$( '.brainpress_course_invite_student_wrapper .invite-submit' ).on( 'click', function() {
			// Really basic validation
			...
			if ( email_valid ) {
				$( '.brainpress_course_invite_student_wrapper .invite-submit' ).prepend( '<i class="fa fa-spinner fa-spin invite-progress"></i> ' );

				BrainPress.Course.set( 'action', 'invite_student' );
				var data = {
					first_name: first_name,
					last_name: last_name,
					email: email,
				...
				BrainPress.Course.set( 'data', data );
				BrainPress.Course.save();
				BrainPress.Course.on( 'brainpress:invite_student_success', function(){
					// Reload page
					window.location.reload();
				});
			}
		...
		$( '.column-actions .remove-invite' ).on( 'click', function(){
			...
			BrainPress.Course.set( 'action', 'remove_student_invitation' );
			BrainPress.Course.set( 'data', data );
			BrainPress.Course.save();

			BrainPress.Course.on( 'brainpress:remove_student_invitation_success', function() {
		...
		$( '.button.facilitator-assign' ).on( 'click', function() {
			...
			var select = $( '[name="facilitators"]' ),
				...
				avatar = _brainpress.instructor_avatars['default'],
			...
			BrainPress.Course.set( 'action', 'add_facilitator' );
			BrainPress.Course.set( 'data', data );
			BrainPress.Course.save();
		});

		// Remove course facilitator
		BrainPress.remove_facilitator = function() {
			var div = $( this ).parent(),
			...
			BrainPress.Course.set( 'action', 'remove_facilitator' );
			BrainPress.Course.set( 'data', data );
			BrainPress.Course.save();
		};
		$( '.facilitator-remove' ).on( 'click', BrainPress.remove_facilitator );

		/**
		 * send email to enroled students
		 */
		$( '.brainpress_course_email_enroled_students_wrapper .send-submit' ).on( 'click', function() {
			// Really basic validation
			...
			$( '.brainpress_course_email_enroled_students_wrapper .send-submit' ).hide();
			$('.brainpress-email-field', message_container).slideUp();
			$('.brainpress-email-sending', message_container).slideDown();

			BrainPress.Course.set( 'action', 'send_email' );
			var data = {
				send_to: send_to,
				subject: subject,
				body: body,
			...
			BrainPress.Course.set( 'data', data );
			BrainPress.Course.save();
			BrainPress.Course.off( 'brainpress:send_email_success' );
			BrainPress.Course.on( 'brainpress:send_email_success', function( data ){
				$('.brainpress-email-sending td', message_container).html(data.message.info);
				$('.brainpress-email-field-subject td', message_container).html(data.message.subject);;
				$('.brainpress-email-field-body td', message_container).html(data.message.body);;
				$('.brainpress-email-field', message_container).slideDown();
			});
			BrainPress.Course.off( 'brainpress:send_email_error' );
			BrainPress.Course.on( 'brainpress:send_email_error', function( data ) {
				$('.brainpress-email-sending td', message_container).html(data.message.info);
				$('.brainpress-email-field-subject td', message_container).html(data.message.subject);;
				$('.brainpress-email-field-body td', message_container).html(data.message.body);;
				$('.brainpress-email-field', message_container).slideDown();
	...
	function bind_remove_button( selector ) {
		$( selector ).on( 'click', function( e ) {
			...
			switch( meta.status ) {
				case 'confirmed':
					message = _brainpress.instructor_delete_confirm;
					if ( 'facilitator' === meta.who ) {
						message = _brainpress.facilitator_delete_confirm;
					}
					...
					if ( window.confirm( message ) ) {
						...
						$( step_box ).find('.button.update.hidden' ).removeClass('hidden');
						BrainPress.Course.set( 'action', 'delete_instructor' );
						data = {
					...
					break;
				case 'pending':
					message = _brainpress.instructor_delete_invite_confirm;
					if ( 'facilitator' === meta.who ) {
						message = _brainpress.facilitator_delete_invite_confirm;
					}
					// Confirm before deleting
					if ( window.confirm( message ) ) {
						$(target).html( '<span class="fa fa-circle-o-notch fa-spin fa-2x fa-fw"></span>' );
						...
						step_box = $( target ).parents('.cp-box-content')[0];
						$( step_box ).find('.button.update.hidden' ).removeClass('hidden');
						BrainPress.Course.set( 'action', 'delete_instructor_invite' );
						data = {
	...
	function bind_brainpress_events() {

		/**
		 * COURSE UPDATE
		 */
		BrainPress.Course.on( 'brainpress:update_course_success', function( data ) {
			if ( data.last_step == data.next_step ) {
				BrainPress.Course.show_message( _brainpress.unit_builder_form.messages.setup.saved, 'success' );
			}
			...
			_brainpress.course_id = data.course_id;
		...
		BrainPress.Course.on( 'brainpress:update_course_error', function( data ) {
			BrainPress.Course.show_message( _brainpress.unit_builder_form.messages.setup.error, 'error' );
		 ...
		 * INSTRUCTOR ACTIONS
		 */
		BrainPress.Course.on( 'brainpress:add_instructor_success', function( data ) {

			var content = '';
			var avatar = _brainpress.instructor_avatars[ 'default' ];
		...
		BrainPress.Course.on( 'brainpress:delete_instructor_success', function( data ) {
			// Remove Instructor or Facilitator Avatar
			$( '#' + data.who + '_holder_' + data.instructor_id ).detach();
			if ( 'instructor' == data.who ) {
				var empty_holder = '<div class="instructor-avatar-holder empty"><span class="instructor-name">' + _brainpress.instructor_empty_message + '</span></div>';
		...
		BrainPress.Course.on( 'brainpress:invite_instructor_success', function( data ) {
			var template = wp.template('course-invitation');
			var email = data.data.email;
			var invite_code = data.invite_code;
			var img = BrainPress.utility.get_gravatar_image( email, 80 );
		...
		BrainPress.Course.on( 'brainpress:invite_instructor_error', function( data ) {
		...
		BrainPress.Course.on( 'brainpress:delete_instructor_invite_success', function( data ) {
			var who = 'instructor';
		...
		// Add facilitator
		BrainPress.Course.on( 'brainpress:add_facilitator_success', function( data ) {
			var template = wp.template('course-person');
		...
		BrainPress.Course.on( 'brainpress:add_facilitator_error', function( data ) {
		...
		// Remove facilitator
		BrainPress.Course.on( 'brainpress:remove_facilitator_success', function( data ) {
		...
		BrainPress.Course.on( 'brainpress:toggle_course_status_error', function() {
			// Toggle back.
		...
		BrainPress.Course.on( 'brainpress:enroll_student_success', function() {
			location.reload();
		} );
		...
		BrainPress.Course.on( 'brainpress:invite_student_success', function( data ) {
			$( '.invite-progress' ).detach();
			$( '.brainpress_course_invite_student_wrapper .invite-submit' ).attr( 'data-nonce', data.nonce );
		} );

		BrainPress.Course.on( 'brainpress:invite_student_error', function( ) {
			$( '.invite-progress' ).detach();
		} );
	...
	function bind_assessment_events() {
		var ungraded_selector = '.brainpress_settings_wrapper.assessment .ungraded-elements [type="checkbox"]';
		var submitted_selector = '.brainpress_settings_wrapper.assessment .submitted-elements [type="checkbox"]';
		...
		$( '.brainpress_settings_wrapper.assessment table' ).treegrid().on( 'expand', function() {
			update_modules( this );
		} );

		$( '.brainpress_settings_wrapper.assessment table .instructor-feedback' ).link_popup( { link_text: '<span class="dashicons dashicons-admin-comments"></span>' } );

		$( '.brainpress_settings_wrapper.assessment [name="course-list"]' ).on( 'change', function() {
			var course_id = $( this ).val();
			location.href = _brainpress.assessment_grid_url + '&course_id=' + course_id;
		} );

		$( '.brainpress_settings_wrapper.assessment .collapse-all-students' ).on( 'click', function() {
			$( '.brainpress_settings_wrapper.assessment table' ).treegrid( 'collapseAll' );
		} );

		$( '.brainpress_settings_wrapper.assessment .expand-all-students' ).on( 'click', function() {
			$( '.brainpress_settings_wrapper.assessment table' ).treegrid( 'expandAll' );
		} );
	}

	function bind_reports_events() {
		$( '.brainpress_settings_wrapper.reports .help-tooltip' ).link_popup( {
			link_text: '<span class="dashicons dashicons-editor-help"></span>',
			offset_x: 20
		} );

		$( '.brainpress_settings_wrapper.reports [name="course-list"]' ).on( 'change', function() {
			var course_id = $( this ).val();
			location.href = _brainpress.assessment_report_url + '&course_id=' + course_id;
		} );

		$( '.brainpress_settings_wrapper.reports .column-report .pdf' ).on( 'click', function() {
	...
	function bind_notification_events() {
		$( '[name*="publish-notification-toggle"]' ).on( 'change', function( ev, state ) {
			BrainPress.Post.prepare( 'update_notification', 'notification:' );
			BrainPress.Post.set( 'action', 'toggle' );
		...
		$( '.delete-notification-link' ).on( 'click', function( ev ) {
			...
			if ( window.confirm( _brainpress.notification_delete ) ) {
				BrainPress.Post.prepare( 'update_notification', 'notification:' );
				BrainPress.Post.set( 'action', 'delete' );
		...
		//BrainPress.Post.on( 'brainpress:notification:delete_error', function( data ) {} );
		//BrainPress.Post.on( 'brainpress:notification:toggle_success', function( data ) { } );
		BrainPress.Post.on( 'brainpress:notification:toggle_error', function( data ) {
		...
		$( '.brainpress_communications_wrapper.notifications [id*="doaction"]' ).on( 'click', function() {
			var action = $( this ).siblings( '[id*="bulk-action-selector"]' ).val();

			if ( 'delete' === action && !window.confirm( _brainpress.notification_bulk_delete ) ) {
				return false;
			}
			...
			if ( undefined !== action && -1 !== action ) {
				...
				BrainPress.Post.prepare( 'update_notification', 'notification:' );
				BrainPress.Post.set( 'action', 'bulk_' + action );
		...
		BrainPress.Post.on( 'brainpress:notification:bulk_publish_success', function() {
			location.reload();
		} );

		BrainPress.Post.on( 'brainpress:notification:bulk_unpublish_success', function() {
			location.reload();
		} );

		BrainPress.Post.on( 'brainpress:notification:bulk_delete_success', function() {
			location.reload();
		} );
	}
	...
	function bind_discussion_events() {
		$( '[name*="publish-discussion-toggle"]' ).on( 'change', function( e, state ) {
			BrainPress.Post.prepare( 'update_discussion', 'discussion:' );
			BrainPress.Post.set( 'action', 'toggle' );
		...
		$( '.delete-discussion-link' ).on( 'click', function( e ) {
			e.stopImmediatePropagation();
			e.preventDefault();
			if ( window.confirm( _brainpress.discussion_delete ) ) {
		...
		BrainPress.Post.on( 'brainpress:discussion:delete_success', function() {
			location.reload();
		} );

		// Could add these events, but won't need it
		//BrainPress.Post.on( 'brainpress:discussion:delete_error', function( data ) {} );
		//BrainPress.Post.on( 'brainpress:discussion:toggle_success', function( data ) {} );
		BrainPress.Post.on( 'brainpress:discussion:toggle_error', function( data ) {
		...
		$( '.brainpress_communications_wrapper.discussions [id*="doaction"]' ).on( 'click', function() {
			var action = $( this ).siblings( '[id*="bulk-action-selector"]' ).val();

			if ( 'delete' === action && ! window.confirm( _brainpress.discussion_bulk_delete ) ) {
				return false;
			}
		...
		BrainPress.Post.on( 'brainpress:discussion:bulk_publish_success', function() {
			location.reload();
		} );

		BrainPress.Post.on( 'brainpress:discussion:bulk_unpublish_success', function() {
			location.reload();
		} );

		BrainPress.Post.on( 'brainpress:discussion:bulk_delete_success', function() {
			location.reload();
		} );

		// Edit Page
		$( '.brainpress_communications_wrapper.discussions select#course_id' ).on( 'change', function() {
			var course_id = $( this ).val();

			$.each( $( '.brainpress_communications_wrapper.discussions select#unit_id' ).find( 'option' ), function( index, item ) {
				if ( 'course' !== $( item ).val() ) {
					$( item ).detach();
				}
			} );
			...
			BrainPress.Post.prepare( 'update_discussion', 'discussion:' );
			BrainPress.Post.set( 'action', 'unit_items' );
		...
		BrainPress.Post.on( 'brainpress:discussion:unit_items_success', function( data ) {
			if ( data.items.length > 0 ) {
				$.each( data.items, function( index, item ) {
					$( '.brainpress_communications_wrapper.discussions select#unit_id' ).append( '<option value="' + item.key + '">' + item.value + '</option>' );
				} );
			}
		} );
	}
	 ...
	 */
	var Search_Params = {
		placeholder: _brainpress.labels.user_dropdown_placeholder,
		allowClear: true,
		ajax: {
			url: _brainpress._ajax_url,
			dataType: 'json',
			delay: 250,
	...
	function formatDataSelection (data ) {
		...
		if ( ! data || ( ! data.gravatar && ! data.display_name ) ) {
			markup = _brainpress.labels.user_dropdown_placeholder;
		} else {
	...
	// UPDATE COURSE
	BrainPress.updateCourse = function() {
		var form = $(this),
	...
	BrainPress.maybeUpdateCourse = function() {
		var form = $( 'form#post' ),
			target = $(this);
	...
	$( document ).ready( function() {
		...
		$( ".course-edit-notification input[type=submit], .course-edit-forums input[type=submit]" ).click( function( event ) {
			...
			if ( '' === $('#titlewrap input[name=post_title]').val() ) {
				if ( is_notification ) {
					errors.push( _brainpress.messages.notification.empty_title );
				} else if ( is_forum ) {
					errors.push( _brainpress.messages.discussion.empty_title );
				} else {
					errors.push( _brainpress.messages.general.empty_title );
				}
			}
			/**
			 * Check content
			...
			if ( show_content_alert ) {
				if ( is_notification ) {
					errors.push( _brainpress.messages.notification.empty_content );
				} else if ( is_forum ) {
					errors.push( _brainpress.messages.discussion.empty_content );
				} else {
					errors.push( _brainpress.messages.general.empty_content );
				}
			}
			if ( errors.length ) {
				alert( errors.join( "\n" ) );
	...
	.on( 'click', '.post-type-course #publish, .post-type-course #search-submit, .post-type-course #post-preview', BrainPress.maybeUpdateCourse )
	.on( 'submit', '.post-type-course form#post', BrainPress.updateCourse );
