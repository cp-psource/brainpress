 ...
 */
class BrainPress_View_Front_EnrollmentPopup {
	 ...
	 * We check, if the theme supports BrainPress, and add different hooks
	 * depending on that condition.
	 *
	 * @since 2.0.0
	 */
	public static function add_hooks() {
		if ( get_theme_support( 'brainpress' ) ) {
			add_filter(
				'wp_footer',
	...
	public static function add_backbone_registration_templates_vp( $content, $course_id, $context ) {
		if ( 'main' == $context ) {
			$scode = sprintf(
				'[brainpress_enrollment_templates course_id="%d"]',
				$course_id
			);
	...
	public static function course_enrollment() {
		...
		if ( empty( $data->action ) ) {
			$json_data['message'] = __( 'Einschreibung: Keine Aktion.', 'brainpress' );
		...
		switch ( $action ) {
			...
			case 'delete_instructor':
				if ( wp_verify_nonce( $data->data->nonce, 'setup-course' ) ) {
					BrainPress_Data_Course::remove_instructor( $data->data->course_id, $data->data->instructor_id );
			...
			case 'add_instructor':
				...
				if ( wp_verify_nonce( $data->data->nonce, 'brainpress_add_instructor' ) ) {
					...
					if ( ! empty( $data->data->invite_code ) ) {
						$success = $json_data['success'] = BrainPress_Data_Instructor::add_from_invitation( $data->data->course_id, $data->data->instructor_id, $data->data->invite_code );
					}
				}
				break;
			...
			case 'delete_instructor_invite':
				if ( wp_verify_nonce( $data->data->nonce, 'setup-course' ) ) {
					BrainPress_Data_Instructor::delete_invitation( $data->data->course_id, $data->data->invite_code );
					$json_data['course_id'] = $data->data->course_id;
			...
			case 'duplicate_course':
				if ( wp_verify_nonce( $data->data->nonce, 'duplicate_course' ) ) {
					...
					if ( ! empty( $the_course ) ) {
						...
						$the_course['post_title'] = $the_course['post_title'] . ' ' . __( 'Kopieren', 'brainpress' );
			...
			case 'signup':
				if ( wp_verify_nonce( $data->data->nonce, 'brainpress_enrollment_action_signup' ) ) {
					$nonce = wp_create_nonce( 'brainpress_enrollment_action' );
				} else {
					$json_data['message'] = __( 'Einschreibung: Ungültige Anfrage. Bitte versuche die Seite neu zu laden.', 'brainpress' );
					wp_send_json_error( $json_data );
					return;
				}
				 ...
				 */
				if ( empty( $username ) ) {
					$signup_errors[] = __( 'Der Benutzername darf nicht leer sein.', 'brainpress' );
					$registration_data_are_valid = false;
				} elseif ( ! validate_username( $username ) ) {
					$signup_errors[] = __( 'Ungültiger Benutzername. Bitte wähle einen anderen.', 'brainpress' );
					$registration_data_are_valid = false;
				} else {
					$user_id = username_exists( $username );
					if ( ! empty( $user_id ) ) {
						$signup_errors[] = __( 'Benutzername existiert bereits. Bitte wähle einen anderen.', 'brainpress' );
						$registration_data_are_valid = false;
					}
				}
				 ...
				 */
				if ( ! is_email( $email ) ) {
					$signup_errors[] = __( 'Email Adresse ist nicht gültig.', 'brainpress' );
					$registration_data_are_valid = false;
				} else {
					$email_exists = email_exists( $email );
					if ( $email_exists ) {
						$signup_errors[] = __( 'E-Mail-Adresse bereits verwendet.', 'brainpress' );
						$registration_data_are_valid = false;
					}
				}
			...
			case 'login':
				...
				if ( ! is_wp_error( $user ) ) {
					...
					$enrolled = BrainPress_Data_Course::student_enrolled( $user->ID, $course_id );
			...
			case 'enroll_student':
				...
				$course_id = (int) $data->data->course_id;
				$type = BrainPress_Data_Course::get_setting( $course_id, 'enrollment_type', 'manually' );

				if ( 'passcode' == $type ) {
					$json_data['success'] = false;
				} else if ( ! empty( $student_id ) && ! empty( $course_id ) && true === BrainPress_Data_Course::enroll_student( $student_id, $course_id ) ) {
					$json_data['student_id'] = $student_id;
					$json_data['course_id'] = $course_id;
					$json_data['success'] = true;
				} else {
					$json_data['error_message'] = __( 'Zu diesem Zeitpunkt konnte ich mich nicht einschreiben.', 'brainpress' );
					$json_data['success'] = false;
				}
			...
			case 'enroll_with_passcode':
				...
				$course_id = $data->data->course_id;
				// Verify passcode
				$course_passcode = BrainPress_Data_Course::get_setting( $course_id, 'enrollment_passcode' );

				if ( $course_passcode != $passcode ) {
					$json_data['success'] = false;
					$json_data['message'] = __( 'IUngültiger PASSCODE!', 'brainpress' );
				} else {
					BrainPress_Data_Course::enroll_student( $student_id, $course_id );
					$json_data['success'] = true;
				}
				$success = isset( $json_data['success'] ) ? $json_data['success'] : false;
				break;
			...
			default:
				...
				$json_data = array_merge( apply_filters( 'brainpress_popup_enrollment', array( 'success' => true ), $action, $step_data ), $json_data );
