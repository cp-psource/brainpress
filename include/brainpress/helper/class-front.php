...
class BrainPress_Helper_Front {
	...
	public static function check_and_redirect( $key, $redirect_not_logged = true ) {
		...
		if ( BrainPress_Core::get_slug( $key ) != $wp->query_vars['pagename'] ) {
			return false;
		}
		/**
		 * redirect non logged
		...
		if ( $redirect_not_logged && ! is_user_logged_in() ) {
			...
			if ( $use_custom ) {
				...
				if ( empty( $login_page ) ) {
					$url = BrainPress_Core::get_slug( 'login', true );
				} else {
		 ...
		 */
		$vp = (int) BrainPress_Core::get_setting( 'pages/'.$key, 0 );
		if ( ! empty( $vp ) ) {
			$post = get_post( $vp );
