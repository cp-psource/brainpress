...
class BrainPress_Helper_Query_Student extends WP_User_Query {
	...
	function do_paging() {
		...
		if ( $this->total_users_for_query > $this->users_per_page ) { // pagination required
			...
			if ( $this->paging_text ) {
				$this->paging_text = sprintf( '<span class="displaying-num">' . __( 'Angezeigt %s&#8211;%s von %s', 'brainpress' ) . '</span>%s', number_format_i18n( ( $this->page_num - 1 ) * $this->users_per_page + 1 ), number_format_i18n( min( $this->page_num * $this->users_per_page, $this->total_users_for_query ) ), number_format_i18n( $this->total_users_for_query ), $this->paging_text
				);
			}
		}
	}
	...
	function page_links() {
		...
		$pagination = new BrainPress_Helper_Pagination();
		...
		$pagination->title( __( 'student', 'brainpress' ), __( 'Studenten', 'brainpress' ) );
