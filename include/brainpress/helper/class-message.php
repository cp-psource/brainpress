<?php
class BrainPress_Helper_Message {
	...
	/**
	 * Initialize class BrainPress_Helper_Message
	 *
	 * @since 2.0.0
	 *
	 */
	...
	public static function set_message_key() {
		...
		self::$message_code = apply_filters( 'brainpress_helper_message_get_message_code', $key );
	}
	...
	public static function add_message_query_arg( $url, $key ) {
		...
		return apply_filters( 'brainpress_helper_message_add_query_arg', $url, $key );
	}
	...
	public static function course_message( $atts ) {
		...
		switch ( self::$message_code ) {
			...
			case 'no-access':
				$message = __( 'Du benötigst ein Mitgliedskonto, um auf diesen Kurs zugreifen zu können.', 'brainpress' );
				$classes[] = 'course-message-alert';
			break;

			case 'only-enroled':
				$message = __( 'Nur eingeschriebene Studenten können auf dieses Kursmaterial zugreifen.', 'brainpress' );
			break;
		...
		$message = apply_filters( 'brainpress_helper_message_message', $message, $message_code, $atts );
		if ( empty( $message ) ) {
			return;
		}
		...
		$content .= '</div>';
		return apply_filters( 'brainpress_helper_message_html', $content, $message, $message_code, $atts );
	}
	...
	public static function course_message_classes() {
		...
		return apply_filters( 'brainpress_helper_message_classes', $classes, self::$message_code );
	}
	...
	private static function _get_not_available_message() {
		if (
			...
			if ( 'module' == $_REQUEST['type'] ) {
				$unit_id = BrainPress_Data_Module::get_unit_id_by_module( $_REQUEST['id'] );
			}
			$course_id = BrainPress_Data_Unit::get_course_id_by_unit( $unit_id );

			$user_id = get_current_user_id();
			$unit_availability_date = BrainPress_Data_Unit::get_unit_availability_date( $unit_id, $course_id, $user_id );
			$when = date( 'M d', BrainPress_Data_Course::strtotime( $unit_availability_date ) );

			return sprintf(
				__( 'Diese Einheit ist ab %s verfügbar', 'brainpress' ),
				sprintf(
		...
		return __( 'Diese Einheit ist nicht verfügbar.', 'brainpress' );
	}
}
