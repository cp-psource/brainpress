...
function ema_modal_window_admin_style($hook) {
	...
	wp_enqueue_style(  'ema_modal_css', 		plugins_url('brainpress-reorder-pages-css.css', __FILE__) );
	...
	wp_enqueue_script( 'ema_reorder_script', 	plugin_dir_url( __FILE__ ) . 'brainpress-reorder-pages-js.js', array(), '1.0' );
	
	wp_enqueue_script( 'ema_modal_script', 		plugin_dir_url( __FILE__ ) . 'brainpress-reorder-pages-modalWindow-js.js', array(), '1.0', true );
	...
	function ema_cp_reorder_sections($check) {
		...
		foreach($queryarr as $var => $val) {
			if(substr($var, 0, 3) != 'ema') {
				$querystring .= '&' . http_build_query(array($var => $val));
			}
		}
