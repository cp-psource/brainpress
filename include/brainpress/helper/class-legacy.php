...
class BrainPress_Helper_Legacy {
		...
		if ( empty( $meta_updated ) ) {
			...
			if ( ! empty( $courses ) ) {
				foreach ( $courses as $course_id ) {
					$old_settings = BrainPress_Data_Course::get_setting( $course_id );
					BrainPress_Data_Course::update_setting( $course_id, true, $old_settings );
...
if ( ! function_exists( 'cp_deep_unserialize' ) ) {
	function cp_deep_unserialize( $serialized_object ) {
		_doing_it_wrong(
			'cp_deep_unserialize',
			'Deprecated BrainPress function (without replacement)',
			'2.0'
		);
...
if ( ! function_exists( 'cp_instructors_drop_down' ) ) {
	function cp_instructors_drop_down( $class = '' ) {
		...
		$content = '';
		$content .= '<select name="instructors" id="instructors" data-placeholder="' . __( 'Wähle einen Kursleiter ...', 'brainpress' ) . '" class="' . $class . '">';
...
if ( ! function_exists( 'cp_students_drop_down' ) ) {
	function cp_students_drop_down() {
		...
		$content = '';
		$content .= '<select name="students" data-placeholder="' . __( 'Wähle einen Studenten ...', 'brainpress' ) . '" class="chosen-select">';
...
if ( ! function_exists( 'brainpress_send_email' ) ) {
	function brainpress_send_email( $email_args = array() ) {
		_doing_it_wrong(
			'brainpress_send_email',
			'Deprecated BrainPress function. Use BrainPress_Helper_Email::send_mail() instead!',
			'2.0'
		);

		if ( 'student_registration' == $email_args['email_type'] ) {
			throw new Exception( 'Deprecated: Use BrainPress_Data_Student::send_registration() instead.' );
		}

		if ( 'enrollment_confirmation' == $email_args['email_type'] ) {
			throw new Exception( 'Deprecated: Use BrainPress_Data_Course::enroll_student() instead.' );
		}

		if ( 'student_invitation' == $email_args['email_type'] ) {
			throw new Exception( 'Deprecated: Use BrainPress_Data_Course::send_invitation() instead.' );
		}

		if ( 'instructor_invitation' == $email_args['email_type'] ) {
			throw new Exception( 'Deprecated: Use BrainPress_Data_Instructor::send_invitation() instead.' );
		}
	}
}

if ( ! function_exists( 'brainpress_unit_pages' ) ) {
	function brainpress_unit_pages( $unit_id, $unit_pagination = false ) {
		_doing_it_wrong(
			'brainpress_unit_pages',
			'Deprecated BrainPress function (without replacement)',
			'2.0'
		);
...
if ( ! function_exists( 'brainpress_unit_module_pagination_ellipsis' ) ) {
	function brainpress_unit_module_pagination_ellipsis( $unit_id, $pages_num ) {
		_doing_it_wrong(
			'brainpress_unit_module_pagination_ellipsis',
			'Deprecated BrainPress function (without replacement)',
			'2.0'
		);

		global $wp, $wp_query, $paged, $brainpress_modules;
...
if ( ! function_exists( 'brainpress_unit_module_pagination' ) ) {
	function brainpress_unit_module_pagination( $unit_id, $pages_num, $check_is_last_page = false ) {
		_doing_it_wrong(
			'brainpress_unit_module_pagination',
			'Deprecated BrainPress function (without replacement)',
			'2.0'
		);

		global $wp, $wp_query, $paged, $brainpress_modules, $brainpress;
		...
		for ( $link_num = 1; $link_num <= $max; $link_num ++ ) {
			$enabled = '';
			if ( $brainpress->is_preview( $unit_id, $link_num ) ) {
...
if ( ! function_exists( 'cp_get_id_by_post_name' ) ) {
	function cp_get_id_by_post_name( $post_name, $post_parent = 0, $type = 'unit' ) {
		_doing_it_wrong(
			...
			'Deprecated BrainPress function. Use BrainPress_Data_Course::by_name() or BrainPress_Data_Unit::by_name()',
			'2.0'
		);
...
if ( ! function_exists( 'cp_is_chat_plugin_active' ) ) {
	function cp_is_chat_plugin_active() {
		...
		*/
		...
		$required_plugin = 'ps-chat/ps-chat.php';

		if ( in_array( $required_plugin, $plugins ) || cp_is_plugin_network_active( $required_plugin ) || preg_grep( '/^ps-chat.*/', $plugins ) || cp_preg_array_key_exists( '/ps-chat.*/', $active_sitewide_plugins ) ) {
			return true;
		} else {
			return false;
		}
...
if ( ! function_exists( 'cp_filter_content' ) ) {
	function cp_filter_content( $content, $none_allowed = false ) {
		throw new Exception( 'Deprecated: Use BrainPress_Helper_Utility::filter_content() instead!' );
	}
}

if ( ! function_exists( 'cp_user_can_register' ) ) {
	function cp_user_can_register() {
		throw new Exception( 'Deprecated: Use BrainPress_Helper_Utility::user_can_register() instead!' );
	}
}
...
if ( ! function_exists( 'cp_get_file_size' ) ) {
	function cp_get_file_size( $url, $human = true ) {
		throw new Exception( 'Deprecated: Use BrainPress_Helper_Utility::get_file_size() instead!' );
	}
}

if ( ! function_exists( 'cp_format_file_size' ) ) {
	function cp_format_file_size( $bytes ) {
		throw new Exception( 'Deprecated: Use BrainPress_Helper_Utility::format_file_size() instead!' );
	}
}

if ( ! function_exists( 'cp_remove_related_videos' ) ) {
	function cp_remove_related_videos( $html, $url, $args ) {
		throw new Exception( 'Deprecated: Use BrainPress_Helper_Utility::remove_related_videos() instead!' );
	}
}
...
if ( ! function_exists( 'truncateHtml' ) ) {
	...
	// @codingStandardsIgnoreEnd
		throw new Exception( 'Deprecated: Use BrainPress_Helper_Utility::truncate_html() instead!' );
	}
}
...
if ( ! function_exists( 'cp_wp_get_image_extensions' ) ) {
	function cp_wp_get_image_extensions() {
		throw new Exception( 'Deprecated: Use BrainPress_Helper_Utility::get_image_extensions() instead!' );
	}
}
...
if ( ! function_exists( 'cp_curPageURL' ) ) {
	...
	// @codingStandardsIgnoreEnd
		throw new Exception( 'Deprecated: Use BrainPress_Helper_Utility::get_current_url() instead!' );
	}
}
...
if ( ! function_exists( 'cp_instructors_avatars_array' ) ) {
	function cp_instructors_avatars_array( $args = array() ) {
		throw new Exception( 'Deprecated: Use BrainPress_Helper_UI::get_user_avatar_array() instead!' );
	}
}
...
if ( ! function_exists( 'cp_admin_ajax_url' ) ) {
	function cp_admin_ajax_url() {
		throw new Exception( 'Deprecated: Use BrainPress_Helper_Utility::get_ajax_url() instead!' );
	}
}
...
if ( ! function_exists( 'cp_can_see_unit_draft' ) ) {
	...
	function cp_can_see_unit_draft() {
		...
		if ( current_user_can( 'brainpress_create_course_unit_cap' ) ) { return true; }
...
if ( ! function_exists( 'cp_set_visited_unit_page' ) ) {
	...
	function cp_set_visited_unit_page( $unit_id, $page_num, $student_id = 0 ) {
		...
		BrainPress_Data_Student::visited_page( $student_id, $course_id, $unit_id, $page_num );
