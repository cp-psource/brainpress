 ...
 */
class BrainPress_Data_Shortcode_Instructor {
	...
	public static function course_instructors( $atts ) {
		...
		$instructor_profile_slug = BrainPress_Core::get_setting(
		...
		extract( shortcode_atts( array(
			...
			'label' => __( 'Kursleiter', 'brainpress' ),
			'label_plural' => __( 'Kursleiter', 'brainpress' ),
			'label_delimeter' => ':&nbsp;',
			'label_tag' => '',
			'count' => false, // Deprecated.
			'list' => false, // Deprecated.
			'link' => false,
			'link_text' => __( 'Profil anzeigen', 'brainpress' ),
		...
		if ( empty( $course_id ) ) {
			...
			$instructors = BrainPress_Data_Course::get_instructors( $course_id, true );
		}
		...
		if ( 'count' != $style ) {
			if ( ! empty( $instructors ) ) {
				foreach ( $instructors as $instructor ) {
					...
					$instructor_hash = BrainPress_Data_Instructor::get_hash( $instructor );

					if ( empty( $instructor_hash ) ) {
						BrainPress_Data_Instructor::create_hash( $instructor );
					}

					$show_username = cp_is_true( BrainPress_Core::get_setting( 'instructor/show_username', true ) );
					$profile_href .= $show_username ? trailingslashit( $instructor->user_login ) : trailingslashit( $hash );

					$display_name = ' ' . apply_filters(
						'brainpress_schema',
						esc_html( BrainPress_Helper_Utility::get_user_name( $instructor->ID, false, false ) ),
						'title'
					);
					...
					switch ( $style ) {
						case 'block':
							...
							if ( ! empty( $summary_length ) ) {
								$content .= '<div class="profile-description">' . BrainPress_Helper_Utility::author_description_excerpt( $instructor, $summary_length ) . '</div>';
							}
	...
	public static function course_instructor_avatar( $atts ) {
		...
		if ( ! empty( $avatar ) ) {
			...
			$content .= '<div class="' . $class . '" style="background: url( ' . $avatar_url . ' ) no-repeat; width: ' . $thumb_size . 'px; height: ' . $thumb_size . 'px;"></div>';
			$content .= '</div>';
		}
	...
	public static function instructor_profile_url( $atts ) {
		...
		if ( get_option( 'show_instructor_username', 1 ) ) {
			...
			$username = trailingslashit(
				BrainPress_Helper_Utility::md5( $instructor->user_login )
			);
		}
