...
class BrainPress_Data_PostFormat {
	 ...
	 * The prefix can be defined via the constant BRAINPRESS_CPT_PREFIX in
	...
	public static function register_post_types() {
		// Add the formats array in BrainPress_Core
		$classes = apply_filters( 'brainpress_post_formats', array() );
	...
	public static function prefix( $post_type = '' ) {
		if ( null === self::$prefix ) {
			...
			/**
			 * Override post type prefixes with BRAINPRESS_CPT_PREFIX defined
			 * in wp-config.php.
			 *
			 * Use this to help with conflicts.
			 * Only define the prefix, without trailing underscore!
			 *   Example:
			 *   define( 'BRAINPRESS_CPT_PREFIX', 'my' )  // GOOD :)
			 *   define( 'BRAINPRESS_CPT_PREFIX', 'my_' ) // BAD!
			 */
			if ( defined( 'BRAINPRESS_CPT_PREFIX' ) ) {
				self::$prefix = strtolower(
					sanitize_html_class( BRAINPRESS_CPT_PREFIX ) . '_'
				);
			}
		}
