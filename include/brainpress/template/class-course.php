 ...
 * Course Template
 **/
class BrainPress_Template_Course {
	...
	public static function course_list_table( $courses = array() ) {
		...
		if ( ! empty( $courses ) ) {
			...
			$table_header = '';
			$table_body = '';
			$certificated = BrainPress_Data_Certificate::is_enabled();

			$table_columns = array(
				'name' => __( 'Kurs', 'brainpress' ),
				'date_enrolled' => __( 'Eingeschrieben am', 'brainpress' ),
				'average' => __( 'Durchschnittlich', 'brainpress' ),
				'status' => __( 'Status', 'brainpress' ),
			);

			if ( $certificated ) {
				$table_columns['certificate'] = __( 'Zertifikat', 'brainpress' );
			}
			...
			foreach ( $courses as $course ) {
				...
				$completion_status = BrainPress_Data_Student::get_course_status( $course->ID, $student_id );
				$course_completed = BrainPress_Data_Student::is_course_complete( $student_id, $course->ID );
					...
					switch ( $column_key ) {
						...
						case 'date_enrolled':
							...
							if ( empty( $date_enrolled ) ) {
								...
								$date_enrolled = date_i18n( $date_format, BrainPress_Data_Course::strtotime( $date_enrolled ) );
							}
						...
						case 'average':
							...
							if ( in_array( $completion_status, $statuses ) ) {
								...
								$average = BrainPress_Data_Student::average_course_responses( $student_id, $course->ID );
								$average .= '%';
							}
						...
						case 'certificate':
							...
							if ( $course_completed ) {
								$certificate_link = BrainPress_Data_Certificate::get_encoded_url( $course->ID, $student_id );
				...
				$allow_workbook = BrainPress_Data_Course::get_setting( $course->ID, 'allow_workbook' );

				if ( $allow_workbook ) {
					$workbook_url = BrainPress_Data_Student::get_workbook_url( $course->ID );
					$workbook_link = sprintf( '<a href="%s" target="_blank">%s</a>', esc_url( $workbook_url ), __( 'Arbeitsmappe', 'brainpress' ) );
				...
				$withdraw_link = sprintf( '<a href="%s" class="cp-withdraw-student">%s</a>', esc_url( $withdraw_link ), __( 'Zurückziehen', 'brainpress' ) );
	...
	public static function course_edit_avatar_pending_invite( $invite, $remove_buttons = false, $type = 'instructor' ) {
		...
		if ( $remove_buttons ) {
			...
			$content .= sprintf(
				...
				esc_attr( $type ),
				esc_html__( 'Ausstehend', 'brainpress' )
			);
			if ( $remove_buttons ) {
				$content .= '<div class="remove"><a><span class="dashicons dashicons-dismiss"></span></a></div>';
			}
