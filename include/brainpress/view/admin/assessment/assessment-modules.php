<?php
	...
	$course_progress = (int) BrainPress_Helper_Utility::get_array_val( $student_progress, 'completion/average' );
...
<div class="cp-actions">
	<button style="display: none;" type="button" title="<?php esc_attr_e( 'Revalidate user submission', 'brainpress' ); ?>" class="button cp-right cp-refresh-progress" data-course="<?php echo $course_id; ?>" data-student="<?php echo $student_id; ?>">
		<span class="fa fa-refresh"></span> <?php esc_html_e( 'Refresh', 'brainpress' ); ?>
	</button>
	<div class="cp-box">
		<label><?php esc_html_e( 'Select Display', 'brainpress' ); ?></label>
		<select id="grade-type" class="medium">
			<option value="all" <?php selected( 'all', $display_type ); ?>><?php esc_html_e( 'Show all modules', 'brainpress' ); ?></option>
			<option value="all_answered" <?php selected( 'all_answered', $display_type ); ?>><?php esc_html_e( 'Show all answered modules', 'brainpress' ); ?></option>
			<option value="all_assessable" <?php selected( 'all_assessable', $display_type ); ?>><?php esc_html_e( 'Show all assessable modules', 'brainpress' ); ?></option>
		</select>
	</div>
</div>
<div class="cp-content modules-answer-wrapper">
	...
	<div class="cp-responses">
		<?php echo BrainPress_View_Admin_Assessment_List::student_assessment( $student_id, $course_id, $student_progress, false, $assess, $display_type ); ?>
	</div>
</div>