 ...
 * BrainPress Email
 *
 * This class is use as base class for brainpress email setup.
 *
 * @since 2.0
 **/
class BrainPress_Email {
	/**
	...
	public function __construct() {
		...
		add_filter( 'brainpress_default_email_settings', array( $this, 'default_email_settings' ) );
	}
	...
	public function admin_hooks() {
		...
		add_filter( 'brainpress_email_settings_sections', array( $this, 'email_settings_view' ) );
	}
	...
	public function default_email_settings( $all_settings ) {
		$email_fields = wp_parse_args(
			...
			array(
				...
				'subject' => __( 'Betreffzeile hier ...', 'brainpress' ),
				'content' => 'Der Inhalt geht hier ...',
		) );
	...
	public function email_fields() {
		...
		$email_fields = BrainPress_Core::get_setting( 'email/' . $this->email_type, $email_fields );
		 ...
		 **/
		$email_fields = apply_filters( 'brainpress_get_email_fields-' . $this->email_type, $email_fields );
	...
	public function email_settings_view() {
		...
		$block = '<tr><th>%s</th><td>%s<input type="text" class="widefat" name="brainpress_settings[email][%s][%s]" value="%s" /></td></tr>';
		...
		$fields = '
			<tr>
				...
				<td>
					<input type="hidden" name="brainpress_settings[email][' . $this->email_type . '][enabled]" value="0" />
					<input type="checkbox" class="widefat" name="brainpress_settings[email][' . $this->email_type . '][enabled]" value="1" '
		...
		if ( ! empty( $mail_tokens ) ) {
			$content_help_text .= sprintf( '<p class="description"><strong>%s</strong>: <br />%s', __( 'Mail Tokens', 'brainpress' ), implode( ', ', $mail_tokens ) );
			$content_help_text .= '<p class="description">* ' . __( 'Diese Token werden durch tatsächliche Daten ersetzt.', 'brainpress' ) . '</p>'; 
		}

		$fields .= '<tr><th>' . __( 'Email Inhalt', 'brainpress' ) . '</th><td>' . $content_help_text;

		ob_start();
		$editor_settings = array(
			'textarea_name' => 'brainpress_settings[email]['. $this->email_type . '][content]',
			'media_buttons' => false,
			'teeny' => true,
	 ...
	 * Use to send email.
	 *
	 * Note: This method is using BrainPress_Helper_Email class
	 *
	...
	public function send_email( $email_args = array(), $tokens = array() ){
		 ...
		 **/
		$vars = apply_filters( 'brainpress_fields_' . $this->email_type, $tokens );

		$message = BrainPress_Helper_Utility::replace_vars( $message, $vars );

		// Use BrainPress_Helper_Email class
		$email_args = wp_parse_args( $email_args, $email_fields );
		$email_args['message'] = $message;

		BrainPress_Helper_Email::send_email( $this->email_type, $email_args );
	}
}
