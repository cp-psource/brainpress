...
class BrainPress_View_Admin_Assessment_Report {

	public static $slug = 'brainpress_reports';
	private static $title = '';
	private static $menu_title = '';

	public static function init() {
		self::$title = __( 'Reports/BrainPress', 'brainpress' );
		self::$menu_title = __( 'Reports', 'brainpress' );
		...
		add_action( 'brainpress_settings_page_pre_render_' . self::$slug, array( __CLASS__, 'process_form' ) );
	...
	public static function add_page( $pages ) {
		$pages[ self::$slug ] = array(
			...
			'cap' => apply_filters( 'brainpress_capabilities', 'brainpress_reports_cap' ),
		);
	...
	public static function process_form() {
		if ( isset( $_REQUEST['action'] ) && 'brainpress_report' === $_REQUEST['action'] ) {
	...
	public static function report_content( $students, $course_id, $unit_id = 'all' ) {
		...
		foreach ( $students as $student_id ) {
			...
			foreach ( $units as $unit_id => $unit_obj ) {
				...
				foreach ( $unit_obj['pages'] as $page ) {
					...
					foreach ( $page['modules'] as $module_id => $module ) {
						...
						$date_display = false !== $response && isset( $response['date'] ) ? $response['date'] : __( 'Not yet submitted', 'brainpress' );
				...
				if ( empty( $assessable_modules ) ) {
					$html .= '
							<tr style="font-style:oblique; font-size: 4mm; background-color: ' . esc_attr( $colors['item_bg'] ) . '; color: ' . esc_attr( $colors['no_items'] ) . ';">
								<td colspan="3"><em>' . esc_html__( 'No assessable items.', 'brainpress' ) . '</em></td>
							</tr>
						';
				}
			...
			$average_display = ! $course_answered && ! $assessable_modules ? '' : sprintf( __( 'Average response grade: %d%%', 'brainpress' ), $average );
			$course_average = $assessable_modules > 0 ? (int) ( $course_total / $course_assessable_modules ) : 0;
			$course_average_display = ! $assessable_modules ? __( 'No assessable items in this course.', 'brainpress' ) : sprintf( __( 'Total Average: %d%%', 'brainpress' ), $course_average );
		...
		$pdf_args['footer'] = __( 'Course Report', 'brainpress' );
	...
	public static function render_page() {
		$content = '<div class="brainpress_settings_wrapper reports wrap">';
	...
	public static function render_report_list() {
		...
		if ( empty( $courses ) ) {
			return esc_html__( 'You do not currently have any courses assigned.', 'brainpress' );
		}

		$selected_course = isset( $_GET['course_id'] ) ? (int) $_GET['course_id'] : $courses[0]->ID;

		$content .= '<div><strong>' . esc_html__( 'Select Course', 'brainpress' ) . '</strong><br />';
		$content .= BrainPress_Helper_UI::get_course_dropdown( 'course-list', 'course-list', $courses, array(
			'class' => 'medium',
		...
		$content .= '
			<form method="POST">
				...
				<input type="hidden" name="action" value="brainpress_report" />
		';

		/**
		 * Student List
		 */
		$list_course = new BrainPress_Helper_Table_ReportStudent();
		...
		$tooltip = '<span class="help-tooltip">' . esc_html__( 'Select entire course for selected students, or just a unit for selected students.', 'brainpress' ) . '</span>';
		$content .= '<div><strong>' . esc_html__( 'Bulk Reporting', 'brainpress' ) . '</strong>' . $tooltip . '<br />';

		$units = BrainPress_Data_Course::get_units( $selected_course );

		$content .= '
			<select name="bulk-report-unit" class="narrow">
				<option value="all">' . esc_html__( 'All units', 'brainpress' ) . '</option>
		';
		...
		$content .= '
			...
			<input type="submit" class="button button-primary" value="' . esc_attr__( 'Generate Report', 'brainpress' ) . '" name="bulk-report-submit" />
		';
