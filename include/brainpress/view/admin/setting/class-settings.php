...
abstract class BrainPress_View_Admin_Setting_Setting {
	...
	public static function process_form( $page, $tab ) {
		...
		if ( isset( $_POST['action'] ) && 'updateoptions' === $_POST['action'] && wp_verify_nonce( $_POST['_wpnonce'], 'update-brainpress-options' ) ) {
	...
	protected static function page_start( $slug, $tab ) {
		...
		$content .= wp_nonce_field( 'update-brainpress-options', '_wpnonce', true, false );
		return $content;
	}
	...
	protected static function radio( $group, $name, $options, $selected = '' ) {
		...
		foreach ( $options as $key => $data ) {
			$content .= '<li>';
			$content .= sprintf(
				'<label><input type="radio" name="brainpress_settings[%s][%s]" %s %s value="%s" /> %s</label>',
				$group,
				$name,
