<?php
	...
	$students = BrainPress_View_Admin_Assessment_List::filter_students( $selected_course, $current_unit, $type );
	$total = count( $students );
	$students = BrainPress_Data_Course::get_students( $selected_course, $per_page, $offset );
...
<div class="cp-assessment-page" data-nonce="<?php esc_attr_e( $nonce ); ?>">
	<?php if ( empty( $courses ) ): ?>
		<p class="description"><?php esc_html_e( 'You currently have no courses assigned.', 'brainpress' ); ?></p>
	<?php else: ?>
		<div class="cp-course-selector">
			<div class="cp-box">
				<label><?php esc_html_e( 'Select Course', 'brainpress' ); ?></label>
				<?php echo BrainPress_Helper_UI::get_course_dropdown( 'course-list', 'course-list', $courses, array( 'class' => 'medium', 'value' => $selected_course ) ); ?>
			</div>
			<div class="cp-box">
				<select id="unit-list">
					<option value="all"><?php esc_html_e( 'Show all', 'brainpress' ); ?></option>
					<option value="all_submitted" <?php selected( 'all_submitted', $current_unit ); ?>><?php esc_html_e('Show all assessable students', 'brainpress' ); ?></option>
			...
			<div class="cp-box">
				<select id="ungraded-list">
					<option value="all"><?php esc_html_e( 'Show graded and ungraded students', 'brainpress' ); ?></option>
					<option value="ungraded" <?php selected( 'ungraded', $type ); ?>><?php esc_html_e( 'Show ungraded students only', 'brainpress' ); ?></option>
					<option value="graded" <?php selected( 'graded', $type ); ?>><?php esc_html_e( 'Show graded students only', 'brainpress' ); ?></option>
				</select>
			</div>
		</div>
		<table class="wp-list-table widefat fixed striped cp-table">
			<thead>
				<th><?php esc_html_e( 'Student', 'brainpress' ); ?></th>
				<th><?php esc_html_e( 'Last Active', 'brainpress' ); ?></th>
				<th class="unit-grade"><?php esc_html_e( 'Grade', 'brainpress' ); ?></th>
				<th width="10%"><?php esc_html_e( 'Submission', 'brainpress' ); ?></th>
			</thead>
			<tbody id="the-list">
				<?php
					$odd = '';
					...
					foreach ( $students as $student ):
						...
						$student_label = BrainPress_Helper_Utility::get_user_name(
							$student_id,
							true
						);
						...
						$final_grade = (int) BrainPress_Helper_Utility::get_array_val(
							$student_progress,
							'completion/average'
						);
						...
						$course_completed = BrainPress_Helper_Utility::get_array_val(
							$student_progress,
					...
					<tr class="cp-content" data-student="<?php echo $student_id; ?>" style="display: none;">
						<td class="cp-responses cp-inline-responses" colspan="4">
							<script type="text/template" id="student-grade-<?php echo $student_id; ?>">
								<?php echo BrainPress_View_Admin_Assessment_List::student_assessment( $student_id, $selected_course, $student_progress ); ?>
							</script>
						</td>
					</tr>
				<?php endforeach; ?>
		...
		<div class="no-student-info" style="display: none;">
			<p class="description"><?php esc_html_e( '0 students found under this unit', 'brainpress' ); ?></p>
		</div>
		<div class="no-assessable-info" style="display: none;">
			<p class="description"><?php esc_html_e( 'There are no assessable students found!', 'brainpress' ); ?></p>
		</div>
		<?php
		$url = remove_query_arg( array( 'unit', 'type' ) );
		echo BrainPress_Helper_UI::admin_paginate( $paged, $total, $per_page, $url );
		?>
	<?php endif; ?>
</div>