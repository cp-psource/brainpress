...
class BrainPress_View_Admin_Setting_Extensions {
	...
	public static function init() {
		...
		add_action(
			'brainpress_settings_process_extensions',
			array( __CLASS__, 'process_form' ),
			10, 2
		);
		add_filter(
			'brainpress_settings_render_tab_extensions',
	...
	public static function add_tabs( $tabs ) {
		$tabs['extensions'] = array(
			'title' => __( 'Erweiterungen', 'brainpress' ),
			'description' => __( 'Erweiterungen und Plugins zur Verbesserung von BrainPress.', 'brainpress' ),
			'order' => 60,
			'is_form' => false,
			'buttons' => 'none',
		);
	...
	public static function return_content( $content, $slug, $tab ) {
		BrainPress_Helper_Extension::init();
		if ( isset( $_POST['action'] ) && 'install-plugin' === $_POST['action'] && 'extensions' === $tab && wp_verify_nonce( $_POST['_wp_nonce'], 'install-plugin' ) ) {

			ob_start();

			echo '<div class="brainpress_settings_wrapper">' .
				'<h1>' . esc_html( BrainPress::$name ) . ' : ' . esc_html__( 'Plugin installieren ...', 'brainpress' ) . '</h1>
                <hr />';
			echo '</div>';
			echo '<h2>' . esc_html( $_POST['plugin_name'] ) . '</h2>';

			// add_filter( 'brainpress_settings_page_main', array( __CLASS__, 'return_content_plugin_install' ) );
			...
			$content .= add_filter( 'brainpress_settings_page_main', array( __CLASS__, 'return_content_plugin_install' ), $content );
		} else {
        ...
        ' . wp_nonce_field( 'update-brainpress-options', '_wpnonce', true, false );
			...
			$content .= BrainPress_Helper_Extension::plugins_table();
		}
	...
	public static function return_content_plugin_install( $content ) {
		...
		return '<a href="' . $return_url . '">' . esc_html__( 'Kehre zu den BrainPress-Einstellungen zurück.', 'brainpress' ) . '</a>';
	}
