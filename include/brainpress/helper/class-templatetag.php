...
class BrainPress_Helper_TemplateTag {
...
if ( ! function_exists( 'cp_student_login_address' ) ) {
	...
	function cp_student_login_address() {
		if ( get_option( 'use_custom_login_form', 1 ) ) {
			$slug = BrainPress_Core::get_setting( 'slugs/login', 'student-login' );
...
if ( ! function_exists( 'cp_flush_rewrite_rules' ) ) {
	...
	/**
	 * flush_rewrite_rules() wrapper for BrainPress.
	 *
...
if ( ! function_exists( 'cp_can_access_course' ) ) {
	...
	function cp_can_access_course( $course_id ) {
		 ...
		 * check student
		 */
		if ( BrainPress_Data_Student::is_enrolled_in_course( get_current_user_id(), $course_id ) ) {
			return true;
		}
		 ...
		 * check instructor
		 */
		if ( BrainPress_Data_Instructor::is_assigned_to_course( get_current_user_id(), $course_id ) ) {
			return true;
		}
...
if ( !function_exists( 'cp_is_academy_page' ) ) {
	function cp_is_academy_page() {
		...
		/**
		 * custom pages from BrainPress
		 */
		if( is_page() ) {
			$pages = BrainPress_Core::get_setting('pages');
