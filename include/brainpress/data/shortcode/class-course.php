 ...
 */
class BrainPress_Data_Shortcode_Course {
	...
	public static function course_details( $atts ) {
		...
		BrainPress_Data_Student::log_student_activity( 'course_seen' );
	...
	public static function course_title( $atts ) {
		...
		$content .= apply_filters( 'brainpress_schema', $title, 'title' );
		$content .= 'yes' == $link ? '</a>' : '';
	...
	public static function course_summary( $atts ) {
		...
		if ( $course && is_object( $course ) ) {
			if ( is_numeric( $length ) ) {
				$content .= BrainPress_Helper_Utility::truncate_html( do_shortcode( $course->post_excerpt ), $length );
			} else {
	...
	public static function course_description( $atts ) {
		...
		$course_title = apply_filters( 'brainpress_schema', $course->post_title, 'title' );
		$content = '<span style="display:none;">' . $course_title . '</span>';

		/**
		 * schema.org
		 */
		$content_schema = apply_filters( 'brainpress_schema', '', 'description' );
	...
	public static function course_start( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'date_format' => get_option( 'date_format' ),
			'label' => __( 'Kursbeginn', 'brainpress' ),
			'label_tag' => 'strong',
			'label_delimeter' => ': ',
			'class' => '',
		), $atts, 'course_start' ) );
		...
		$course_id = (int) $course_id;
		if ( empty( $course_id ) ) { return ''; }
		$date_format = apply_filters( 'brainpress_course_courses_list_date_format', sanitize_text_field( $date_format ) );
		$time_format = apply_filters( 'brainpress_course_courses_list_time_format', get_option( 'time_format' ) );
		$label = sanitize_text_field( $label );
		$label_tag = sanitize_html_class( $label_tag );
		$label_delimeter = sanitize_text_field( $label_delimeter );
		$class = sanitize_html_class( $class );

		$start_date = BrainPress_Data_Course::get_setting( $course_id, 'course_start_date' );
		$open_ended = BrainPress_Data_Course::get_setting( $course_id, 'course_open_ended' );
		...
		if ( $open_ended || empty( $start_date ) ) {
			$content .= __( 'Bereits gestartet', 'brainpress' );
		} else {
			$content .= str_replace( ' ', '&nbsp;', date_i18n( $date_format, BrainPress_Data_Course::strtotime( $start_date ) ) );
			// Add time if different to '00:00:00'
			$content .= ( date( 'H:i:s', BrainPress_Data_Course::strtotime( $start_date ) ) != '00:00:00' ) ? str_replace( ' ', '&nbsp;', ' / ' . date_i18n( $time_format , BrainPress_Data_Course::strtotime( $start_date ) ) ) : '';
		}
		$content .= '</div>';
	...
	public static function course_length( $atts ) {
		...
		$open_ended = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'course_open_ended' ) );
		if ( $open_ended ) {
			return '';
		}

		$start_date = BrainPress_Data_Course::get_setting( $course_id, 'course_start_date' );
		$end_date = BrainPress_Data_Course::get_setting( $course_id, 'course_end_date' );

		$length = ceil( ( BrainPress_Data_Course::strtotime( $end_date ) - BrainPress_Data_Course::strtotime( $start_date ) ) / 604800 );
	...
	public static function course_end( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'date_format' => get_option( 'date_format' ),
			'label' => __( 'Kursenddatum: ', 'brainpress' ),
			'label_tag' => 'strong',
			'label_delimeter' => ':',
			'no_date_text' => __( 'Kein Enddatum', 'brainpress' ),
			'class' => '',
		), $atts, 'course_end' ) );
		...
		$end_date = BrainPress_Data_Course::get_setting( $course_id, 'course_end_date' );
		$open_ended = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'course_open_ended', false ) );
		...
		$content .= $open_ended ? $no_date_text : str_replace( ' ', '&nbsp;', date_i18n( $date_format, BrainPress_Data_Course::strtotime( $end_date ) ) );
		$content .= '</div>';
	...
	public static function course_dates( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'date_format' => get_option( 'date_format' ),
			'label' => __( 'Kurstermine: ', 'brainpress' ),
			'label_tag' => 'strong',
			'label_delimeter' => ':',
			'no_date_text' => __( 'Kein Enddatum', 'brainpress' ),
			'alt_display_text' => __( 'Unbegrenzt', 'brainpress' ),
			'show_alt_display' => 'no',
			'class' => '',
		), $atts, 'course_dates' ) );
		...
		$end_output = $open_ended ? $no_date_text : str_replace( ' ', '&nbsp;', ( date_i18n( $date_format, BrainPress_Data_Course::strtotime( $end_date ) ) ) );
		...
		if ( $show_alt_display && $open_ended ) {
			...
			$content .= str_replace( ' ', '&nbsp;', date_i18n( $date_format, BrainPress_Data_Course::strtotime( $start_date ) ) ) . ' - ' . $end_output;
		}
	...
	public static function course_enrollment_start( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'date_format' => get_option( 'date_format' ),
			'label' => __( 'Einschreibungsbeginn: ', 'brainpress' ),
			'label_tag' => 'strong',
			'label_delimeter' => ':',
			'no_date_text' => __( 'Jederzeit einschreiben', 'brainpress' ),
			'class' => '',
		...
		$start_date = BrainPress_Data_Course::get_setting( $course_id, 'enrollment_start_date' );
		$start_date = BrainPress_Data_Course::strtotime( $start_date );
		$date = str_replace( ' ', '&nbsp;', date_i18n( $date_format, $start_date ) );
		$open_ended = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'enrollment_open_ended', false ) );
		...
		//$content .= $open_ended ? $no_date_text : str_replace( ' ', '&nbsp;', date_i18n( $date_format, BrainPress_Data_Course::strtotime( $start_date ) ) );
	...
	public static function course_enrollment_end( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'date_format' => get_option( 'date_format' ),
			'label' => __( 'Einschreibungsende: ', 'brainpress' ),
			'label_tag' => 'strong',
			'label_delimeter' => ':',
			'no_date_text' => __( 'Jederzeit einschreiben ', 'brainpress' ),
			'show_all_dates' => 'no',
		...
		$end_date = BrainPress_Data_Course::get_setting( $course_id, 'enrollment_end_date' );
		$open_ended = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'enrollment_open_ended', false ) );
	...
	public static function course_enrollment_dates( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'date_format' => get_option( 'date_format' ),
			'label' => __( 'Einschreibungstermine: ', 'brainpress' ),
			'label_enrolled' => __( 'Du hast Dich eingeschrieben: ', 'brainpress' ),
		...
		if ( 'yes' == strtolower( $show_enrolled_display ) ) {
			$enrollment_date = BrainPress_Data_Course::student_enrolled( get_current_user_id(), $course_id );
			if ( ! empty( $enrollment_date ) ) {
				$enrollment_date = date_i18n( $date_format, BrainPress_Data_Course::strtotime( $enrollment_date ) );
		...
		if ( empty( $enrollment_date ) ) {
			if ( $show_alt_display && $open_ended ) {
				...
				$content .= $open_ended ? $no_date_text : str_replace( ' ', '&nbsp;', date_i18n( $date_format, BrainPress_Data_Course::strtotime( $start_date ) ) ) . ' - ' . str_replace( ' ', '&nbsp;', date_i18n( $date_format, BrainPress_Data_Course::strtotime( $end_date ) ) );
			}
		} else {
			// User is enrolled
	...
	public static function course_enrollment_type( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'anyone_text' => __( 'Jeder', 'brainpress' ),
			'class' => '',
			'label_delimeter' => ':',
			'label_tag' => 'strong',
			'label' => __( 'Wer kann sich einschreiben?: ', 'brainpress' ),
			'manual_text' => __( 'Die Studenten werden von Kursleitern hinzugefügt.', 'brainpress' ),
			'passcode_text' => __( 'Für die Anmeldung ist ein Passcode erforderlich.', 'brainpress' ),
			'prerequisite_text' => __( 'Die Studenten müssen zuerst %s ausfüllen.', 'brainpress' ),
			'registered_text' => __( 'Registrierte Benutzer.', 'brainpress' ),
		...
		$enrollment_type = BrainPress_Data_Course::get_setting( $course_id, 'enrollment_type' );
		...
		switch ( $enrollment_type ) {
			...
			case 'prerequisite':
				$prereq = BrainPress_Data_Course::get_prerequisites( $course_id );
		...
		$enrollment_text = apply_filters( 'brainpress_course_enrollment_type_text', $enrollment_text );
	...
	public static function course_class_size( $atts ) {
		extract( shortcode_atts( array(
			...
			'label_delimeter' => ':',
			'label_tag' => 'strong',
			'label' => __( 'Klassengröße: ', 'brainpress' ),
			'no_limit_text' => __( 'Unlimitiert', 'brainpress' ),
			'remaining_text' => __( '(%d Plätze frei)', 'brainpress' ),
			'show_no_limit' => 'no',
			'show_remaining' => 'yes',
		...
		$is_limited = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'class_limited', false ) );
		$class_size = (int) BrainPress_Data_Course::get_setting( $course_id, 'class_size' );
		...
		if ( $is_limited ) {
			...
			if ( $show_remaining ) {
				$remaining = $class_size - (int) BrainPress_Data_Course::count_students( $course_id );
	...
	public static function course_cost( $atts ) {
		...
		extract( shortcode_atts( array(
			...
			'label_delimeter' => ': ',
			'label_tag' => 'strong',
			'label' => __( 'Preis:&nbsp;', 'brainpress' ),
			'no_cost_text' => __( 'FREI', 'brainpress' ),
			'show_icon' => 'no',
		), $atts, 'course_cost' ) );
		...
		$is_paid = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'payment_paid_course', false ) );

		$content = '';

		if ( $is_paid ) {
			$content .= apply_filters( 'brainpress_shortcode_course_cost', '', $course_id );
		} else {
			if ( $show_icon ) {
	...
	public static function course_language( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'label' => __( 'Kurssprache: ', 'brainpress' ),
			'label_tag' => 'strong',
			'label_delimeter' => ':',
			'class' => '',
	...
	public static function course_category( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'label' => __( 'Kurskategorie: ', 'brainpress' ),
			'label_tag' => 'strong',
			'label_delimeter' => ':',
			'no_category_text' => __( 'Keine', 'brainpress' ),
			'class' => '',
		...
		$categories = BrainPress_Data_Course::get_course_categories( $course_id );
	...
	public static function course_list_image( $atts ) {
		...
		$image_src = BrainPress_Data_Course::get_setting( $course_id, 'listing_image', '' );
		 ...
		 * schema.org
		 */
		$schema = apply_filters( 'brainpress_schema', '', 'image' );
		/**
		 * wrapper start
		 */
		$content = sprintf(
	...
	public static function course_featured_video( $atts ) {
		...
		$video_src = BrainPress_Data_Course::get_setting( $course_id, 'featured_video', '' );
		...
		if ( ! empty( $video_extension ) ) {
			...
			if ( preg_match( '/youtube.com|youtu.be/', $video_src ) ) {
				add_filter(
					'oembed_result',
					array( 'BrainPress_Helper_Utility', 'remove_related_videos' ),
					10,
					3
				);
			}
	...
	public static function course_media( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'class' => '',
			'height' => BrainPress_Core::get_setting( 'course/image_height' ),
			'list_page' => 'no',
			'priority' => '', // Gives priority to video (or image).
			'type' => '', // Default, video, image.
			'width' => BrainPress_Core::get_setting( 'course/image_width' ),
			'wrapper' => '',
		), $atts, 'course_media' ) );
		...
		if ( ! $list_page ) {
			$type = empty( $type ) ? BrainPress_Core::get_setting( 'course/details_media_type', 'default' ) : $type;
			$priority = empty( $priority ) ? BrainPress_Core::get_setting( 'course/details_media_priority', 'video' ) : $priority;
		} else {
			$type = empty( $type ) ? BrainPress_Core::get_setting( 'course/listing_media_type', 'default' ) : $type;
			$priority = empty( $priority ) ? BrainPress_Core::get_setting( 'course/listing_media_priority', 'image' ) : $priority;
		}
		...
		$course_video = BrainPress_Data_Course::get_setting( $course_id, 'featured_video' );
	...
	public static function course_action_links( $atts ) {
		...
		$course_start_date = BrainPress_Data_Course::get_setting( $course_id, 'course_start_date' );
		$course_end_date = BrainPress_Data_Course::get_setting( $course_id, 'course_end_date' );
		$open_ended_course = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'course_open_ended' ) );
		...
		if ( ! empty( $student_id ) && BrainPress_Data_Course::student_enrolled( $student_id, $course_id ) ) {
			if ( ( ( BrainPress_Data_Course::strtotime( $course_start_date ) <= $now && BrainPress_Data_Course::strtotime( $course_end_date ) >= $now ) || ( BrainPress_Data_Course::strtotime( $course_end_date ) >= $now ) ) || $open_ended_course ) {
		...
		if ( $withdraw_link_visible ) {
			$content .= '<a href="' . wp_nonce_url( '?withdraw=' . $course_id, 'withdraw_from_course_' . $course_id, 'course_nonce' ) . '" onClick="return withdraw();">' . esc_html__( 'Zurückziehen', 'brainpress' ) . '</a> | ';
		}
		$content .= '<a href="' . get_permalink( $course_id ) . '">' . esc_html__( 'Kursdetails', 'brainpress' ) . '</a>';

		// Add certificate link.
		if ( CP_IS_PREMIUM ) {
			// COMPLETION LOGIC.
			// $content .= CP_Basic_Certificate::get_certificate_link( get_current_user_id(), $course_id, __( 'Certificate', 'brainpress' ), ' | ' );
		}
	...
	public static function course_time_estimation( $atts ) {
		...
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'label' => __( 'Geschätzte Dauer:&nbsp;', 'brainpress' ),
			'label_tag' => 'strong',
			'label_delimeter' => ': ',
			'wrapper' => 'no',
			'class' => '',
		...
		$content .= BrainPress_Data_Course::get_time_estimation( $course_id );
	...
	public static function get_parent_course_id( $atts ) {
		return BrainPress_Data_Course::get_current_course_id();
	}
}
