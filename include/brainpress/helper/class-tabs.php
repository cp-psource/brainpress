...
class BrainPress_Helper_Tabs {
	...
	public static function render_tabs( $tabs, $tab_content, $hidden_args, $page, $active, $echo = true, $mode = 'vertical', $extra = '', $tab_arg = 'tab' ) {
        ...
        if ( empty( $active ) ) {
            $content .= sprintf(
                '<p>%s</p>',
                __( 'Diese Registerkarte existiert nicht mehr. Bitte wähle eine andere.', 'brainpress' )
            );
        } else {
            ...
            if ( 'both' === $tabs[ $active ]['buttons'] || 'top' === $tabs[ $active ]['buttons'] ) {
                ...
                $content .= apply_filters( 'brainpress_admin_setting_before_top_save', '', $active, $tabs );
                $content .= '<input class="button-primary" type="submit" value="' . esc_attr__( 'Einstellungen speichern', 'brainpress' ) . '" name="submit_settings_header"></p>';
            }
            ...
            if ( 'both' === $tabs[ $active ]['buttons'] || 'bottom' === $tabs[ $active ]['buttons'] ) {
                $content .= '<hr /><p class="section-save-button">
                    <input class="button-primary" type="submit" value="' . esc_attr__( 'Einstellungen speichern', 'brainpress' ) . '" name="submit_settings_section">
                    </p>';
            }
		...
		// }
		if ( $echo ) {
			echo apply_filters( 'brainpress_' . $page . '_tabs_content', $content, $tab_content );
		} else {
			return apply_filters( 'brainpress_' .$page . '_tabs_content', $content, $tab_content );
		}
	}
}
