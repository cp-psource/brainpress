 ...
 */
class BrainPress_Data_Certificate {
		...
		return array(
			...
			'post_args' => array(
				'labels' => array(
					...
					'singular_name' => __( 'Zertifikat', 'brainpress' ),
					'add_new' => __( 'Erstelle neu', 'brainpress' ),
					'add_new_item' => __( 'Neues Zertifikat erstellen', 'brainpress' ),
					'edit_item' => __( 'Zertifikat bearbeiten', 'brainpress' ),
					'edit' => __( 'Bearbeiten', 'brainpress' ),
					'new_item' => __( 'Neues Zertifikat', 'brainpress' ),
					'view_item' => __( 'Zertifikat ansehen', 'brainpress' ),
					'search_items' => __( 'Zertifikate suchen', 'brainpress' ),
					'not_found' => __( 'Keine Zertifikate gefunden', 'brainpress' ),
					'not_found_in_trash' => __( 'Keine Zertifikate im Papierkorb gefunden', 'brainpress' ),
					'view' => __( 'Zertifikat ansehen', 'brainpress' ),
				),
				'public' => false,
				'show_ui' => false,
				'show_in_menu' => false,
	...
	public static function is_enabled() {
		if ( null === self::$is_enabled ) {
			$flag = BrainPress_Core::get_setting(
	...
	public static function send_certificate( $certificate_id ) {
		...
		return BrainPress_Helper_Email::send_email(
	...
	public static function deprecated_get_pdf_file_name( $course_id, $student_id, $add_extension = true ) {
		...
		$pdf_file = BrainPress_Helper_PDF::cache_path() . $filename.'.pdf';
		return $pdf_file;
	}
	...
	public static function get_pdf_file_name( $course_id, $student_id, $basedir = 'include' ) {
		...
		BrainPress_Helper_PDF::check_dir( $subdirectory );
		...
		if ( 'no-base-dir' == $basedir ) {
			...
			$pdf_file = BrainPress_Helper_PDF::cache_path( $subdirectory ) . $filename;
		}
		return $pdf_file;
	}
	...
	protected static function create_certificate( $student_id, $course_id ) {
		...
		wp_update_post(
			apply_filters( 'brainpress_pre_insert_post', $post )
		);
	...
	protected static function fetch_params( $certificate_id ) {
		...
		if ( in_array( $course->post_status, $valid_stati ) ) {
			$course_address = BrainPress_Core::get_slug( 'course/', true ) . $course->post_name . '/';
		} else {
		...
		$params['unit_list'] = BrainPress_Data_Course::get_units_html_list( $course_id );
	...
	protected static function get_certificate_content( $certificate_id ) {
		...
		$content = BrainPress_Core::get_setting(
			'basic_certificate/content'
		);

		// Check if custom certificate is enable
		$course_id = $data['course_id'];
		$is_override = BrainPress_Data_Course::get_setting( $course_id, 'basic_certificate' );

		if ( cp_is_true( $is_override ) ) {
			$content = BrainPress_Data_Course::get_setting( $course_id, 'basic_certificate_layout', $content );
		} else {
			...
			if ( $use_cp_default ) {
				$content = BrainPress_View_Admin_Setting_BasicCertificate::default_certificate_content();
			}
		}
		 ...
		 * @param (array) $vars.
		 **/
		$vars = apply_filters( 'brainpress_basic_certificate_vars', $vars );
	...
	public static function generate_pdf_certificate( $course_id, $student_id = '', $download = false, $post = array() ) {
		...
		if ( count( $post ) > 0 || $is_override ) {
			...
			$settings = BrainPress_Core::get_setting( 'basic_certificate' );
			$background = BrainPress_Helper_Utility::get_array_val( $settings, 'background_image' );
			$orientation = BrainPress_Helper_Utility::get_array_val( $settings, 'orientation' );
			$margins = (array) BrainPress_Helper_Utility::get_array_val( $settings, 'margin' );
			$filename = self::get_pdf_file_name( $course_id, $student_id, 'no-base-dir' );
			$text_color = $text_color = BrainPress_Helper_Utility::convert_hex_color_to_rgb( BrainPress_Core::get_setting( 'basic_certificate/text_color' ), array() );
			$logo = array();
			$logo_image = BrainPress_Helper_Utility::get_array_val( $settings, 'logo_image' );
			if ( ! empty( $logo_image  ) ) {
				$logo_positions = BrainPress_Helper_Utility::get_array_val( $settings, 'logo' );
				$logo  = array(
					'file' => $logo_image,
					'x'	=> $logo_positions['x'],
					'y'	=> $logo_positions['y'],
			 ...
			 */
			if ( $is_override ) {
				$margins = BrainPress_Data_Course::get_setting( $course_id, 'cert_margin', array() );
				...
				$text_color = BrainPress_Helper_Utility::convert_hex_color_to_rgb( BrainPress_Data_Course::get_setting( $course_id, 'cert_text_color' ), $text_color );
				$certificate_logo = BrainPress_Data_Course::get_setting( $course_id, 'certificate_logo', '' );
				if ( ! empty( $certificate_logo ) ) {
					$logo_positions = BrainPress_Data_Course::get_setting( $course_id, 'logo_position', '' );
					$logo  = array(
			 ...
			 **/
			$html = apply_filters( 'brainpress_basic_certificate_html', $html, $course_id, $student_id );
			$certificate_title = apply_filters( 'brainpress_certificate_title', __( 'Abschlusszertifikat', 'brainpress' ) );
			$args = array(
			...
			return BrainPress_Helper_PDF::make_pdf( $html, $args );
		}
		return false;
	}
	...
	public static function get_encoded_url( $course_id, $student_id ) {
		 ...
		 * get by default
		 */
		$file = BrainPress_Data_Certificate::get_pdf_file_name( $course_id, $student_id );
		$url = self::url_prepare( $file, $course_id, $student_id );
		if ( ! empty( $url ) ) {
			return $url;
		}
		 ...
		 *
		 */
		$file = BrainPress_Data_Certificate::deprecated_get_pdf_file_name( $course_id, $student_id );
		$url = self::url_prepare( $file, $course_id, $student_id );
		if ( ! empty( $url ) ) {
			return $url;
		}
	...
	public static function url_prepare( $file, $course_id, $student_id ) {
		...
		return BrainPress_View_Front_Course::download_certificate_link( $course_id, $student_id );
	}
	...
	public static function get_vars( $course_id, $student_id ) {
		...
		if ( $vars['CERTIFICATE_URL'] ) {
			$vars['CERTIFICATE_BUTTON'] = sprintf(
				...
				__( 'Lade Dein Zertifikat herunter', 'brainpress' )
			);
		}
		return $vars;
	}
