...
class BrainPress_Data_Course {
	...
	public static function get_format() {
	    $register_post_type_array = array(
			...
			'post_args' => array(
				'labels' => array(
					'name' => _x( BrainPress::$name, 'post type & admin menu', 'brainpress' ),
					'singular_name' => __( 'Kurs', 'brainpress' ),
					'add_new' => __( 'Neuer Kurs', 'brainpress' ),
					'add_new_item' => __( 'Neuen Kurs erstellen', 'brainpress' ),
					'edit_item' => __( 'Kurs bearbeiten', 'brainpress' ),
					'edit' => __( 'Bearbeiten', 'brainpress' ),
					'new_item' => __( 'Neuer Kurs', 'brainpress' ),
					'view_item' => __( 'Kurs anzeigen', 'brainpress' ),
					'search_items' => __( 'Suche Kurse', 'brainpress' ),
					'not_found' => __( 'Keine Kurse gefunden', 'brainpress' ),
					'not_found_in_trash' => __( 'Keine Kurse im Papierkorb gefunden', 'brainpress' ),
					'view' => __( 'Kurs anzeigen', 'brainpress' ),
					'all_items' => __( 'Kurse', 'brainpress' ),
				),
				'public' => true,
				...
				'capabilities' => array(
					'edit_posts' => 'brainpress_create_course_cap',
					'edit_post' => 'brainpress_update_course_cap',
					'delete_post' => 'brainpress_delete_course_cap',
					'delete_posts' => 'brainpress_delete_course_cap',
					'edit_published_posts' => 'brainpress_update_course_cap',
					'edit_private_posts' => 'brainpress_update_course_cap',
					'edit_others_posts' => 'brainpress_update_course_cap',
					'delete_others_posts' => 'brainpress_delete_course_cap',
					'delete_published_posts' => 'brainpress_delete_course_cap',
					'publish_posts' => 'brainpress_change_course_status_cap',
					'create_posts' => 'brainpress_create_course_cap',
				),
				'query_var' => true,
				'rewrite' => array(
					'slug' => BrainPress_Core::get_slug( 'course' ),
					'with_front' => false,
				),
				'supports' => array( 'slug', 'thumbnail' ),
				'taxonomies' => array( 'course_category' ),
				'supports' => array( 'slug' ),
				'menu_icon' => BrainPress::$url . 'asset/img/brainpress-icon.png',
			),
		);
		/**
		 * check is logged?
		...
		$courses = BrainPress_Data_Instructor::count_courses( $user_id );
		if ( empty( $courses ) ) {
			return $register_post_type_array;
		}
		/**
		 ...
		 */
		$instructor_capabilities = BrainPress_Core::get_setting( 'instructor/capabilities' );
		/**
		 * check has access to courses submenu
		 */
		if ( ! isset( $instructor_capabilities['brainpress_courses_cap'] ) || empty( $instructor_capabilities['brainpress_courses_cap'] ) ) {
		 ...
		 */
		$check_keys = array(
			'brainpress_change_course_status_cap',
			'brainpress_change_my_course_status_cap',
			'brainpress_delete_course_cap',
			'brainpress_delete_course_cap',
			'brainpress_delete_my_course_cap',
			'brainpress_delete_my_course_cap',
			'brainpress_update_course_cap',
			'brainpress_update_my_course_cap',
			'brainpress_view_others_course_cap',
		);
	...
	public static function get_taxonomy() {
		return array(
			...
			'taxonomy_args' => apply_filters(
				'brainpress_register_course_category',
				array(
					'labels' => array(
						'name' => __( 'Kurskategorien', 'brainpress' ),
						'singular_name' => __( 'Kurskategorie', 'brainpress' ),
						'search_items' => __( 'Suche nach Kurskategorien', 'brainpress' ),
						'all_items' => __( 'Alle Kurskategorien', 'brainpress' ),
						'edit_item' => __( 'Kurskategorien bearbeiten', 'brainpress' ),
						'update_item' => __( 'Kurskategorie aktualisieren', 'brainpress' ),
						'add_new_item' => __( 'Neue Kurskategorie hinzufügen', 'brainpress' ),
						'new_item_name' => __( 'Neuer Name der Kurskategorie', 'brainpress' ),
						'menu_name' => __( 'Kurskategorien', 'brainpress' ),
					),
					'hierarchical' => true,
					'sort' => true,
					'args' => array( 'orderby' => 'term_order' ),
					'rewrite' => array(
						'slug' => BrainPress_Core::get_setting(
							'slugs/category',
							'course_category'
						),
					),
					...
					'capabilities' => array(
						'manage_terms' => 'brainpress_course_categories_manage_terms_cap',
						'edit_terms' => 'brainpress_course_categories_edit_terms_cap',
						'delete_terms' => 'brainpress_course_categories_delete_terms_cap',
						'assign_terms' => 'brainpress_courses_cap',
					),
				)
			),
		);
	...
	public static function get_message( $key, $alternate = '' ) {
		...
		return ! empty( self::$messages[ $key ] ) ? BrainPress_Helper_Utility::filter_content( self::$messages[ $key ] ) : BrainPress_Helper_Utility::filter_content( $alternate );
	}

	public static function get_default_messages( $key = '' ) {
		return apply_filters(
			'brainpress_course_messages',
			array(
				'ca' => __( 'Neuer Kurs erfolgreich hinzugefügt!', 'brainpress' ),
				'cu' => __( 'Kurs erfolgreich aktualisiert.', 'brainpress' ),
				'usc' => __( 'Einheitenstatus erfolgreich geändert', 'brainpress' ),
				'ud' => __( 'Einheit erfolgreich gelöscht', 'brainpress' ),
				'ua' => __( 'Neue Einheit erfolgreich hinzugefügt!', 'brainpress' ),
				'uu' => __( 'Einheit erfolgreich aktualisiert.', 'brainpress' ),
				'as' => __( 'Der Student wurde erfolgreich zur Klasse hinzugefügt.', 'brainpress' ),
				'ac' => __( 'Neue Klasse wurde erfolgreich hinzugefügt.', 'brainpress' ),
				'dc' => __( 'Die ausgewählte Klasse wurde erfolgreich gelöscht.', 'brainpress' ),
				'us' => __( 'Der ausgewählte Student wurde erfolgreich vom Kurs zurückgezogen.', 'brainpress' ),
				'usl' => __( 'Ausgewählte Studenten wurden erfolgreich aus dem Kurs zurückgezogen.', 'brainpress' ),
				'is' => __( 'Einladung erfolgreich gesendet.', 'brainpress' ),
				'ia' => __( 'Erfolgreich als Kursleiter hinzugefügt.', 'brainpress' ),
			),
			$key
		);
	}
	...
	public static function update( $course_id, $data ) {
		...
		do_action( 'brainpress_course_pre_update', $course_id, $data );
		...
		$post = array(
			...
			'post_type' => self::get_post_type_name(),
			'meta_input' => array(
				'brainpress_version' => BrainPress::$version,
			),
		);
		...
		if ( ! $new_course ) {
			$post['post_excerpt'] = $course && isset( $data->course_excerpt ) ? BrainPress_Helper_Utility::filter_content( $data->course_excerpt ) : $course->post_excerpt;
			$post['post_content'] = $course && isset( $data->course_description ) ? BrainPress_Helper_Utility::filter_content( $data->course_description ) : $course->post_content;
			$post['post_title'] = $course && isset( $data->course_name ) ? BrainPress_Helper_Utility::filter_content( $data->course_name ) : $course->post_title;
			...
			if ( isset( $data->course_excerpt ) ) {
				$post['post_excerpt'] = BrainPress_Helper_Utility::filter_content( $data->course_excerpt );
			}
			if ( isset( $data->course_description ) ) {
				$post['post_content'] = BrainPress_Helper_Utility::filter_content( $data->course_description );
			}
			if ( isset( $data->course_name ) ) {
				$post['post_title'] = BrainPress_Helper_Utility::filter_content( $data->course_name );
		...
		$course_id = wp_insert_post( apply_filters( 'brainpress_pre_insert_post', $post ) );
		...
		if ( ! empty( $course_id ) ) {
			...
			foreach ( $data as $key => $value ) {
				...
				$value = BrainPress_Helper_Utility::object_to_array( $value );
				...
				if ( preg_match( '/meta_/i', $key ) ) {// every field name with prefix "meta_" will be saved as post meta automatically
					self::set_setting( $settings, str_replace( 'meta_', '', $key ), BrainPress_Helper_Utility::filter_content( $value ) );
				}
				...
				if ( $can_manage_categories && ( 'course_category' == $key || 'meta_course_category' == $key ) ) {
					if ( isset( $data->meta_course_category ) ) {
						self::set_setting(
							$settings,
							'course_category',
							BrainPress_Helper_Utility::filter_content( $value )
						);

						if ( is_array( BrainPress_Helper_Utility::object_to_array( $data->meta_course_category ) ) ) {
				...
				if ( 'meta_listing_image' == $key ) {
					...
					// $course_image_width = BrainPress_Core::get_setting( 'course/image_width', 235 );
					// $course_image_height = BrainPress_Core::get_setting( 'course/image_height', 225 );
					//
					...
					// legacy
					// update_post_meta( $course_id, '_thumbnail_id', BrainPress_Helper_Utility::filter_content( $value ) );
					// } else {
					...
					// legacy
					// update_post_meta( $course_id, '_thumbnail_id', BrainPress_Helper_Utility::filter_content( $new_file_path ) );
					// }
					// } else {
					// legacy
					// update_post_meta( $course_id, '_thumbnail_id', BrainPress_Helper_Utility::filter_content( $value, true ) );
					// }
					// Remove Thumbnail
				...
				if ( 'instructor' == $key ) {
					...
					if ( serialize( array( $value ) ) !== serialize( $old_post_meta ) || 0 == $value ) {
						...
						BrainPress_Helper_Utility::delete_user_meta_by_key( 'course_' . $course_id );
					}
			...
			if ( $new_course ) {
				 ...
				 * @since 1.2.1
				 */
				do_action( 'brainpress_course_created', $course_id, $settings );
			} else {
	...
	public static function add_instructor( $course_id, $instructor_id ) {
		...
		if ( ! in_array( $instructor_id, $instructors ) ) {
			BrainPress_Data_Instructor::added_to_course( $instructor_id, $course_id );
	...
	public static function remove_instructor( $course_id, $instructor_id ) {
		...
		BrainPress_Data_Instructor::removed_from_course( $instructor_id, $course_id );
		/**
	...
	public static function get_setting( $course_id, $key = true, $default = null ) {
		...
		$defaults = array(
			...
			'setup_step_6' => '',
			'setup_step_7' => '',
			'course_language' => __( 'Deutsch', 'brainpress' ),
			'course_view' => 'normal',
			...
			'class_limited' => '',
			'class_size' => '',
			'enrollment_type' => BrainPress_Data_Course::get_enrollment_type_default( $course_id ),
			'payment_paid_course' => false,
			'enrollment_passcode' => '',
			'pre_completion_title' => __( 'Fast fertig!', 'brainpress' ),
			'pre_completion_content' => '',
			'minimum_grade_required' => 100,
			'course_completion_title' => __( 'Glückwunsch, Du hast bestanden!', 'brainpress' ),
			'course_completion_content' => '',
			'course_failed_title' => __( 'Entschuldigung, Du hast diesen Kurs nicht bestanden!', 'brainpress' ),
			'course_failed_content' => '',
			'basic_certificate_layout' => BrainPress_View_Admin_Setting_BasicCertificate::default_certificate_content(),
	...
	public static function update_setting($course_id, $value, $key=true) {
		...
		if ( true === $key ) {
			...
			$settings = BrainPress_Helper_Utility::set_array_value( $settings, $key, $value );
		}
	...
	public static function delete_setting( $course_id, $key = true ) {
		...
		if ( true === $key ) {
			...
			// Replace only one setting
			$settings = BrainPress_Helper_Utility::unset_array_value( $settings, $key );
		}
	...
	public static function get_post_category_name() {
		return BrainPress_Data_PostFormat::prefix( self::$post_taxonomy );
	}
	...
	public static function get_listing_image( $course_id ) {
		$url = BrainPress_Data_Course::get_setting(
			$course_id,
			'listing_image'
		);
		...
		return apply_filters(
			'brainpress_course_listing_image',
			$url,
			$course_id
		);
	}
	...
	public static function get_units_with_modules( $course_id, $status = array( 'publish' ) ) {
		...
		foreach ( $units as $unit ) {
			$items = BrainPress_Helper_Utility::set_array_value( $items, $unit->ID . '/order', get_post_meta( $unit->ID, 'unit_order', true ) );
			$items = BrainPress_Helper_Utility::set_array_value( $items, $unit->ID . '/unit', $unit );
			...
			if ( is_array( $page_titles ) ) {
				...
				foreach ( $page_titles as $page_id => $page_title ) {
					...
					$items = BrainPress_Helper_Utility::set_array_value(
						$items,
	...
	public static function get_units_with_modules3( $course_id, $status = array( 'publish' ) ) {
		...
		$post_args = array(
			'post_type' => array(
				BrainPress_Data_Unit::get_post_type_name(),
				BrainPress_Data_Module::get_post_type_name(),
			),
			'post_parent' => $course_id,
			'posts_per_page' => -1,
			'order' => 'ASC',
		...
		$unit_cpt = BrainPress_Data_Unit::get_post_type_name();
		$module_cpt = BrainPress_Data_Module::get_post_type_name();
		...
		foreach ( $query->posts as $post ) {
			...
			if ( $module_cpt == $post->post_type ) {
				...
				$combine = BrainPress_Helper_Utility::set_array_value( $combine, $path . '/title', $page_title );
				$combine = BrainPress_Helper_Utility::set_array_value( $combine, $path . '/description', $page_description );
				$combine = BrainPress_Helper_Utility::set_array_value( $combine, $path . '/feature_image', $page_image );
				$combine = BrainPress_Helper_Utility::set_array_value( $combine, $path . '/visible', $page_visibility );

				$path = $post->post_parent . '/pages/' . $page . '/modules/' . $post->ID;
				$combine = BrainPress_Helper_Utility::set_array_value( $combine, $path, $post );

				$previous_parent = $post->post_parent;

			} elseif ( $unit_cpt == $post->post_type ) {
				$combine = BrainPress_Helper_Utility::set_array_value( $combine, $post->ID . '/order', get_post_meta( $post->ID, 'unit_order', true ) );
				$combine = BrainPress_Helper_Utility::set_array_value( $combine, $post->ID . '/unit', $post );
			}
		}
	...
	public static function get_unit_modules(
		 ...
		 * sanitize unit_id
		 */
		$is_unit = BrainPress_Data_Unit::is_unit( $unit_id );
		if ( ! $is_unit ) {
			return array();
		}
	...
	public static function is_paid_course( $course_id ) {
		 ...
		 */
		if ( $is_paid && class_exists( 'BrainPress_Helper_Integration_WooCommerce' ) ) {
			if ( class_exists( 'WooCommerce' ) ) {
				$is_paid = BrainPress_Helper_Integration_WooCommerce::$is_active? 'on':'off';
				$is_paid = cp_is_true( $is_paid );
				return $is_paid;
			}
		}
	...
	public static function get_certified_student_ids( $course_id ) {
		...
		if ( ! empty( $student_ids ) ) {
			foreach ( $student_ids as $student_id ) {
				$completed = BrainPress_Data_Student::is_course_complete( $student_id, $course_id );
	...
	public static function enroll_student( $student_id, $course_id, $class = '', $group = '' ) {
		 ...
		 */
		if ( false == apply_filters( 'brainpress_enroll_student', true, $student_id, $course_id ) ) {
			return;
		}
		 ...
		 * @since 1.2.2
		 */
		do_action( 'brainpress_student_enrolled', $student_id, $course_id );

		// Update course count
		BrainPress_Data_Student::count_enrolled_courses_ids( $student_id, true );

		/**
		 * Log student activity
		 */
		BrainPress_Data_Student::log_student_activity( 'enrolled', $student_id );

		// Reset students count
		BrainPress_Data_Instructor::reset_students_count( $instructors );
		return true;
	}
	...
	public static function withdraw_student( $student_id, $course_id ) {
		...
		$meta_keys[] = BrainPress_Helper_EmailAlert::META_NOTICE_PREFIX . 'course_' . $course_id;
		foreach ( $unit_ids as $unit_id ) {
			$meta_keys[] = BrainPress_Helper_EmailAlert::META_NOTICE_PREFIX . 'unit_' . $unit_id;
		}
		 ...
		 */
		$certificate_id = BrainPress_Data_Certificate::get_certificate_id( $student_id, $course_id );
		if ( ! empty( $certificate_id ) ) {
			BrainPress_Data_Certificate::delete_certificate( $certificate_id );
		}
		...
		do_action( 'brainpress_student_withdrawn', $student_id, $course_id );

		// Update student course count
		$enrolled_courses = BrainPress_Data_Student::count_enrolled_courses_ids( $student_id, true );
	...
	public static function send_invitation( $email_data ) {
		...
		BrainPress_Data_Course::set_last_course_id( (int) $email_data['course_id'] );
		 ...
		 **/
		if ( 'passcode' == $type ) {
			$type = BrainPress_Helper_Email::COURSE_INVITATION_PASSWORD;
		} else {
			$type = BrainPress_Helper_Email::COURSE_INVITATION;
		}
	...
	public static function get_time_estimation( $course_id ) {
		 ...
		 */
		return apply_filters( 'brainpress_course_get_time_estimation', $duration, $course_id, $hours, $minutes, $seconds, $total_seconds );
	}
	...
	public static function structure_visibility( $course_id ) {
		if ( ! isset( self::$structure_visibility[ $course_id ] ) || empty( self::$structure_visibility[ $course_id ] ) ) {
			$units = array_filter(
				BrainPress_Data_Course::get_setting(
					$course_id,
			...
			foreach ( array_keys( $pages ) as $key ) {
				...
				if ( in_array( $unit, array_keys( $units ) ) ) {
					$visibility = BrainPress_Helper_Utility::set_array_value(
						$visibility,
						$unit . '/' . $page ,
						true
					);
			...
			foreach ( array_keys( $modules ) as $key ) {
				...
				$is_visible = BrainPress_Helper_Utility::get_array_val(
					$visibility,
					$unit . '/' . $page
				);
	...
	public static function previewability( $course_id ) {
		...
		if ( empty( self::$previewability ) ) {
			...
			foreach ( array_keys( $pages ) as $key ) {
				list( $unit, $page ) = explode( '_', $key );
				$preview_structure = BrainPress_Helper_Utility::set_array_value(
					$preview_structure,
					$unit . '/' . $page,
					true
				);
	...
	public static function can_view_page( $course_id, $unit_id, $page = 1, $student_id = false ) {
		...
		$enrolled = ! empty( $student_id ) ? BrainPress_Data_Course::student_enrolled( $student_id, $course_id ) : false;
		$instructors = array_filter( BrainPress_Data_Course::get_instructors( $course_id ) );
	...
	public static function can_view_module( $course_id, $unit_id, $module_id, $page = 1, $student_id = false ) {
		...
		$enrolled = ! empty( $student_id ) ? BrainPress_Data_Course::student_enrolled( $student_id, $course_id ) : false;
		$instructors = BrainPress_Data_Course::get_instructors( $course_id );
		$is_instructor = in_array( $student_id, $instructors );
	...
	public static function can_view_unit( $course_id, $unit_id, $student_id = false ) {
		...
		$enrolled = ! empty( $student_id ) ? BrainPress_Data_Course::student_enrolled( $student_id, $course_id ) : false;
		$can_update_course = BrainPress_Data_Capabilities::can_update_course( $course_id, $student_id );

		if ( ! $enrolled && ! $can_update_course ) {
			$can_preview = BrainPress_Helper_Utility::get_array_val(
				$preview,
	...
	public static function get_next_accessible_module(
		...
		$can_update_course = BrainPress_Data_Capabilities::can_update_course( $course_id );
		$student_id = get_current_user_id();
		$instructors = array_filter( BrainPress_Data_Course::get_instructors( $course_id ) );
		$is_instructor = in_array( $student_id, $instructors );
		$is_enrolled = BrainPress_Data_Course::student_enrolled( $student_id, $course_id );
		...
		if ( $is_enrolled && $current_module ) {
			$current_module_done = BrainPress_Data_Module::is_module_done_by_student(
				$current_module,
				$student_id
			);
		}
	...
	public static function get_prev_accessible_module(
		...
		$new_sequence = array();
		$valid = true;
		$hide_section = BrainPress_Data_Course::get_setting( $course_id, 'focus_hide_section', false );
	...
	public static function get_course_navigation_items( $course_id ) {
		...
		if ( ! isset( $Items[ $course_id ] ) ) {
			...
			if ( $has_full_access ) {
				...
				$units = BrainPress_Data_Course::get_units_with_modules( $course_id, $statuses );
				...
				foreach ( $units as $unit_id => $unit ) {
					...
					if ( $is_student ) {
						...
						$is_available = BrainPress_Data_Unit::is_unit_available(
							$course_id,
							$unit_id,
							$prev_unit_id
						);
					...
					$unit_link = BrainPress_Data_Unit::get_unit_url( $unit_id );
				...
				$completion_url = $course_link . trailingslashit( BrainPress_Core::get_slug( 'completion' ) );
				...
				$preview_course = BrainPress_Data_Course::get_setting(
					$course_id,
					'structure_preview_modules',
					array()
				);
	...
	public static function get_current_course_id() {
		...
		$course_id = BrainPress_Data_Course::by_name( $coursename, true );
	...
	public static function get_permalink( $course_id ) {
		$base_url = BrainPress_Core::get_slug( 'courses/', true );
	...
	public static function get_course( $course_id = 0 ) {
		...
		$duration = ceil( ( BrainPress_Data_Course::strtotime( $end_date ) - BrainPress_Data_Course::strtotime( $start_date ) ) / 86400 );

		$course->start_date = date_i18n( $date_format, BrainPress_Data_Course::strtotime( $start_date ) );
		$course->end_date = $duration > 0 ? date_i18n( $date_format, BrainPress_Data_Course::strtotime( $end_date ) ) : '--';
		$course->duration = $duration > 0 ? sprintf( _n( '%s Day', '%s Days', $duration, 'brainpress' ), $duration ) : __( 'Unbegrenzt', 'brainpress' );

		// Links
		$course->permalink = self::get_course_url( $course_id );
		$course->edit_link = add_query_arg(
			array(
				'page' => BrainPress_View_Admin_Course_Edit::$slug,
				'id' => $course_id,
				'action' => 'edit',
			),
			admin_url( 'admin.php' )
		...
		$course = apply_filters( 'brainpress_get_course', $course, $course_id );
	...
	static public function duplicate_course( $data ) {
		...
		$the_course = BrainPress_Helper_Utility::object_to_array( $the_course );
		$the_course['post_author'] = get_current_user_id();
		$the_course['comment_count'] = 0;
		if ( apply_filters( 'brainpress_course_duplicated_add_copy', true ) ) {
			$the_course['post_title'] = sprintf(
				_x( '%s Kopie', 'Standardtitel für einen duplizierten Kurs. Variable ist Originaltitel.', 'brainpress' ),
		...
		$course_data = BrainPress_Helper_Utility::object_to_array( BrainPress_Data_Course::get_units_with_modules( $course_id, array(
			'publish',
			'draft',
		) ) );
		$course_data = BrainPress_Helper_Utility::sort_on_key( $course_data, 'order' );
		...
		do_action( 'brainpress_course_duplicated', $new_course_id, $course_id );
	...
	public static function get_course_url( $course_id = 0 ) {
		...
		if ( ! empty( $course_id ) ) {
			...
			$course_url = BrainPress_Core::get_slug( 'course/', true );
	...
	public static function is_course_available( $course_id, $student_id = 0 ) {
		...
		if ( $is_available ) {
			...
			if ( ! $is_open_ended && ! empty( $end_date ) ) {
				...
				if ( false === $is_available ) {
					...
					$is_student = BrainPress_Data_Course::student_enrolled( $student_id, $course_id );
		...
		$is_student = BrainPress_Data_Student::is_enrolled_in_course( $student_id, $course_id );
	...
	public static function get_course_availability_status( $course_id, $user_id = 0 ) {
		...
		if ( ! $is_course_available ) {
			...
			if ( $start_date > $now ) {
				$status = sprintf( __( 'Dieser Kurs wird am %s geöffnet', 'brainpress' ), date_i18n( $date_format, $start_date ) );
			} else {
				...
				if ( ! $is_open_ended && ! empty( $end_date ) ) {
					...
					if ( $end_date < $now ) {
						$status = __( 'Dieser Kurs ist bereits geschlossen.', 'brainpress' );
					}
				}
			}
		}
		...
		if ( ! empty( $status ) ) {
			 ...
			 * @param (int) $course_id
			 **/
			$status = apply_filters( 'brainpress_course_availability_status', $status, $course_id );
		}
	...
	public static function can_access( $course_id, $unit_id = 0, $module_id = 0, $student_id = 0, $page = 1 ) {
		...
		if ( empty( $error_message ) ) {
			if ( ! empty( $unit_id ) ) {
				$previous_unit_id = BrainPress_Data_Unit::get_previous_unit_id( $course_id, $unit_id );
				$is_unit_available = BrainPress_Data_Unit::is_unit_available( $course_id, $unit_id, $previous_unit_id );

				if ( ! $is_unit_available ) {
					$unit_availability_date = BrainPress_Data_Unit::get_unit_availability_date( $unit_id, $course_id );

					if ( ! empty( $unit_availability_date ) ) {
						$error_message = sprintf( __( 'Diese Einheit ist ab %s verfügbar', 'brainpress' ), date_i18n( $date_format, self::strtotime( $unit_availability_date ) ) );
					} else {
				...
				if ( empty( $error_message ) && ! empty( $previous_unit_id ) ) {
					...
					if ( $previous_modules ) {
						foreach ( $previous_modules as $prev_module_index => $_module_id ) {
							$is_done = BrainPress_Data_Module::is_module_done_by_student( $_module_id, $student_id );

							if ( ! $is_done ) {
								$first_line = __( 'Du musst alle ERFORDERLICHEN Module vor dieser Einheit abschließen.', 'brainpress' );
								$error_message = BrainPress_Helper_UI::get_message_required_modules( $first_line );
								continue;
							}
						}
					}
				...
				if ( empty( $error_message ) && $page > 1 ) {
					...
					foreach ( $previous_modules as $prev_module_index => $_module_id ) {
						...
						if ( ! $is_done ) {
							$first_line = __( 'Du musst alle ERFORDERLICHEN Module vor diesem Abschnitt abschließen.', 'brainpress' );
							$error_message = BrainPress_Helper_UI::get_message_required_modules( $first_line );
							continue;
						}
					}
				...
				if ( count( $modules ) ) {
					foreach ( $modules as $module_index => $_module_id ) {
						...
						if ( ! $is_done ) {
							$first_line = __( 'Du musst alle ERFORDERLICHEN Module vor diesem Modul abschließen.', 'brainpress' );
							$error_message = BrainPress_Helper_UI::get_message_required_modules( $first_line );
							continue;
						} else {
							/**
							 ...
							 **/
							$attributes = BrainPress_Data_Module::attributes( $_module_id );
							...
							if ( cp_is_true( $is_assessable ) && cp_is_true( $is_required ) ) {
								...
								$grades = BrainPress_Data_Student::get_grade( $student_id, $course_id, $unit_id, $_module_id );
								$grade = BrainPress_Helper_Utility::get_array_val( $grades, 'grade' );
		 ...
		 * @param (int) $module_id
		 **/
		$error_message = apply_filters( 'brainpress_inaccessable_error_message', $error_message, $course_id, $unit_id, $module_id );
	...
	public static function course_class( $course_id, $user_id = 0 ) {
		...
		$course_image = BrainPress_Data_Course::get_setting( $course_id, 'listing_image' );
		$is_enrolled = false;
		...
		if ( $user_id > 0 ) {
			$is_enrolled = BrainPress_Data_Student::is_enrolled_in_course( $user_id, $course_id );
			$is_enrolled = cp_is_true( $is_enrolled );

			if ( $is_enrolled ) {
				$student_progress = BrainPress_Data_Student::get_completion_data( $user_id, $course_id );
	...
	public static function get_units_html_list( $course_id ) {
		...
		if ( $units ) {
			...
			foreach ( $units as $unit ) {
				$is_unit_available = BrainPress_Data_Unit::is_unit_available( $course_id, $unit->ID, $previous_unit_id );
	...
	public static function current_and_upcoming_courses( $args = array(), $student_id = 0 ) {
		 ...
		 * Order Parameter
		 */
		$selected_dir = BrainPress_Core::get_setting( 'course/order_by_direction', 'ASC' );
		...
		if ( false === $is_admin ) {
			...
			$enrolled_courses = (array) BrainPress_Data_Student::get_enrolled_courses_ids( $student_id );

			if ( ! empty( $enrollment_ended_courses ) ) {
				foreach ( $enrollment_ended_courses as $pos => $post_id ) {
					$is_instructor = BrainPress_Data_Capabilities::can_update_course( $post_id, $student_id );
	...
	public static function get_enrollment_types_array( $course_id = 0 ) {
		$enrollment_types = array(
			'manually' => __( 'Nur manuell hinzugefügt', 'brainpress' ),
		);
		if ( BrainPress_Helper_Utility::users_can_register() ) {
			$enrollment_types = array_merge( $enrollment_types, array(
				'anyone' => __( 'Alle registrierten Benutzer', 'brainpress' ),
				'passcode' => __( 'Alle registrierten Benutzer mit einem Passcode', 'brainpress' ),
				'prerequisite' => __( 'Registrierte Benutzer, die die erforderlichen Kurse abgeschlossen haben', 'brainpress' ),
			) );
		} else {
			$enrollment_types = array_merge( $enrollment_types, array(
				'registered' => __( 'Alle registrierten Benutzer', 'brainpress' ),
				'passcode' => __( 'Alle registrierten Benutzer mit einem Passcode', 'brainpress' ),
				'prerequisite' => __( 'Registrierte Benutzer, die die erforderlichen Kurse abgeschlossen haben', 'brainpress' ),
			) );
		}
		$enrollment_types = apply_filters( 'brainpress_course_enrollment_types', $enrollment_types, $course_id );
	...
	public static function get_enrollment_type_default( $course_id = 0 ) {
		...
		$default = BrainPress_Core::get_setting( 'course/enrollment_type_default', $default );
		return apply_filters( 'brainpress_course_enrollment_type_default', $default, $course_id );
	}
	...
	public static function get_defaults_setup_pages_content() {
		 ...
		 * Pre-Completion Page
		 */
		$defaults['pre_completion']['title'] = __( 'Fast fertig!', 'brainpress' );
		$defaults['pre_completion']['content'] = sprintf( '<h3>%s</h3>', __( 'Du hast den Kurs abgeschlossen!', 'brainpress' ) );
		$defaults['pre_completion']['content'] .= sprintf( '<p>%s</p>', __( 'Deine eingereichte Arbeitsmappe wird überprüft, und Du wirst von mir erfahren, ob Du bestanden oder nicht bestanden hast.', 'brainpress' ) );
		/**
		 * Course Completion Page
		 */
		$defaults['course_completion']['title'] = __( 'Glückwunsch, Du hast bestanden!', 'brainpress' );
		$defaults['course_completion']['content'] = sprintf( '<p>%s</p>', __( 'Woohoo! Du hast COURSE_NAME bestanden!', 'brainpress' ) );

		/**
		 * Course Fail Page
		 */
		$defaults['course_failed']['title'] = __( 'Entschuldigung, Du hast diesen Kurs nicht bestanden!', 'brainpress' );
		$defaults['course_failed']['content'] = __( 'Es tut mir leid zu sagen, dass Du COURSE_NAME nicht bestanden hast. Viel Glück beim nächsten Mal!', 'brainpress' );
		/**
		 * Filter for defaults values allow in easy way change all defaults values.
		 *
		 * @since 2.0.0
		 */
		$defaults = apply_filters( 'brainpress_pages_defaults', $defaults );
		return $defaults;
	}
	...
	public static function get_prerequisites( $course_id ) {
		$courses = BrainPress_Data_Course::get_setting( $course_id, 'enrollment_prerequisite', null );
		if ( empty( $courses ) ) {
			return array();
		}
		if ( ! is_array( $courses ) ) {
	...
	private static function send_enrollment_notification_to_student( $course_id, $student ) {
		...
		self::$email_type = BrainPress_Helper_Email::ENROLLMENT_CONFIRM;
		 ...
		 **/
		$notify_student = apply_filters( 'brainpress_notify_student', true );
	...
	private static function send_enrollment_notification_to_instructors( $course_id, $student ) {
		...
		foreach ( $instructors as $instructor ) {
			 ...
			 * @param (bool) true Set to false to disable notification.
			 **/
			$notify_instructors = apply_filters( 'brainpress_notify_instructors', true );
			...
			if ( is_email( $email_args['email'] ) && $notify_instructors ) {
				BrainPress_Helper_Email::send_email(
					BrainPress_Helper_Email::INSTRUCTOR_ENROLLMENT_NOTIFICATION,
					$email_args
				);
			}
		}
