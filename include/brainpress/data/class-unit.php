...
class BrainPress_Data_Unit {
	...
	public static function init_hooks() {
		 ...
		 * @since 2.0.0
		 */
		add_action( 'brainpress_unit_added', array( __CLASS__, 'show_new_on_list' ), 10, 3 );
	}
	...
	public static function get_format() {
		return array(
			...
			'post_args' => array(
				'labels' => array(
					'name' => __( 'Einheiten', 'brainpress' ),
					'singular_name' => __( 'Einheit', 'brainpress' ),
					'add_new' => __( 'Erstelle neu', 'brainpress' ),
					'add_new_item' => __( 'Neue Einheit erstellen', 'brainpress' ),
					'edit_item' => __( 'Einheit bearbeiten', 'brainpress' ),
					'edit' => __( 'Bearbeiten', 'brainpress' ),
					'new_item' => __( 'Neue Einheit', 'brainpress' ),
					'view_item' => __( 'Einheit anzeigen', 'brainpress' ),
					'search_items' => __( 'Einheiten suchen', 'brainpress' ),
					'not_found' => __( 'Keine Einheiten gefunden', 'brainpress' ),
					'not_found_in_trash' => __( 'Keine Einheiten im Papierkorb gefunden', 'brainpress' ),
					'view' => __( 'Einheit anzeigen', 'brainpress' ),
				),
				'public' => false,
				'show_ui' => false,
	...
	public static function get_time_estimation( $unit_id, $data, $default = '1:00' ) {
		...
		foreach ( $data[ $unit_id ]['pages'] as $page_id => $page ) {
			...
			foreach ( $page['modules'] as $module_id => $module ) {
				$duration = BrainPress_Data_Module::get_time_estimation( $module_id, $default );
				...
				$time = BrainPress_Helper_Utility::get_time( $seconds, $minutes, $hours );
				/**
				 * Increase page time
				 */
			 ...
			 * Page time
			 */
			$time = BrainPress_Helper_Utility::get_time( $page_seconds );
			$estimations = BrainPress_Helper_Utility::set_array_value( $estimations, 'pages/' . $page_id . '/estimation', $time['time'] );
			$estimations = BrainPress_Helper_Utility::set_array_value( $estimations, 'pages/' . $page_id . '/components/hours', $time['hours'] );
			$estimations = BrainPress_Helper_Utility::set_array_value( $estimations, 'pages/' . $page_id . '/components/minutes', $time['minutes'] );
			$estimations = BrainPress_Helper_Utility::set_array_value( $estimations, 'pages/' . $page_id . '/components/seconds', $time['seconds'] );
			/**
			 * Increase unit time
			 */
		 ...
		 * Unit time
		 */
		$time = BrainPress_Helper_Utility::get_time( $unit_seconds );
		$estimations = BrainPress_Helper_Utility::set_array_value( $estimations, 'unit/estimation', $time['time'] );
		$estimations = BrainPress_Helper_Utility::set_array_value( $estimations, 'unit/components/hours', $time['hours'] );
		$estimations = BrainPress_Helper_Utility::set_array_value( $estimations, 'unit/components/minutes', $time['minutes'] );
		$estimations = BrainPress_Helper_Utility::set_array_value( $estimations, 'unit/components/seconds', $time['seconds'] );
		/**
		 ...
		 */
		return apply_filters( 'brainpress_unit_get_time_estimation', $estimations, $unit_id );
	}
	...
	public static function get_unit_availability_status( $course, $unit, $previous_unit = 0 ) {
		...
		if ( $previous_unit_id && $is_available ) {
			...
			$mandatory_done = BrainPress_Data_Student::is_mandatory_done(
				$student_id, $course_id, $previous_unit_id, $student_progress
			);

			$unit_completed = BrainPress_Data_Student::is_unit_complete(
				$student_id, $course_id, $previous_unit_id, $student_progress
			);

			$status = BrainPress_Helper_Utility::set_array_value(
			...
			if ( $is_available && $force_current_unit_successful_completion ) {
				$is_available = BrainPress_Data_Student::unit_answers_are_correct( $student_id, $course_id, $previous_unit );
				BrainPress_Helper_Utility::set_array_value( $status, 'passed_required/enabled', true );
				BrainPress_Helper_Utility::set_array_value( $status, 'passed_required/result', $is_available );
			}
		}
		 ...
		 * @since 1.2.2
		 * */
		do_action( 'brainpress_unit_availble', $is_available, $unit_id );
		...
		$is_available = apply_filters(
			'brainpress_filter_unit_availability',
			$is_available,
			$unit_id
		);
	...
	public static function get_unit_availability_date( $unit_id, $course_id, $date_format = null, $student_id = 0 ) {
		...
		$is_open_ended = BrainPress_Data_Course::get_setting( $course_id, 'course_open_ended' );
		$course_start = BrainPress_Data_Course::get_setting( $course_id, 'course_start_date' );
		$course_end = BrainPress_Data_Course::get_setting( $course_id, 'course_end_date' );
		$start_date = BrainPress_Data_Course::strtotime( $course_start ); // Converts date to UTC.
		$end_date = BrainPress_Data_Course::strtotime( $course_end ); // Converts date to UTC.
		...
		if ( 'after_delay' == $status_type ) {
			...
			if ( $delay_days > 0 ) {
				...
				$base_date = BrainPress_Data_Course::strtotime( $course_start ); // UTC value.
			...
			$due_date = BrainPress_Data_Course::strtotime( $due_on ); // UTC value.
	...
	public static function get_previous_unit_id( $course_id, $unit_id ) {
		...
		$units = BrainPress_Data_Course::get_units( $course_id, $status );
		$valid = true;
	...
	public static function get_instructors( $unit_id, $objects = false ) {
		...
		return BrainPress_Data_Course::get_instructors( $course_id, $objects );
	}
	...
	public static function is_unit_structure_visible( $course_id, $unit_id, $user_id = 0 ) {
		...
		$can_update_course = BrainPress_Data_Capabilities::can_update_course( $course_id, $user_id );
		...
		$structure_visibility = BrainPress_Data_Course::structure_visibility( $course_id );
	...
	public static function is_page_structure_visible( $course_id, $unit_id, $page_number, $user_id = 0 ) {
		...
		$page_modules = BrainPress_Helper_Utility::get_array_val(
			$visibility,
	...
	public static function show_new_on_list( $unit_id, $course_id ) {
		...
		BrainPress_Data_Course::update_setting( $course_id, 'structure_visible_units', $visible_units );
	...
	public static function show_page( $unit_id, $page_id, $course_id = false ) {
		...
		$visible_pages[ $id ] = 1;
		BrainPress_Data_Course::update_setting( $course_id, 'structure_visible_pages', $visible_pages );
	}
	...
	public static function delete_section( $unit_id, $page_number ) {
		 ...
		 */
		BrainPress_Data_Module::move_to_first_page( $unit_id, $page_number );
		/**
		 * move modules one page down
		 */
		BrainPress_Data_Module::decrease_page_number( $unit_id, $page_number );
		/**
