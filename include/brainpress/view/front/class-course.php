 ...
 */
class BrainPress_View_Front_Course {
	...
	/**
	 * Custom template file to load when the brainpress theme is used.
	 *
	 * @var string
	 */
	...
	public static function init() {
		...
		add_action(
			'wp_enqueue_scripts',
			array( __CLASS__, 'brainpress_front_css' )
		);
		...
		BrainPress_View_Front_EnrollmentPopup::init();

		/**
		 * BrainPress discussion
		 */
		BrainPress_Helper_Discussion::init();
	...
	public static function init_admin() {
		...
		BrainPress_View_Front_EnrollmentPopup::init_admin();
	...
	public static function maybe_save_discussion() {
		...
		$content = BrainPress_Helper_Utility::filter_content(
			$_POST['discussion_content']
		);
		$title = BrainPress_Helper_Utility::filter_content(
		...
		$args = array(
			...
			'post_type' => BrainPress_Data_Discussion::get_post_type_name(),
			'post_status' => 'publish',
		...
		$url = BrainPress_Core::get_slug( 'course/', true ) .
			get_post_field( 'post_name', $course_id ) . '/' .
			BrainPress_Core::get_slug( 'discussions/' );
	...
	public static function handle_module_uploads() {
		...
		if ( ! $course_id && ! $unit_id && ! $module_id ) {
			...
			$json_data['response'] = __( 'Ungültige Daten übermittelt', 'brainpress' );
			...
			if ( $student_id ) {
				$student_progress = BrainPress_Data_Student::get_completion_data(
					$student_id,
					$course_id
				);
			}
			...
			if ( $can_retry ) {
				...
				if ( ! $is_enabled ) {
					$json_data['response'] = __( 'Maximal zulässige Wiederholungsversuche überschritten.', 'brainpress' );
					$json_data['success'] = false;
					$process_file = false;
				}
			}
		...
		if ( $process_file ) {
			...
			$upload_overrides = array(
				'test_form' => false,
				'mimes' => BrainPress_Helper_Utility::allowed_student_mimes(),
			);
			...
			if ( isset( $_FILES ) ) {
				foreach ( $_FILES as $file ) {
					...
					try {
						...
						if ( isset( $response['error'] ) ) {
							...
							// Record the response
							if ( $student_id ) {
								BrainPress_Data_Student::module_response(
									$student_id,
									$course_id,
									$unit_id,
									$module_id,
				...
				$json_data['response'] = __( 'Keine Dateien hochgeladen.', 'brainpress' );
				$json_data['success'] = false;
			}
		}
		...
		if ( $ajax ) {
			...
			$json_data['html'] = BrainPress_Data_Shortcode_Template::brainpress_focus_item( $attr );
	...
	public static function render_course_main( $course_id = 0 ) {
		...
		if ( $theme_file ) {
			...
			if ( $course_post && BrainPress_Admin_Courses::_is_course( $course_post ) ) {
				$content = apply_filters( 'the_content', $course_post->post_content );
			}
		} else {
			$content = BrainPress_Template_Course::course();
		}
		return $content;
	}
	...
	public static function render_course_unit( $post_id ) {
		...
		if ( $theme_file ) {
			...
			$content = BrainPress_Template_Unit::unit_with_modules();
		}
	...
	public static function render_course_unit_archive() {
		...
		if ( $theme_file ) {
			...
			$content = BrainPress_Template_Unit::unit_archive();
		}
	...
	public static function render_course_archive() {
		...
		if ( $theme_file ) {
			...
			$content = BrainPress_Template_Course::course_archive();
		}
	...
	public static function render_course_discussion() {
		...
		if ( $theme_file ) {
			...
			$content = BrainPress_Template_Communication::render_discussion();
		}
	...
	public static function render_new_course_discussion() {
		...
		if ( $theme_file ) {
			...
			$content = BrainPress_Template_Communication::render_new_discussion();
		}
	...
	public static function render_course_discussion_archive() {
		...
		if ( $theme_file ) {
			...
			$content = BrainPress_Template_Communication::render_discussions();
		}
	...
	public static function render_course_grades_archive() {
		...
		if ( $theme_file ) {
			...
			$content = BrainPress_Template_Grades::render();
		}
	...
	public static function render_course_workbook() {
		...
		if ( $theme_file ) {
			...
			$content = BrainPress_Template_Workbook::render_workbook();
		}
	...
	public static function render_course_notifications_archive() {
		...
		if ( $theme_file ) {
			...
			$content = BrainPress_Template_Communication::render_notifications();
		}
	...
	public static function remove_canonical( $wp_query ) {
		...
		// Is the user on a BrainPress page?
	...
	public static function no_access_redirect( $course_id ) {
		$course_url = BrainPress_Data_Course::get_permalink( $course_id );
		 ...
		 **/
		do_action( 'brainpress_no_access_redirect', $course_url, $course_id );
		...
		if ( is_user_logged_in() ) {
			$course_url = BrainPress_Helper_Message::add_message_query_arg( $course_url, 'only-enroled' );
		} else {
			$course_url = BrainPress_Helper_Message::add_message_query_arg( $course_url, 'no-access' );
		}
		 ...
		 * @param (int) $course_id
		 **/
		$course_url = apply_filters( 'brainpress_no_access_redirect_url', $course_url, $course_id );
	...
	public static function archive_redirect() {
		$archive_url = BrainPress_Core::get_slug( 'courses/', true );
	 ...
	 * BrainPress-template (theme) or VirtualPage to display.
	 *
	 * @since  2.0.0
	...
	public static function parse_request( $wp ) {
		...
		BrainPress_Helper_Utility::$is_singular = false;
		BrainPress_Helper_Utility::set_the_course_subpage( '' );
		$is_other_cp_page = false;
		$is_focus = false;

		if ( ! empty( $wp->query_vars['coursename'] ) ) {
			$course_name = $wp->query_vars['coursename'];
			$cp->course_id = BrainPress_Data_Course::by_name( $cp->cp_course, true );
			$mode = get_post_meta( $cp->course_id, 'cp_course_view', true );
			$is_focus = 'focus' == $mode;
		}

		if ( array_key_exists( 'brainpress_focus', $wp->query_vars ) ) {
			$cp->is_focus = (1 == $wp->query_vars['brainpress_focus']);
		}
		...
		if ( $cp->is_focus ) {
			...
			$shortcode = sprintf(
				'[brainpress_focus_item course="%d" unit="%d" type="%s" item_id="%d"]',
				$cp->course_id,
				$unit_id,
				$type,
				$item_id
		...
		if ( $cp->is_course && BrainPress_Core::get_slug( 'category' ) == $cp->cp_course ) {
		...
		if ( $cp->cp_course ) {
			$cp->course_id = BrainPress_Data_Course::by_name( $cp->cp_course, true );
			$cp->can_preview = BrainPress_Data_Capabilities::can_update_course( $cp->course_id );
			...
			if ( isset( $_REQUEST['action'] ) && isset( $_REQUEST['_wpnonce'] ) ) {
				if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'enroll_student' ) ) {
					...
					$type = BrainPress_Data_Course::get_setting( $cp->course_id, 'enrollment_type' );
					//Enroll user only if passcode not required for course
					if ( 'passcode' != $type ) {
						BrainPress_Data_Course::enroll_student( $user_id, $cp->course_id );
					}
				}
			}
		...
		if ( $cp->student_id && $cp->course_id ) {
			$cp->is_enrolled = BrainPress_Data_Course::student_enrolled(
				$cp->student_id,
				$cp->course_id
			);
			...
			$cp->is_instructor = in_array(
				$cp->student_id,
				BrainPress_Data_Course::get_instructors( $cp->course_id )
			);
		}
		...
		if ( $cp->is_unit ) {
			...
			$is_cp = (0 == strpos( $wp->request, BrainPress_Core::get_slug( 'courses' ) ) );

			if ( $is_cp && BrainPress_Data_Course::get_post_type_name() == $post_type ) {
		...
		if ( isset( $wp->query_vars['paged'] ) ) {
			...
			BrainPress_Helper_Utility::set_the_pagination( $cp->pagination );
		}

		// We have a course-slug, find the ID.
		if ( $cp->course_id ) {
			$cp->can_preview = BrainPress_Data_Capabilities::can_update_course( $cp->course_id );
			//$preview = BrainPress_Data_Course::previewability( $cp->course_id );
			//$cp->can_preview = $preview['has_previews'];

			$cp->title = sprintf(
				'%s | %s',
				__( 'Course', 'brainpress' ),
				get_post_field( 'post_title', $cp->course_id )
			);

			BrainPress_Helper_Utility::set_the_course( $cp->course_id );
		}

		// ---------------------------------------------------------------------
		if ( $cp->is_course ) {
			// This is a single course page!
			BrainPress_Helper_Utility::$is_singular = true;

			$user_id = get_current_user_id();
			$can_update_course = BrainPress_Data_Capabilities::can_update_course( $cp->course_id );
			$course_url = BrainPress_Data_Course::get_course_url( $cp->course_id );

			// Redirect user to units overview
			if ( false === $can_update_course && BrainPress_Data_Course::student_enrolled( $user_id, $cp->course_id ) ) {
				$units_overview = $course_url . BrainPress_Core::get_slug( 'unit/' );
			 ...
			 **/
			$show_title = apply_filters( 'brainpress_single_show_title', true, $cp->course_id );
			$theme_file = locate_template( array( 'single-course.php' ) );
			...
			$cp->vp_args = array(
				...
				'callback' => array( 'BrainPress_Template_Course', 'course' ),
				'context' => 'main',
				'content' => '',
				'type' => BrainPress_Data_Course::get_post_type_name(),
				'is_singular' => true,
				'ID' => $cp->course_id,
			);
			// -----------------------------------------------------------------
			...
			if ( $cp->is_enrolled ) {
				$student_progress = BrainPress_Data_Student::get_completion_data( $cp->student_id, $cp->course_id );
				BrainPress_Data_Student::get_calculated_completion_data( $cp->student_id, $cp->course_id, $student_progress );
				$is_failed = BrainPress_Helper_Utility::get_array_val(
					$student_progress,
					'completion/failed'
				);
				...
				if ( cp_is_true( $is_failed ) ) {
					...
					$is_course_completed = BrainPress_Data_Student::is_course_complete( $cp->course_id, $cp->student_id, $student_progress );
				...
				$page_title = BrainPress_Data_Course::get_setting( $cp->course_id, $title_slug );
				$page_content = BrainPress_Data_Course::get_setting( $cp->course_id, $content_slug );
				...
				$tokens = array(
					...
					'COURSE_SUB_TITLE' => BrainPress_Data_Course::get_setting( $cp->course_id, 'course_subtitle' ),
					...
					'DOWNLOAD_CERTIFICATE_BUTTON' => sprintf( '<a href="%s" class="download-certificate-button">%s</a>', esc_url( self::download_certificate_link( $cp->course_id ) ), __( 'Zertifikat herunterladen', 'brainpress' ) ),
					'STUDENT_WORKBOOK' => $workbook,
				);

				$page_content = BrainPress_Helper_Utility::replace_vars( $page_content, $tokens );
			...
			BrainPress_Helper_Utility::set_the_course_category( $cp->cp_category );

			$course_taxonomy = BrainPress_Data_Course::get_taxonomy();
			$tax = get_term_by(
				'slug',
				$cp->cp_category,
			...
			if ( $tax ) {
				$cp->title = sprintf(
					'%s %s',
					__( 'Kurse in', 'brainpress' ),
					$tax->name
				);
			} elseif ( 'all' === $cp->cp_category ) {
				$cp->title = __( 'Alle Kurse', 'brainpress' );
			} else {
			...
			$cp->vp_args = apply_filters(
				'brainpress_category_page_args',
				array(
					'slug' => 'course_archive',
					'title' => $cp->title,
					'show_title' => true,
					'content' => '',
					'callback' => array( 'BrainPress_Template_Course', 'course_archive' ),
			...
			BrainPress_Helper_Utility::set_the_course_subpage( 'discussions' );
			...
			if ( $discussion ) {
				$comment_status = 'open';
				BrainPress_Data_Discussion::$last_discussion = $discussion->ID;
			} else {
				$comment_status = 'closed';
				BrainPress_Data_Discussion::$last_discussion = '';
			}

			$cp->title = sprintf(
				'%s | %s',
				__( 'Diskussionen', 'brainpress' ),
				get_post_field( 'post_title', $cp->course_id )
			);
			...
			BrainPress_Helper_Utility::set_the_course_subpage( 'grades' );

			$cp->title = sprintf(
				'%s | %s',
				__( 'Bewertungen', 'brainpress' ),
				get_post_field( 'post_title', $cp->course_id )
			);
			...
			BrainPress_Helper_Utility::set_the_course_subpage( 'workbook' );

			$cp->title = sprintf(
				'%s | %s',
				__( 'Arbeitsmappe', 'brainpress' ),
				get_post_field( 'post_title', $cp->course_id )
			);
			...
			BrainPress_Helper_Utility::set_the_course_subpage( 'notifications' );

			$cp->title = sprintf(
				'%s | %s',
				__( 'Benachrichtigungen', 'brainpress' ),
				get_post_field( 'post_title', $cp->course_id )
			);
			...
			BrainPress_Helper_Utility::set_the_course_subpage( 'units' );
			...
			$cp->vp_args = array(
				...
				'title' => get_the_title( $cp->course_id ),
				'content' => '',
				'callback' => array( 'BrainPress_Template_Unit', 'unit_archive' ),
				'context' => 'units',
				'ID' => $cp->course_id,
				'type' => BrainPress_Data_Unit::get_post_type_name() . '_archive',
				'is_singular' => true,
			);
			// -----------------------------------------------------------------
			...
			if ( ! $cp->is_enrolled && ! $cp->can_preview && ! $cp->is_instructor ) {
				...
				$view_mode = BrainPress_Data_Course::get_setting( $cp->course_id, 'course_view', 'normal' );
				/**
				 * check free preview
				 */
				if ( isset( $wp->query_vars['module_id'] ) ) {
					if ( 'focus' == $view_mode ) {
						...
						$can_be_previewed = BrainPress_Data_Module::can_be_previewed( $module_id );
					}
				} else {
					$unit_id = BrainPress_Data_Unit::by_name(
						$wp->query_vars['unitname'],
						true,
						$cp->course_id
					);
					$can_be_previewed = BrainPress_Data_Unit::can_be_previewed( $unit_id );
				}
				if ( ! $can_be_previewed ) {
					self::no_access_redirect( $cp->course_id );
				}
			...
			$post_id = BrainPress_Data_Unit::by_name(
				$wp->query_vars['unitname'],
				true,
				$cp->course_id
			);
			...
			BrainPress_Helper_Utility::set_the_post( $post_id );
			...
			$cp->vp_args = array(
				...
				'title' => get_the_title( $cp->course_id ),
				'content' => apply_filters(
					'brainpress_view_course_unit',
					self::render_course_unit( $post_id ),
					$cp->course_id,
					$post_id
				),
				'type' => BrainPress_Data_Unit::get_post_type_name(),
			...
			$cp->vp_args = array(
				...
				'show_title' => false,
				'content' => apply_filters(
					'brainpress_view_course_archive',
		 ...
		 * @param (object) $cp.
		 **/
		$cp->vp_args = apply_filters( 'brainpress_virtual_page', $cp->vp_args, $cp );
		...
		if ( $cp->vp_args ) {
			...
			$pg = new BrainPress_Data_VirtualPage( $cp->vp_args );
			self::$title = $cp->title;
	...
	public static function get_valid_post_types() {
		$pt_course = BrainPress_Data_Course::get_post_type_name();
		$pt_unit = BrainPress_Data_Unit::get_post_type_name();
	...
	public static function brainpress_front_css() {
		...
		if ( in_array( $post_type, $valid_types ) ) {
			$style = BrainPress::$url . 'asset/css/brainpress_front.css';
			//wp_enqueue_style( 'brainpress_general', $style, array( 'dashicons' ), BrainPress::$version );

			$style = BrainPress::$url . 'asset/css/bbm.modal.css';
			//wp_enqueue_style( 'brainpress_bbm_modal', $style, array(), BrainPress::$version );
		}
	}

	// Some themes think having an author bio makes it ok to display it... not for BrainPress.
	...
	public static function process_course_ajax() {
		...
		switch ( $action ) {
			case 'record_module_response':
				...
				if ( BrainPress_Data_Course::get_course_status( $course_id ) == 'closed' ) {
					$json_data['message'] = __( 'Dieser Kurs ist abgeschlossen, Du kannst keine Antworten mehr einreichen.', 'brainpress' );
					wp_send_json_error( $json_data );
				}

				BrainPress_Data_Student::module_response( $student_id, $course_id, $unit_id, $module_id, $response );

				$data = BrainPress_Helper_Utility::object_to_array( $data );

				if ( 'input-quiz' == $module_type ) {

					$quiz_result = BrainPress_Data_Module::get_quiz_results( $student_id, $course_id, $unit_id, $module_id );
					$json_data['quiz_result_screen'] = BrainPress_Data_Module::quiz_result_content( $student_id, $course_id, $unit_id, $module_id, $quiz_result );
					$json_data['results'] = $quiz_result;

				}

				// Check if it is the last unit
				$units = BrainPress_Data_Course::get_units( $course_id, array( 'publish' ) );
				if ( $units ) {
					$last_unit = array_pop( $units );

					if ( ! empty( $last_unit->ID ) && $last_unit->ID == $unit_id ) {
						// Check if it is the last module
						$modules = BrainPress_Data_Course::get_unit_modules( $unit_id );
						...
						if ( ! empty( $last_module->ID ) && $last_module->ID == $module_id ) {
							...
							BrainPress_Data_Student::get_calculated_completion_data( $student_id, $course_id, $student_progress );
			...
			case 'calculate_completion':
				...
				if ( $student_id > 0 && $course_id > 0 ) {
					...
					BrainPress_Debugger::log( sprintf( 'Kursfortschritt über Ajax ID aktualisiert: %d, STUDENT: %d', $course_id, $student_id ) );
				}
			...
			case 'comment_add_new':
				$json_data = BrainPress_Data_Discussion::comment_add_new( $data, $json_data );
				$success = true;
		}

		if ( $success ) {
			BrainPress_Data_Student::log_student_activity( 'module_answered', $json_data['student_id'] );
	...
	public static function permalink( $permalink, $post, $leavename ) {
		switch ( $post->post_type ) {
			...
			case BrainPress_Data_Discussion::get_post_type_name():

				$course_id = (int) get_post_meta( $post->ID, 'course_id', true );
				$course_id = ! empty( $course_id ) ? $course_id : BrainPress_Helper_Utility::the_course( true );

				if ( ! empty( $course_id ) ) {
					$course = get_post( $course_id );
					$discussion_url = BrainPress_Core::get_slug( 'courses/', true ) . $course->post_name . '/';
					$permalink = $discussion_url . BrainPress_Core::get_slug( 'discussion/' ) . $post->post_name;
				} else {
					return '';
				}
				break;

			case BrainPress_Data_Notification::get_post_type_name():
				break;

			case BrainPress_Data_Unit::get_post_type_name():
				break;
	...
	public static function discussion_page_num_link( $result ) {
		...
		$discussion_url = BrainPress_Core::get_slug( 'courses/', true ) . get_post_field( 'post_name', $course_id ) . '/';
		$discussion_url = $discussion_url . BrainPress_Core::get_slug( 'discussion/' ) . get_post_field( 'post_name', self::$discussion );
	...
	public static function add_discussion_comment_meta( $comment_id ) {
		if ( ! empty( BrainPress_Data_Discussion::$last_discussion ) ) {
			add_comment_meta( $comment_id, 'context', BrainPress_Data_Discussion::get_post_type_name() );
		}
	}
	...
	public static function temp_update_discussion_comments( $comments, $post_id ) {
		$discussion_type = BrainPress_Data_Discussion::get_post_type_name();
	...
	public static function add_edit_to_admin_bar_menu( $bar ) {
		...
		switch ( $post->post_type ) {
			...
			break;
			case 'unit':
				$course_id = BrainPress_Data_Unit::get_course_id_by_unit( $post );
				$tab = 'units';
			break;
		}
		if ( empty( $course_id ) ) {
	...
	public static function download_certificate_link( $course_id, $student_id = 0 ) {
		...
		$download_link = add_query_arg(
			array(
				...
				'action' => 'certificate',
				'nonce' => wp_create_nonce( 'brainpress_download_certificate' ),
			),
			site_url( '/' )
		);
	...
	public static function download_certificate() {
		if ( isset( $_REQUEST['action'] ) && 'certificate' == $_REQUEST['action']
			&& ! empty( $_REQUEST['nonce'] ) && wp_verify_nonce( $_REQUEST['nonce'], 'brainpress_download_certificate' ) && isset( $_REQUEST['course_id'] ) ) {
			$course_id = (int) $_REQUEST['course_id'];
			...
			if ( $student_id != $current_user_id ) {
				if ( ! BrainPress_Data_Capabilities::can_update_course( $course_id ) ) {
					_e( 'Cheatin&#8217; uh?', 'brainpress' );
					exit;
				}
			}
			BrainPress_Data_Certificate::generate_pdf_certificate( $course_id, $student_id, true );
			exit;
		}
	}
}
