...
/**
 * Helper functions.
 * Integrate other plugins with BrainPress.
 *
 * Note: This file is only loaded, if the Woo-Integration is enabled in
 * BrainPress - so we do not need to check if setting 'woocommerce/use' is true.
 *
 * @package  BrainPress
 */

/**
 * Integrates WooCommerce with BrainPress.
 */
class BrainPress_Helper_Integration_WooCommerce {
	...
	public static function init() {
		/**
		 * Always setup _brainpress javasctipt object.
		 */
		add_filter(
			'brainpress_localize_object',
		...
		self::$is_active = true;
		add_filter( 'brainpress_is_woocommerce_active', '__return_true' );
		add_filter( 'brainpress_payment_supported', array( __CLASS__, 'is_payment_supported' ), 10, 2 );
		// Add additional fields to Course Setup Step 6 if paid is checked
		add_filter(
			'brainpress_course_setup_step_6_paid',
		 ...
		 */
		add_filter(
			'brainpress_is_user_purchased_course',
			array( __CLASS__, 'is_user_purchased_course' ),
			10, 3
		);
		add_action(
			'brainpress_course_updated',
		...
		add_filter(
			'brainpress_shortcode_course_cost',
		 ...
		 */
		add_action(
			'brainpress_registration_form_end',
			array( __CLASS__, 'add_to_cart_template' ),
			10, 1
		);
		/** This filter is documented in include/brainpress/data/class-course.php */
		add_filter(
			'brainpress_enroll_student',
		...
		/**
		 * WooCommerce payment complete -> BrainPress enroll student.
		 */
	...
	public static function change_order_status( $order_id, $old_status, $new_status ) {
		...
		self::remove_filter_brainpress_enroll_student();
		...
		foreach ( $items as $item ) {
			...
			if ( 'completed' === $new_status ) {
				BrainPress_Data_Course::enroll_student( $user_id, $course_id );
			}
		}
	}

	public static function is_payment_supported( $payment_supported, $course_id ) {
		return BrainPress_Core::get_setting( 'woocommerce/enabled', false );
	}
	...
	public static function product_settings( $content, $course_id ) {
		...
		$mp_content = '
			<div class="wide">
				<label>' .
					esc_html__( 'WooCommerce Produkt Einstellungen', 'brainpress' ) .
					'</label>
				<p class="description">' . esc_html__( 'Dein Kurs wird ein neues Produkt in WooCommerce sein. Gib unten die Zahlungseinstellungen Deines Kurses ein.', 'brainpress' ) . '</p>
				<label class="normal required">
					' . esc_html__( 'Voller Preis', 'brainpress' ) . '
				</label>
				<input type="text" name="meta_mp_product_price" value="' . BrainPress_Data_Course::get_setting( $course_id, 'mp_product_price', '' ) . '" />


				<label class="normal">
					' . esc_html__( 'Angebotspreis', 'brainpress' ) . '
				</label>
				<input type="text" name="meta_mp_product_sale_price" value="' . BrainPress_Data_Course::get_setting( $course_id, 'mp_product_sale_price', '' ) . '" /><br >

				<label class="checkbox narrow">
					<input type="checkbox" name="meta_mp_sale_price_enabled" ' . BrainPress_Helper_Utility::checked( BrainPress_Data_Course::get_setting( $course_id, 'mp_sale_price_enabled', false ) ) . ' />
					<span>' . esc_html__( 'Aktiviere Angebotspreis', 'brainpress' ) . '</span>
				</label>

				<label class="normal">
					<span> ' . esc_html__( 'Kurs SKU:', 'brainpress' ) . '</span>
				</label>
				<input type="text" name="meta_mp_sku" placeholder="' . sprintf( __( 'z.B. %s0001', 'brainpress' ), apply_filters( 'brainpress_course_sku_prefix', 'CP-' ) ) . '" value="' . BrainPress_Data_Course::get_setting( $course_id, 'woo/sku', '' ) . '" /><br >
				<label class="checkbox narrow">
					<input type="checkbox" name="meta_mp_auto_sku" ' . BrainPress_Helper_Utility::checked( BrainPress_Data_Course::get_setting( $course_id, 'mp_auto_sku', false ) ) . ' />
					<span>' . esc_html__( 'Automatisch generierte Lagerhaltungseinheiten (SKUs)', 'brainpress' ) . '</span>
				</label>';

		$product_id = BrainPress_Data_Course::get_setting( $course_id, 'woo/product_id', false );
		...
		if ( $product_id ) {
			...
			$mp_content .= sprintf( __( 'WooCommerce Produkt ID: %d', 'brainpress' ), $product_id );
			$mp_content .= sprintf(
				...
				__( 'Bearbeite WooCommerce Produkt', 'brainpress' )
			);
			$mp_content .= '</p> ';
		}
	...
	public static function update_product( $course_id, $settings ) {
		...
		if ( ! self::$is_active ) {
			do_action( 'brainpress_mp_update_product', $course_id );
			return true;
		}
		...
		$post = array(
			...
			'post_title'   => BrainPress_Helper_Utility::filter_content( $course->post_title, true ),
			'post_type'    => self::$product_ctp,
			'post_content' => BrainPress_Helper_Utility::filter_content( $course->post_content, true ),
		);
		...
		if ( isset( $settings['payment_paid_course'] ) && 'on' == $settings['payment_paid_course'] ) {
			...
			if ( 'on' == $automatic_sku ) {
				...
				$sku[0] = BrainPress_Helper_Utility::filter_content( ( ! empty( $settings['mp_sku'] ) ? $settings['mp_sku'] : '' ), true );
			}
			if ( self::$is_active ) {
				BrainPress_Data_Course::update_setting( $course_id, 'woo/product_id', $post_id );
				BrainPress_Data_Course::update_setting( $course_id, 'woo/sku', $sku[0] );
				$price	  = BrainPress_Helper_Utility::filter_content( ( ! empty( $settings['mp_product_price'] ) ? $settings['mp_product_price'] : 0 ), true );
				$sale_price = BrainPress_Helper_Utility::filter_content( ( ! empty( $settings['mp_product_sale_price'] ) ? $settings['mp_product_sale_price'] : 0 ), true );
				...
				update_post_meta( $post_id, 'mp_sale_price_enabled', BrainPress_Helper_Utility::filter_content( ( ! empty( $settings['mp_sale_price_enabled'] ) ? $settings['mp_sale_price_enabled'] : '' ), true ) );
				update_post_meta( $post_id, 'cp_course_id', $course_id );
				// Resave product meta
				BrainPress_Data_Course::update_setting( $course_id, 'mp_product_price', $price );
				BrainPress_Data_Course::update_setting( $course_id, 'mp_product_sale_price', $sale_price );
				/**
				 * Action for WooCommerce product after BrainPress update.
				 *
				 * @since 2.0.5
				 *
				 * @param integer $post_id WooCommerce product ID.
				 * @param integer $course_id BrainPress course ID.
				 */
				do_action( 'brainpress_woocommerce_product_updated', $post_id, $course_id );
			}
			...
			if ( $product_id && 0 != $product_id ) {
				if ( self::$is_active ) {
					...
					if ( 'delete' == $unpaid ) {
						BrainPress_Data_Course::delete_setting( $course_id, 'woo' );
	...
	public static function update_course_when_deleting_product( $product_id ) {
		...
		BrainPress_Data_Course::delete_setting( $course_id, 'woo' );
		BrainPress_Data_Course::update_setting( $course_id, 'payment_paid_course', 'off' );
	}
	...
	public static function update_product_when_deleting_course( $course_id ) {
		...
		$course_post_type = BrainPress_Data_Course::get_post_type_name();
		/**
	...
	public static function add_post_parent_metabox() {
		add_meta_box( 'cp_woo_post_parent', __( 'Parent Course', 'brainpress' ), array( __CLASS__, 'cp_woo_post_parent_box_content' ), self::$product_ctp, 'side', 'default' );
	}
...
<?php
	...
	public static function update_course_from_product( $product_id, $post, $before_update ) {
		...
		foreach ( $meta as $key => $value ) {
			BrainPress_Data_Course::update_setting( $course_id, $key, $value );
		}
		/**
		 * Set or update thumbnail.
		 */
	...
	public static function show_course_message_woocommerce_order_details_after_order_table( $order ) {
		...
		printf( '<h2 class="cp_woo_header">%s</h2>', esc_html__( 'Kurs', 'brainpress' ) );
		printf( '<p class="cp_woo_thanks">%s</p>', esc_html__( 'Vielen Dank, dass Du dich für den Kurs eingeschrieben hast. Wir hoffen, Du genießt Deine Erfahrung.', 'brainpress' ) );
		if ( is_user_logged_in() && 'wc-completed' == $order_status ) {
			echo '<p class="cp_woo_dashboard_link">';
			printf(
				__( 'Den Kurs findest Du in Deinem <a href="%s">Dashboard</a>', 'brainpress' ),
				( method_exists( 'BrainPress_Core', 'get_slug' ) ) ? BrainPress_Core::get_slug( 'student_dashboard', true ) : ''
			);
			echo '</p><hr />';
		}
	}
	...
	public static function enroll_button( $content, $course_id, $user_id, $button_option ) {
		 ...
		 */
		if ( ! BrainPress_Helper_Utility::$is_singular ) {
			return $content;
		}
		/**
		 * do not chane for free courses
		 */
		if ( ! BrainPress_Data_Course::is_paid_course( $course_id ) ) {
			return $content;
		}
		/**
...
<?php
		...
		<script type="text/template" id="modal-view-woo-template" data-type="modal-step" data-modal-action="paid_enrollment">
			<div class="bbm-modal__topbar">
				<h3 class="bbm-modal__title">
					<?php esc_html_e( 'Kurs zum Warenkorb hinzufügen.', 'brainpress' ); ?>
				</h3>
			</div>
			<div class="bbm-modal__section">
				<p>
					<?php esc_html_e( 'Du kannst diesen Kurs jetzt in den Warenkorb legen.', 'brainpress' ); ?>
				</p>
	...
	public static function allow_student_to_enroll( $enroll_student, $student_id, $course_id ) {
		...
		return ! BrainPress_Data_Course::is_paid_course( $course_id );
	}
	...
	public static function use_redirect_woo_to_course() {
		...
		$redirect_woo_to_course = BrainPress_Core::get_setting( 'woocommerce/redirect', false );
	...
	/**
	 * Add extension settings to _brainpress javascript array
	 *
	 * @since 2.0.0
	 *
	...
	public static function woocommerce_before_main_content() {
		...
		while ( have_posts() ) {
			...
			$course_status = BrainPress_Data_Course::is_course_available( $course_id );
			if ( $course_status ) {
				continue;
			}
	 ...
	 * @since 2.0.7
	 */
	private static function remove_filter_brainpress_enroll_student() {
		/**
