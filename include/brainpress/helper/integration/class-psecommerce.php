...
class BrainPress_Helper_Integration_PSeCommerce {
	...
	public static function init() {
		...
		/** This filter is documented in include/brainpress/helper/class-javascript.php */
		add_filter(
			'brainpress_localize_object',
	...
	public static function replace_image( $img_src ) {
		...
		$image = BrainPress_Data_Course::get_setting( $course_id, 'listing_image' );
		if ( ! empty( $image ) ) {
			return $image;
		}
		return $img_src;
	...
	public static function product_settings( $content, $course_id ) {
		...
		$mp_content = '
			<div class="wide">
				...
				<input type="text" name="meta_mp_sku" placeholder="' . sprintf( __( 'z.B. %s0001', 'brainpress' ), apply_filters( 'brainpress_course_sku_prefix', 'CP-' ) ) . '" value="' . BrainPress_Data_Course::get_setting( $course_id, 'mp_sku', '' ) . '" /><br >
				<label class="checkbox narrow">
					...
					<span>' . esc_html__( 'Automatisch generierte Lagerbestandseinheiten (SKUs)', 'brainpress' ) . '</span>
				</label>';
	...
	public static function update_product_meta( $product_id, $settings, $course_id ) {
		...
		if (
			...
			$sku_prefix = apply_filters( 'brainpress_course_sku_prefix', 'CP-' );
	...
	public static function update_product_from_course( $course_id, $settings ) {
		...
		$is_paid = BrainPress_Data_Course::is_paid_course( $course_id );
		// Update and publish
		if ( $product_id && $is_paid ) {
			self::update_product_meta( $product_id, $settings, $course_id );
	...
	public static function update_course_from_product( $product_id, $post, $before_update ) {
		...
		$settings = BrainPress_Data_Course::get_setting( $course_id );

		BrainPress_Data_Course::set_setting( $settings, 'mp_sku', $sku );
		BrainPress_Data_Course::set_setting( $settings, 'mp_product_price', $price );
		BrainPress_Data_Course::set_setting( $settings, 'mp_product_sale_price', $sale_price );
		BrainPress_Data_Course::set_setting( $settings, 'mp_sale_price_enabled', $is_sale );
		BrainPress_Data_Course::set_setting( $settings, 'payment_paid_course', $is_paid );
		BrainPress_Data_Course::update_setting( $course_id, true, $settings );
	...
	public static function shortcode_cost( $content, $course_id ) {
		$product_id = BrainPress_Data_Course::get_setting(
			$course_id,
			'mp_product_id',
			false
		);
	...
	public static function update_course_when_deleting_product( $product_id ) {
		...
		BrainPress_Data_Course::delete_setting( $course_id, 'mp_product_id' );
	}
	...
	public static function get_product_id( $course_id = false ) {
		$product_id = BrainPress_Data_Course::get_setting( $course_id, 'mp_product_id', false );
		/**
	 ...
	 * @since 2.0.0
	 *
	 * @param array $localize_array BrainPress javascript configuration.
	 */
	...
	public static function order_notification_subject( $subject, $order ) {
		if ( self::_get_order_course_id( $order->ID ) ) {
			$subject = get_option( 'mp_order_email_subject', __( 'Bestellbestätigung', 'brainpress' ) );
		}
		/**
		 ...
		 */
		return apply_filters( 'brainpress_order_notification_subject', $subject );
	}
	...
	public static function order_email_body( $content, $order ) {
		...
		$order_message_body = BrainPress_Helper_Utility::replace_vars(
			self::_get_order_content_email(),
			apply_filters(
				'brainpress_order_email_vars',
				$vars
			)
		);
		 ...
		 */
		return apply_filters( 'brainpress_order_email_body', $order_message_body );
	}
	...
	public static function order_email_from( $email ) {
		 ...
		 */
		return apply_filters( 'brainpress_order_email_from', $order_from_email );
	}
	...
	public static function order_email_from_name( $name ) {
		 ...
		 */
		return apply_filters( 'brainpress_order_email_from_name', $order_from_name );
	}
	...
	public static function add_to_cart_template( $atts ) {
		 ...
		 */
		if ( ! BrainPress_Data_Course::is_paid_course( $atts['course_id'] ) ) {
			return;
		}
		?>
		<script type="text/template" id="modal-view-mp-template" data-type="modal-step" data-modal-action="paid_enrollment">
			...
			<div class="bbm-modal__section">
				<p><?php esc_html_e( 'Du kannst diesen Kurs jetzt in den Warenkorb legen.', 'brainpress' ); ?></p>
				<p><?php printf(
					'<a href="%s">%s</a>',
					esc_url( get_permalink( $atts['course_id'] ) ),
					sprintf(
						esc_html__( 'Zeige Kurs: %s', 'brainpress' ),
	...
	private static function _get_order_content_email() {
		$default_mp_order_content_email = sprintf( __( 'Vielen Dank für Deine Bestellung %1$s,
...
Hochachtungsvoll,
Dein %5$s Team', 'brainpress' ), 'CUSTOMER_NAME', '<a href="COURSE_ADDRESS">COURSE_TITLE</a>', '<a href="STUDENT_DASHBOARD">' . __( 'Dashboard', 'brainpress' ) . '</a>', '<a href="COURSES_ADDRESS">COURSES_ADDRESS</a>', 'BLOG_NAME' );
	...
	public static function body_class( $classes ) {
		...
		if ( BrainPress_Data_Course::is_course() ) {
			global $post;
