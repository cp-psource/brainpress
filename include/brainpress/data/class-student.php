...
class BrainPress_Data_Student {
	...
	public static function update_student_data( $student_id, $student_data ) {
		 ...
		 * Update student data.
		 */
		$student_data = apply_filters( 'brainpress_student_update_data', $student_data );
		if ( wp_update_user( $student_data ) ) {
			 ...
			 * @since 1.2.2
			 */
			do_action( 'brainpress_student_updated', $student_id );
			return true;
		}
		return false;
	}
	...
	public static function init_completion_data( $student_id, $course_id ) {
		return BrainPress_Helper_Utility::set_array_value( array(), 'version', BrainPress::$version );
	}
	...
	public static function get_completion_data( $student_id, $course_id ) {
		...
		if ( empty( $data ) ) {
			$data = apply_filters( 'brainpress_get_student_progress', array(), $student_id, $course_id );
	...
	public static function visited_page( $student_id, $course_id, $unit_id, $page, &$data = false ) {
		...
		$data = BrainPress_Helper_Utility::set_array_value( $data, 'units/' . $unit_id . '/visited_pages/' . $page, $page );
		$data = BrainPress_Helper_Utility::set_array_value( $data, 'units/' . $unit_id . '/last_visited_page', $page );
	...
	public static function visited_module( $student_id, $course_id, $unit_id, $module_id, &$data = false ) {
		...
		$data = BrainPress_Helper_Utility::set_array_value( $data, 'completion/' . $unit_id . '/modules_seen/' . $module_id, true );
		self::update_completion_data( $student_id, $course_id, $data );
		return $data;
	}
	...
	public static function module_response( $student_id, $course_id, $unit_id, $module_id, $response, &$data = false, $refresh = false ) {
		...
		switch ( $attributes['module_type'] ) {
			...
			case 'input-quiz':
				$result = BrainPress_Data_Module::get_quiz_results(
					$student_id,
					$course_id,
					$unit_id,
					$module_id,
				...
				break;
			case 'input-form':
				$result = BrainPress_Data_Module::get_form_results(
					$student_id,
					$course_id,
					$unit_id,
					$module_id,
		...
		$grade = apply_filters(
			'brainpress_autograde_module_response',
			$grade,
			$module_id,
			$student_id
		);
		...
		if ( isset( $attributes['mandatory'] ) && $attributes['mandatory'] ) {
			...
			$mandatory = (int) BrainPress_Helper_Utility::get_array_val( $data, $key );
			$data = BrainPress_Helper_Utility::set_array_value( $data, $key, $mandatory + 1 );
		}

		$data = BrainPress_Helper_Utility::set_array_value( $data, 'units/' . $unit_id . '/responses/' . $module_id . '/', $response_data );
	...
	public static function get_responses( $student_id, $course_id, $unit_id, $module_id, $response_only = false, &$data = false ) {
		...
		$responses = BrainPress_Helper_Utility::get_array_val( $data, 'units/' . $unit_id . '/responses/' . $module_id );
	...
	public static function record_grade(
		...
		if ( empty( $responses ) ) {
			$responses = array();
			$data = BrainPress_Helper_Utility::set_array_value(
				$data,
				'units/' . $unit_id . '/responses/' . $module_id,
				$responses
			);
	...
	public static function record_feedback(
		...
		$feedback_data = array(
			...
			'feedback' => BrainPress_Helper_Utility::filter_content( $feedback_new ),
			'date' => current_time( 'mysql' ),
			'draft' => $is_draft,
		);
	...
	public static function get_calculated_completion_data( $student_id, $course_id, &$student_progress = false ) {
		...
		$is_done = BrainPress_Helper_Utility::get_array_val(
			$student_progress,
		...
		$course_mode = BrainPress_Data_Course::get_setting( 'course_view' );
		$is_normal_mode = 'focus' != $course_mode;
		$require_assessment = 0;

		foreach ( $units as $unit_id => $unit ) {
			$unit_count += 1;
			$is_unit_available = BrainPress_Data_Unit::is_unit_available( $course_id, $unit_id, $previous_unit_id, false, $student_id );
				...
				foreach ( $unit['pages'] as $page_number => $modules ) {
					...
					if ( ! empty( $modules['modules'] ) ) {
						foreach ( $modules['modules'] as $module_id => $module ) {
							...
							$is_module_structure_visible = BrainPress_Data_Unit::is_module_structure_visible( $course_id, $unit_id, $module_id, $student_id );
							...
							if ( $is_answerable ) {
								...
								$had_passed = BrainPress_Helper_Utility::get_array_val(
									$student_progress,
									'completion/' . $unit_id . '/passed/' . $module_id
								);
								$had_answered = BrainPress_Helper_Utility::get_array_val(
								...
								if ( 'discussion' == $module_type ) {
									$last_answer = BrainPress_Data_Discussion::have_comments( $student_id, $module_id );
								...
								if ( ! empty( $last_answer ) ) {
									...
									if ( ! cp_is_true( $had_answered ) ) {
										do_action( 'brainpress_student_module_attempted', $student_id, $module_id, get_post_field( 'post_tile', $module_id ), $unit_id, $course_id );
									}
									...
									$grade = BrainPress_Helper_Utility::get_array_val(
										$grades,
										'grade'
									);
										...
										if ( $is_assessable ) {
											...
											if ( $pass ) {
												...
												if ( ! cp_is_true( $had_passed ) ) {
													do_action( 'brainpress_student_module_passed', $student_id, $module_id, get_post_field( 'post_tile', $module_id ), $unit_id, $course_id );
												}
					...
					$visited = BrainPress_Helper_Utility::get_array_val(
						$student_progress,
			...
			$was_completed = BrainPress_Helper_Utility::get_array_val(
				$student_progress,
			...
			if ( $is_unit_completed ) {
				...
				if ( ! cp_is_true( $was_completed ) ) {
					do_action( 'brainpress_student_unit_completed', $student_id, $unit_id, $unit['unit']->post_title, $course_id );
				}
			}
		}
		...
		if ( 0 === $require_assessment ) {
			...
			$minimum_grade_required = (int) BrainPress_Data_Course::get_setting( $course_id, 'minimum_grade_required', 100 );
		...
		if ( ! $is_done && $is_completed ) {
			...
			do_action(
				'brainpress_student_course_completed',
				$student_id,
				$course_id,
			...
			BrainPress_Data_Certificate::generate_certificate(
				$student_id,
				$course_id
			);
		}
	...
	public static function get_mandatory_completion( $student_id, $course_id, $unit_id, &$data = false ) {
		 ...
		 */
		if ( ! empty( $unit_id ) && is_numeric( $unit_id ) ) {
			$completed = BrainPress_Helper_Utility::get_array_val(
				$data,
				'completion/' . $unit_id . '/completed_mandatory'
			);
		}
		return array(
			'required' => BrainPress_Data_Unit::get_number_of_mandatory( $unit_id ),
			'completed' => $completed,
		);
	}
	...
	public static function get_unit_progress( $student_id, $course_id, $unit_id, &$data = false ) {
		 ...
		 */
		if ( ! empty( $unit_id ) && is_numeric( $unit_id ) ) {
			$completed = (int) BrainPress_Helper_Utility::get_array_val(
				$data,
				'completion/' . $unit_id . '/progress'
			);
		}
	...
	public static function get_course_progress( $student_id, $course_id, &$data = false ) {
		...
		return (int) BrainPress_Helper_Utility::get_array_val(
			$data,
			'completion/progress'
		);
	}
	...
	public static function is_mandatory_done( $student_id, $course_id, $unit_id, &$data = false ) {
		...
		if ( ! empty( $unit_id ) && is_numeric( $unit_id ) ) {
			 ...
			 * boolean value!
			 */
			$all_mandatory = BrainPress_Helper_Utility::get_array_val(
				$data,
				'completion/' . $unit_id . '/all_mandatory'
			);
			if ( $all_mandatory ) {
				$required_steps = BrainPress_Helper_Utility::get_array_val(
					$data,
					'completion/' . $unit_id . '/required_steps'
				);
				$completed = BrainPress_Helper_Utility::get_array_val(
	...
	public static function count_course_responses( $student_id, $course_id, $data = false ) {
		...
		foreach ( $units as $key => $unit ) {
			$modules = BrainPress_Helper_Utility::get_array_val(
				$data,
				'units/' . $key . '/responses'
			);
	...
	public static function send_registration( $student_id, $user_data = array() ) {
		...
		$sent = BrainPress_Helper_Email::send_email(
			BrainPress_Helper_Email::REGISTRATION,
			$email_args
		);
	...
	public static function get_workbook_url( $course_id ) {
		...
		$workbook_url = $course_url . trailingslashit( BrainPress_Core::get_slug( 'workbook' ) );
	...
	public static function get_admin_workbook_link( $student_id, $course_id ) {
		$workbook_link = add_query_arg(
			array(
				'page' => BrainPress_View_Admin_Student::get_slug(),
				'view' => 'workbook',
				'course_id' => $course_id,
	...
	public static function get_all_unit_progress( $student_id, $course_id, $unit_id, &$data = false ) {
		 ...
		 */
		$count_mandatory_seen_as_step = apply_filters( 'brainpress_count_mandatory_seen_as_step', false );
		$modules_ids = BrainPress_Data_Module::get_modules_ids_by_unit( $unit_id );
			...
			if (
				...
				if ( $count_mandatory_seen_as_step ) {
					...
					if ( $attributes['mandatory'] ) {
						$is_module_completed = BrainPress_Data_Student::is_module_completed( $course_id, $unit_id, $module_id, $student_id );
	...
	public static function is_section_seen( $course_id, $unit_id, $page, $student_id = 0 ) {
		...
		$student_progress = self::get_completion_data( $student_id, $course_id );
		$is_unit_visited = BrainPress_Helper_Utility::get_array_val(
			$student_progress,
		...
		if ( ! $completed ) {
			...
			$modules = BrainPress_Data_Course::get_unit_modules( $unit_id, array( 'publish' ), true, false, array( 'page' => $page ) );

			if ( count( $modules ) > 0 ) {
				$count = 0;
				foreach ( $modules as $module_id ) {
					$is_module_seen = BrainPress_Helper_Utility::get_array_val(
						$student_progress,
						'completion/' . $unit_id . '/modules_seen/' . $module_id
					);
	...
	public static function is_module_completed( $course_id, $unit_id, $module_id, $student_id = 0 ) {
		...
		$is_seen = BrainPress_Helper_Utility::get_array_val(
			$student_progress,
			'completion/' . $unit_id . '/modules_seen/' . $module_id
		);
	...
	public static function my_courses( $student_id = 0, $courses = array() ) {
		...
		if ( empty( $courses ) ) {
			$course_ids = BrainPress_Data_Student::get_enrolled_courses_ids( $student_id );
		...
		foreach ( $courses as $course ) {
			$course_id = $course->ID;
			$course_setting = BrainPress_Data_Course::get_setting( $course_id );
			$start_date = ! empty( $course_setting['course_start_date'] ) ? BrainPress_Data_Course::strtotime( $course_setting['course_start_date'] ) : 0;
			$end_date = ! empty( $course_setting['course_end_date'] ) ? BrainPress_Data_Course::strtotime( $course_setting['course_end_date'] ) : 0;
	...
	public static function get_course_status( $course_id, $student_id = 0, $return_human_readable_label = true ) {
		...
		$labels = array(
			'certified' => __( 'Zertifiziert', 'brainpress' ),
			'failed' => __( 'Gescheitert', 'brainpress' ),
			'awaiting-review' => __( 'Warten auf Überprüfung', 'brainpress' ),
			'ongoing' => __( 'Laufend', 'brainpress' ),
			'incomplete' => __( 'Unvollständig', 'brainpress' ),
		);
		...
		if ( $is_completed ) {
			...
			if ( 100 == $course_progress ) {
				$failed = BrainPress_Helper_Utility::get_array_val(
					$student_progress,
					'completion/failed'
				);
	...
	public static function withdraw_from_course() {
		...
		if ( isset( $_REQUEST['_wpnonce'] ) && ! empty( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'brainpress_student_withdraw' ) ) {
	...
	public static function get_admin_profile_url( $student_id ) {
		$nonce = wp_create_nonce( BrainPress_Admin_Students::get_view_profile_nonce_action( $student_id ) );
	...
	public static function log_visited_course( $course_id, $unit_id = 0, $page_number = 1, $module_id = 0 ) {
		...
		$key = 'brainpress_last_visited_' . $course_id;
		$value = array(
			'unit' => $unit_id,
			'page' => $page_number,
			'module' => $module_id,
	...
	public static function get_last_visited_url( $course_id ) {
		...
		$key = 'brainpress_last_visited_' . $course_id;
		$link = $course_url . BrainPress_Core::get_slug( 'units/' );
		...
		if ( ! empty( $last_visited ) ) {
			...
			if ( ! empty( $last_visited['unit'] ) ) {
				$link = BrainPress_Data_Unit::get_unit_url( (int) $last_visited['unit'] );
	...
	public static function unit_answers_are_correct( $student_id, $course_id, $unit_id, $student_progress = false ) {
		...
		foreach ( $unit['pages'] as $page_number => $page ) {
			...
			foreach ( $modules as $module_id => $module ) {
				...
				$completed = BrainPress_Data_Student::is_module_completed( $course_id, $unit_id, $module->ID, $student_id );
