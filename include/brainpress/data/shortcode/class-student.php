 ...
 */
class BrainPress_Data_Shortcode_Student {
	...
	public static function init() {
		...
		add_shortcode(
			'brainpress_enrollment_templates',
			array( __CLASS__, 'brainpress_enrollment_templates' )
		);
		...
		add_action( 'brainpress_focus_item_preload', array( __CLASS__, 'log_student_visit' ) );
		add_action( 'brainpress_normal_items_loaded', array( __CLASS__, 'log_normal_student_visit' ), 10, 3 );
	}

	public static function courses_student_dashboard( $atts ) {
		$content = BrainPress_Template_Student::dashboard();

		return $content;
	}

	public static function courses_student_settings( $atts ) {
		$content = BrainPress_Template_Student::student_settings();

		return $content;
	}

	public static function student_registration_form() {
		$content = BrainPress_Template_Student::registration_form();
	...
	public static function student_workbook_table( $args ) {
		...
		$workbook_is_active = BrainPress_Helper_Utility::checked( BrainPress_Data_Course::get_setting( $course_id, 'allow_workbook', false ) );

		if ( false == $workbook_is_active ) {
			$content = sprintf( '<p class="message">%s</p>', __( 'Für diesen Kurs ist keine Arbeitsmappe verfügbar.', 'brainpress' ) );
			return $content;
		}
		...
		$args = shortcode_atts(
			array(
				'course_id' => $course_id,
				'unit_id' => false,
				'module_column_title' => __( 'Element', 'brainpress' ),
				'title_column_title' => __( 'Module', 'brainpress' ),
				'submission_date_column_title' => __( 'Eingereicht', 'brainpress' ),
				'response_column_title' => __( 'Antworten', 'brainpress' ),
				'grade_column_title' => __( 'Bewertung', 'brainpress' ),
				'comment_column_title' => __( 'Feedback', 'brainpress' ),
				'module_response_description_label' => __( 'Beschreibung', 'brainpress' ),
				'comment_label' => __( 'Kommentar', 'brainpress' ),
				'view_link_label' => __( 'Ansehen', 'brainpress' ),
				...
				'pending_grade_label' => __( 'Ausstehend', 'brainpress' ),
				'unit_unread_label' => __( 'Einheit ungelesen', 'brainpress' ),
				'unit_read_label' => __( 'Einheit gelesen', 'brainpress' ),
				'single_correct_label' => __( 'Richtig', 'brainpress' ),
				'single_incorrect_label' => __( 'Falsch', 'brainpress' ),
				'no_content_label' => __( 'Diese Einheit hat keine Aktivitäten.', 'brainpress' ),
		...
		$content = '';
		$unit_list = BrainPress_Data_Course::get_units_with_modules( $course_id );
		$unit_list = BrainPress_Helper_Utility::sort_on_key( $unit_list, 'order' );
		...
		if ( $show_course_progress && empty( $unit_id ) ) {
			$content .= '<h3 class="course-completion-progress">' . esc_html__( 'Kursabschluss: ', 'brainpress' ) . '<small>' . BrainPress_Data_Student::get_course_progress( $student_id, $course_id, $student_progress ) . '%</small>' . '</h3>';
		}
		...
		foreach ( $unit_list as $unit_id => $unit ) {
			...
			$progress = BrainPress_Data_Student::get_unit_progress( $student_id, $course_id, $unit_id, $student_progress );
			$format = '<tr class="row-unit"><th colspan="2" class="workbook-unit unit-%s">%s</th><th style="text-align: right" class="td-right">%s: %s</th></tr>';
			$content .= sprintf( $format, $unit_id, $unit['unit']->post_title, __( 'Fortschritt', 'brainpress' ), $progress . '%' );
					...
					foreach ( $page['modules'] as $module_id => $module ) {
						...
						if ( in_array( $module_type, $excluded_modules ) ) {
							if ( ( 'auto' == $graded_by || empty( $graded_by ) ) && ! empty( $response ) ) {
								$grade = __( 'Ausstehend', 'brainpress' );
							}
						}
						...
						switch ( $attributes['module_type'] ) {
							...
							case 'input-upload':
								if ( $response ) {
									...
									$file_size = $file_size ? BrainPress_Helper_Utility::format_file_size( $file_size ) : '';
									...
									$response_display = '<a href="' . esc_url( $url ) . '" class="button button-download">' . esc_html( $file_name ) . ' ' . BrainPress_Helper_Utility::filter_content( $file_size ) . '</a>';
								} else {
									$response_display = '';
								}
								break;
								...
								foreach ( $questions as $q_index => $question ) {
									...
									if ( ! empty( $response_display[ $q_index ] ) ) {
										...
										foreach ( $answers as $a_index => $answer ) {
											if ( ! empty( $answer ) ) {
												$the_answer = BrainPress_Helper_Utility::get_array_val(
													$attributes,
							...
							case 'input-text': case 'input-textarea':
									$response_display = empty( $response_display ) ? __( 'Keine Antwort!', 'brainpress' ) : $response_display;
						...
						$response_date = ! isset( $response['date'] ) ? '' : date_i18n( get_option( 'date_format' ), BrainPress_Data_Course::strtotime( $response['date'] ) );
						...
						$first_last = BrainPress_Helper_Utility::get_user_name( $feedback_by );
						...
						if ( 'Pending' === $grade_display && false === $is_assessable ) {
							$grade_display = __( 'Nicht bewertbar', 'brainpress' );
						}
						...
						$content .= sprintf( '<td style="text-align: right" class="td-right">%s</td>', $grade_display );
						$content .= '</tr>';
			...
			if ( 0 == $module_count ) {
				$content .= sprintf( '<tr><td colspan="3" class="non-gradable">%s</td></tr>', __( 'Keine bewertbaren Module in dieser Einheit.', 'brainpress' ) );
			}
		}
		$content .= '</table></div>';
	...
	public static function brainpress_enrollment_templates( $atts ) {
		/**
		...
		if (
			isset( $post->brainpress_enrollment_templates_was_already_loaded )
			&& $post->brainpress_enrollment_templates_was_already_loaded
		) {
			return;
		}
		$post->brainpress_enrollment_templates_was_already_loaded = true;
		...
		$nonce = wp_create_nonce( 'brainpress_enrollment_action' );
		...
		<script type="text/template" id="modal-template">
			<div class="enrollment-modal-container"
				...
				data-course-is-paid="<?php esc_html_e( intval( BrainPress_Data_Course::is_paid_course( $course_id ) ) ); ?>"
			></div>
		</script>

		<?php if ( apply_filters( 'brainpress_registration_form_step-1', true ) ) : ?>
		<script type="text/template" id="modal-view1-template" data-type="modal-step" data-modal-action="signup">
			<div class="bbm-modal-nonce signup" data-nonce="<?php echo wp_create_nonce( 'brainpress_enrollment_action_signup' ); ?>"></div>
			<div class="bbm-modal__topbar">
				<h3 class="bbm-modal__title">
					<?php esc_html_e( 'Neues Konto erstellen', 'brainpress' ); ?>
				</h3>
			...
			<input type="submit" class="bbm-button done signup button cta-button" value="<?php esc_attr_e( 'Ein Konto erstellen', 'brainpress' ); ?>" />
			<a href="#" class="cancel-link">
				<?php esc_html_e( 'Abbrechen', 'brainpress' ); ?>
			</a>
			</div>
		</script>
		<?php endif; ?>

		<?php if ( apply_filters( 'brainpress_registration_form_step-2', true ) ) : ?>
		<script type="text/template" id="modal-view2-template" data-type="modal-step" data-modal-action="login">
			<div class="bbm-modal-nonce login" data-nonce="<?php echo wp_create_nonce( 'brainpress_enrollment_action_login' ); ?>"></div>
			<div class="bbm-modal__topbar">
				<h3 class="bbm-modal__title">
					<?php esc_html_e( 'Melde dich in deinem Konto an', 'brainpress' ); ?>
				</h3>
				<span id="error-messages"></span>
			</div>
			<div class="bbm-modal__section">
			...
			<div class="bbm-modal__bottombar">
			<input type="submit" class="bbm-button done login button cta-button" value="<?php esc_attr_e( 'Anmelden', 'brainpress' ); ?>" />
			<a href="#" class="cancel-link"><?php esc_html_e( 'Abbrechen', 'brainpress' ); ?></a>
			</div>
		</script>
		<?php endif; ?>

		<?php if ( apply_filters( 'brainpress_registration_form_step-3', true ) ) : ?>
		<script type="text/template" id="modal-view3-template" data-type="modal-step" data-modal-action="enrolled">
			<div class="bbm-modal__topbar">
				<h3 class="bbm-modal__title">
					<?php esc_html_e( 'Erfolgreich eingeschrieben.', 'brainpress' ); ?>
				</h3>
			</div>
			<div class="bbm-modal__section">
				<p>
					<?php esc_html_e( 'Herzliche Glückwünsche! Du hast dich erfolgreich eingeschrieben. Klicke unten, um loszulegen.', 'brainpress' ); ?>
				</p>
				<a href="<?php echo get_permalink( BrainPress_Helper_Utility::the_course( true ) ) . BrainPress_Core::get_slug( 'units' ); ?>"><?php _e( 'Start Learning', 'brainpress' ); ?></a>
			</div>
			<div class="bbm-modal__bottombar">
			</div>
		</script>
		...
		<?php if ( apply_filters( 'brainpress_registration_form_step-4', true ) ) : ?>
		<script type="text/template" id="modal-view4-template" data-type="modal-step" data-modal-action="passcode">
			<div class="bbm-modal__topbar">
                <h3 class="bbm-modal__title"><?php esc_html_e( 'Zu diesem Zeitpunkt kann ich mich nicht einschreiben.', 'brainpress' ); ?>
				</h3>
			</div>
            <div class="bbm-modal__section"><?php
			printf( '<p>%s</p>', esc_html__( 'Für die Anmeldung ist ein Passcode erforderlich. Klicke unten, um zum Kurs zurückzukehren.', 'brainpress' ) );
?>
                    <a href="<?php echo get_permalink( BrainPress_Helper_Utility::the_course( true ) ) . BrainPress_Core::get_slug( 'units' ); ?>"><?php _e( 'Geh zurück zum Kurs!', 'brainpress' ); ?></a>
			</div>
			<div class="bbm-modal__bottombar">
			</div>
		</script>
		...
		<?php
		do_action( 'brainpress_registration_form_end', $atts );
	...
	public static function course_progress( $atts ) {
		...
		return number_format_i18n(
			BrainPress_Data_Student::get_course_progress(
				get_current_user_id(),
				$course_id
			),
			$decimal_places
	...
	public static function course_unit_progress( $atts ) {
		...
		$progress = number_format_i18n(
			BrainPress_Data_Student::get_unit_progress(
				get_current_user_id(),
				$course_id,
				$unit_id
			),
	...
	public static function course_mandatory_message( $atts ) {
		extract(
			shortcode_atts(
				array(
					'course_id' => BrainPress_Helper_Utility::the_course( true ),
					'unit_id' => BrainPress_Helper_Utility::the_post( true ),
					'message' => __( '%d von %d erforderlichen Elementen abgeschlossen.', 'brainpress' ),
				),
				$atts,
				'course_mandatory_message'
			)
		...
		$mandatory = BrainPress_Data_Student::get_mandatory_completion(
			$student_id,
			$course_id,
			$unit_id
		);
	...
	public static function course_unit_percent( $atts ) {
		$shortcode_atts = shortcode_atts( array(
			...
			'format' => false,
			'style' => 'flat',
			'tooltip_alt' => __( 'Prozent der Einheit fertiggestellt', 'brainpress' ),
		 ...
		 **/
		$shortcode_atts = apply_filters( 'brainpress_unit_progress_wheel_atts', $shortcode_atts );
		...
		$percent_value = (int) BrainPress_Data_Student::get_unit_progress( get_current_user_id(), $course_id, $unit_id );
		 ...
		 */
		$is_unit_available = BrainPress_Data_Unit::is_unit_available( $course_id, $unit_id, null );
		 ...
		 **/
		$content = apply_filters( 'brainpress_unit_progress_wheel', $content, $course_id, $unit_id, $percent_value );
	...
	public static function student_grades_table( $args ) {
		...
		foreach ( $unit_list as $unit_id => $unit ) {
			...
			$content .= sprintf( '<td style="text-align: right" class="td-right">%s: %d%%</td>', __( 'Fortschritt', 'brainpress' ), $progress );
			$content .= '</tr>';
			...
			$grade_display = __( 'Keine bewertbaren Module unter dieser Einheit.', 'brainpress' );
			if ( 0 != $module_count ) {
				$grade_display = __( '%d von %d Elementen abgeschlossen.', 'brainpress' );
	...
	public static function log_student_visit( array $array ) {
		...
		if ( 'section' == $type ) {
			...
			$page_number = BrainPress_Data_Shortcode_Template::get_module_page( $course_id, $unit_id, $module_id );
		}

		BrainPress_Data_Student::log_visited_course( $course_id, $unit_id, $page_number, $module_id );
	}
	...
	public static function log_normal_student_visit( $course_id, $unit_id, $page_number ) {
		BrainPress_Data_Student::log_visited_course( $course_id, $unit_id, $page_number );
	}
}
