...
class BrainPress_View_Admin_Instructor {
	public static $slug = 'brainpress_instructors';
	...
	public static function init() {
		self::$title = __( 'Kurse/Kursleiter', 'brainpress' );
		self::$menu_title = __( 'Kursleiter', 'brainpress' );

		add_filter(
			'brainpress_admin_valid_pages',
			array( __CLASS__, 'add_valid' )
		);
		add_filter(
			'brainpress_admin_pages',
		...
		add_action(
			'brainpress_admin_' . self::$slug,
			array( __CLASS__, 'render_page' )
		);

		add_action(
			'brainpress_settings_page_pre_render_' . self::$slug,
			array( __CLASS__, 'pre_process' )
		);

		add_action(
			'brainpress_course_deleted',
	...
	public static function pre_process() {
		if ( empty( $_GET['action'] ) ) {
			self::$table_manager = new BrainPress_Helper_Table_Instructor;
	...
	public static function remove_instructor( $instructor_id ) {
		...
		$assigned_courses = BrainPress_Data_Instructor::get_assigned_courses_ids( $instructor );

		if ( ! empty( $assigned_courses ) ) {
			foreach ( $assigned_courses as $course_id ) {
				BrainPress_Data_Course::remove_instructor( $course_id, $instructor_id );
			}
		}
	}
		...
		<div class="wrap nocoursesub cp-wrap">
				...
				<div id="course-left">
					<div class="course-holder-wrap" id="edit-sub">
						<div class="sidebar-name no-movecursor">
							<h3><?php esc_html_e( 'Kurse', 'brainpress' ); ?></h3>
						</div>
										...
										<tr <?php echo $style; ?>>
											...
											<td style="width:25%;">
												<div class="course_additional_info">
													<div>
														<span class="info_caption"><?php esc_html_e( 'Start', 'brainpress' ); ?></span>
														<span class="info">
															<?php echo $course->start_date; ?>
														</span>
													</div>
													<div>
														<span class="info_caption"><?php esc_html_e( 'Ende', 'brainpress' ); ?></span>
														<span class="info"><?php echo $course->end_date; ?></span>
													</div>
													<div>
														<span class="info_caption"><?php esc_html_e( 'Dauer', 'brainpress' ); ?></span>
			...
			<div class="course-liquid-right">
				<div class="course-holder-wrap">
					<div class="sidebar-name no-movecursor">
						<h3><?php esc_html_e( 'Profil', 'brainpress' ); ?></h3>
					</div>
						...
						<div class="sidebar-inner">
							<div class="instructors-info" id="instructors-info">
										...
										<tr>
											...
											<td>
												<div class="instructor_additional_info">
													...
													<div>
														<span class="info_caption"><?php esc_html_e( 'Email', 'brainpress' ); ?></span>
														<span class="info">
								...
								<div class="edit-profile-link">
									<a href="<?php echo get_edit_user_link( $instructor_id ); ?>"><?php esc_html_e( 'Profil bearbeiten', 'brainpress' ); ?></a>
								</div>
							</div>
						</div>
					</div>
