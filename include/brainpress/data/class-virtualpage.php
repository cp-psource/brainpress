<?php
/**
 * Handles the virtual pages used by BrainPress.
 *
 * When the brainpress theme is used, these virtual pages are not used because
 * the theme already comes with special template files for BrainPress pages.
 *
 * @since  2.0.0
 *
 * @package WordPress
...
class BrainPress_Data_VirtualPage {
	 ...
	 * Usually this is set to a virtual post-type, e.g. 'brainpress_instructor'.
	 *
	 * @var string
	 */
	protected $type = 'page';
	...
	public function __construct( $args ) {
		...
		if ( isset( $args['is_singular'] ) ) {
			...
			// Check status
			if ( 'publish' != $post->post_status && false === BrainPress_Data_Capabilities::can_update_course( $post->ID ) ) {
				return;
			}
		}
	...
	public function virtual_page( $posts ) {
		...
		if ( $this->ID ) {
			BrainPress_Helper_Utility::set_the_post( $this->ID );
	...
	public function virtual_content( $content ) {
		if ( $this->callback ) {
			...
			$content = apply_filters( 'brainpress_view_course', $content, $this->ID, $this->context );
		}
	...
	public function virtual_comments( $template ) {
		if ( 'closed' == $this->comment_status ) {
			$template = BrainPress::$path . 'lib/BrainPress/Template/no-comment.php';
		}
