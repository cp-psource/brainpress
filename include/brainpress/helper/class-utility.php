...
class BrainPress_Helper_Utility {
	...
	public static function enable_extended_upload( $mime_types = array() ) {
		...
		$valid_pages = array( 'brainpress_settings', 'brainpress_course', 'brainpress' );
		$matches = false;
	...
	public static function set_array_val( &$a, $path_input, $value ) {
		BrainPress_Helper_Legacy::deprecated_function( __CLASS__.'::'.__FUNCTION__, '2.0.5', 'BrainPress_Helper_Utility::set_array_value()' );
		$path = array();
		if ( is_array( $path_input ) ) {
			$path = $path_input;
		} else {
	...
	public static function unset_array_val( &$a, $path_input ) {
		BrainPress_Helper_Legacy::deprecated_function( __CLASS__.'::'.__FUNCTION__, '2.0.5', 'BrainPress_Helper_Utility::unset_array_value()' );
		$path = array();
		if ( is_array( $path_input ) ) {
			$path = $path_input;
		} else {
	...
	public static function get_image_extensions() {
		return apply_filters(
			'brainpress_allowed_image_extensions',
			array(
				'jpg',
				'jpeg',
				'jpe',
	 ...
	 *
	 * @since  1.0.0
	 * @return bool If BrainPress allows user signup.
	 */
	public static function users_can_register() {
		...
		if ( null === $_allow_register ) {
			 ...
			 * BrainPress registration only.
			 *
			 * @since 2.0.0
			 * @var bool $_allow_register
			 */
			$_allow_register = apply_filters(
				'brainpress_users_can_register',
				$_allow_register
			);
		}
	...
	public static function is_payment_supported() {
		...
		return apply_filters(
			...
			false,
			BrainPress_Data_Course::last_course_id()
		);
	}
	...
	public static function force_download_file_request() {
		if ( isset( $_GET['fdcpf'] ) ) {
			...
			if ( ! empty( $_GET['c'] ) && ! empty( $_GET['u'] ) ) {
				...
				BrainPress_Data_Certificate::generate_pdf_certificate( $course_id, $student_id );
			}
	...
	public static function download_file_request( $requested_file ) {
		...
		$force_download_parameters = apply_filters(
			'brainpress_force_download_parameters',
			array(
				'timeout' => 60,
				'user-agent' => BrainPress::$name . ' / ' . BrainPress::$version . ';',
			)
		);
		...
		if ( ! empty( $body ) ) {
			...
			_e( 'Something went wrong.', 'brainpress' );
		}
		exit();
	}
	...
	public static function open_course_zip_object() {
		if ( isset( $_GET['oacpf'] ) ) {
			...
			echo '<a href="' . esc_url_raw( wp_get_referer() ) . $append_url . '" style="padding: 5px; font-size: 12px; text-decoration: none; opacity: 0.3; background: #3C3C3C; color: #fff; font-family: helvetica, sans-serif; position: absolute; top: 2; left: 2;"> &laquo; ' . esc_html__( 'Back to Course', 'brainpress' ) . '</a>';
	...
	public static function the_course( $id_only = false ) {
		$id = BrainPress_Data_Course::last_course_id();
	...
	public static function the_course_category() {
		return BrainPress_Data_Course::$last_course_category;
	}

	public static function the_course_subpage() {
		return BrainPress_Data_Course::$last_course_subpage;
	}

	public static function set_the_course( $post ) {
		if ( is_object( $post ) ) {
			BrainPress_Data_Course::set_last_course_id( (int) $post->ID );
		} else {
			BrainPress_Data_Course::set_last_course_id( (int) $post );
		}
	}

	public static function set_the_course_category( $category ) {
		BrainPress_Data_Course::$last_course_category = sanitize_text_field( $category );
	}

	public static function set_the_course_subpage( $page ) {
		BrainPress_Data_Course::$last_course_subpage = sanitize_text_field( $page );
	}
	...
	public static function allowed_student_mimes() {
		return apply_filters(
			'brainpress_allowed_student_mimes',
			array(
				'txt' => 'text/plain',
				'pdf' => 'application/pdf',
				'zip' => 'application/zip',
	...
	public static function add_nav_menu_meta_boxes() {
		add_meta_box( 'brainpress_endpoints_nav_link', __( 'BrainPress Menü', 'brainpress' ), array( __CLASS__, 'nav_menu_links' ), 'nav-menus', 'side', 'low' );
	}
	...
	public static function nav_menu_links() {
		$end_points = array(
			'courses' => array(
				'label' => __( 'Kurse', 'brainpress' ),
				'value' => '#brainpress-endpoints-courses',
			),
			'login' => array(
				'label' => __( 'Anmelden/Abmelden', 'brainpress' ),
				'value' => '#brainpress-endpoints-login',
			),
			'dashboard' => array(
				'label' => __( 'Dashboard', 'brainpress' ),
				'value' => '#brainpress-endpoints-dashboard',
			),
			'profile' => array(
				'label' => __( 'Profil', 'brainpress' ),
				'value' => '#brainpress-endpoints-profile',
			),
		);
		?>
		<div id="posttype-brainpress-endpoints" class="posttypediv">
			<div id="tabs-panel-brainpress-endpoints" class="tabs-panel tabs-panel-active">
				<ul id="brainpress-endpoints-checklist" class="categorychecklist form-no-clear">
					<?php
					$i = -1;
			...
			<p class="button-controls">
				<span class="list-controls">
					<a href="<?php echo admin_url( 'nav-menus.php?page-tab=all&selectall=1#posttype-brainpress-endpoints' ); ?>" class="select-all"><?php _e( 'Wähle Alle', 'brainpress' ); ?></a>
				</span>
				<span class="add-to-menu">
					<input type="submit" class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e( 'Zum Menü hinzufügen', 'brainpress' ); ?>" name="add-post-type-menu-item" id="submit-posttype-brainpress-endpoints">
	...
	public static function add_site_vars( $vars = array() ) {
		 ...
		 * @param array $vars Array of site vars.
		 */
		return apply_filters( 'brainpress_site_vars', $vars );
	}
	...
	{
		return (boolean) apply_filters('brainpress_display_password_strength_meter', true);
	}
	...
	{
		return apply_filters('brainpress_min_password_length', 6);
	}
