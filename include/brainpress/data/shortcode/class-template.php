 ...
 */
class BrainPress_Data_Shortcode_Template {
	...
	public static function init() {
		...
		add_shortcode(
			'brainpress_dashboard',
			array( __CLASS__, 'brainpress_dashboard' )
		);
		add_shortcode(
			'brainpress_focus_item',
			array( __CLASS__, 'brainpress_focus_item' )
		);
		add_shortcode(
			'brainpress_quiz_result',
			array( __CLASS__, 'brainpress_quiz_result' )
		);
		...
		if ( apply_filters( 'brainpress_custom_signup', true ) ) {
			/**
		...
		add_action('brainpress_after_signup_email', array( 'BrainPress_Helper_UI', 'password_strength_meter'));
	}
	...
	public static function course_archive( $a ) {
		...
		$a = shortcode_atts( array(
			'category' => BrainPress_Helper_Utility::the_course_category(),
			'posts_per_page' => 10,
			'show_pager' => true,
			'echo' => false,
		...
		if ( ! empty( $a['courses_type'] ) && 'current_and_upcoming' == $a['courses_type'] ) {
			$query = BrainPress_Data_Course::current_and_upcoming_courses( $post_args );
		} else {
		...
		$template = apply_filters( 'brainpress_template_course_archive', $template, $a );

		foreach ( $query->posts as $post ) {
			BrainPress_Helper_Utility::set_the_course( $post );
		...
		$content = apply_filters( 'brainpress_course_archive_content', $content, $a );
	...
	public static function course_enroll_box( $a ) {
		...
		$template = apply_filters( 'brainpress_template_course_enroll_box', $template, $course_id, $a );
	...
	public static function course_list_box( $a ) {
		$a = shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'clickable' => false,
			'clickable_label' => __( 'Kursdetails', 'brainpress' ),
			'override_button_text' => '',
			'override_button_link' => '',
			'button_label' => __( 'Details', 'brainpress' ),
			'echo' => false,
		...
		$url = BrainPress_Data_Course::get_course_url( $course_id );
		...
		if ( is_user_logged_in() ) {
			$student_progress = BrainPress_Data_Student::get_completion_data( get_current_user_id(), $course_id );
			...
			if ( $show_withdraw_link && ! $completed ) {
				$withdraw_link = add_query_arg(
					array(
						'_wpnonce' => wp_create_nonce( 'brainpress_student_withdraw' ),
						'course_id' => $course_id,
						'student_id' => get_current_user_id(),
					)
				);
				$withdraw_from_course = sprintf( '<a href="%s" class="cp-withdraw-student">%s</a>', esc_url( $withdraw_link ), __( 'Zurückziehen', 'brainpress' ) );
			}
		}
		$completion_class = BrainPress_Data_Course::course_class( $course_id );
		...
		if ( ! empty( $a['override_button_text'] ) && ! empty( $a['override_button_link'] ) ) {
			$button_text = '<button class="brainpress-course-link" data-link="' . esc_url( $a['override_button_link'] ) . '">' . esc_attr( $a['override_button_text'] ) . '</button>';
		}

		/**
		 * schema.org
		 */
		$schema = apply_filters( 'brainpress_schema', '', 'itemscope' );
...
'.$withdraw_from_course.'
		...
		$template = apply_filters( 'brainpress_template_course_list_box', $template, $course_id, $a );
	...
	public static function course_page( $a ) {
		...
		$template = '<div class="course-wrapper"'.$schema.'>
			...
			[course_description label="' . __( 'Über diesen Kurs', 'brainpress' ) . '" course_id="' . $course_id . '"]
			[course_structure course_id="' . $course_id . '"]
		</div>
		';

		$template = apply_filters( 'brainpress_template_course_page', $template, $course_id, $a );
	...
	public static function instructor_page( $a ) {
		$a = shortcode_atts( array(
			'instructor_id' => BrainPress_View_Front_Instructor::$last_instructor,
			'echo' => false,
			'heading_title' => __( 'Kurse', 'brainpress' ),
		), $a, 'instructor_page' );
		 ...
		 * schema.org
		 */
		$schema = apply_filters( 'brainpress_schema', '', 'itemscope-person' );
		 ...
		 * schema.org
		 */
		$schema = apply_filters( 'brainpress_schema', '', 'description' );

		$template .= '<div class="instructor-bio"'.$schema.'>' . BrainPress_Helper_Utility::filter_content( get_user_meta( $instructor_id, 'description', true ) ) . '</div>
		...
		$template = apply_filters( 'brainpress_template_instructor_page', $template, $instructor_id, $a );
	...
	public static function brainpress_dashboard( $a ) {
		$a = shortcode_atts( array(
			'user_id' => get_current_user_id(),
			'echo' => false,
		), $a, 'brainpress_dashboard' );
		...
		$template = '<div class="brainpress-dashboard-wrapper">
		...
		$template = sprintf( $template, $user_id, __( 'Eingeschriebene Kurse', 'brainpress' ) );

		$template = apply_filters( 'brainpress_template_dashboard_page', $template, $user_id, $a );
	...
	public static function brainpress_focus_item( $a ) {
		$a = shortcode_atts(
			array(
				'course' => '',
				'unit' => '',
				'type' => '',
				'item_id' => 0,
				'pre_text' => __( '&laquo; Zurück', 'brainpress' ),
				'next_text' => __( 'Weiter &raquo;', 'brainpress' ),
				'next_section_title' => __( 'Fahre mit dem nächsten Abschnitt fort', 'brainpress' ),
				'next_module_title' => __( 'Fahre mit dem nächsten Modul fort', 'brainpress' ),
				'next_section_text' => __( 'Nächster Abschnitt', 'brainpress' ),
				'echo' => false,
			),
			$a,
			'brainpress_focus_item'
		...
		do_action( 'brainpress_focus_item_preload', $a );
		...
		BrainPress_Helper_Utility::set_the_course( $course_id );
		...
		if ( 'section' === $type ) {
			...
			if ( BrainPress_Data_Course::get_setting( $course_id, 'focus_hide_section', false ) ) {
				$next_modules = BrainPress_Data_Course::get_unit_modules(
					$unit_id,
					array( 'publish' ),
					true,
					false,
				...
				if ( empty( $mod ) ) {
					$type = 'no_access';
				} elseif ( ! BrainPress_Data_Course::can_view_module( $course_id, $unit_id, $mod, $page ) ) {
					$type = 'no_access';
				}
			}
			if ( 0 === $unit_id ) {
		...
		$page_info = BrainPress_Data_Unit::get_page_meta( $unit_id, $page );

		if ( $breadcrumbs ) {
			// Course.
			$c_link = BrainPress_Data_Course::get_course_url( $course_id );
			$a_link = $c_link . BrainPress_Core::get_slug( 'units/' );
			...
			$a_link = '<a href="' . esc_url( $a_link ) . '" class="breadcrumb-course-units crumb">' . esc_html__( 'Einheiten', 'brainpress' ) . '</a>';
			$u_link_url = $u_link;
		...
		if ( ! $is_enrolled && ! $is_instructor ) {
			if ( 'section' == $type ) {
				$can_view = BrainPress_Data_Course::can_view_page( $course_id, $unit_id, $page, $student_id );
			}
			if ( 'module' == $type ) {
				$attributes = BrainPress_Data_Module::attributes( $item_id );
				if ( 'output' === $attributes['mode'] ) {
					$can_view = BrainPress_Data_Course::can_view_module( $course_id, $unit_id, $item_id, $page, $student_id );
				} else {
					$can_view = false;
				}
			}
		...
		if ( 'module' == $type && ! $can_view && ! $can_update_course ) {
			$can_be_previewed = BrainPress_Data_Module::can_be_previewed( $item_id );
			$view_type = 'preview';
		}
		/**
		 * sanitize type
		...
		$error_message = BrainPress_Data_Course::can_access( $course_id, $unit_id, $item_id, $student_id, $page, $type );

		switch ( $type ) {
			case 'section':
				if ( $is_enrolled ) {
					BrainPress_Data_Student::visited_page(
						$student_id,
						$course_id,
						$unit_id,
						$page,
				...
				$is_unit_available = $is_course_available ? BrainPress_Data_Unit::is_unit_available( $course_id, $unit_id, null ) : $is_course_available;
				...
				if ( empty( $error_message ) ) {
					...
					$template = apply_filters( 'brainpress_template_focus_item_section', $template, $a );

					$content .= $template;

					$content .= '</div>'; // .focus-main

					$next = BrainPress_Data_Course::get_next_accessible_module(
						$course_id,
						$unit_id,
						$page,
						false
					);
					$prev = BrainPress_Data_Course::get_prev_accessible_module(
						$course_id,
						$unit_id,
						$page,
						false
			...
			case 'module':
				if ( $is_enrolled ) {
					BrainPress_Data_Student::visited_module(
						$student_id,
						$course_id,
						$unit_id,
						$item_id,
				...
				$modules = BrainPress_Data_Course::get_unit_modules( $unit_id, $modules_status, true, false, array( 'page' => $page ) );
				...
				$module = get_post( $item_id );
				$attributes = BrainPress_Data_Module::attributes( $module );
				...
				$module_passed = BrainPress_Helper_Utility::get_array_val(
					$student_progress,
				...
				$method = 'template';
				$template = 'BrainPress_Template_Module';
				...
				if ( 'input-quiz' == $attributes['module_type'] && $is_mandatory ) {
					$quiz_result = BrainPress_Data_Module::get_quiz_results(
						$student_id,
						$course_id,
						$unit_id,
						$module->ID
				...
				if ( 'normal' == $view_type && ( $is_enrolled || $can_update_course ) ) {
					...
					if ( ! empty( $next['not_done'] ) ) {
						...
						$title = __( 'Du musst dieses ERFORDERLICHE Modul abschließen, bevor Du fortfahren kannst.', 'brainpress' );
					}
				...
				$template = apply_filters( 'brainpress_focus_mode_module_template', $template, $content, $item_id );

				break;

			case 'no_access':
				$content = do_shortcode( '[brainpress_enrollment_templates]' );
				$content .= '<div class="focus-wrapper">';
				$content .= '<div class="focus-main section">';

				$content .= '<div class="no-access-message">' . __( 'Du hast derzeit keinen Zugriff auf diesen Teil des Kurses. Melde Dich jetzt an, um vollen Zugriff auf den Kurs zu erhalten.', 'brainpress' ) . '</div>';
				$content .= do_shortcode( '[course_join_button course_id="' . $course_id . '"]' );

				$content .= '</div>'; // .focus-main
				$content .= '</div>'; // .focus-wrapper

				$template = apply_filters( 'brainpress_no_access_message', $content, $course_id, $unit_id );
				break;
			...
			case '404_module':
				...
				switch ( $type ) {
					case '404':
						$content .= __( 'Diese Einheit existiert nicht.', 'brainpress' );
					break;
					case '404_module':
						$content .= __( 'Dieses Modul existiert nicht.', 'brainpress' );
					break;
				}
				$content .= '</p></div>';
				$content .= do_shortcode(
					sprintf(
						...
						esc_attr( $course_id ),
						esc_attr__( 'Kursdetails anzeigen', 'brainpress' )
					)
				);
				$content .= '</div>'; // .focus-main
				$content .= '</div>'; // .focus-wrapper

				$template = apply_filters( 'brainpress_404_message', $content, $course_id, $unit_id );
		...
		if ( $breadcrumbs ) {
			$content = '<div class="brainpress-breadcrumbs ' . $type . '">' . $breadcrumb_trail . '</div>' . $content;
		}
	...
	protected static function tpl_mandatory_not_completed() {
		...
		<script type="text/template" id="modal-next-step-is-mandatory" data-type="modal-step" data-modal-action="mandatory">
			<div class="bbm-modal__topbar">
				<h3 class="bbm-modal__title"><?php _e( 'Dies ist ein ERFORDERLICHES Modul.', 'brainpress' ); ?></h3>
			</div>
			<div class="bbm-modal__section">
<?php
		$first_line = __( 'Du musst dieses ERFORDERLICHE Modul abschließen, bevor Du fortfahren kannst.', 'brainpress' );
		echo BrainPress_Helper_UI::get_message_required_modules( $first_line );
?>
			</div>
			<div class="bbm-modal__bottombar"><a class="cancel-link"><?php _e( 'Schließen', 'brainpress' ); ?></a></div>
		</script>
		<?php
		return ob_get_clean();
	}
	...
	public static function show_nav_button( $button, $title, $classes, $link_title = '', $next = false, $rel = '' ) {
		 ...
		 * @param  string $link_title Tooltip title of the link.
		 */
		return apply_filters( 'brainpress_data_shortcode_template_show_nav_button', $res, $button, $title, $classes, $link_title, $next );
	}

	public static function brainpress_quiz_result( $a ) {
		$a = shortcode_atts( array(
			'course_id' => false,
			'unit_id' => false,
			'module_id' => false,
		...
		$template = BrainPress_Data_Module::quiz_result_content(
			$student_id,
			$course_id,
			$unit_id,
			$module_id
		...
		$template = apply_filters(
			'brainpress_template_quiz_results_shortcode',
			$template,
			$a
		);
	...
	public static function cp_pages( $atts ) {
		...
		switch ( $page ) {
			case 'enrollment_process':
				BrainPress_View_Front_Student::render_enrollment_process_page();
				break;

			case 'student_login':
				BrainPress_View_Front_Login::render_student_login_page();
				break;

			case 'student_signup':
				BrainPress_View_Front_Signup::render_student_signup_page();
				break;

			case 'student_dashboard':
				BrainPress_View_Front_Student::render_student_dashboard_page( false, $atts );
				break;
			...
			default:
				_e( 'Seite kann nicht gefunden werden', 'brainpress' );
		}

		$content = wpautop( ob_get_clean(), apply_filters( 'brainpress_pages_content_preserve_line_breaks', true ) );
	...
	public static function messaging_submenu( $atts ) {
		...
		if ( isset( $brainpress->inbox_subpage ) ) {
			$subpage = $brainpress->inbox_subpage;
		} else {
			$subpage = '';
		}
		...
		$url_inbox = $brainpress->get_inbox_slug( true );
		$url_messages = $brainpress->get_sent_messages_slug( true );
		$url_compose = $brainpress->get_new_message_slug( true );
	...
	public static function process_registration_form() {
		if ( ! is_user_logged_in() ) {
			BrainPress_UserLogin::process_registration_form();
		}
	}
	...
	public static function course_signup( $atts ) {
		...
		if ( is_user_logged_in() ) {
			return __( 'Du bist bereits eingeloggt.', 'brainpress' );
		}
		...
		extract(
			shortcode_atts(
				array(
					...
					'failed_login_text' => __( 'ungültiger Benutzername oder Passwort.', 'brainpress' ),
					'login_tag' => 'h3',
					'login_title' => __( 'Einloggen', 'brainpress' ),
					'login_url' => '',
					'logout_url' => '',
					'page' => isset( $_REQUEST['page'] ) ? $_REQUEST['page'] : '',
					'redirect_url' => '', // Redirect on successful login or signup.
					'signup_tag' => 'h3',
					'signup_title' => __( 'Anmelden', 'brainpress' ),
					'signup_url' => '',
				),
				$atts,
				'course_signup'
		...
		$signup_url = empty( $signup_url ) ? BrainPress_Core::get_slug( 'signup', true ) : $signup_url;
		$login_url = empty( $login_url ) ? BrainPress_Core::get_slug( 'login', true ) : $login_url;
		...
		switch ( $page ) {
			case 'signup':
				if ( ! is_user_logged_in() ) {
					if ( BrainPress_Helper_Utility::users_can_register() ) {
						$content .= BrainPress_UserLogin::get_registration_form( $redirect_url, $login_url, $signup_title, $signup_tag );
					} else {
						$content .= __( 'Registrierungen sind nicht erlaubt.', 'brainpress' );
					}
				} else {
					if ( ! empty( $redirect_url ) ) {
						wp_redirect( esc_url_raw( urldecode( $redirect_url ) ) );
					} else {
						wp_redirect( esc_url_raw( BrainPress_Core::get_slug( 'student_dashboard', true ) ) );
					}
					exit;
				}
				break;
			...
			case 'login':
				$content = '<div class="brainpress-form brainpress-form-login">';
				...
				ob_start();
				do_action( 'brainpress_before_login_form' );
				$content .= ob_get_clean();
				$content .= '<form name="loginform" id="student-settings" class="student-settings login-form" method="post">';
				ob_start();
				do_action( 'brainpress_after_start_form_fields' );
				$content .= ob_get_clean();
				/**
				 * Username
				 */
				$content .= '<label class="username">';
				$content .= '<span>' . esc_html__( 'Benutzername oder E-mail Adresse', 'brainpress' ) . '</span>';
				...
				$content .= '<span>' . esc_html__( 'Passwort', 'brainpress' ) . '</span>';
				$content .= '<input type="password" name="pwd" value="' . ( isset( $_POST['pwd'] ) ? esc_attr( $_POST['pwd'] ) : '' ) . '"/>';
				$content .= '</label>';
				ob_start();
				do_action( 'brainpress_form_fields' );
				$content .= ob_get_clean();

				$redirect_to = BrainPress_Core::get_slug( 'student_dashboard', true );
				...
				$content .= '<label class="signup-link full">' . ( BrainPress_Helper_Utility::users_can_register() ? sprintf( __( 'Du hast noch kein Konto? %s%s%s jetzt!', 'brainpress' ), '<a href="' . $signup_url . '">', __( 'Create an Account', 'brainpress' ), '</a>' ) : '' ) . '</label>';
				$content .= '<label class="forgot-link half-left"><a href="' . esc_url( wp_lostpassword_url() ) . '">' . esc_html__( 'Passwort vergessen?', 'brainpress' ) . '</a></label>';
				$content .= '<label class="submit-link half-right"><input type="submit" name="wp-submit" id="wp-submit" class="apply-button-enrolled" value="' . esc_attr__( 'Einloggen', 'brainpress' ) . '"></label>';
				...
				ob_start();
				do_action( 'brainpress_before_end_form_fields' );
				$content .= ob_get_clean();

				$content .= '</form>';

				ob_start();
				do_action( 'brainpress_after_login_form' );
				$content .= ob_get_clean();
				$content .= '</div>';
	...
	public static function course_signup_form( $atts ) {
		...
		extract( shortcode_atts(
			array(
				...
				'login_link_id' => '',
				'login_link_class' => '',
				'login_link_label' => __( 'Du hast bereits ein Konto? <a href="%s" class="%s" id="%s">Melde dich in deinem Konto an</a>!', 'brainpress' ),
				'signup_link_url' => '#',
				'signup_link_id' => '',
				'signup_link_class' => '',
				'signup_link_label' => __( 'Du hast noch kein Konto? <a href="%s" class="%s" id="%s">Erstelle ein Konto</a> jetzt!', 'brainpress' ),
				'forgot_password_label' => __( 'Passwort vergessen?', 'brainpress' ),
				'submit_button_class' => '',
		...
		$signup_url = BrainPress_Core::get_slug( 'signup', true );
					...
					if ( BrainPress_Helper_Utility::users_can_register() ) {
						...
						$content .= '
							...
							<label class="firstname">
								<span>' . esc_html__( 'Vorname', 'brainpress' ) . ':</span>
						...
						$content .= '
							<label class="lastname">
								<span>' . esc_html__( 'Nachname', 'brainpress' ) . ':</span>
						...
						$content .= '
							<label class="username">
								<span>' . esc_html__( 'Benutzername', 'brainpress' ) . ':</span>
						...
						$content .= '<span>' . esc_html__( 'E-Mail', 'brainpress' ) . ':</span>';
						...
						$content .= '<span>' . esc_html__( 'Bestätige das Passwort', 'brainpress' ) . ':</span>';
						...
						$content .= '<input type="checkbox" name="confirm_weak_password" value="1" /> ' . __( 'Bestätige die Verwendung eines schwachen Passworts', 'brainpress' );
						$content .= '</label> ';
						...
						$content .= __( 'Anmeldungen sind nicht erlaubt.', 'brainpress' );
					}
				}
				break;
			...
			case 'login':
				...
				$content .= '
					...
					<label class="password">
						<span>' . esc_html__( 'Passwort', 'brainpress' ) . '</span>
				...
				if ( apply_filters( 'brainpress_popup_allow_account', true ) ) {
					$content .= '
						<label class="existing-link full">
							' . $signup_link_label . '
						</label>
						<label class="forgot-link half-left">
							<a href="' . esc_url( wp_lostpassword_url() ) . '">' . esc_html__( 'Passwort vergessen?', 'brainpress' ) . '</a>
						</label>';
				}
	...
	public static function get_module_page( $course_id, $current_unit_id, $current_module_id ) {
		...
		$units = BrainPress_Data_Course::get_units_with_modules( $course_id, $status );
	...
	public static function term_link( $termlink, $term, $taxonomy ) {
		$course_category_name = BrainPress_Data_Course::get_post_category_name();
		if ( $course_category_name != $taxonomy ) {
			return $termlink;
		}
		$courses_slug = BrainPress_Core::get_setting( 'slugs/course', 'courses' );
