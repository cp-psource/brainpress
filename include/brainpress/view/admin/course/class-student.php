...
class BrainPress_View_Admin_Course_Student {
	...
	public static function render() {
		...
		$content = '';
		ob_start();
		$list_course->search_box( __( 'Search', 'brainpress' ), 'student' );
		$list_course->views();
		$list_course->display();
		...
		if ( BrainPress_Data_Capabilities::can_invite_students( $course_id ) ) {
			...
			if ( ! empty( $invited_students ) ) {
				...
				$content .= '<h2>'. esc_html__( 'Invited Students', 'brainpress' ) . '</h2>';
				$content .= '<p class="description">' . esc_html__( 'List of invited students.', 'brainpress' ) . '</p>';
				$content .= '<table class="wp-list-table widefat fixed striped">';
				$content .= '<thead><tr><th>' . __( 'First Name', 'brainpress' ) . '</th>';
				$content .= '<th>'. __( 'Last Name', 'brainpress' ) . '</th><th>' . __( 'Email', 'brainpress' ) . '</th><th></th></tr></thead>';
				foreach ( $invited_students as $student_email => $student_data ) {
					...
					$content .= sprintf(
						...
						'',
						esc_attr( __( 'Resend Invitation', 'brainpress' ) ),
					...
					$content .= sprintf(
						...
						'',
						esc_attr( __( 'Remove Invitation', 'brainpress' ) ),
			...
			$content .= '<h2>' . esc_html__( 'Invite Student', 'brainpress' ) .'</h2>';
			$content .= '<label class="invite-firstname"><span>' . esc_html__( 'First Name', 'brainpress' ) . '</span><input type="text" name="invite-firstname"></label>';
			$content .= '<label class="invite-lastname"><span>' . esc_html__( 'Last Name', 'brainpress' ) . '</span><input type="text" name="invite-lastname"></label>';
			$content .= '<label class="invite-email"><span>' . esc_html__( 'E-mail', 'brainpress' ) . '</span><input type="text" name="invite-email"></label>';
			$content .= '<div class="invite-submit button button-primary" name="invite-submit" data-nonce="' . $nonce . '">' . esc_html__( 'Invite', 'brainpress' ) . '</div>';
			$content .= '</div>';
		}

		$content .= '<hr /><div class="brainpress_course_email_enroled_students_wrapper">';
		$content .= '<h2>' . esc_html__( 'Send an email to enrolled students', 'brainpress' ) .'</h2>';
		if ( count( $list_course->items ) ) {
			...
			$content .= sprintf( '<th class="row">%s</th>', __( 'Send to', 'brainpress' ) );
			$content .= '<td><select name="send_to" class="widefat">';
			$send_options = array(
				'all' => __( 'All students', 'brainpress' ),
				'all_with_submission' => __( 'All students with submissions', 'brainpress' ),
			);
			$units = BrainPress_Data_Course::get_units( $course_id );

			if ( ! empty( $units ) ) {
				foreach ( $units as $unit ) {
					$send_options[ $unit->ID ] = __( sprintf( 'All students with submissions of &rarr; %s', $unit->post_title ), 'brainpress' );
				}
			}
			...
			$content .= '<th scope="row"><label class="brainpress-email-subject"><span>' . esc_html__( 'Email subject', 'brainpress' ) . '</span></label></th>';
			$content .= sprintf(
				'<td><input type="text" name="email-subject" class="large-text" id="brainpress-email-subject" /></td>',
				__( 'Enter email subject', 'brainpress' )
			);
			$content .= '</tr>';
			...
			$content .= '<th scope="row"><label class="brainpress-email-body"><span>' . esc_html__( 'Email body', 'brainpress' ) . '</span></label></th>';
			$content .= '<td>';
			$content .= sprintf(
				'<p class="description">%s</p>',
				__( 'These codes will be replaced with actual data: STUDENT_FIRST_NAME, STUDENT_LAST_NAME, STUDENT_LOGIN, BLOG_NAME, WEBSITE_NAME, WEBSITE_ADDRESS, LOGIN_ADDRESS, COURSE_ADDRESS, COURSE_EXCERPT, COURSE_NAME, COURSE_OVERVIEW, COURSES_ADDRESS', 'brainpress' )
			);
			...
			$content .= sprintf(
				...
				esc_attr( $nonce ),
				esc_html__( 'Send', 'brainpress' )
			);

		} else {
			$content .= sprintf(
				'<p>%s</p>',
				__( 'You can not send email, there is no enroled studens in this course.', 'brainpress' )
			);
		}
		$content .= '</div>';
	...
	public static function render_student_courses( $student ) {
		...
		printf( '<h2 id="courses">%s</h2>', __( 'Courses', 'brainpress' ) );
		$enrolled_courses = BrainPress_Data_Student::get_enrolled_courses_ids( $student->ID );
		if ( empty( $enrolled_courses ) ) {
			echo wpautop( __( 'No enrolled courses.', 'brainpress' ) );
			return;
		}
...
<thead>
	<tr>
		<th scope="col" class="column-slug"><span><?php _e( 'Title', 'brainpress' ); ?></span></th>
		<th scope="col"><span><?php _e( 'Excerpt', 'brainpress' ); ?></span></th>
		<th scope="col" class="column-rating"><span class="dashicons dashicons-calendar-alt"></span> <span><?php _e( 'Enrolled', 'brainpress' ); ?></span></th>
		<th scope="col" class="column-rating"><span class="dashicons dashicons-calendar-alt"></span> <span><?php _e( 'Start', 'brainpress' ); ?></span></th>
		<th scope="col" class="column-rating"><span class="dashicons dashicons-calendar-alt"></span> <span><?php _e( 'End', 'brainpress' ); ?></span></th>
		<th scope="col" class="column-rating"><span class="dashicons dashicons-clock"></span> <span><?php _e( 'Duration', 'brainpress' ); ?></span></th>
	</tr>
</thead>
<?php
...
foreach ( $enrolled_courses as $course_id ) {
	...
	<div class="row-actions">
		<span class="edit"><a href="<?php echo  admin_url( 'admin.php?page=course_details&course_id=' . $course_id ); ?>" target="_blank"><?php _e( 'Edit', 'brainpress' ); ?></a> | </span>
		<span class="view"><a href="<?php echo get_permalink( $course_id ); ?>" target="_blank"><?php _e( 'View', 'brainpress' ); ?></a></span>
	</td>
	...
	<td><?php echo $is_open_end_course? __( 'Open-ended', 'brainpress' ) : date_i18n( $date_format, strtotime( BrainPress_Data_Course::get_setting( $course_id, 'course_end_date', true ) ) ); ?></td>
	<td><?php
	if ( $is_open_end_course ) {
		_e( '&infin; Days', 'brainpress' );
	} else {
		...
		$days = intval( $days );
		printf( _n( '%s Day', '%s Days', $days, 'brainpress' ), $days );
	}
	?></td>
	</tr>
	<?php
