 ...
 * Units Builder Class
 **/
class BrainPress_View_Admin_Course_UnitBuilder {
	...
	public static function render() {
		...
		$info_text = __( 'Unit Builder is loading...', 'brainpress' );
	...
	public static function view_templates( $template = false ) {
		...
		$templates = array(
				...
				<script type="text/template" id="unit-builder-template">
					<div class="tab-container vertical unit-builder-container">
						...
						<div id="sticky-wrapper" class="sticky-wrapper sticky-wrapper-tabs">
							<div class="tabs"></div>' .
							( $can_create_units ?
							'<div class="sticky-buttons"><div class="button button-add-new-unit"><i class="fa fa-plus-square"></i> ' . __( 'Add New Unit', 'brainpress' ) . '</div></div>' : '' )
						. '</div>
					</div>
					<div class="tab-content tab-content-vertical unit-builder-content">
						...
						<div class="section static unit-builder-no-access" style="display:none;">'. __( 'You do not have sufficient access to edit this unit!', 'brainpress' ) . '</div>
					</div>
					</div>
				</script>
			',
			...
			'unit_builder_header' => '
				...
				<div class="unit-detail" data-cid="<%- unit_cid %>">
					<h3><i class="fa fa-cog"></i>' . __( 'Unit Settings', 'brainpress' ) . '<div class="unit-state">' .
						BrainPress_Helper_UI::toggle_switch(
							'unit-live-toggle',
							'unit-live-toggle',
							array(
								'left' => __( 'Draft', 'brainpress' ),
								'right' => __( 'Live', 'brainpress' ),
							)
						)
					. '</h3>
					<label for="unit_name">' . __( 'Unit Title', 'brainpress' ) . '</label>
					<input id="unit_name" class="wide" type="text" value="<%= unit_title %>" name="post_title" spellcheck="true">
					<div class="unit-additional-info">
					<label class="unit-description">' . __( 'Unit Description', 'brainpress' ) . '</label>
				...
				array(
					...
					'title' => __( 'Unit Featured Image', 'brainpress' ),
					'value' => '<%= unit_feature_image %>', // Add _s template
				)
			) . '
					</div>
					<div class="unit-availability">
						<label for="unit_availability">'. __( 'Unit Availability', 'brainpress' ) . '</label>
						<select id="unit_availability" class="narrow" name="meta_unit_availability">
							<option value="instant"<%= unit_availability == "instant" ? " selected=\"selected\"" : "" %>>'. __( 'Instantly available', 'brainpress' ) . '</option>
							<option value="on_date"<%= unit_availability == "on_date" ? " selected=\"selected\"" : "" %>>'. __( 'Available on', 'brainpress' ) . '</option>
							<option value="after_delay"<%= unit_availability == "after_delay" ? " selected=\"selected\"" : "" %>>'. __( 'Available after', 'brainpress' ) . '</option>
						</select>
						<div class="div-inline ua-div div-on_date" style="display:none;">
							<div class="date"><input id="dpinputavailability" class="dateinput" type="text" value="<%= unit_date_availability %>" name="meta_unit_date_availability" placeholder="'. __( 'instantly', 'brainpress' ) . '" spellcheck="true" /></div>
						</div>
						<div class="div-inline ua-div div-after_delay" style="display:none;">
							<input type="number" min="0" max="9999" name="meta_unit_delay_days" value="<%=unit_delay_days%>" placeholder="'. __( 'e.g. 7', 'brainpress' ) . '" /> <span>'. __( 'Day(s)', 'brainpress' ) . '</span>
						</div>
					</div>
					<div class="progress-next-unit">
						<label>'. esc_html__( 'Progress to next unit', 'brainpress' ) . '</label>
				...
				sprintf( '%s <em>%s</em> %s',
					esc_html__( 'User needs to', 'brainpress' ),
					esc_html__( 'answer', 'brainpress' ),
					esc_html__( 'all required assessments and view all pages in order to access the next unit', 'brainpress' )
				) . '</span></label>
			...
			sprintf( '%s <em>%s</em> %s',
				esc_html__( 'User also needs to', 'brainpress' ),
				esc_html__( 'pass', 'brainpress' ),
				esc_html__( 'all required assessments', 'brainpress' )
			) . '</span></label>
					</div>
				</div>
				<div class="unit-buttons">
					<div class="button unit-save-button">' . __( 'Save Whole Units', 'brainpress' ) . '</div>
					<a href="#" data-href="'. esc_attr( BrainPress_Data_Course::get_course_url( $course_id ) ) . BrainPress_Core::get_slug( 'units/' ) . '" class="button button-preview" target="_blank">'. __( 'Preview', 'brainpress' ) . '</a>
					<div class="button unit-delete-button"><i class="fa fa-trash-o"></i> ' . __( 'Delete Unit', 'brainpress' ) . '</div></div>
				</script>
			',
			'unit_builder_content_placeholder' => '
				<script type="text/template" id="unit-builder-content-placeholder">
				<div class="loading">
				' . esc_html__( 'Loading modules...', 'brainpress' ) . '
				</div>
				</script>
			',
			...
			'unit_builder_content_pager' => '
				<script type="text/template" id="unit-builder-pager-template">
					<label>' . esc_html__( 'Unit Sections', 'brainpress' ) . '</label>
					<ul>
			...
			'unit_builder_content_pager_info' => '
				<script type="text/template" id="unit-builder-pager-info-template">
					...
					<div class="unit-buttons"><div class="button unit-delete-page-button hidden"><i class="fa fa-trash-o"></i> ' . esc_html__( 'Delete Section', 'brainpress' ) . '</div></div>
					<label>' . esc_html__( 'Section Title', 'brainpress' ) . '</label>
					<p class="description">' . esc_html__( 'The label will be displayed on the Course Overview and Unit page', 'brainpress' ) . '</p>
					<input type="text" value="<%= page_label_text %>" name="page_title" class="wide" />
					<label class="page-description">' . esc_html__( 'Section Description', 'brainpress' ) . '</label>
				...
				array(
					'placeholder' => __( 'Add Image URL or Browse for Image', 'brainpress' ),
					'title' => __( 'Section Image', 'brainpress' ),
					'value' => '<%= page_feature_image %>', // Add _s template
				)
			) . '
					<label><input type="checkbox" value="on" name="show_page_title" <%= page_label_checked %> /><span>' . esc_html__( 'Show section header as part of unit', 'brainpress' ) . '</span></label>
					</div>
				</script>
			',
			'unit_builder_modules' => '
				<script type="text/template" id="unit-builder-modules-template">
					'. __( 'Modules! This template wont be used... its just here for testing.', 'brainpress' ) . '
				</script>
			',
			'unit_builder_footer' => '
				...
				<div class="button unit-save-button">' . __( 'Save Whole Units', 'brainpress' ) . '</div>
				<a href="#" data-href="'. esc_attr( BrainPress_Data_Course::get_course_url( $course_id ) ) . BrainPress_Core::get_slug( 'units/' ) . '" class="button button-preview" target="_blank">'. __( 'Preview', 'brainpress' ) . '</a>
				<!-- <a class="button button-preview" href="#">'. __( 'Preview', 'brainpress' ) . '</a> -->
				' .
					BrainPress_Helper_UI::toggle_switch(
						'unit-live-toggle-2',
						'unit-live-toggle-2',
		...
		$templates['unit_builder_content_components'] = '
				<script type="text/template" id="unit-builder-components-template">
					<label class="bigger">' . esc_html__( 'Modules', 'brainpress' ) . '</label>
					<p class="description">' . esc_html__( 'Click to add module elements to the unit', 'brainpress' ) . '</p>';
		...
		switch ( $task ) {
			case 'units':
				...
				if ( empty( $units ) ) {
					...
					$unit = array(
						'post_title' => __( 'Untitled Unit', 'brainpress' ),
