...
class BrainPress_View_Admin_Course_Export {

	private static $slug = 'brainpress_export_import';
	...
	public static function init() {
		self::$title = __( 'Export', 'brainpress' );
		self::$menu_title = __( 'Export', 'brainpress' );
		...
		add_filter( 'brainpress_admin_page_main', array( __CLASS__, 'add_admin_notice' ) );
	}
	...
	public static function render_page() {
		self::_show_message();
		echo '<div class="wrap">';
		echo BrainPress_Helper_UI::get_admin_page_title(
			self::$menu_title,
			self::$menu_title
		);
		echo '</div>';
	...
	public static function add_page( $pages ) {
		$pages[ self::$slug ] = array(
			...
			'cap' => apply_filters( 'brainpress_capabilities', 'brainpress_courses_cap' ),
			'parent' => BrainPress_View_Admin_Setting::get_slug(),
		);
		return $pages;
	}

	/**
	 * Add Export page slug to valid BrainPress pages.
	 *
	 * @since 2.0.0
	 *
	 * @param array Array of BrainPress pages.
	 * @return array Array of BrainPress pages extende by Export page slug.
	 */
	...
	private static function _export() {
		...
		foreach ( self::$courses as $course_id ) {
			...
			if ( BrainPress_Data_Capabilities::can_update_course( $course_id, $user_id ) ) {
		 ...
		 */
		if ( empty( $course_ids ) ) {
			$slug = BrainPress_View_Admin_BrainPress::get_slug();
			$url = add_query_arg(
				array(
					'page' => $slug,
					'm' => 'no-courses',
		...
		$date = date( 'Y-m-d' );
		$wp_filename = $sitename . 'brainpress.' . $date;
		 ...
		 * get course_category
		 */
		$name = BrainPress_Data_Course::get_post_category_name();
		$terms = get_terms(
			$name,
			array(
				'hide_empty' => false,
		...
		$unit_ids = BrainPress_Data_Unit::get_unit_ids_by_course_ids( $course_ids );
		$module_ids = BrainPress_Data_Module::get_module_ids_by_unit_ids( $unit_ids );
		...
		function wxr_site_url() {
			...
			if ( is_multisite() ) {
				return network_home_url(); } // ClassicPress (single site): the blog URL.
...
<?php
	...
	public static function add_admin_notice( $content ) {
		...
		switch ( $_REQUEST['m'] ) {
			case 'no-courses':
				$content = BrainPress_Helper_UI::admin_notice(
					__( 'Export Courses: please select some courses first.', 'brainpress' ),
					'error'
				).$content;
			break;
		}
