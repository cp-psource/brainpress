...
(function( $ ) {
	BrainPress.Views = BrainPress.Views || {};
	BrainPress.Models = BrainPress.Models || {};
	BrainPress.Collections = BrainPress.Collections || {};
	BrainPress.Helpers = BrainPress.Helpers || {};

	/** Expand the BrainPress Object for handling Modules **/
	BrainPress.Helpers.Module = BrainPress.Helpers.Module || {};
	BrainPress.Helpers.Module.quiz = BrainPress.Helpers.Module.quiz || {};
	BrainPress.Helpers.Module.form = BrainPress.Helpers.Module.form || {};

	BrainPress.Helpers.Module.refresh_ui = function() {
		...
		$.each( $( '.unit-builder-modules .editor' ), function( index, editor ) {
			...
			if ( "undefined" === typeof tinyMCE.editors[id] ) {
				...
				BrainPress.editor.create( editor, id, name, content, false, height );
			}
		} );
		...
		var element = 0;
		if ( BrainPress.UnitBuilder.activeModuleRef && BrainPress.UnitBuilder.activeModuleRef.length > 0 ) {
			var active = $( '[data-cid="' + BrainPress.UnitBuilder.activeModuleRef + '"]' )[ 0 ];
			...
			stop: function( event, ui ) {
				...
				$.each( modules, function( index, module ) {
					...
					if ( current_order !== new_order ) {
						BrainPress.UnitBuilder.module_collection._byId[ cid ].set_meta( 'module_order', new_order );
		...
		$( '.unit-builder-tabs ul' ).sortable( {
			stop: function() {
				...
				$.each( units, function( index, unit ) {
					...
					if ( current_order !== new_order ) {
						var meta = BrainPress.UnitBuilder.unit_collection._byId[ cid ].get( 'meta' );
						meta[ 'unit_order' ] = new_order;
						BrainPress.UnitBuilder.unit_collection._byId[ cid ].set( 'meta', meta );
						BrainPress.UnitBuilder.unit_collection._byId[ cid ].set( 'flag', 'dirty' );
						$( unit ).addClass( 'dirty' );
					}
				} );
			}
		...
		$( '.unit-builder-pager ul li[data-page="' + BrainPress.UnitBuilder.activePage + '"]' ).addClass( 'active' );
		// ====== BRAINPRESS UI TOGGLES =====
		if ( 'publish' === BrainPress.UnitBuilder.activeUnitStatus ) {
		...
		var activeUnit = BrainPress.UnitBuilder.unit_collection.get( BrainPress.UnitBuilder.activeUnitRef ),
			unit_user_cap = activeUnit && activeUnit.get( 'user_cap' ) ? activeUnit.get( 'user_cap' ) : {},
			can_publish = unit_user_cap['brainpress_change_unit_status_cap'];
		$( '.brainpress-ui-toggle-switch' ).each( function() {
			var	ui = $( this ),
				ui_name = ui.attr( 'name' ),
				is_toggle = true;
			if ( 'publish-course-toggle' === ui_name ) {
				if ( ! BrainPress.current_user_can( 'brainpress_change_status_cap' ) ) {
					is_toggle = false;
				}
			}
			...
			if ( is_toggle ) {
				ui.brainpress_ui_toggle();
			}
		});
		// Hide delete button
		var unit_delete_button = BrainPress.UnitBuilder.$el.find( '.unit-delete-button' ),
			can_edit = unit_user_cap['brainpress_delete_course_units_cap'];
		unit_delete_button[ can_edit ? 'show' : 'hide' ]();
		// Delete Page button
		if ( BrainPress.UnitBuilder.totalPages === 1 ) {
		...
		$( '.unit-builder-pager ul li[data-page]' ).droppable( {
			...
			drop: function( event, ui ) {
				...
				var current_page = BrainPress.UnitBuilder.activePage;
				var mod_el = ui.draggable;
				if ( current_page !== page ) {
					var mod_ref = $( $( mod_el ).find( '.module-holder' )[ 0 ] ).attr( 'data-cid' );
					var meta = BrainPress.UnitBuilder.module_collection._byId[ mod_ref ].get( 'meta' );
					meta[ 'module_page' ] = page;
					meta[ 'module_order' ] = meta[ 'module_order' ] + 900;
					BrainPress.UnitBuilder.module_collection._byId[ mod_ref ].set( 'meta', meta );
					BrainPress.UnitBuilder.module_collection._byId[ mod_ref ].trigger( 'change', BrainPress.UnitBuilder.module_collection._byId[ mod_ref ] );
					$( mod_el ).detach();
				} else {
				}
			}
		...
		$( '.dateinput' ).datepicker( {
			dateFormat: 'yy-mm-dd'
			//firstDay: brainpress.start_of_week
		} );
		$( '.date' ).off( 'sync' );
		...
		$( '.unit-builder-body .quiz-action-button').on( 'click', function() {
			...
			switch( type ) {
				case 'single':
					...
					question_type = _brainpress.unit_builder.question_type.single;
					question_content += '<div class="answer-group">';
					question_content += '<div class="answer"><input type="radio" name="' + radio_name + '" value="" />';
					question_content += '<input class="component-radio-answer wide" type="text" value="' + _brainpress.unit_l8n.pre_answers.a + '" /><span class="remove-quiz-item"><i class="fa fa-trash-o"></i></span></div>';
					question_content += '<div class="answer"><input type="radio" name="' + radio_name + '" value="" />';
					question_content += '<input class="component-radio-answer wide" type="text" value="' + _brainpress.unit_l8n.pre_answers.b + '" /><span class="remove-quiz-item"><i class="fa fa-trash-o"></i></span></div>';
					question_content += '</div>';
					question_content += '<a class="add-quiz-item">' + _brainpress.unit_builder_add_answer_label + '</a>';
					break;
				case 'multiple':
					question_type = _brainpress.unit_builder.question_type.multiple;
					question_content += '<div class="answer-group">';
					question_content += '<div class="answer"><input type="checkbox" name="" value="" />';
					question_content += '<input class="component-checkbox-answer wide" type="text" value="' + _brainpress.unit_l8n.pre_answers.a + '" name="" /><span class="remove-quiz-item"><i class="fa fa-trash-o"></i></span></div>';
					question_content += '<div class="answer"><input type="checkbox" name="" value="" />';
					question_content += '<input class="component-checkbox-answer wide" type="text" value="' + _brainpress.unit_l8n.pre_answers.b + '" name="" /><span class="remove-quiz-item"><i class="fa fa-trash-o"></i></span></div>';
					question_content += '</div>';
					question_content += '<a class="add-quiz-item">' + _brainpress.unit_builder_add_answer_label + '</a>';
					break;
				case 'short':
					question_type = _brainpress.unit_builder.question_type.short;
					question_content += '<div class="answer-group">';
					question_content += '<label data-key="label" class="wide">';
					question_content += '<span class="label">' + _brainpress.unit_builder_form_pleaceholder_label + '</span>';
					question_content += '<span class="description">' + _brainpress.unit_builder_form_pleaceholder_desc + '</span>';
					...
					break;
				case 'long':
					question_type = _brainpress.unit_builder.question_type.long;
					question_content += '<div class="answer-group">';
					question_content += '<label data-key="label" class="wide">';
					question_content += '<span class="label">' + _brainpress.unit_builder_form_pleaceholder_label + '</span>';
					question_content += '<span class="description">' + _brainpress.unit_builder_form_pleaceholder_desc + '</span>';
			...
			BrainPress.Helpers.Module.quiz.update_meta( mod_el );
			BrainPress.Helpers.Module.quiz.bind_buttons();
		} );
		BrainPress.Helpers.Module.quiz.bind_add_item();
		BrainPress.Helpers.Module.form.bind_buttons();
		// ===== FORM BUTTONS =====
		...
		$( '.unit-builder-body .form-action-button').on( 'click', function() {
			...
			switch( type ) {
				...
				case 'selectable':
					...
					question_type = _brainpress.unit_builder.question_type.selectable;
					question_content += '<div class="answer-group">';
					question_content += '<div class="answer"><input type="radio" name="' + radio_name + '" value="" />';
					question_content += '<input class="component-select-answer wide" type="text" value="' + _brainpress.unit_l8n.pre_answers.a + '" /><span class="remove-form-item"><i class="fa fa-trash-o"></i></span></div>';
					question_content += '<div class="answer"><input type="radio" name="' + radio_name + '" value="" />';
					question_content += '<input class="component-select-answer wide" type="text" value="' + _brainpress.unit_l8n.pre_answers.b + '" /><span class="remove-form-item"><i class="fa fa-trash-o"></i></span></div>';
					question_content += '</div>';
					question_content += '<a class="add-form-item">' + _brainpress.unit_builder_add_answer_label + '</a>';
					break;
			}
			question_content += '</div>'; // .question-answer
			// Same for all
			...
			content += '</div>';
			$(container).append( content );
			BrainPress.Helpers.Module.form.update_meta( mod_el );
			BrainPress.Helpers.Module.form.bind_buttons();
		} );
		BrainPress.Helpers.Module.form.bind_buttons();		
		...
		$.each( quiz, function( index, item ) {
			...
			switch( item.type ) {
				case 'single':
					...
					//question_content += '<input type="text" value="' + _brainpress.unit_l8n.pre_answers.a + '" name="" /><span class="remove-item"><i class="fa fa-trash-o"></i></span></div>';
					//question_content += '<div class="answer"><input type="radio" name="" value="" />';
					//question_content += '<input type="text" value="' + _brainpress.unit_l8n.pre_answers.b + '" name="" /><span class="remove-item"><i class="fa fa-trash-o"></i></span></div>';
					//
					//question_content += '</div>';
					//question_content += '<a class="add-item">' + _brainpress.unit_builder_add_answer_label + '</a>';
					question_type = _brainpress.unit_builder.question_type.single;
					question_content += '<div class="answer-group">';
	...
	BrainPress.Helpers.Module.quiz.update_meta = function( quiz_el ) {

		var cid = $( quiz_el).attr('data-cid');
		var questions = {};

		var module = BrainPress.UnitBuilder.module_collection._byId[ cid ];
	...
	BrainPress.Helpers.Module.quiz.bind_add_item = function() {
		...
		$('.quiz-question .add-quiz-item').on( 'click', function() {
			...
			BrainPress.Helpers.Module.quiz.bind_remove_item();
			BrainPress.Helpers.Module.quiz.bind_checkboxes();
			BrainPress.Helpers.Module.quiz.bind_textboxes();
		} );
	};

	BrainPress.Helpers.Module.quiz.bind_checkboxes = function() {
	...
	BrainPress.Helpers.Module.quiz.bind_textboxes = function() {
	...
	BrainPress.Helpers.Module.quiz.bind_remove_item = function() {
	...
	BrainPress.Helpers.Module.quiz.bind_remove_question = function() {
		// Remove Quiz
	...
	BrainPress.Helpers.Module.quiz.bind_buttons = function() {
		BrainPress.Helpers.Module.quiz.bind_add_item();
		BrainPress.Helpers.Module.quiz.bind_remove_item();
		BrainPress.Helpers.Module.quiz.bind_remove_question();
		BrainPress.Helpers.Module.quiz.bind_checkboxes();
		BrainPress.Helpers.Module.quiz.bind_textboxes();
	};


	BrainPress.Helpers.Module.form.render_component = function( module ) {
	...
	BrainPress.Helpers.Module.form.update_meta = function( form_el ) {
	...
	BrainPress.Helpers.Module.form.bind_add_item = function() {
		...
		$('.quiz-question .add-form-item').on( 'click', function() {
			...
			$(question).find('.answer-group').append( content );
			BrainPress.Helpers.Module.form.bind_remove_item();
			BrainPress.Helpers.Module.form.bind_checkboxes();
			BrainPress.Helpers.Module.form.bind_textboxes();
		} );
	};

	BrainPress.Helpers.Module.form.bind_checkboxes = function() {
		$('.quiz-question [type="checkbox"], .quiz-question [type="radio"]').off( 'change' );
		$('.quiz-question [type="checkbox"], .quiz-question [type="radio"]').on( 'change', function() {
			var mod_el = $( this).parents('.module-holder')[0];
			BrainPress.Helpers.Module.form.update_meta( mod_el );
	...
	BrainPress.Helpers.Module.form.bind_textboxes = function() {
		$('.quiz-question [type="text"], .quiz-question textarea').off( 'keyup' );
		$('.quiz-question [type="text"], .quiz-question textarea').on( 'keyup', function() {
			var mod_el = $( this).parents('.module-holder')[0];
			BrainPress.Helpers.Module.form.update_meta( mod_el );
	...
	BrainPress.Helpers.Module.form.bind_remove_item = function() {
	...
	BrainPress.Helpers.Module.form.bind_remove_question = function() {
		// Remove Quiz
		$('.quiz-question .quiz-question-remove').off( 'click' );
		$('.quiz-question .quiz-question-remove').on( 'click', function() {
	...
	BrainPress.Helpers.Module.form.bind_buttons = function() {
		BrainPress.Helpers.Module.form.bind_add_item();
		BrainPress.Helpers.Module.form.bind_remove_item();
		BrainPress.Helpers.Module.form.bind_remove_question();
		BrainPress.Helpers.Module.form.bind_checkboxes();
		BrainPress.Helpers.Module.form.bind_textboxes();
	};	
	 ...
	 * Delete page/section.
	 */
	BrainPress.Helpers.Module.delete_page = function( e, unit_id, page ) {
		var nonce = $( '#unit-builder' ).attr( 'data-nonce' );
		BrainPress.UnitBuilder.module_collection.url = _brainpress._ajax_url + '?action=unit_builder&task=modules_update_delete_section&course_id=' + _brainpress.course_id + '&unit_id=' + BrainPress.UnitBuilder.activeUnitID + '&page=' + BrainPress.UnitBuilder.activePage + '&wp_nonce=' + nonce + '&page='+page;
		Backbone.sync( 'update', BrainPress.UnitBuilder.module_collection, {
	...
	BrainPress.Helpers.Module.save_unit = function( e, custom_event ) {
		...
		if ( 0 < requireds.length ) {
			...
			if ( 0 < errors.length ) {
				...
				alert( _brainpress.unit_builder_form.messages.required_fields + "\n" + errors.join( "\n" ) );
				return false;
			}
		}
		// Save modules first... just in case the unit is deleted to avoid orphans
		BrainPress.UnitBuilder.module_collection.url = _brainpress._ajax_url + '?action=unit_builder&task=modules_update&course_id=' + _brainpress.course_id + '&unit_id=' + BrainPress.UnitBuilder.activeUnitID + '&page=' + BrainPress.UnitBuilder.activePage + '&wp_nonce=' + nonce + '&x=1';
		Backbone.sync( 'update', BrainPress.UnitBuilder.module_collection, {
			success: function( response ) {
				$( '#unit-builder' ).attr( 'data-nonce', response[ 'nonce' ] );
			},
		...
		BrainPress.UnitBuilder.unit_collection.url = _brainpress._ajax_url + '?action=unit_builder&task=units_update&course_id=' + _brainpress.course_id + '&wp_nonce=' + nonce;
		Backbone.sync( 'update', BrainPress.UnitBuilder.unit_collection, {
			success: function( response ) {
				$( '.save-progress' ).detach();
				nonce = response[ 'nonce' ];
				$( '#unit-builder' ).attr( 'data-nonce', nonce );
				BrainPress.UnitBuilder.unit_collection.trigger( custom_event, BrainPress.UnitBuilder.unit_collection );
				BrainPress.Helpers.Module.unit_show_message( _brainpress.unit_builder_form.messages.successfully_saved, 'success' );
			},
			error: function() {
				$( '.save-progress' ).detach();
				$( e.currentTarget ).prepend( '<i class="fa fa-info-circle save-progress"></i> ' );
				BrainPress.Helpers.Module.unit_show_message( _brainpress.unit_builder_form.messages.error_while_saving, 'error' );
			}
		} );

		// Reset URL
		BrainPress.UnitBuilder.unit_collection.url = _brainpress._ajax_url + '?action=unit_builder&task=units&course_id=' + _brainpress.course_id;

		/**
		 * Add message
		 */
		BrainPress.Helpers.Module.unit_show_message( _brainpress.unit_builder_form.messages.saving_unit, 'info' );
	};

	BrainPress.Helpers.Module.unit_add_show_message = function( message, notice_class ) {
	...
	BrainPress.Helpers.Module.unit_show_message = function( message, notice_class ) {
	...
	BrainPress.Helpers.Module.toggle_unit_state = function() {
		var nonce = $( '#unit-builder' ).attr( 'data-nonce' );
		...
		this.switch = function( state, unit_id, unit_ref ) {
			...
			var current_state = BrainPress.UnitBuilder.unit_collection._byId[ unit_ref ].get( 'post_status' );

			if ( current_state !== state ) {
				BrainPress.UnitBuilder.unit_collection._byId[ unit_ref ].set( 'post_status', state );
				BrainPress.UnitBuilder.unit_collection._byId[ unit_ref ].trigger( 'change', BrainPress.UnitBuilder.unit_collection._byId[ unit_ref ] );
		...
		var unit_id = BrainPress.UnitBuilder.activeUnitID;
		var unit_ref = BrainPress.UnitBuilder.activeUnitRef;
		var state = BrainPress.UnitBuilder.unit_collection._byId[ unit_ref ].get( 'post_status' );
		state = 'publish' === state ? 'draft' : 'publish';

		BrainPress.UnitBuilder.unit_collection.url = _brainpress._ajax_url + '?action=unit_builder&task=unit_toggle&course_id=' + _brainpress.course_id + '&wp_nonce=' + nonce + '&state=' + state + '&unit_id=' + unit_id;
		Backbone.sync( 'update', BrainPress.UnitBuilder.unit_collection, {
			success: function( response ) {
				self.switch( response[ 'post_status' ], unit_id, unit_ref );
				$( '#unit-builder' ).attr( 'data-nonce', response[ 'nonce' ] );
				BrainPress.UnitBuilder.activeUnitStatus = response[ 'post_status' ];
			},
	...
	BrainPress.Helpers.Module.render_module = function( module, current_order ) {
		var types = _brainpress.unit_builder_module_types;
		var labels = _brainpress.unit_builder_module_labels;
		var content;
		var data;

		if ( module.module_type() && _brainpress.unit_builder_templates[ module.module_type() ].trim().length > 0 ) {
			data = JSON.parse( _brainpress.unit_builder_templates[ module.module_type() ] );
		}

		if ( undefined === data || undefined === _brainpress.unit_builder_module_types[ data[ 'type' ] ] ) {
			return '';
		}
		...
		function module_mandatory() {
			return '<label class="module-mandatory">' +
				'<input type="checkbox" name="meta_mandatory[' + module.cid + ']" value="1" ' + BrainPress.utility.checked(data['mandatory'], 1) + ' />' +
		...
		function module_assessable() {
			return '<label class="module-assessable">' +
				'<input type="checkbox" data-target=".module-minimum-grade" name="meta_assessable[' + module.cid + ']" value="1" ' + BrainPress.utility.checked(data['assessable'], 1) + ' />' +
		...
		function module_use_timer() {
			return '<label class="module-use-timer">' +
				'<input type="checkbox" data-target=".module-duration" name="meta_use_timer[' + module.cid + ']" value="1" ' + BrainPress.utility.checked(data['use_timer'], 1) + ' />' +
		...
		function module_allow_retries() {
			return '<label class="module-allow-retries">' +
				'<input type="checkbox" name="meta_allow_retries[' + module.cid + ']" value="1" ' + BrainPress.utility.checked(data['allow_retries'], 1) + ' />' +
		...
		if ( ( types[ data[ 'type' ] ][ 'body' ] && 'hidden' !== types[ data[ 'type' ] ][ 'body' ] ) || !types[ data[ 'type' ] ][ 'body' ] ) {
			...
			content += '<div class="module module-title"><h4 class="label">' + labels[ 'module_title' ] + '</h4>' +
				...
				'<input type="checkbox" class="show-title" name="meta_show_title[' + module.cid + ']" value="1" ' + BrainPress.utility.checked( data[ 'show_title' ], 1 ) + ' data-target=".module-title-text"/>' +
			...
			if ( 'input-upload' === module_type ) {
					...
					content += '<label class="module-assessable-2">'
						+ '<input type="checkbox" name="meta_instructor_assessable[' + module.cid + ']" value="1" ' + BrainPress.utility.checked(data['instructor_assessable'], 1) + ' />'
			...
			BrainPress.Helpers.Module.render_components( module, data ) +
			'</div>';
	...
	BrainPress.Helpers.Module.render_components = function( module, data ) {
		var content = '';
			...
			$.each( items, function( idx, item ) {
				...
				switch ( item_type ) {
					...
					case 'radio-select':
						...
						//answers = answers.length > 0 ? BrainPress.utility.unserialize( answers ) : item['answers'];
						...
						$.each( answers, function( index, answer ) {
							...
							content += '<div class="answer"><input type="radio" name="' + option_name + '" value="' + index + '" ' + BrainPress.utility.checked( parseInt( selected ), index ) + ' />';
							content += '<input type="text" ' + attr + ' value="' + answer + '" name="' + name + '[]" /> <span class="remove-item"></span><i class="fa fa-trash-o"></i></span></div>';
						} );
						content += '</div>';
						content += '<a class="add-item">' + _brainpress.unit_builder_add_answer_label + '</a>';
						break;
					...
					case 'checkbox-select':
						...
						//selected = selected.length > 0 ? BrainPress.utility.unserialize( selected ) : item['selected'];
					...
					case 'media-caption-settings':
						...
						'<label><input type="checkbox" value="1" name="' + enable_name + '" ' + BrainPress.utility.checked( show_caption_value, 1 ) + ' />' +
						'<span>' + show_caption + '</span></label>' +
						'<div ' + option_class + '>' +
						'<label><input type="radio" value="media" name="' + option_name + '" ' + BrainPress.utility.checked( caption_type, 'media' ) + ' />' +
						'<span>' + media_caption + '</span></label>' +
						'<div class="existing">' + no_caption + '</div>' +
						'<label><input type="radio" value="custom" name="' + option_name + '" ' + BrainPress.utility.checked( caption_type, 'custom' ) + ' />' +
						...
						if ( component_data.media_url && component_data.media_url.length > 0 ) {
							BrainPress.utility.attachment_by_url( component_data.media_url, '.' + component_selector + ' .existing', no_caption );
					...
					case 'media-browser':
						...
						content += BrainPress.UI.browse_media_field( name, name, {
							value: value,
							type: media_type,
					...
					case 'checkbox':
						...
						content += '<label class="normal"><input type="checkbox" value="1" name="' + name + '" ' + BrainPress.utility.checked( value, 1 ) + ' />' +
					...
					case 'quiz':
						content += BrainPress.Helpers.Module.quiz.render_component( module );
						break;

					case 'form':
						content += BrainPress.Helpers.Module.form.render_component( module );
						break;
	...
	/** Add the BrainPress Unit Builder Views **/
		// Parent View / Models / Collections
	BrainPress.Views.UnitBuilder = Backbone.View.extend( {
		initialize: function() {
			...
			this.unit_collection = new BrainPress.Collections.UnitTabs();
			this.unit_collection.url = _brainpress._ajax_url + '?action=unit_builder&task=units&course_id=' + _brainpress.course_id;
			this.unit_collection.fetch();

			// Holds the modules for the current unit
			this.module_collection = new BrainPress.Collections.UnitModules();

			// Displays the tabs
			this.tabViewCollection = new BrainPress.Views.UnitTabViewCollection( {
			...
			this.headerView = new BrainPress.Views.UnitBuilderHeader();
			this.headerView.parentView = this;

			// Displays the content
			this.contentView = new BrainPress.Views.UnitBuilderBody( {
		...
		fetchModules: function( unit_id, page ) {
			...
			this.module_collection.url = _brainpress._ajax_url + '?action=unit_builder&task=modules&course_id=' + _brainpress.course_id + '&unit_id=' + unit_id + '&page=' + page;
		...
		saveUnit: function( e ) {
			...
			BrainPress.Helpers.Module.save_unit( e );
		},
		toggleUnitState: function( e ) {
			BrainPress.Helpers.Module.toggle_unit_state( e );
		},
		deleteModule: function( e ) {
			if ( window.confirm( _brainpress.unit_builder_delete_module_confirm ) ) {
		...
		deletePage: function( e ) {
			var self = this;
			if ( ! window.confirm( _brainpress.unit_builder_delete_page_confirm ) ) {
				return;
			}
			var page = parseInt( this.activePage );
			// Trigger save
			BrainPress.Helpers.Module.delete_page( e, self.activeUnitID, page );
			/**
			 * reload window
			 */
			location.reload();
		...
		deleteUnit: function( e ) {
			...
			if ( window.confirm( _brainpress.unit_builder_delete_unit_confirm ) ) {
		...
		newUnit: function( e ) {
			...
			var unit = new BrainPress.Models.Unit();
			unit.set_meta( 'unit_order', (count + 1) );
			unit.set_meta( 'page_title', { page_1: '' } );
			unit.set_meta( 'show_page_title', [ true ] );
			unit.set( 'post_title', _brainpress.unit_builder_new_unit_title );

			self.unit_collection.add( unit );
			BrainPress.Helpers.Module.save_unit( e, 'new_success' );

			var unitView = new BrainPress.Views.UnitTabView( { model: unit, tagName: 'li' } );
			...
			self.unit_collection.on( 'new_success', function() {
				self.unit_collection.fetch( {
					success: function() {
						...
						BrainPress.Helpers.Module.refresh_ui();
						// Update Unit Count
						$( $( 'li[data-tab="units"] a' )[ 0 ] ).html( $( $( 'li[data-tab="units"] a' )[ 0 ] ).html().replace( /\d+/, $( '.unit-builder-tabs ul li' ).length ) );
					}
				} );
		...
		changeActive: function( e ) {
			...
			this.unit_collection.each( function( unit ) {
				if ( parseInt( model_id ) === parseInt( unit.get( 'ID' ) ) ) {
					BrainPress.Helpers.changeUnit( unit, self );
				}
			} );
		},
	...
	BrainPress.Models.Unit = Backbone.Model.extend( {
		initialize: function() {
			this.url = _brainpress._ajax_url + '?action=unit_builder&task=unit_update&course_id=' + _brainpress.course_id + '&unit_id=' + BrainPress.UnitBuilder.activeUnitID;
	...
	BrainPress.Models.Module = Backbone.Model.extend( {
		initialize: function() {
			var nonce = $( '#unit-builder' ).attr( 'data-nonce' );
			this.url = _brainpress._ajax_url + '?action=unit_builder&task=module_add&course_id=' + _brainpress.course_id + '&unit_id=' + BrainPress.UnitBuilder.activeUnitID + '&wp_nonce=' + nonce;
			this.on( 'change', this.process_changed, this );
			this.on( 'sync', this.model_saved, this );
		},
		...
		from_template: function( template ) {
			...
			var data = JSON.parse( _brainpress.unit_builder_templates[ template ] );
			this.set( 'ID', data[ 'id' ] );
		...
		model_saved: function( model ) {
			BrainPress.UnitBuilder.activeModuleRef = model.cid;
			BrainPress.UnitBuilder.gotoAdded = true;
		},
		process_changed: function() {
			BrainPress.UnitBuilder.activeModuleRef = this.cid;
			this.set( 'flag', 'dirty' );
		}
	...
	BrainPress.Collections.UnitTabs = Backbone.Collection.extend( {
		model: BrainPress.Models.Unit
	} );

	BrainPress.Collections.UnitModules = Backbone.Collection.extend( {
		model: BrainPress.Models.Module
	} );

	// Single Tab View
	BrainPress.Views.UnitTabView = Backbone.View.extend( {
	...
	// Tab Collection View
	BrainPress.Views.UnitTabViewCollection = Backbone.View.extend( {
		initialize: function() {
	...
	BrainPress.Helpers.changeUnit = function( unit, self, page ) {
		self = BrainPress.UnitBuilder;
		if ( undefined === page ) {
			page = 1;
		}
		...
		if ( ! user_cap['brainpress_update_course_unit_cap'] ) {
	...
	// Unit Header View
	BrainPress.Views.UnitBuilderHeader = Backbone.View.extend( {
		initialize: function() {
			...
			BrainPress.Events.on( 'editor:keyup', this.editorChanged, this );
		},
		events: {
		...
		editorChanged: function( e ) {
			...
			if ( undefined !== the_id && re.test( the_id  ) ) {
				var el = $( '#' + the_id );
				var el_val = BrainPress.editor.content( the_id );
	...
	BrainPress.Views.UnitBuilderFooter = Backbone.View.extend( {
		render: function() {
	...
	// Unit Body View
	BrainPress.Views.UnitBuilderBody = Backbone.View.extend( {
		initialize: function() {
			this.initial = true;
			this.pagerView = new BrainPress.Views.UnitBuilderPager( { className: 'section unit-builder-pager' } );
			this.pagerView.parentView = this;
			this.pagerView.template_variables = {};

			this.pagerViewInfo = new BrainPress.Views.UnitBuilderPagerInfo( { className: 'section unit-builder-pager-info' } );
			this.pagerViewInfo.parentView = this;
			this.pagerViewInfo.template_variables = {};

			this.componentsView = new BrainPress.Views.UnitBuilderComponents( { className: 'section unit-builder-components' } );
			this.componentsView.parentView = this;
			this.componentsView.template_variables = {};

			this.modulesView = new BrainPress.Views.UnitBuilderModules( { className: 'section unit-builder-modules' } );
			this.modulesView.parentView = this;
			this.modulesView.template_variables = {};

			this.footerView = new BrainPress.Views.UnitBuilderFooter( { className: 'unit-buttons' } );
		...
		render: function() {
			...
			if ( this.initial ) {
				...
				if ( BrainPress.UnitBuilder.gotoAdded === true ) {
					BrainPress.UnitBuilder.gotoAdded = false;
					var last_added = $( '[data-cid=' + BrainPress.UnitBuilder.activeModuleRef + ']' );
		...
		BrainPress.Helpers.Module.unit_add_show_message( '<i class="fa fa-circle-o-notch fa-spin"></i> ' + _brainpress.unit_builder_form.messages.adding_module, 'info' );
			var el = e.currentTarget;
			var module_type = $( el ).attr( 'class' ).match( /module-(\w|-)*/g )[ 0 ].trim().replace( 'module-', '' );
			//Count current elements
			var count = $( '.module-holder' ).length;
			var module = new BrainPress.Models.Module();
			...
			//$( '.section.unit-builder-modules' ).append( BrainPress.Helpers.Module.render_module( module, (count + 1) ) );
			//BrainPress.Helpers.Module.refresh_ui();
		},
		fieldChanged: function( e ) {
			var el = $( e.currentTarget );
		...
		editorChanged: function( e ) {
			...
			var value = BrainPress.editor.content( el_name );
	...
	BrainPress.Views.UnitBuilderPager = Backbone.View.extend( {
	...
	BrainPress.Views.UnitBuilderPagerInfo = Backbone.View.extend( {
		initialize: function() {
			BrainPress.Events.on( 'editor:keyup', this.editorChanged, this );
		},
		events: {
	...
	BrainPress.Views.UnitBuilderComponents = Backbone.View.extend( {
		render: function( options ) {
	...
	// Unit Body Modules
	BrainPress.Views.UnitBuilderModules = Backbone.View.extend( {
		render: function() {
			var self = this;
			...
			this.parentView.model.each( function( module ) {
				var moduleView = new BrainPress.Views.ModuleView( {
					model: module,
					tagName: 'div',
	...
	BrainPress.Views.ModuleView = Backbone.View.extend( {
		render: function( module, order ) {
			var self = this;
			self.$el.empty();

			//// Not using a template here
			self.$el.append( BrainPress.Helpers.Module.render_module( module, order ) );
			//
			BrainPress.Helpers.Module.refresh_ui();
	...
	function init_course_builder() {
		BrainPress.UnitBuilder = new BrainPress.Views.UnitBuilder( { el: '#unit-builder' } );
	}
