...
class BrainPress_Template_Module {
	...
	protected static function attributes( $module_id ) {
		return BrainPress_Data_Module::attributes( $module_id );
	}
	...
	protected static function get_response( $module_id, $student_id, $all = false ) {
		...
		$response = BrainPress_Data_Student::get_response( $student_id, $course_id, $unit_id, $module_id );
	...
	public static function get_student_answer( $module_id, $student_id ) {
		 ...
		 **/
		$content = apply_filters( 'brainpress_student_reponse_template', $content, $module_id, $module_type );
	...
	public static function get_module_status( $module_id, $student_id ) {
		...
		$grades = (array) BrainPress_Helper_Utility::get_array_val( $response, 'grades' );
		$grades = array_pop( $grades );
		$grade = (int) BrainPress_Helper_Utility::get_array_val( $grades, 'grade' );
		...
		if ( in_array( $module_type, $assessables ) ) {
			if ( ! empty( $attributes['assessable'] ) ) {
				$graded_by = BrainPress_Helper_Utility::get_array_val( $grades, 'graded_by' );
				if ( 'auto' === $graded_by || empty( $grades ) ) {
					$status = __( 'Ausstehend', 'brainpress' );
				} elseif ( $pass ) {
					$status = __( 'Bestanden', 'brainpress' );
				} else {
				    $status = __( 'Gescheitert', 'brainpress' );
				}
			} else {
				$status = __( 'Nicht bewertbar', 'brainpress' );
			}
		} else {
			$status = $pass ? __( 'Bestanden', 'brainpress' ) : __( 'Gescheitert', 'brainpress' );
		}
	...
	public static function template( $module_id = 0, $is_focus = false, $view_type = 'normal' ) {
		 ...
		 **/
		do_action( 'brainpress_module_view', $module_id, $student_id );

		// Marked module and page as visited
		$module_page = get_post_meta( $module_id, 'module_page', true );
		BrainPress_Data_Student::visited_module( $student_id, $course_id, $unit_id, $module_id );
		BrainPress_Data_Student::visited_page( $student_id, $course_id, $unit_id, $module_page );
		...
		if ( $is_focus ) {
			$content .= wp_nonce_field( 'brainpress_submit_modules', '_wpnonce', true, false );
			$content .= sprintf( '<div class="cp-error">%s</div>', apply_filters( 'brainpress_before_unit_modules', '' ) );
		}
		...
		if ( method_exists( __CLASS__, $method ) ) {
			...
			if ( empty( self::$student_progress ) ) {
				$student_progress = self::$student_progress = BrainPress_Data_Student::get_completion_data( $student_id, $course_id );
			} else {
				$student_progress = self::$student_progress;
			}

			// Record module visit
			BrainPress_Data_Student::visited_module( $student_id, $course_id, $unit_id, $module_id, self::$student_progress );

			$retry = 'TRY';
			$element_class = array( 'module-elements' );
			if ( $is_module_answerable ) {
				$responses = BrainPress_Data_Student::get_responses( $student_id, $course_id, $unit_id, $module_id, true, $student_progress );
				...
				$try_again_label = __( 'Versuche es nochmal', 'brainpress' );

				switch ( $module_type ) {
					case 'input-upload':
						$try_again_label = __( 'Lade eine andere Datei hoch', 'brainpress' );
					break;
				}
				...
				if ( empty( $attributes['allow_retries'] ) ) {
					...
					if ( $response_count > $attempts ) {
						...
						$retry .= sprintf(
							...
							sprintf( __( 'Anzahl der fehlgeschlagenen Versuche: %d', 'brainpress' ), $response_count )
						);
					}
				}
				...
				if ( ! empty( $attributes['use_timer'] ) && ! empty( $attributes['duration'] ) ) {
					...
					$timer_info = __( 'Sitzung abgelaufen', 'brainpress' ) . $retry;
			...
			if ( 'closed' == $course_status ) {
				...
				$module_warning = sprintf( $format, esc_html__( 'Dieser Kurs ist abgeschlossen, Du kannst keine Antworten mehr einreichen.', 'brainpress' ) );
				 ...
				 * @since 2.0
				 **/
				$module_elements .= apply_filters( 'brainpress_course_readonly_message', $module_warning, $module_id, $course_id );
			}
			 ...
			 * @since 2.0
			 **/
			$content .= apply_filters( 'brainpress_module_template', $module_elements, $module_type, $module_id );
			 ...
			 * @since 2.0.6
			 **/
			$module_container_classes = apply_filters( 'brainpress_module_container_classes', array( 'cp-module-content' ), $module_type, $module_id );
	...
	private static function render_module_head( $module, $attributes = false ) {
		...
		if ( $show_title ) {
			...
			if ( $mandatory ) {
				$content .= sprintf( '<span class="is-mandatory">%s</span>', __( 'Erforderlich', 'brainpress' ) );
			}
			$content .= '</h4>';
		} else if ( $mandatory ) {
			$content .= sprintf( '<div class="is-mandatory">%s</div>', __( 'Erforderlich', 'brainpress' ) );
		}
		 ...
		 * @since 2.0
		 **/
		$content = apply_filters( 'brainpress_module_header', $content, $module );
	...
	private static function do_caption_media( $data, $module = null ) {
		...
		$attachment = BrainPress_Helper_Utility::attachment_from_url( $url );

		if ( 'media' === $caption_source ) {

			if ( ! empty( $attachment ) ) {

				$alt_text = BrainPress_Helper_Utility::filter_content( get_post_meta( $attachment->ID, '_wp_attachment_image_alt', true ) );
				if ( empty( $alt_text ) ) {
					$alt_text = isset( $data['caption_custom_text'] ) ? BrainPress_Helper_Utility::filter_content( $data['caption_custom_text'] ) : '';
				}
			...
			$the_caption = isset( $data['caption_custom_text'] ) ? BrainPress_Helper_Utility::filter_content( $data['caption_custom_text'] ) : '';
		}
		...
		if ( 'video' === $type ) {
			$player_width = BrainPress_Helper_Utility::get_array_val( $data, 'video_player_width' );
			$player_width = $player_width ? $player_width : '640';
			$player_height = BrainPress_Helper_Utility::get_array_val( $data, 'video_player_height' );
			$player_height = $player_height ? $player_height : '360';
			$autoplay = BrainPress_Helper_Utility::get_array_val( $data, 'video_autoplay' ) ? 'autoplay' : '';
			$loop = BrainPress_Helper_Utility::get_array_val( $data, 'video_loop' ) ? 'loop' : '';
			$controls = BrainPress_Helper_Utility::get_array_val( $data, 'video_hide_controls' ) ? '' : 'controls';
			...
			?>
				<video
					...
					src="<?php echo $url; ?>"
					data-setup='<?php echo BrainPress_Helper_Utility::create_video_js_setup_data( $url, $data ); ?>'
					<?php echo $controls; ?>
					<?php echo $autoplay; ?>
					<?php echo $loop; ?>>
				</video>
	...
	public static function render_download( $module, $attributes = false ) {
		...
		if ( isset( $attributes['file_url'] ) ) {
			...
			$file_size = BrainPress_Helper_Utility::get_file_size( $url );
			...
			$after_content = sprintf(
				...
				esc_url( $url ),
				esc_html( $link_text ),
				BrainPress_Helper_Utility::filter_content( $filesize )
			);
	...
	public static function render_zipped( $module, $attributes = false ) {
		...
		if ( isset( $attributes['zip_url'] ) && ! empty( $attributes['primary_file'] ) ) {
			...
			$content .= sprintf(
				...
				__( 'Primärdatei nicht festgelegt, bitte komme später wieder.', 'brainpress' )
			);
		}
	...
	private static function comment_form( $post_id, $attributes = false ) {
		...
		$enrolled = false;
		$student_id = get_current_user_id();
		$course_id = BrainPress_Data_Module::get_course_id_by_module( $post_id );
		 ...
		 */
		if ( false == $enrolled ) {
			$enrolled = BrainPress_Data_Capabilities::can_update_course( $course_id );
		}
		if ( false == $enrolled ) {
			return '';
		}
		...
		$args = array(
			...
			'title_reply' => __( 'Hier posten', 'brainpress' ),
			'label_submit' => __( 'Beitrag', 'brainpress' ),
			'must_log_in' => '',
			'logged_in_as' => '',
			'action' => '',
		 ...
		 * @since 2.0.8
		 */
		$args = apply_filters( 'brainpress_comment_form_args', false );

		add_filter( 'comment_form_submit_button', array( 'BrainPress_Template_Discussion', 'add_subscribe_button' ) );
		...
		remove_filter( 'comment_form_submit_button', array( 'BrainPress_Template_Discussion', 'add_subscribe_button' ) );
		return $comment_form;
	}
	...
	public static function comment_list( $module_id ) {
		 ...
		 * @since 2.0.8
		 */
		$args = apply_filters( 'brainpress_comment_list_args', $args );
	...
	public static function render_input_upload( $module, $attributes = false, $student_progress = false, $disabled = false ) {
		...
		$content = sprintf( $format, $module->ID, $disabled_attr, __( 'Datei ändern', 'brainpress' ), __( 'Datei hochladen', 'brainpress' ), __( 'Datei hochladen', 'brainpress' ) );

		$upload_types = BrainPress_Helper_Utility::allowed_student_mimes();
		...
		$content .= sprintf( $format,
			__( 'Akzeptiertes Dateiformat: ', 'brainpress' ),
			implode( ' ', $upload_types ),
			__( 'Maximale Dateigröße:', 'brainpress' ),
	...
	public static function render_input_quiz( $module, $attributes = false, $student_progress = false, $disabled = false ) {
		 ...
		 */
		$content = apply_filters( 'brainpress_template_module_render_input_quiz_content', $content );
		return $content;
	}
	...
	public static function quiz_result_content( $student_id, $course_id, $unit_id, $module_id, $quiz_result = false ) {
		...
		$passed_message = ! empty( $quiz_result['passed'] ) ? __( 'Du hast das Quiz erfolgreich bestanden. Hier sind Deine Ergebnisse.', 'brainpress' ) : __( 'Du hast das Quiz diesmal nicht bestanden. Hier sind Deine Ergebnisse.', 'brainpress' );

		$template = '<div class="module-quiz-questions">
			<div class="brainpress-quiz-results ' . esc_attr( $passed_class ) . '">
				<div class="quiz-message"><p class="result-message">' . $passed_message . '</p></div>
				<div class="quiz-results">
					<table>
					<tr><th>' . esc_html__( 'Fragen insgesamt', 'brainpress' ) . '</th><td>' . esc_html( $quiz_result['total_questions'] ) . '</td></tr>
					<tr><th>' . esc_html__( 'Richtig', 'brainpress' ) . '</th><td>' . esc_html( $quiz_result['correct'] ) . '</td></tr>
					<tr><th>' . esc_html__( 'Falsch', 'brainpress' ) . '</th><td>' . esc_html( $quiz_result['wrong'] ) . '</td></tr>
					<tr><th>' . esc_html__( 'Bewertung', 'brainpress' ) . '</th><td>' . esc_html( $quiz_result['grade'] ) . '%</td></tr>
					</table>
				</div>
			</div>
		</div>';
		...
		$template = apply_filters( 'brainpress_template_quiz_results', $template, $attributes );
	...
	public static function form_result_content( $student_id, $course_id, $unit_id, $module_id, $form_result = false ) {
		...
		if ( empty( $form_result ) ) {
			$form_result = BrainPress_Data_Module::get_form_results( $student_id, $course_id, $unit_id, $module_id );
		}
		...
		if ( ! empty( $form_result['pending'] ) ) {
            ...
            $passed_message = !empty($form_result['passed']) ? __('You have successfully passed the form. Here are your results.', 'brainpress') : __('You did not pass the form this time. Here are your results.', 'brainpress');
            $template = '<div class="module-form-questions">
                <div class="brainpress-form-results ' . esc_attr($passed_class) . '">
                    <div class="form-message"><p class="result-message">' . $passed_message . '</p></div>
                    <div class="form-results">
                        <table>
                        <tr><th>' . esc_html__('Total Questions', 'brainpress') . '</th><td>' . esc_html($form_result['total_questions']) . '</td></tr>
                        <tr><th>' . esc_html__('Correct', 'brainpress') . '</th><td>' . esc_html($form_result['correct']) . '</td></tr>
                        <tr><th>' . esc_html__('Incorrect', 'brainpress') . '</th><td>' . esc_html($form_result['wrong']) . '</td></tr>
                        <tr><th>' . esc_html__('Grade', 'brainpress') . '</th><td>' . esc_html($form_result['grade']) . '%</td></tr>
                        </table>
                    </div>
                </div>
            </div>';
		...
		// Can't use shortcodes this time as this also loads via AJAX
		$template = apply_filters( 'brainpress_template_form_results', $template, $attributes );
		return $template;
	}
}
