...
class BrainPress_View_Front_General {

	public static function init() {
		// BrainPress Menus
		if ( cp_is_true( BrainPress_Core::get_setting( 'general/show_brainpress_menu', 1 ) ) ) {

			/**
			 * Create BrainPress basic menus automatically.
			 *
			 * @since 1.0.0
			 */
	...
	public static function main_navigation_links( $sorted_menu_items, $args ) {
		$current_url = BrainPress_Helper_Utility::get_current_url();
		...
		if ( $args->theme_location == $theme_location ) {
			...
			$courses->title = __( 'Kurse', 'brainpress' );
			$courses->description = '';
			$courses->menu_item_parent = 0;
			$courses->ID = 'cp-courses';
			$courses->db_id = '';
			$courses->classes = array();
			$courses->url = BrainPress_Core::get_slug( 'courses', true );
			...
			if ( $is_in ) {
				...
				$dashboard->title = __( 'Dashboard', 'brainpress' );
				$dashboard->description = '';
				$dashboard->menu_item_parent = 0;
				$dashboard->ID = 'cp-dashboard';
				$dashboard->db_id = - 9998;
				$dashboard->url = BrainPress_Core::get_slug( 'student_dashboard', true );
				...
				*/
				...
				$dashboard_courses->title = __( 'Meine Kurse', 'brainpress' );
				...
				$dashboard_courses->url = BrainPress_Core::get_slug( 'student_dashboard', true );
				...
				$settings_profile->title = __( 'Mein Profil', 'brainpress' );
				...
				$settings_profile->url = BrainPress_Core::get_slug( 'student_settings', true );
				...
				//
				// $settings_inbox->title = __( 'Inbox', 'brainpress' ) . $unread_count;
			...
			$login = new stdClass();
			if ( $is_in ) {
				$login->title = __( 'Abmelden', 'brainpress' );
			} else {
				$login->title = __( 'Anmelden', 'brainpress' );
			}
			$login->description = '';
	...
	public static function get_log_in_out_link() {
		$use_custom = cp_is_true( BrainPress_Core::get_setting( 'general/use_custom_login', 1 ) );
		if ( is_user_logged_in() ) {
			return wp_logout_url();
		}
		if ( $use_custom ) {
			$login_page = BrainPress_Core::get_setting( 'pages/login', false );
			if ( empty( $login_page ) ) {
				return BrainPress_Core::get_slug( 'login', true );
			}
	...
	public static function handle_custom_endpoints( $sorted_menu_items, $args ) {
		...
		foreach ( $sorted_menu_items as $index => $item ) {
			...
			switch ( $item->url ) {
				case '#brainpress-endpoints-login':
					if ( is_user_logged_in() ) {
						if ( __( 'Anmelden', 'brainpress' ) == $item->title  ) {
							$item->title = __( 'Abmelden', 'brainpress' );
						}
					}
					$item->url = self::get_log_in_out_link();
				break;
				case '#brainpress-endpoints-courses':
					$item->url = BrainPress_Core::get_slug( 'courses', true );
				break;
				case '#brainpress-endpoints-dashboard':
					$item->url = BrainPress_Core::get_slug( 'student_dashboard', true );
				break;
				case '#brainpress-endpoints-profile':
					$item->url = BrainPress_Core::get_slug( 'student_settings', true );
				break;
			}
