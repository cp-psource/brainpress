...
class BrainPress_Helper_UI {
	...
	public static function browse_media_field( $id, $name, $args = array() ) {
		...
		$args['placeholder'] = isset( $args['placeholder'] ) ? sanitize_text_field( $args['placeholder'] ) : __( 'Medien-URL hinzufügen oder Nach Medien suchen', 'brainpress' );
		$args['button_text'] = isset( $args['button_text'] ) ? sanitize_text_field( $args['button_text'] ) : __( 'Durchsuche', 'brainpress' );
		...
		if ( 'image' === $args['type'] ) {
			$supported_extensions = implode( ', ', BrainPress_Helper_Utility::get_image_extensions() );
		}
		if ( 'audio' === $args['type'] ) {
		...
		$content .= '
			...
			<div class="invalid_extension_message">' . sprintf( esc_html__( 'Die Erweiterung der Datei ist ungültig. Bitte verwende eine der folgenden Möglichkeiten: %s', 'brainpress' ), $supported_extensions ) . '</div>
		</div>';
	...
	public static function get_user_avatar_array( $args = array(), $meta_options = array() ) {
		...
		$args = BrainPress_Helper_Utility::merge_distinct( $args, $args_default );

		if ( defined( 'BRAINPRESS_INSTRUCTOR_ROLE' ) ) {

			$args['role'] = BRAINPRESS_INSTRUCTOR_ROLE;
	...
	public static function get_course_dropdown( $id, $name, $courses = false, $options = array() ) {
		...
		if ( false === $courses ) {
			$courses = get_posts( 'post_type=' . BrainPress_Data_Course::get_post_type_name() );
		}
	...
	public static function get_unit_dropdown( $id, $name, $course_id, $units = false, $options = array() ) {
		...
		if ( 'all' !== $course_id ) {
			...
			$units = BrainPress_Helper_Utility::sort_on_object_key( $units, 'unit_order' );
		}
	...
	public static function get_discussion_module_dropdown( $id, $name, $unit_id, $units_and_modules, $options = array() ) {
		...
		foreach ( $unit['pages'] as $page ) {
			foreach ( $page['modules'] as $module ) {
				...
				$meta = BrainPress_Data_Module::attributes( $module );
				if ( BrainPress_Helper_UI_Module::OUTPUT_DISCUSSION != $meta['module_type'] ) {
					continue;
				}
	...
	public static function get_user_dropdown( $id, $name, $options = array() ) {
		...
		$args = array(
			...
			'meta_compare' => '',
			'meta_query' => array(),
			'include' => apply_filters( 'brainpress_user_dropdown_include', $include_users, $context ),
			'exclude' => apply_filters( 'brainpress_user_dropdown_exclude', $exclude_users, $context ),
			'orderby' => 'display_name',
			'order' => 'ASC',
			'offset' => '',
			'search' => '',
		...
		$roles = apply_filters( 'brainpress_allowed_roles', array(), $context );
		$get_by_role = apply_filters( 'brainpress_allowed_users_by_role', ! empty( $roles ) );
	...
	public static function course_pendings_instructors_avatars( $course_id, $options = array() ) {
		...
		if ( BrainPress_Data_Capabilities::can_assign_course_instructor( $course_id ) ) {
		...
		foreach ( $instructor_invites as $invite ) {
			$content .= BrainPress_Template_Course::course_edit_avatar_pending_invite( $invite, $remove_buttons, 'instructor' );
		}
		return $content;
	}
	...
	public static function course_instructors_avatars( $course_id, $options = array(), $show_pending = false ) {
		...
		$instructors = BrainPress_Data_Course::get_setting( $course_id, 'instructors', array() );
		$instructors = array_filter( $instructors );

		if ( empty( $instructors ) ) {
			// Set current user the default instructor
			BrainPress_Data_Course::add_instructor( $course_id, get_current_user_id() );
		...
		foreach ( $instructors as $instructor ) {
			$content .= BrainPress_Template_Course::course_edit_avatar( $instructor, $remove_buttons, 'instructor' );
		}
	...
	public static function toggle_switch( $id, $name, $options = array() ) {
		...
		$content = '
			<div id="' . esc_attr( $id ) . '" class="toggle-switch brainpress-ui-toggle-switch ' . esc_attr( $control_class ) . ' ' . $state . '" name="' . esc_attr( $name ) . '" ' . $data . '>';
	...
	public static function admin_per_page_add_options( $name, $label ) {
		...
		$args = array(
			'label' => $label,
			'default' => 10,
			'option' => sprintf( 'brainpress_%s_%s', $name, $option ),
		);
...
<?php
	...
	public static function get_admin_edit_title_field( $value, $label = '' ) {
		if ( empty( $label ) ) {
			$label = __( 'Gib hier den Titel ein.', 'brainpress' );
		}
	...
	public static function get_message_required_modules( $error_message ) {
		...
		$error_message .= PHP_EOL;
		$error_message .= __( 'Bitte drücke den Zurück-Button links, um fortzufahren.', 'brainpress' );
	...
	public static function course_facilitator_avatars( $course_id, $options = array(), $show_pending = false ) {
		$facilitators = BrainPress_Data_Facilitator::get_course_facilitators( $course_id, false );
		$content = '';
		$can_assigned_facilitator = BrainPress_Data_Capabilities::can_assign_facilitator( $course_id );
		if ( count( $facilitators ) > 0 ) {
			foreach ( $facilitators as $facilitator_id => $userdata ) {
				$content .= BrainPress_Template_Course::course_edit_avatar( $userdata, $can_assigned_facilitator, 'facilitator' );
			}
		}
		// Pending from invites
		if ( $show_pending ) {
			$invites = BrainPress_Data_Facilitator::get_invitations_by_course_id( $course_id );
			if ( ! empty( $invites ) ) {
				foreach ( $invites as $invite ) {
					$content .= BrainPress_Template_Course::course_edit_avatar_pending_invite( $invite, $can_assigned_facilitator, 'facilitator' );
				}
			}
		}
	...
	public static function check_logout_redirect( $url = false ) {
		 ...
		 */
		$dashbord = BrainPress_Core::get_slug( 'student_dashboard', false );
	...
	public static function course_edit_checkbox( $checkbox, $course_id = null ) {
		...
		$content .= sprintf(
			...
			BrainPress_Helper_Utility::checked( BrainPress_Data_Course::get_setting( $course_id, $checkbox['meta_key'], false ) )
		);
