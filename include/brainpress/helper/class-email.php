...
class BrainPress_Helper_Email {

	/**
	 * Email type.
	 * Used by BrainPress_Data_Certificate::send_certificate().
	 */
	const BASIC_CERTIFICATE = 'basic_certificate';

	/**
	 * Email type.
	 * Used by BrainPress_Data_Student::send_registration().
	 */
	const REGISTRATION = 'registration';

	/**
	 * Email type.
	 * Used by BrainPress_Data_Course::enroll_student().
	 */
	...
	/**
	 * Email type.
	 * Used by BrainPress_Data_Course::send_invitation().
	 */
	...
	/**
	 * Email type.
	 * Used by BrainPress_Data_Instructor::send_invitation().
	 */
	const INSTRUCTOR_INVITATION = 'instructor_invitation';

	/**
	 * Email type.
	 * Used by BrainPress_Data_Facilitator::send_invitation().
	 */
	...
	/**
	 * Email type.
	 * Used BrainPress_Data_Student::notify_student()
	 **/
	const COURSE_START_NOTIFICATION = 'course_start';

	/**
	 * Email type.
	 * Used BrainPress_Template_Discussion::notify_all()
	 **/
	const DISCUSSION_NOTIFICATION = 'discussion_notification';

	/**
	 * Email type.
	 * Used at BrainPress_Helper_EmailAlerts::unit_started()
	 **/
	const UNIT_STARTED_NOTIFICATION = 'unit_started';

	/**
	 * Email type.
	 * Used at BrainPress_Helper_EmailAlerts::instructor_module_feedback()
	 **/
	...
	public static function get_email_fields( $email_type ) {
		return apply_filters(
			'brainpress_get_email_fields-' . $email_type,
			array(
	...
	public static function send_email( $type, $args ) {
		 ...
		 * @param (bool) $send
		 **/
		$send = BrainPress_Data_Unsubscribe::is_send( $type, $args );
	...
	/**
	 * Send a BrainPress email template to a single user.
	 *
	 * @since  1.0.0
	...
	protected static function process_and_send( $type, $args ) {
		...
		$email = array(
			'to' => apply_filters(
				'brainpress_email_to_address',
				sanitize_email( $args['to'] ),
				$args
			),
			'subject' => apply_filters(
				'brainpress_email_subject',
				sanitize_text_field( $args['subject'] ) ,
				$args
			),
			'message' => apply_filters(
				'brainpress_email_message',
				$args['message'],
				$args
			),
			'headers' => apply_filters(
				'brainpress_email_headers',
				array(
					'Content-Type' => 'text/html',
				)
			),
			'attachments' => apply_filters(
				'brainpress_email_attachments',
				isset( $args['attachments'] ) ? $args['attachments'] : array()
			),
		);

		$email = apply_filters(
			'brainpress_email_fields',
			$email,
			$args,
			$type
		);
		$email = apply_filters(
			'brainpress_email_fields-' . $type,
			$email,
			$args
		);

		// Good one to hook if you want to hook WP specific filters (e.g. changing from address)
		do_action( 'brainpress_email_pre_send', $args, $type );
		do_action( 'brainpress_email_pre_send-' . $type, $args );

		if ( apply_filters( 'brainpress_email_strip_slashed', true, $args, $type ) ) {
		...
		$email['message'] = BrainPress_Helper_Utility::filter_content( $email['message'] );
		 ...
		 * @var array $args Email parameters passed to the BrainPress function.
		 */
		$result = apply_filters(
			'brainpress_send_email',
			null,
			$email,
			$args,
			$type
		...
		do_action( 'brainpress_email_sent', $args, $type, $result );
		do_action( 'brainpress_email_sent-' . $type, $args, $result );
	...
	protected static function enabled( $email_type ) {
		$fields = BrainPress_Helper_Setting_Email::get_defaults( $email_type );
	 ...
	 * Triggered by BrainPress_Data_Certificate::send_certificate()
	 *
	...
	protected static function basic_certificate_message( $args, $content ) {
		...
		$vars = array(
			...
			'CERTIFICATE_URL' => esc_url( BrainPress_Data_Certificate::get_encoded_url( $course_id, $args['student_id'] ) ),
		 ...
		 * @param (int) $course_id
		 **/
		$vars = apply_filters( 'brainpress_fields_' . self::BASIC_CERTIFICATE, $vars, $course_id );

		return BrainPress_Helper_Utility::replace_vars( $content, $vars );
	}

	/**
	 * Email body for new user registration/welcome email.
	 * Triggered by BrainPress_Data_Student::send_registration()
	 *
	 * @since  2.0.0
	 * @param  array $args Email params.
	 * @param  string $content Default email content, with placeholders.
	...
	protected static function registration_message( $args, $content ) {
		 ...
		 * @param array $vars
		 **/
		$vars = apply_filters( 'brainpress_fields_' . self::REGISTRATION, $vars );
	 ...
	 * Triggered by BrainPress_Data_Course::enroll_student()
	 *
	 * @since  2.0.0
	 * @param  array $args Email params.
	 * @param  string $content Default email content, with placeholders.
	...
	protected static function enrollment_confirm_message( $args, $content ) {
		 ...
		 * @param (int) $course_id
		 **/
		$vars = apply_filters( 'brainpress_fields_' . self::ENROLLMENT_CONFIRM, $vars, $course_id );
	...
	protected static function instructor_enrollment_notification_message( $args, $content ) {
		 ...
		 * @param (int) $course_id
		 **/
		$vars = apply_filters('brainpress_fields_' . self::INSTRUCTOR_ENROLLMENT_NOTIFICATION, $vars, $course_id);
	 ...
	 * Triggered by BrainPress_Data_Course::send_invitation()
	 *
	 * @since  2.0.0
	 * @param  array $args Email params.
	 * @param  string $content Default email content, with placeholders.
	...
	protected static function course_invitation_message( $args, $content ) {
		...
		$vars = array(
			...
			'COURSE_ADDRESS' => esc_url( $course_address ),
			'PASSCODE' => BrainPress_Data_Course::get_setting( $course_id, 'enrollment_passcode', '' ),
		);
		$vars = BrainPress_Helper_Utility::add_site_vars( $vars );
		 ...
		 * @param (int) $course_id
		 **/
		$vars = apply_filters( 'brainpress_fields_' . self::COURSE_INVITATION, $vars, $course_id );
	 ...
	 * Triggered by BrainPress_Data_Instructor::send_invitation()
	 *
	 * @since  2.0.0
	 * @param  array $args Email params.
	 * @param  string $content Default email content, with placeholders.
	...
	protected static function instructor_invitation_message( $args, $content ) {
		 ...
		 **/
		$vars = apply_filters( 'brainpress_fields_' . self::INSTRUCTOR_INVITATION, $vars, $course_id );
		$message = BrainPress_Helper_Utility::replace_vars( $content, $vars );
		 ...
		 **/
		$message = apply_filters( 'brainpress_course_invitation_message', $message, $course_id );
	 ...
	 * Triggered by BrainPress_Data_facilitator::send_invitation()
	 *
	 * @since  2.0.0
	 * @param  array $args Email params.
	 * @param  string $content Default email content, with placeholders.
	...
	protected static function facilitator_invitation_message( $args, $content ) {
		 ...
		 * @param array $course_id
		 **/
		$vars = apply_filters( 'brainpress_fields_' . self::FACILITATOR_INVITATION, $vars, $course_id );
		$message = BrainPress_Helper_Utility::replace_vars( $content, $vars );
	...
	public static function course_start_notification_message( $args, $content ) {
		 ...
		 * @param array $course_id
		 **/
		$vars = apply_filters( 'brainpress_fields_' . self::COURSE_START_NOTIFICATION, $vars, $course_id );
		 ...
		 * @param int    $course_id The course_id the message is associated to.
		 **/
		$message = apply_filters( 'brainpress_course_start_notification_message', $message, $course_id );
	...
	public static function discussion_notification_message( $args, $content ) {
		 ...
		 * @param array $course_id
		 **/
		$vars = apply_filters( 'brainpress_fields_' . self::DISCUSSION_NOTIFICATION, $vars, $course_id );
		 ...
		 * @param (int) $course_id The course_id the message is associated to.
		 **/
		$message = apply_filters( 'brainpress_discussion_notification_message', $message, $course_id );
	...
	public static function units_started_notification_message( $args, $content ) {
		...
		$course_id = BrainPress_Data_Unit::get_course_id_by_unit( $unit );
		...
		if ( in_array( $course->post_status, $valid_stati ) ) {
			$course_address = BrainPress_Core::get_slug( 'course/', true ) . $unit->post_name . '/';
		} else {
			$course_address = get_permalink( $course_id );
		}

		$unit_address = $course_address . BrainPress_Core::get_slug( 'units/' ) . $unit->post_name . '/';
		$unsubscribe_link = '';  // @todo: NOT IMPLEMENTED YET!!!
		 ...
		 * @param array $course_id
		 **/
		$vars = apply_filters( 'brainpress_fields_' . self::UNIT_STARTED_NOTIFICATION, $vars, $course_id );
		 ...
		 * @param int    $course_id The course_id the message is associated to.
		 **/
		$message = apply_filters( 'brainpress_units_started_notification_message', $message, $course_id );
	...
	public static function units_started_notification_subject( $args, $subject ) {
		...
		$subject = BrainPress_Helper_Utility::replace_vars( $subject, $vars );
		 ...
		 * @param int    $course_id The course_id the message is associated to.
		 **/
		$subject = apply_filters( 'brainpress_units_started_notification_subject', $subject, $course_id );
	...
	public static function instructor_module_feedback_notification_message( $args, $content ) {
		...
		$course_grade = BrainPress_Helper_Utility::get_array_val(
			$student_progress,
			'completion/average'
		);
		 ...
		 * @param array $course_id
		 */
		$vars = apply_filters( 'brainpress_fields_' . self::INSTRUCTOR_MODULE_FEEDBACK_NOTIFICATION, $vars, $course_id );
		 ...
		 * @param int    $course_id The course_id the message is associated to.
		 */
		$message = apply_filters( 'brainpress_instructor_module_feedback_notification_message', $message, $course_id );
