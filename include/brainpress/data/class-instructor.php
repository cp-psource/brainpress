...
class BrainPress_Data_Instructor {
	...
	public static function get_accessable_courses( $user_id = '', $post_status = 'publish' ) {
		...
		if ( ! user_can( $user_id, 'manage_options' ) ) {
			$can_search = false;
			if ( user_can( $user_id, 'brainpress_update_my_course_cap' ) ) {
				$args['author'] = $user_id;
				$can_search = true;
			}
			if ( user_can( $user_id, 'brainpress_update_course_cap' ) ) {
		...
		if ( count( $posts ) > 0 ) {
			foreach ( $posts as $index => $post ) {
				$can_update = BrainPress_Data_Capabilities::can_update_course( $post->ID );
	...
	public static function remove_instructor_status( $user ) {
		...
		BrainPress_Data_Capabilities::drop_instructor_capabilities( $user_id );
	}
	// */
	...
	public static function instructor_by_hash( $hash ) {
		...
		// Check cache first!
		$user_id = wp_cache_get( $hash, 'brainpress_userhash' );
		...
		if ( empty( $user_id ) ) {
			...
			wp_cache_add( $hash, $user_id, 'brainpress_userhash' );
		}
		...
		if ( ! $number_of_courses || $refresh ) {
			...
			if ( $meta_keys ) {
				...
				$course_ids = array_map(
					array( 'BrainPress_Data_Instructor', 'filter_course_meta_array' ),
					$meta_keys
				);
				$course_ids = array_filter( $course_ids );
				foreach ( $course_ids as $course_id ) {
					$id = preg_replace( '/[^\d]/', '', $course_id );
					if ( ! BrainPress_Data_Course::is_course( $id ) ) {
	...
	public static function added_to_course( $instructor_id, $course_id ) {
		...
		if ( empty( $assigned_courses_ids ) ) {
			BrainPress_Data_Capabilities::assign_instructor_capabilities( $instructor );
		}
	...
	public static function removed_from_course( $instructor_id, $course_id ) {
		...
		if ( ! BrainPress_Data_Student::is_enrolled_in_course( $instructor_id, $course_id ) ) {
		 ...
		 **/
		if ( empty( $assigned_courses_ids ) ) {
			BrainPress_Data_Capabilities::drop_instructor_capabilities( $instructor );
	...
	public static function send_invitation( $course_id, $email, $first_name, $last_name ) {
		...
		$sent = BrainPress_Helper_Email::send_email(
			BrainPress_Helper_Email::INSTRUCTOR_INVITATION,
			$email_args
		);
		...
		if ( $sent ) {
			if ( ! $invite_exists ) {
				...
				$return_data = BrainPress_Helper_Utility::set_array_value(
					$return_data,
					'message/sent',
					__( 'Einladung erfolgreich gesendet.', 'brainpress' )
				);
				...
				$return_data = BrainPress_Helper_Utility::set_array_value(
					$return_data,
					'message/exists',
					__( 'Einladung existiert bereits. Die Einladung wurde erneut gesendet.', 'brainpress' )
				);
			}
		} else {
			// Email not sent.
			...
			$return_data = BrainPress_Helper_Utility::set_array_value(
				$return_data,
				'message/send_error',
				__( 'E-Mail konnte nicht gesendet werden.', 'brainpress' )
			);
		};
	...
	public static function invitation_data( $localize_array ) {
		...
		if ( $invitation_data = self::is_course_invite() ) {
			...
			$invitation_data->nonce = wp_create_nonce( 'brainpress_add_instructor' );
	...
	public static function add_from_invitation( $course_id, $instructor_id, $invitation_code ) {
		...
		if ( ! empty( $invite_data['email'] ) && $invite_data['email'] == $userdata->user_email ) {
			BrainPress_Data_Course::add_instructor( $course_id, $instructor_id );
			BrainPress_Data_Capabilities::assign_instructor_capabilities( $userdata );
			BrainPress_Data_Instructor::delete_invitation( $course_id, $invite_data['code'] );
			 ...
			 *
			 */
			do_action( 'brainpress_instructor_invite_confirmed', $course_id, $instructor_id );
		 ...
		 *
		 */
		do_action( 'brainpress_instructor_invite_confirm_fail', $course_id, $instructor_id );
	...
	public static function get_students_count( $instructor_id, $refresh = false ) {
		...
		$count = get_user_meta( $instructor_id, 'brainpress_followers_count', true );
		if ( false === $count || $refresh ) {
			...
			update_user_meta( $instructor_id, 'brainpress_followers_count', $count );
		}
		if ( empty( $count ) ) {
			return 0;
		}
