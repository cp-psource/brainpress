 ...
 */
class BrainPress_Helper_EmailAlert {
	 ...
	 */
	const SCHEDULE_NAME = 'brainpress_scheduled_mails';
	 ...
	 * This function is called on every BrainPress admin page via action
	 * `brainpress_admin_render_page` (by class BrainPress_Helper_Setting)
	 *
	 * @since  2.0.0
	 */
	...
	protected static function send_email_course_start( $student, $notify_key, $course_id ) {
		...
		$res = BrainPress_Helper_Email::send_email(
			BrainPress_Helper_Email::COURSE_START_NOTIFICATION,
			$variables
		);
	...
	protected static function send_email_unit_start( $student, $notify_key, $unit_id ) {
		...
		// Finally send notification!
		$res = BrainPress_Helper_Email::send_email(
			BrainPress_Helper_Email::UNIT_STARTED_NOTIFICATION,
			$variables
		);
	...
	public static function init_email_task() {
		...
		self::$today = apply_filters(
			'brainpress_scheduled_email_date',
			self::$today,
			$today_stamp
		);
		...
		self::$max_emails = apply_filters(
			'brainpress_scheduled_email_batch_size',
			self::$max_emails
		);
		self::$batch_delay = apply_filters(
			'brainpress_scheduled_email_batch_delay',
			self::$batch_delay
		);
	}
	...
	protected static function get_courses_that_start_today() {
		...
		foreach ( $all_courses as $course ) {
			$start_date = BrainPress_Data_Course::get_setting( $course->ID, 'course_start_date' );
			$start_stamp = BrainPress_Data_Course::strtotime( $start_date ); // UTC.
	...
	protected static function get_units_that_start_today() {
		...
		foreach ( $all_units as $unit ) {
			...
			$start_date = BrainPress_Data_Unit::get_unit_availability_date( $unit->ID, $course_id, 'Y-m-d' );

			$course_start_date = BrainPress_Data_Course::get_setting( $course_id, 'course_start_date' );
			$course_start_stamp = BrainPress_Data_Course::strtotime( $course_start_date ); // UTC.
