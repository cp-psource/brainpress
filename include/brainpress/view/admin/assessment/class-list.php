...
class BrainPress_View_Admin_Assessment_List {

	public static $slug = 'brainpress_assessments';
	...
	public static function init() {
		...
		self::$title = __( 'Assessments/BrainPress', 'brainpress' );
		self::$menu_title = __( 'Assessments', 'brainpress' );
		self::$feedback_email = new BrainPress_View_Admin_Assessment_FeedbackEmail;
	...
	public static function add_page( $pages ) {
		$pages[ self::$slug ] = array(
			...
			'cap' => apply_filters( 'brainpress_capabilities', 'brainpress_assessment_cap' ),
		);
	...
	public static function update_assessment() {
		...
		switch ( $data->action ) {
			case 'update':
				...
				$old_grade = BrainPress_Data_Student::get_grade( $student_id, $course_id, $unit_id, $module_id, false, false, $student_progress );
				if ( ! empty( $old_grade['grade'] ) ) { $old_grade = $old_grade['grade']; }

				$old_feedback = BrainPress_Data_Student::get_feedback( $student_id, $course_id, $unit_id, $module_id, false, false, $student_progress );
	...
	public static function render_page() {
		...
		$content = '<div class="brainpress_settings_wrapper assessment wrap">';
		...
		if ( isset( $_REQUEST['view_answer'] ) ) {
			...
			$content .= '<span class="cp-right cp-back">'
				. '<a href="'. esc_url( $back_url ) . '"><span class="dashicons dashicons-arrow-left-alt2"></span> '. __( 'Back', 'brainpress' ) . '</a>'
				. '</span>';
		}

		$content .= BrainPress_Helper_UI::get_admin_page_title( self::$menu_title );
	...
	public static function render_assessment() {
		...
		$courses = BrainPress_Data_Instructor::get_accessable_courses( $user_id, true );
		...
		if ( empty( $courses ) ) {
			$content .= sprintf( '<p class="description">%s</p>', __( 'You currently have no courses assigned.', 'brainpress' ) );
		} else {
			$content .= '<div class="cp-course-selector"><br />
				<div class="cp-box">
					<label>' . esc_html__( 'Select Course', 'brainpress' ) . '</label>
					' . BrainPress_Helper_UI::get_course_dropdown( 'course-list', 'course-list', $courses, array( 'class' => 'medium', 'value' => $selected_course ) )
					.
				'</div>
				<div class="cp-box">
					<select id="unit-list">
						<option value="all">' . esc_html__( 'Show all', 'brainpress' ) . '</option>
						<option value="all_submitted"' . selected( 'all_submitted', $current_unit, false ) . '>' . esc_html__( 'Show all assessable students', 'brainpress' ) . '</option>';
			...
			foreach ( $units as $unit ) {
				...
				<div class="cp-box">
					<select id="ungraded-list">
						<option value="all">' . esc_html__( 'Show graded and ungraded students', 'brainpress' ) . '</option>
						<option value="ungraded" ' . selected( 'ungraded', $type, false ) . '>' . esc_html__( 'Show ungraded students only', 'brainpress' ) . '</option>
						<option value="graded" ' . selected( 'graded', $type, false ) . '>' . esc_html__( 'Show graded students only', 'brainpress' ) . '</option>
					</select>
				</div>
			</div>';
			...
			$content .= sprintf( '<div class="cp-loader-info" style="display:none;"><span class="fa fa-spinner fa-spin"></span> %s</div>',
				__( 'Fetching students...', 'brainpress' )
			);
		}
	...
	public static function get_students_table( $course_id, $the_unit = 'all', $type = 'all', $paged = 1 ) {
		...
		$results = BrainPress_View_Admin_Assessment_List::filter_students( $course_id, $the_unit, $type );
		...
		if ( empty( $total ) ) {
			return sprintf( '<br><br><p class="description">%s</p>', __( 'There are no students found.', 'brainpress' ) );
		}

		$table = '
			<table>
			<tr>
				<td>' . __( 'Students Found:', 'brainpress' ) . ' ' . $total . '</td>
				<td>' . __( 'Modules:', 'brainpress' ) . ' <span class="cp-total-assessable">' . $results['assessable'] . '</span></td>
				<td>' . __( 'Passing Grade: ', 'brainpress' ) . ' <span class="cp-pasing-grade">' . $results['passing_grade'] . '%</span></td>
			</tr>
			</table>
		';
		...
		$table .= '<table class="wp-list-table widefat fixed striped cp-table">
			<thead>
				<th>' . esc_html__( 'Student', 'brainpress' ) . '</th>
				<th>' . esc_html__( 'Last Active', 'brainpress' ) . '</th>
				<th class="unit-grade">' . esc_html__( 'Grade', 'brainpress' ) . '</th>
				<th width="5%">' . esc_html__( 'Modules', 'brainpress' ) . '</th>
				<th width="5%">' . esc_html__( 'View All', 'brainpress' ) . '</th>
			</thead>
			<tbody>
		';
		...
		foreach ( $students as $student ) {
			...
			$student_label = BrainPress_Helper_Utility::get_user_name( $student_id, true );
			$student_progress = BrainPress_Data_Student::get_completion_data( $student_id, $course_id );
			$last_active = '';
				...
				foreach ( $units as $unit_id => $unit ) {
					if ( ! empty( $units[ $unit_id ]['responses'] ) ) {
						...
						foreach ( $responses as $module_id => $response ) {
							...
							if ( ! empty( $last['date'] ) ) {
								$date = BrainPress_Data_Course::strtotime( $last['date'] );
					...
					<tr class="cp-content" data-student="' . $student_id . '" style="display: none;">
						<td class="cp-responses cp-inline-responses" colspan="5">
							<script type="text/template" id="student-grade-' . $student_id . '">
								' . BrainPress_View_Admin_Assessment_List::student_assessment( $student_id, $course_id, $student_progress, $the_unit, ( $the_unit != 'all' ) ) . '
							</script>
						</td>
					</tr>';
		}
		...
		$table .= '<br><br><div class="no-student-info" style="display: none;">
			<p class="description">' . esc_html__( '0 students found under this unit', 'brainpress' ) . '</p>
		</div>
		<div class="no-assessable-info" style="display: none;">
			<p class="description">' . esc_html__( 'There are no assessable students found!', 'brainpress' ) . '</p>
		</div>';
		...
		$table .= BrainPress_Helper_UI::admin_paginate( $paged, $total, $per_page, $url );
	...
	public static function student_assessment( $student_id, $course_id, $student_progress = false, $activeUnit = 'all', $assess = false, $display = false ) {
			...
			foreach ( $unit['pages'] as $page_number => $page ) {
				...
				foreach ( $page['modules'] as $module_id => $module ) {
					...
					if ( $require_instructor_assessment ) {
						...
						$unit_grade += $grade;
						$pass = BrainPress_Helper_Utility::get_array_val(
							$student_progress,
							...
							$page_content .= '
									<div class="cp-right cp-assessment-div">
										<span class="cp-check cp-right ' . $pass_class . '">
											' . ( 'green' === trim( $pass_class ) ? __( 'Pass', 'brainpress' ) : __( 'Fail', 'brainpress' ) ) . '
										</span>
										<label class="cp-assess-label"> ' . __( 'Assessment Result', 'brainpress' ) . '</label><br />
										<p class="brainpress-tooltip description cp-min-tooltip">
											' . __( 'The minimum grade required is: ', 'brainpress' ) .'
											' . $min_grade . '
										</p>
										<p class="brainpress-tooltip cp-grade-tooltip">
											...
											<button type="button" class="button-primary module-submit disabled">
												' . __( 'Submit Grade', 'brainpress' ) . '
											</button>
										</p>
									</div>';
							if ( $is_required ) {
								$page_content .= '<span class="cp-required cp-right">' . __( 'Required', 'brainpress' ) . '</span>';
							}
						} else {
							$page_content .= '
									<span class="cp-check cp-right ' . $pass_class . '">
						...
						if ( 0 === count( $response ) ) {
							$page_content .= sprintf( '<div class="cp-answer-box"><span class="dashicons dashicons-no"></span> %s</div>', __( 'No answer!', 'brainpress' ) );
						} else {
							...
							if ( in_array( $module_type, $allowed_for_feedback ) ) {
								...
								$feedback_by = ! empty( $feedback['feedback'] ) ? '- ' . BrainPress_Helper_Utility::get_user_name( $feedback['feedback_by'] ) : '';
								$add_label = $has_feedback ? __( 'Edit Feedback', 'brainpress' ) : __( 'Add Feedback', 'brainpress' );

								$page_content .= '<div class="cp-instructor-feedback">
										<h4>' . __( 'Instructor Feedback', 'brainpress' ) . '</h4>
										<div class="cp-right cp-feedback-buttons">
											<button type="button" class="button-primary edit-feedback">' . $add_label . '</button>
											<button type="button" class="button-primary delete-feedback'. ( $has_feedback ? '' : ' disabled' ) . '">' . __( 'Delete Feedback', 'brainpress' ) . '</button>
											<button type="button" class="button-primary send-feedback disabled">' . __( 'Send Feedback', 'brainpress' ) . '</button>
										</div>
									';
								...
								$page_content .= sprintf( '<p class="description" %s>%s</p>', empty( $feedback_text ) ? '' : $hide, __( 'Write your feedback!', 'brainpress' ) );
		...
		if ( empty( $content ) ) {
			$content .= sprintf( '<p class="div-info description">%s</p>', __( 'There are no assessable items!', 'brainpress' ) );
		}
