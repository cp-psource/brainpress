...
(function( $ ) {
	...
	BrainPress.editor = BrainPress.editor || {};

	if ( undefined !== window.getUserSetting ) {
		BrainPress.editor.init_mode = window.getUserSetting( 'editor' );
	}

	BrainPress.editor.create = function( target, id, name, content, append, height ) {
		...
		if ( ! _brainpress._dummy_editor ) {
			var cp_wp_editor = $( '#cp-wp-editor' );
			if ( cp_wp_editor.length > 0 ) {
				_brainpress._dummy_editor = cp_wp_editor.html();
			}
		}

		var editor = _brainpress._dummy_editor;
		...
		if ( undefined !== options ) {
			...
			options.init_instance_callback = 'BrainPress.editor.on_init'; // code to execute after editor is created
			options.cache_suffix = '';
			options.setup = function( ed ) {
				ed.on( 'keyup', function() {
					BrainPress.Events.trigger('editor:keyup',ed);
				} );

				/**
				 * Markup changing catch-all hack
				 *
				 * Basically, trick the BrainPress listeners into believing
			...
			$('textarea#' + id ).on( 'keyup', function() {
				BrainPress.Events.trigger('editor:keyup',this);
			});
	...
	BrainPress.editor.content = function( id, content ) {
	...
	BrainPress.editor.set_height = function( id, height ) {
		$( '#wp-' + id + '-editor-container' ).removeAttr( 'rows' );
		$( '#wp-' + id + '-wrap iframe' ).css( 'height', height + 'px' );
	};

	BrainPress.editor.on_init = function( instance ) {
		var mode = BrainPress.editor.init_mode;
		...
		var mce_button = '<button id="' + instance.id + '-visual' + '" class="wp-switch-editor switch-tmce" type="button">' + _brainpress.editor_visual + '</button>';
		var qt_button = '<button id="' + instance.id + '-text' + '" class="wp-switch-editor switch-html" type="button">' + _brainpress.editor_text + '</button>';
	...
	BrainPress.utility.merge_distinct = function( array1, array2 ) {
		var merged = array1;
	...
	BrainPress.utility.update_object_by_path = function( object, path, value ) {
	...
	BrainPress.utility.get_object_path = function( object, search_key, search_value, base ) {
		if ( undefined === base ) {
			base = '';
		}
		...
		while ( keys.length > 0 ) {
			...
			if ( _.isObject( object[ key ] ) ) {
				...
				return BrainPress.utility.get_object_path( object[ key ], search_key, search_value, base );
			} else {
	...
	BrainPress.utility.in_array = function( value, array ) {
		return array.indexOf( value ) > -1;
	};

	BrainPress.utility.is_valid_url = function( str ) {
	...
	BrainPress.utility.valid_media_extension = function( filename, type ) {
		...
		var audio_extensions = _brainpress.allowed_audio_extensions;
		var video_extensions = _brainpress.allowed_video_extensions;
		var image_extensions = _brainpress.allowed_image_extensions;
		...
		if ( type === 'audio_url' ) {
			if ( BrainPress.utility.in_array( extension, audio_extensions ) ) {
				return true;
			} else {
				if ( BrainPress.utility.is_valid_url( filename ) && extension.length > 5 ) {
					return true;
				} else {
					if ( ! filename.length ) {
						return true;
		...
		if ( type === 'video_url' ) {
			if ( BrainPress.utility.in_array( extension, video_extensions ) ) {
				return true;
			} else {
				if ( BrainPress.utility.is_valid_url( filename ) && extension.length > 5 ) {
					return true;
		...
		if ( type === 'image_url' ) {
			if ( BrainPress.utility.in_array( extension, image_extensions ) ) {
				return true;
			} else {
				if ( BrainPress.utility.is_valid_url( filename ) && extension.length > 5 ) {
					return true;
		...
		if ( _brainpress.allowed_extensions ) {
			if ( BrainPress.utility.in_array( extension, _brainpress.allowed_extensions ) ) {
				return true;
			} else {
				if ( BrainPress.utility.is_valid_url( filename ) && extension.length > 5 ) {
					return true;
	...
	BrainPress.utility.pad = function( num, size ) {
		var s = num + '';
	...
	BrainPress.utility.unserialize = function( data ) {
	...
	BrainPress.utility.hashcode = function(s) {
	...
	// Webkit MD5 method
	BrainPress.utility.md5 = function( s ) {
		function _L( k, d ) {
			return (k << d) | (k >>> (32 - d));
		}
	...
	BrainPress.utility.get_gravatar = function( email, size, default_image, allowed_rating, force_default ) {
		...
		return ('https://secure.gravatar.com/avatar/' + BrainPress.utility.md5( email.toLowerCase().trim() ) + '?size=' + size + '&default=' + encodeURIComponent( default_image ) + '&rating=' + allowed_rating + (force_default === 'y' ? '&forcedefault=' + force_default : ''));
	};

	BrainPress.utility.get_gravatar_image = function( email, size, alt, default_image, allowed_rating, force_default ) {
		var url = BrainPress.utility.get_gravatar( email, size, default_image, allowed_rating, force_default );
	...
	BrainPress.utility.fix_checkboxes = function( items, selector, false_value ) {
	...
	BrainPress.utility.is_equal = function( current_val, expected_val, strict ) {
		if ( undefined === strict ) {
			/*jshint ignore:start*/
	...
	BrainPress.utility.checked = function( current_val, expected_val ) {
		return BrainPress.utility.is_equal( current_val, expected_val ) ? 'checked="checked"' : '';
	};

	BrainPress.utility.event_supported = function() {
		var TAGNAMES = {
	...
	BrainPress.utility.attachment_by_url = function( url, target, fallback, field ) {
		var model = new BrainPress.Models.utility.Attachment();
		model.get_attachment( url, target, fallback, field );
	};

	BrainPress.utility.hex_to_rgb = function( hex ) {
	...
	BrainPress.UI.toggle_switch = function( id, name, options ) {
		if ( undefined === options ) {
			options = [];
		}
		...
		content +='<div id="' + id + '" class="toggle-switch brainpress-ui-toggle-switch ' + control_class + ' ' + state + '" name="' + name + '" ' + data + '>';
	...
	BrainPress.UI.browse_media_field = function( id, name, args ) {
		if ( undefined === name ) {
			name = id;
		}
		...
		args.invalid_message = args.invalid_message ? args.invalid_message : _brainpress.invalid_extension_message;
		args.description = args.description ? args.description : '';

		var supported_extensions = false;

		if ( 'image' === args.type ) {
			supported_extensions = _brainpress.allowed_image_extensions.join( ', ' );
		}

		if ( 'audio' === args.type ) {
			supported_extensions = _brainpress.allowed_audio_extensions.join( ', ' );
		}

		if ( 'video' === args.type ) {
			supported_extensions = _brainpress.allowed_video_extensions.join( ', ' );
		}

		if ( ! supported_extensions && _brainpress.allowed_extensions ) {
			supported_extensions = _brainpress.allowed_extensions.join( ', ' );
		}
	...
	BrainPress.UI.link_popup = function( id, name, args ) {
		var content = '';
	...
	$.fn.extend( {
		browse_media_field: function() {
			return this.each( function() {
				...
				$( parent ).on( 'keyup', function() {
					...
					if ( BrainPress.utility.valid_media_extension( $( parent ).val(), parent ) ) {//extension is allowed
				...
				$( this ).on( 'click', function() {
					...
					wp.media.string.props = function( props, attachment ) {
						...
						if ( BrainPress.utility.valid_media_extension( attachment.url, target_url_field ) ) {//extension is allowed
		...
		brainpress_timer: function( options ) {
			options = options || {
					seconds: 10,
					running: false,
					action: 'none'
			...
			switch( options.action ) {
				...
				case 'none':
					...
					if (has_hours) {
						duration += BrainPress.utility.pad(d_hours, 2) + ':';
					}
					duration += BrainPress.utility.pad(d_minutes, 2) + ':';
					duration += BrainPress.utility.pad(d_seconds, 2);
					$(self.find('.cp-counter-clock')[0]).replaceWith('<div class="cp-counter-clock">' + duration + '</div>');
					self.append('<input class="cp-counter-start" type="button" value="' + _brainpress.labels['module_start_quiz'] + '">');
					self.find('.cp-counter-start').on('click', function() {
						options.action = 'start';
						self.brainpress_timer( options );
						$(self.find('.cp-counter-start')).detach();
						$( options.toggle_element).show();
						$( '.focus-nav').addClass('not-active');
						$( '.brainpress-breadcrumbs a').addClass('not-active');
					...
					return this.each(function() {
						...
						var timeinterval = setInterval(function() {
							...
							if (seconds > 0) {
								...
								//$( '.brainpress-breadcrumbs a').removeClass('not-active');
		...
		link_popup: function( options ) {
			...
			this.each( function( index, item ) {
				...
				content = BrainPress.UI.link_popup( id, name, args );
	...
	BrainPress.Models.utility = BrainPress.Models.utility || {};

	BrainPress.Models.utility.Attachment = Backbone.Model.extend( {
		initialize: function() {
		...
		get_attachment: function( attachment_url, target, fallback, field ) {
			...
			this.url = _brainpress._ajax_url + '?action=attachment_model&task=get&url=' + attachment_url;
			this.fetch();
		},
		process: function() {
	...
	// User Capabilities
	BrainPress.current_user_can = function ( cap ) {
		return _brainpress.is_super_admin  || _brainpress.user_caps[cap];
	};
	...
	***** Settings > Basic Certificate *****
	...
	(function( $ ) {
		function check_extension( url, field ) {
			...
			if ( BrainPress.utility.valid_media_extension( url, field ) ) {
	...
	***** Settings > Instructor Capabilities *****
	...
	(function() {
		...
		function on_dash_click() {
			// Hide boxes.
			show_settings( '.cp-content-box.course', [this, '.brainpress_courses_cap input'] );
			show_settings( '.cp-content-box.course-category', [this, '.brainpress_courses_cap input'] );
			show_settings( '.cp-content-box.course-unit', [this, '.brainpress_courses_cap input'] );
			show_settings( '.cp-content-box.instructor', [this, '.brainpress_instructors_cap input'] );
			show_settings( '.cp-content-box.student', [this, '.brainpress_students_cap input'] );
			show_settings( '.cp-content-box.notification', [this, '.brainpress_notifications_cap input'] );
			show_settings( '.cp-content-box.discussion', [this, '.brainpress_discussions_cap input'] );

			// Hide submenu settings.
			show_settings( '.capability-list .brainpress_courses_cap', [this] );
			show_settings( '.capability-list .brainpress_instructors_cap', [this] );
			show_settings( '.capability-list .brainpress_students_cap', [this] );
			show_settings( '.capability-list .brainpress_assessment_cap', [this] );
			show_settings( '.capability-list .brainpress_reports_cap', [this] );
			show_settings( '.capability-list .brainpress_notifications_cap', [this] );
			show_settings( '.capability-list .brainpress_discussions_cap', [this] );
			show_settings( '.capability-list .brainpress_settings_cap', [this] );
		}
		...
		$(document)
			.on( 'change', '.capability-list .brainpress_dashboard_cap input', on_dash_click )
			.on( 'change', '.capability-list .brainpress_courses_cap input', on_course_click )
			.on( 'change', '.capability-list .brainpress_instructors_cap input', on_instructor_click )
			.on( 'change', '.capability-list .brainpress_students_cap input', on_student_click )
			.on( 'change', '.capability-list .brainpress_notifications_cap input', on_notification_click )
			.on( 'change', '.capability-list .brainpress_discussions_cap input', on_discussion_click );
	})();
