...
class BrainPress_Helper_Table_CourseStudent extends WP_Users_List_Table {
	...
	public function __construct() {
		 ...
		 * set course ID
		 */
		if ( BrainPress_Data_Course::is_course( $this->course_id ) ) {
			$this->can_withdraw_students = BrainPress_Data_Capabilities::can_withdraw_students( $this->course_id );
		}
		...
		$this->filter_options = array(
			'all' => __( 'Alle', 'brainpress' ),
			'yes' => __( 'Zertifiziert', 'brainpress' ),
			'no' => __( 'Nicht zertifiziert', 'brainpress' ),
		);
	...
	public function student_row_actions( $actions, $item ) {
		...
		$profile_link = BrainPress_Data_Student::get_admin_profile_url( $item->ID );
		...
		if ( current_user_can( 'edit_users' ) ) {
			$actions['edit_user_profile'] = sprintf(
				...
				__( 'Benutzerprofil bearbeiten', 'brainpress' )
			);
		}
		...
		if ( $this->can_withdraw_students ) {
			$actions['trash'] = sprintf(
				...
				esc_html__( 'Zurückziehen', 'brainpress' )
			);
		}
		return $actions;
	}
	...
	public function prepare_items() {
		 ...
		 * fil certificates
		 */
		$certificates = BrainPress_Data_Certificate::get_certificated_students_by_course_id( $this->course_id );
	...
	public function extra_tablenav( $which ) {
		...
		if ( 'bottom' === $which && $this->add_new ) {
			...
			?>
			<div class="brainpress_course_add_student_wrapper">
			<?php
			$nonce = wp_create_nonce( 'add_student' );
			$withdraw_nonce = wp_create_nonce( 'withdraw_all_students' );

			if ( BrainPress_Data_Capabilities::can_assign_course_student( $course_id ) ) {
				$class_limited = BrainPress_Data_Course::get_setting( $course_id, 'class_limited' );
				...
				if ( $class_limited ) {
					...
					if ( 0 === $class_size || $class_size > $total_items ) {
						...
						printf(
							'<span>%s</span>',
							__( 'Du kannst keinen Studenten hinzufügen, das Klassenlimit ist erreicht.', 'brainpress' )
						);
					}
				} else {
					$add_form_to_add_student = true;
				...
				if ( $add_form_to_add_student ) {
					$name = 'student-add';
					$id = 'student-add';
					if ( apply_filters( 'brainpress_use_default_student_selector', false ) ) {
						$user_selector = BrainPress_Helper_UI::get_user_dropdown(
							$id,
							$name,
							array(
								'placeholder' => __( 'Student wählen...', 'brainpress' ),
								'class' => 'chosen-select narrow',
								'exclude' => $this->students,
								'context' => 'students',
							)
						);
					} else if ( apply_filters( 'brainpress_use_select2_student_selector', true ) ) {
						$nonce_search = BrainPress_Admin_Students::get_search_nonce_name( $course_id );
						...
						$user_selector = '<input type="text" id="' . $id .'" name="' . $name . '" placeholder="' . esc_attr__( 'Benutzer-ID eingeben', 'brainpress' ) . '" />';
					}
					$user_selector = apply_filters( 'brainpress_student_selector', $user_selector, $id, $name );
					echo $user_selector;
					printf(
						' <input type="button" class="add-new-student-button button" data-nonce="%s" value="%s" >',
						esc_attr( $nonce ),
						esc_attr__( 'Student hinzufügen', 'brainpress' )
					);
				}
			}
			...
			if ( BrainPress_Data_Capabilities::can_withdraw_students( $course_id ) ) {
			?>
				<a class="withdraw-all-students" data-nonce="<?php echo $withdraw_nonce; ?>" href="#"><?php esc_html_e( 'Ziehe alle Studenten zurück', 'brainpress' ); ?></a>
			<?php
			}
			?>
			<br />
	...
	public function no_items() {
		...
		if ( BrainPress_Data_Capabilities::can_assign_course_student( $course_id ) || BrainPress_Data_Capabilities::can_invite_students( $course_id ) ) {
			esc_html_e( 'In diesem Kurs sind keine Studenten eingeschrieben. Füge sie unten hinzu.', 'brainpress' );
		} else {
			esc_html_e( 'In diesem Kurs sind keine Studenten eingeschrieben.', 'brainpress' );
		}
	}
	...
	public function column_certificates( $item_id ) {
		...
		if ( 'yes' == $item->data->certified ) {
			return sprintf( '<span class="cp-certified">%s</span>', esc_html__( 'Zertifiziert', 'brainpress' ) );
		}
