...
class BrainPress_Data_Module {
	...
	public static function module_init_hooks() {
		 ...
		 * @since 2.0.0
		 */
		add_action( 'brainpress_module_added', array( __CLASS__, 'show_on_list' ), 10, 3 );
	}
	...
	public static function get_format() {
		...
		return array(
			...
			'post_args' => array(
				'labels' => array(
					'name' => __( 'Module', 'brainpress' ),
					'singular_name' => __( 'Modul', 'brainpress' ),
					'add_new' => __( 'Erstelle neu', 'brainpress' ),
					'add_new_item' => __( 'Neues Modul erstellen', 'brainpress' ),
					'edit_item' => __( 'Modul bearbeiten', 'brainpress' ),
					'edit' => __( 'Bearbeiten', 'brainpress' ),
					'new_item' => __( 'Neues Modul', 'brainpress' ),
					'view_item' => __( 'Modul anzeigen', 'brainpress' ),
					'search_items' => __( 'Suche Module', 'brainpress' ),
					'not_found' => __( 'Keine Module gefunden', 'brainpress' ),
					'not_found_in_trash' => __( 'Keine Module im Papierkorb gefunden', 'brainpress' ),
					'view' => __( 'Modul anzeigen', 'brainpress' ),
				),
	...
	public static function get_time_estimation( $module_id, $default = '1:00', $formatted = false ) {
		...
		if ( ! $formatted ) {
			 ...
			 */
			return apply_filters( 'brainpress_module_get_time_estimation', $duration, $module_id, $hours, $minutes, $seconds );
		}
		return '';
	}
	...
	public static function discussion_module_link( $location, $comment ) {
		...
		$location = esc_url_raw( $course_link . BrainPress_Core::get_slug( 'unit/' ) . get_post_field( 'post_name', $course_id ) . '#module-' . $comment->comment_post_ID );
		return $location;
	}
	...
	public static function discussion_post_link( $location, $post ) {
		...
		$course_id = get_post_field( 'post_parent', $unit_id );
		$course_link = get_permalink( $course_id );
		$location = esc_url_raw( $course_link . BrainPress_Core::get_slug( 'unit/' ) . get_post_field( 'post_name', $unit_id ) . '#module-' . $post->ID );
		return $location;
	}
	...
	public static function discussion_edit_redirect( $location, $comment_id ) {
		...
		if ( self::$post_type === $post_type ) {
			...
			$course_id = get_post_field( 'post_parent', $unit_id );
			$course_link = get_permalink( $course_id );
			$location = esc_url_raw( $course_link . BrainPress_Core::get_slug( 'unit/' ) . get_post_field( 'post_name', $unit_id ) . '#comment-' . $comment->comment_ID );
		}
	...
	public static function discussion_reply_link( $location, $args, $comment, $post ) {
		...
		// $course_id = get_post_field( 'post_parent', $unit_id );
		// $course_link = get_permalink( $course_id );
		// $location = esc_url_raw( $course_link . BrainPress_Core::get_slug( 'unit/' ) . get_post_field('post_name', $unit_id ) . '#module-' . $comment->comment_post_ID );
		// }
		if ( 'module' === $post->post_type ) {
			if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) {
				...
				$course_id = get_post_field( 'post_parent', $unit_id );
				$course_link = get_permalink( $course_id );
				$location = $course_link . BrainPress_Core::get_slug( 'unit/' ) . get_post_field( 'post_name', $unit_id );
				$location .= '&replytocom=' . $comment->comment_ID;
				$location .= '&module=' . $post->ID;
				// $location .= '#module-' . $post->ID;
				$onclick = sprintf( 'return BrainPress.utility.addComment.moveForm( "%1$s-%2$s", "%2$s", "%3$s", "%4$s" )',
	...
	public static function discussion_cancel_reply_link( $formatted_link, $location, $text ) {
		...
		if ( empty( $text ) ) {
			$text = __( 'Klicke hier, um die Antwort abzubrechen.', 'brainpress' ); }
	...
	public static function get_quiz_results( $student_id, $course_id, $unit_id, $module_id, $response = false, $data = false ) {
		...
		if ( false === $data ) {
			$data = BrainPress_Data_Student::get_completion_data( $student_id, $course_id );
		}

		if ( false === $response ) {
			$response = BrainPress_Data_Student::get_response( $student_id, $course_id, $unit_id, $module_id, false, $data );
		...
		$remaining_message = sprintf(
			esc_html__( 'Dein letzter Versuch war erfolglos. Versuche es nochmal. %s', 'brainpress' ),
			$remaining_message
		);
	...
	*/
	public static function get_form_results( $student_id, $course_id, $unit_id, $module_id, $response = false, $data = false ) {
		...
		$grades = BrainPress_Data_Student::get_grade( $student_id, $course_id, $unit_id, $module_id );
		$minimum_grade = (int) $attributes['minimum_grade'];

		$total_questions = count( $attributes['questions'] );
		$gross_correct = 0;
		$grade = BrainPress_Helper_Utility::get_array_val( $grades, 'grade', 0 );
		...
		if ( $is_assessable ) {
			...
			if ( 'auto' == $graded_by ) {
				return array(
					...
					'passed' => false,
					'attributes' => $attributes,
					'message' => __( 'Deine Einreichung wartet auf die Beurteilung des Kursleiters.', 'brainpress' ),
				);
			}
		}
	...
	public static function quiz_result_content( $student_id, $course_id, $unit_id, $module_id, $quiz_result = false ) {
		...
		$passed_heading = ! empty( $quiz_result['passed'] ) ? __( 'Erfolg!', 'brainpress' ) : __( 'Quiz nicht bestanden.', 'brainpress' );
		...
		$template = '<div class="module-quiz-questions"><div class="brainpress-quiz-results ' . esc_attr( $passed_class ) . '">
			...
			if ( $can_retry ) {
				...
				if ( $is_enabled ) {
					...
					$template .= sprintf(
						...
						esc_attr( $module_id ),
						__( 'Versuche es nochmal!', 'brainpress' )
					);
					$template .= ' </div>';
				}
			}
		...
		if ( isset( $mandatory_modules[ $module_id ] ) ) {
			switch ( $mandatory_modules[ $module_id ] ) {
				...
				default:
					...
					$completion_data = BrainPress_Data_Student::get_completion_data( $student_id, $course_id );
					$response = BrainPress_Helper_Utility::get_array_val(
						$completion_data,
						'units/' . $unit_id . '/responses/' . $module_id
					);
	...
	public static function get_modules_ids_by_unit( $unit_id ) {
		...
		$unit_post_type = BrainPress_Data_Unit::get_post_type_name();
	...
	public static function get_course_id_by_module( $module ) {
		...
		return BrainPress_Data_Unit::get_course_id_by_unit( $unit_id );
	}
	...
	public static function get_instructors( $module_id, $objects = false ) {
		...
		return BrainPress_Data_Unit::get_instructors( $unit_id, $objects );
	}
	...
	public static function add_instructors_to_comments_args( $args ) {
		...
		$args['brainpress_instructors'] = self::get_instructors( $post->ID );
		return $args;
	}
	...
	public static function show_on_list( $module_id, $unit_id, $meta ) {
		$course_id = BrainPress_Data_Unit::get_course_id_by_unit( $unit_id );
		...
		BrainPress_Data_Course::update_setting( $course_id, 'structure_visible_modules', $visible_modules );
		/**
		...
		if ( ! isset( $visible_modules[ $id ] ) ) {
			BrainPress_Data_Unit::show_page( $unit_id, $page_id, $course_id );
		}
	}
	...
	public static function get_all_modules_ids_by_type( $type, $course_id = null ) {
		...
		if ( ! empty( $course_id ) ) {
			$units = BrainPress_Data_Course::get_units( $course_id, array( 'any' ), true );
			if ( empty( $units ) ) {
				return array();
			}
	...
	public static function decrease_page_number( $unit_id, $page_number ) {
		if ( ! BrainPress_Data_Unit::is_unit( $unit_id ) ) {
			return;
		}
		$args = array(
			'post_type' => self::get_post_type_name(),
