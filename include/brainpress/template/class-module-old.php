	...
	public static function template( $module_id = 0 ) {
		...
		if ( method_exists( __CLASS__, $method ) ) {
			...
			if ( empty( self::$student_progress ) ) {
				$student_progress = self::$student_progress = BrainPress_Data_Student::get_completion_data( get_current_user_id(), $course_id );
			} else {
				$student_progress = self::$student_progress;
			}

			$responses = BrainPress_Data_Student::get_responses( get_current_user_id(), $course_id, $unit_id, $module_id, true, $student_progress );
			...
			$course_status = BrainPress_Data_Course::get_course_status($course_id) == 'closed';
			if ( $course_status ){
				$disabled = true;
			}
			// RESUBMIT LOGIC
			$action = false === $disabled ? '<div><a class="module-submit-action">' . esc_html__( 'Antwort einreichen', 'brainpress' ) . '</a></div>' : '';
...
*/
	...
	private static function do_caption_media( $data ) {
		...
		if ( 'video' === $type ) {
			...
			if ( $hide_related ) {
				add_filter( 'oembed_result', array( 'BrainPress_Helper_Utility', 'remove_related_videos' ), 10, 3 );
			}
...
/*
		...
		$content .= '<div class="comments-list-container">' . BrainPress_Template_Discussion::get_comments( $module->ID ) . '</div>';
		//      $content .= '<div class="comments-nav-container">' . BrainPress_Template_Discussion::get_comments_nav( $module->ID, $module->post_parent, 5 ) . '</div>';
...
*/
	...
	public static function render_input_checkboxOLD( $module, $attributes = false, $student_progress = false ) {
		...
		if ( ! empty( $attributes['answers'] ) ) {
			...
			// RESUBMIT LOGIC
			$action = ! $disabled ? '<div><a class="module-submit-action">' . esc_html__( 'Antwort einreichen', 'brainpress' ) . '</a></div>' : '';
		...
		$content .= $course_status ? apply_filters('brainpress_course_readonly_message', '<div class="module-warning"><p>' . esc_html__( 'Dieser Kurs ist abgeschlossen, Du kannst keine Antworten mehr einreichen.', 'brainpress' ) . '</p></div>') : '';
...
///		$content .= '</div>'; // module_footer
	...
	private static function render_module_result( $module, $attributes, $args ) {
		...
		$grade = BrainPress_Data_Student::get_grade( $student_id, $course_id, $unit_id, $module_id, $response_key, false, $student_progress );
		$grade = $grade['grade'];
		$feedback = BrainPress_Data_Student::get_feedback( $student_id, $course_id, $unit_id, $module_id, $response_key, false, $student_progress );
		...
		if ( cp_is_true( $attributes['assessable'] ) ) {
			if ( 'input-upload' == $attributes['module_type'] ) {
				$content .= '<p>' . esc_html__( 'Diese Datei wurde erfolgreich hochgeladen. Es wird Teil Deiner Bewertung in diesem Kurs sein.', 'brainpress' ) . '</p>';
			}
			if ( $grade > - 1 ) {
				$content .= '<p><strong>' . esc_html__( 'Bewertung:', 'brainpress' ) . '</strong> ' . $grade . '%</p>';
			} else {
				$content .= '<p><strong>' . esc_html__( 'Nicht bewertet', 'brainpress' ) . '</strong></p>';
			}
		} elseif ( 'input-upload' == $attributes['module_type'] ) {
			$content .= '<p>' . esc_html__( 'Diese Datei wurde erfolgreich hochgeladen. Es wird nicht Teil Deiner Bewertung in diesem Kurs sein.', 'brainpress' ) . '</p>';
		}
		$content .= '</div>';

		if ( ( $attributes['minimum_grade'] > $grade || ( ! cp_is_true( $attributes['assessable'] ) ) && $attributes['minimum_grade'] <= $grade ) && ! $disabled ) {
			$content .= '<div class="resubmit"><a>' . esc_html__( 'Erneut einreichen', 'brainpress' ) . '</a></div>';
		}
		if ( $feedback && ! empty( $feedback ) ) {
			$content .= '<div class="feedback"><strong>' . esc_html__( 'Feedback:', 'brainpress' ) . '</strong><br/> ' . $feedback . '</div>';
		}
	...
	public static function render_input_radioOLD( $module, $attributes = false ) {
		...
		if ( ! empty( $attributes['answers'] ) ) {
			...
			// RESUBMIT LOGIC
			$action = ! $disabled ? '<a class="module-submit-action">' . esc_html__( 'Antwort einreichen', 'brainpress' ) . '</a>' : '';
	...
	public static function render_input_select( $module, $attributes = false ) {
		...
		if ( ! empty( $attributes['answers'] ) ) {
			...
			if ( ! empty( $responses ) ) {
				...
				$content .= '</ul>';
				// $meh = '<p><span class="label">' . esc_html__( 'Response: ', 'brainpress' ) . '</span>
	...
	public static function render_input_text( $module, $attributes = false ) {
		...
		if ( ! empty( $responses ) ) {
			...
			$content .= '<div class="module-response">
				<p><span class="label">' . esc_html__( 'Antwort: ', 'brainpress' ) . '</span>
					' . $last_response . '
				</p>
			</div>';
...
//		$content .= '</div>'; // module_footer
	...
	public static function render_input_upload( $module, $attributes = false ) {
		...
		// RESUBMIT LOGIC
		$action = ! $disabled ? '<div><a class="module-submit-action">' . esc_html__( 'Datei einreichen', 'brainpress' ) . '</a></div>' : '';
		...
		$content .= '<div class="module-elements ' . $element_class . '">
						...
						<label class="file">
							...
							<span class="button" data-change="'.esc_attr__('Change File', 'brainpress' ).'" data-upload="'.esc_attr__('Datei hochladen', 'brainpress' ).'">'.esc_attr__('Datei hochladen', 'brainpress' ).'</span>
						</label>
		...
		if ( ! empty( $responses ) ) {
			...
			if ( isset( $last_response['url'] ) ) {
				...
				$content .= '<div class="module-response">
					<p class="file_holder"><span class="label">' . esc_html__( 'Hochgeladene Datei: ', 'brainpress' ) . '</span>
						<a href="' . esc_url( $url ) . '">' . esc_html( $file_name ) . ' ' . BrainPress_Helper_Utility::filter_content( $filesize ) . '</a>
					</p>
				</div>';
			}
	...
	public static function render_input_quiz( $module, $attributes = false ) {
		...
		$quiz_result = BrainPress_Data_Module::get_quiz_results( get_current_user_id(), $course_id, $unit_id, $module_id, false, $student_progress );

		// Is the quiz already passed?
		$already_passed = ! empty( $quiz_result ) && ! empty( $quiz_result['passed'] );
		$quiz_result_content = ! empty( $quiz_result ) ? do_shortcode( '[brainpress_quiz_result course_id="' . $course_id . '" unit_id="' . $unit_id . '" module_id="' . $module_id . '" student_id="' . get_current_user_id() . '"]' ) : '';
		...
		if ( ! empty( $attributes['questions'] ) && ! $already_passed && ! $disabled ) {
			...
			$remaining_message = ! $unlimited ? sprintf( __( 'Du hast noch %d Versuche.', 'brainpress' ), $remaining ) : '';

			if ( ! empty( $responses ) && ! $already_passed ) {
				$remaining_message = sprintf( esc_html__( 'Dein letzter Versuch war erfolglos. Versuche es nochmal. %s', 'brainpress' ), $remaining_message );

				if ( ! $unlimited && 1 > $remaining ) {
					$remaining_message = esc_html__( 'Dein letzter Versuch war erfolglos. Du kannst es nicht mehr versuchen.', 'brainpress' );
				}
			...
			$seconds = BrainPress_Helper_Utility::duration_to_seconds( $attributes['duration'] );
			if ( $use_timer && ! empty( $seconds ) ) {
				$data_ref = 'ref_' . strrev( BrainPress_Helper_Utility::hashcode( '' . $seconds ) );
			...
			// RESUBMIT LOGIC
			$action = '<a class="module-submit-action">' . esc_html__( 'Sende Quizantworten', 'brainpress' ) . '</a>';
			$content .= $action;
...
//		$content .= '</div>'; // module_footer
	...
	public static function render_input_form( $module, $attributes = false ) {
		...
		// RESUBMIT LOGIC
		$action = '<a class="module-submit-action">' . esc_html__( 'Einreichen', 'brainpress' ) . '</a>';
		$content .= $action;
