 ...
 * views.
 */
class BrainPress_Data_Shortcode_CourseTemplate {
	...
	public static function course_join_button( $atts ) {
		global $brainpress, $enrollment_process_url, $signup_url, $wp_query;

		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'access_text' => __( 'Beginne zu lernen', 'brainpress' ),
			'class' => '',
			'continue_learning_text' => __( 'Weiter lernen', 'brainpress' ),
			'course_expired_text' => __( 'Nicht verfügbar', 'brainpress' ),
			'course_full_text' => __( 'Kurs voll', 'brainpress' ),
			'details_text' => __( 'Details', 'brainpress' ),
			'enrollment_closed_text' => __( 'Einschreibung geschlossen', 'brainpress' ),
			'enrollment_finished_text' => __( 'Einschreibungen abgeschlossen', 'brainpress' ),
			'enroll_text' => __( 'Jetzt einschreiben!', 'brainpress' ),
			'instructor_text' => __( 'Zugang zum Kurs', 'brainpress' ),
			'list_page' => false,
			'not_started_text' => __( 'Nicht verfügbar', 'brainpress' ),
			'passcode_text' => __( 'Passcode erforderlich', 'brainpress' ),
			'prerequisite_text' => __( 'Voraussetzung Erforderlich', 'brainpress' ),
			'signup_text' => __( 'Jetzt einschreiben!', 'brainpress' ),
		...
		if ( $can_update_course ) {
			$enroll_text = __( 'Einschreiben', 'brainpress' );
		}

		$now = BrainPress_Data_Course::time_now();
		$general_settings = BrainPress_Core::get_setting( 'general' );

		$course->enroll_type = BrainPress_Data_Course::get_setting( $course_id, 'enrollment_type' );
		$course->course_start_date = BrainPress_Data_Course::get_setting( $course_id, 'course_start_date' );
		$course->course_start_date = BrainPress_Data_Course::strtotime( $course->course_start_date );
		$course->course_end_date = BrainPress_Data_Course::get_setting( $course_id, 'course_end_date' );
		$course->enrollment_start_date = BrainPress_Data_Course::get_setting( $course_id, 'enrollment_start_date' );
		$course->enrollment_end_date = BrainPress_Data_Course::get_setting( $course_id, 'enrollment_end_date' );
		$course->open_ended_course = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'course_open_ended' ) );
		$course->open_ended_enrollment = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'enrollment_open_ended' ) );
		$course->prerequisite = BrainPress_Data_Course::get_prerequisites( $course_id );
		$course->is_paid = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'payment_paid_course' ) );
		$course->course_started = ! $course->open_ended_course && ! empty( $course->course_end_date ) && BrainPress_Data_Course::strtotime( $course->course_start_date ) <= $now ? true : false;
		$course->enrollment_started = BrainPress_Data_Course::strtotime( $course->enrollment_start_date ) <= $now ? true : false;
		$course->course_expired = ! $course->open_ended_course && ! empty( $course->course_end_date ) && BrainPress_Data_Course::strtotime( $course->course_end_date ) <= $now ? true : false;
		$course->enrollment_expired = ! empty( $course->enrollment_end_date ) && BrainPress_Data_Course::strtotime( $course->enrollment_end_date ) <= $now ? true : false;
		$course->full = BrainPress_Data_Course::is_full( $course_id );
		$course_progress = 0;
		...
		if ( is_user_logged_in() ) {
			...
			$student_enrolled = BrainPress_Data_Course::student_enrolled( $student_id, $course_id );
			$is_instructor = BrainPress_Data_Instructor::is_assigned_to_course( $course_id, $student_id );
			$course_progress = BrainPress_Data_Student::get_course_progress( $student_id, $course_id );
			if ( 100 === $course_progress ) {
				$continue_learning_text = __( 'Abgeschlossen', 'brainpress' );
				$class .= ' course-completed-button';
			} else {
				$meta_key = BrainPress_Data_Course::get_last_seen_unit_meta_key( $course_id );
				...
				if ( is_array( $last_seen_unit ) && isset( $last_seen_unit['unit_id'] ) && isset( $last_seen_unit['page'] ) ) {
					...
					$is_unit = BrainPress_Data_Unit::is_unit( $last_seen_unit['unit_id'] );
					if ( $is_unit ) {
						$continue_learning_link = $course_link = BrainPress_Data_Unit::get_url( $last_seen_unit['unit_id'], $last_seen_unit['page'] );
					}
				}
			}
		} else {
			...
			if ( false === $is_custom_login ) {
				...
				$signup_url = BrainPress_Core::get_slug( 'login/', true );
		...
		$is_single = BrainPress_Helper_Utility::$is_singular;
		$buttons = apply_filters(
			'brainpress_course_enrollment_button_options',
			array(
				'full' => array(
				...
				'continue' => array(
					...
					'attr' => array(
						...
						'data-link' => empty( $continue_learning_link )? BrainPress_Data_Student::get_last_visited_url( $course_id ) : $continue_learning_link,
					),
					'type' => 'link',
				),
			),
		...
		$buttons = apply_filters( 'brainpress_coursetemplate_join_button', $buttons );

		// Determine the button option.
		if ( ! $student_enrolled && ! $is_instructor ) {
			// For vistors and non-enrolled students.
			$enrollment_start_date = BrainPress_Data_Course::strtotime( $course->enrollment_start_date );
			...
			if ( $course->full ) {
				...
				if ( ! empty( $student_id ) ) {
					...
					foreach ( $prerequisites as $prerequisite ) {
						if ( BrainPress_Data_Course::student_enrolled( $student_id, $prerequisite ) && BrainPress_Data_Student::is_course_complete( $student_id, $prerequisite ) ) {
							$completed += 1;
						}
					}
			...
			if ( empty( $button_option ) || 'enroll' == $button_option ) {
				...
				$user_can_register = BrainPress_Helper_Utility::users_can_register();
				...
				$force_signup = apply_filters( 'brainpress_course_enrollment_force_registration', false );
				...
				if ( ( empty( $student_id ) && $user_can_register && empty( $button_option ) ) || $force_signup ) {
					...
					switch ( $course->enroll_type ) {
						...
						case 'prerequisite':
							...
							foreach ( $prerequisites as $prerequisite ) {
								if ( BrainPress_Data_Course::student_enrolled( $student_id, $prerequisite ) && BrainPress_Data_Student::is_course_complete( $student_id, $course_id ) ) {
									$completed += 1;
								}
							}
			...
			$progress = BrainPress_Data_Student::get_course_progress( get_current_user_id(), $course_id );
			...
			} elseif ( ! $is_single && false === strpos( $_SERVER['REQUEST_URI'], BrainPress_Core::get_setting( 'slugs/student_dashboard', 'courses-dashboard' ) ) ) {
		...
		$button_option = apply_filters( 'brainpress_course_enrollment_button_option', $button_option );

		// Prepare the button.
		if ( ( ! $is_single && ! is_page() ) || $list_page ) {
			$button_url = get_permalink( $course_id );
			global $post;
			if ( BrainPress_Data_Course::is_course( $post ) ) {
			...
			//$button = apply_filters( 'brainpress_enroll_button_content', '', $course );
			if ( empty( $button_option ) || ( 'manually' == $course->enroll_type && ! ( 'access' == $button_option || 'continue' == $button_option ) ) ) {
				return apply_filters( 'brainpress_enroll_button', $button, $course_id, $student_id, $button_option );
			}
			 ...
			 */
			$is_wp_script = wp_script_is( 'brainpress-front-js' );
		...
		if ( is_user_logged_in() ) {
			...
			$is_course_instructor = BrainPress_Data_Instructor::is_course_instructor( $user_id, $course_id );
	...
	public static function course_structure( $atts ) {
		...
		extract( shortcode_atts( array(
			'course_id' => BrainPress_Helper_Utility::the_course( true ),
			'free_text' => __( 'Vorschau', 'brainpress' ),
			'free_show' => 'true',
			'free_class' => 'free',
			'show_title' => 'no',
			'show_label' => 'no',
			...
			'show_divider' => 'yes',
			'show_estimates' => 'no',
			'label' => __( 'Kursstruktur', 'brainpress' ),
			'class' => '',
			'deep' => false,
		...
		$view_mode = BrainPress_Data_Course::get_setting( $course_id, 'course_view', 'normal' );
		$with_modules = false;
		$counter = 0;
		...
		$structure_visible = cp_is_true(
			BrainPress_Data_Course::get_setting( $course_id, 'structure_visible' )
		);

		if ( ! $structure_visible ) { return ''; }

		$time_estimates = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'structure_show_duration' ) );

		$preview = BrainPress_Data_Course::previewability( $course_id );
		$visibility = BrainPress_Data_Course::structure_visibility( $course_id );
		$structure_level = BrainPress_Data_Course::get_setting( $course_id, 'structure_level' );
		...
		if ( ! empty( $student_id ) ) {
			$enrolled = BrainPress_Data_Course::student_enrolled( $student_id, $course_id );
		}
		if ( $enrolled ) {
			$student_progress = BrainPress_Data_Student::get_completion_data( $student_id, $course_id );
		}

		$units = BrainPress_Data_Course::get_units_with_modules( $course_id, array( 'publish' ) );
		...
		$is_course_available = BrainPress_Data_Course::is_course_available( $course_id );
		$can_update_course = BrainPress_Data_Capabilities::can_update_course( $course_id );
		...
		foreach ( $units as $unit_id => $unit ) {
			$is_unit_visible = BrainPress_Data_Unit::is_unit_structure_visible( $course_id, $unit_id );
			if ( ! $is_unit_visible ) {
				continue;
			}
			$the_unit = $unit['unit'];
			$previous_unit_id = BrainPress_Data_Unit::get_previous_unit_id( $course_id, $the_unit->ID );
			$is_unit_available = $is_course_available ? BrainPress_Data_Unit::is_unit_available( $course_id, $the_unit, $previous_unit_id ) : $is_course_available;
			$unit_link = BrainPress_Core::get_slug( 'courses/', true ) .
				$course_slug . '/' .
				BrainPress_Core::get_slug( 'unit/' ) .
				$unit['unit']->post_name;
			$estimation = BrainPress_Data_Unit::get_time_estimation( $unit_id, $units );
			if ( $last_module_id > 0 && $clickable ) {
				// Check if the last module is already answered.
				$is_last_module_done = BrainPress_Data_Module::is_module_done_by_student( $last_module_id, $student_id );
			...
			if (
				...
				if ( empty( $last_unit ) ) {
					...
					$unit_available = BrainPress_Data_Unit::is_unit_available( $course_id, $unit_id, $last_unit );
				}
				if ( $unit_available ) {
			 ...
			 * Show empty units?
			 */
			$show_empty_units = cp_is_true( BrainPress_Data_Course::get_setting( $course_id, 'structure_show_empty_units' ) );

			foreach ( $unit['pages'] as $key => $page ) {

				// Hide pages if it is not set as visible
				$show_page = BrainPress_Data_Unit::is_page_structure_visible( $course_id, $unit_id, $key, $student_id );
				...
				$page_title = empty( $page['title'] ) ? sprintf( __( 'Unbenannte Seite %s', 'brainpress' ), $count ) : $page['title'];
				...
				if ( $show_page_tilte ) {
					...
					if ( $time_estimates ) {
						...
						$page_estimate = apply_filters( 'brainpress_page_estimation', $page_estimate, $estimation['pages'][ $key ] );
				...
				// Add Module Level.
				$structure_level = BrainPress_Data_Course::get_setting( $course_id, 'structure_level', 'unit' );
				...
				if ( $deep || 'section' === $structure_level || 'unit' === $structure_level ) {
					...
					foreach ( $page['modules'] as $m_key => $module ) {
						...
						$is_module_visible = BrainPress_Data_Unit::is_module_structure_visible( $course_id, $unit_id, $m_key, $student_id );
						...
						$attributes = BrainPress_Data_Module::attributes( $m_key );
						...
						$module_seen = BrainPress_Helper_Utility::get_array_val( $student_progress, 'completion/' . $unit_id . '/modules_seen/' . $m_key );
						$module_passed = BrainPress_Helper_Utility::get_array_val( $student_progress, 'completion/' . $unit_id . '/passed/' . $m_key );
						$module_answered = BrainPress_Helper_Utility::get_array_val( $student_progress, 'completion/' . $unit_id . '/answered/' . $m_key );
						...
						if ( $prev_module_id > 0 ) {
							$is_done = BrainPress_Data_Module::is_module_done_by_student( $prev_module_id, $student_id );
							if ( false === $is_done ) {
								$clickable = false;
							} else {
						...
						$list_content .= sprintf(
							...
							esc_attr__( 'Vorschau', 'brainpress' ),
							$module_title
						);
						$list_content .= '</div>';
						$list_content .= '</li>';
	...
	public static function unit_archive_list( $atts ) {
		...
		BrainPress_Data_Student::log_student_activity( 'course_unit_seen' );

		$course_id = (int) $course_id;
		if ( empty( $course_id ) ) { return ''; }

		$with_modules = cp_is_true( $with_modules );
		$course_base_url = BrainPress_Data_Course::get_course_url( $course_id );
		$can_update_course = BrainPress_Data_Capabilities::can_update_course( $course_id );

		if ( ! $with_modules ) {
			$unit_mode = BrainPress_Data_Course::get_setting( $course_id, 'structure_level', 'unit' );
		...
		$units_with_modules = BrainPress_Data_Course::get_units_with_modules( $course_id, $unit_status );
		$units_with_modules = BrainPress_Helper_Utility::sort_on_key( $units_with_modules, 'order' );

		if ( ! $with_modules ) {
			$units = BrainPress_Data_Course::get_units(
				BrainPress_Helper_Utility::the_course( true ),
				$unit_status
			);
		} else {
		...
		foreach ( $units as $unit ) {
			...
			// Hide hidden unit
			$is_unit_structure_visible = BrainPress_Data_Unit::is_unit_structure_visible( $course_id, $unit_id, $student_id );
			if ( ! $is_unit_structure_visible ) { continue; }

			if ( is_object( $unit ) ) { $unit = get_object_vars( $unit ); }

			$can_view = BrainPress_Data_Course::can_view_unit( $course_id, $unit_id );
			...
			if ( $enrolled ) {
				...
				$is_unit_completed = BrainPress_Helper_Utility::get_array_val(
					$student_progress,
				...
				if ( cp_is_true( $is_unit_completed ) ) {
					...
					$is_first_page_seen = BrainPress_Data_Student::is_section_seen( $course_id, $unit_id, 1, $student_id );
			...
			if ( 'publish' != $the_unit->post_status && $can_update_course ) {
				$title_suffix = esc_html__( ' [ENTWURF]', 'brainpress' );
			}
			...
			if ( ! $clickable ) {
				$unit_status = __( 'Diese Einheit ist verfügbar, Du musst jedoch alle ERFORDERLICHEN Module vor dieser Einheit abschließen.', 'brainpress' );
			 ...
			 */
			$add_open_date = apply_filters( 'brainpress_unit_add_open_date', $add_open_date, $unit_id, $course_id );

			if ( $add_open_date ) {
				/**
				 * return date with known format
				 */
				$unit_availability_date = BrainPress_Data_Unit::get_unit_availability_date( $unit_id, $course_id, 'c' );
				$_unit_date = BrainPress_Data_Course::strtotime( $unit_availability_date );
				$now = BrainPress_Data_Course::time_now();
				...
				if ( ! empty( $unit_availability_date ) && $_unit_date > $now && 'expired' != $unit_availability_date ) {
					...
					if ( 'instant' == $status_type ) {
						$unit_status = esc_attr__( 'Du musst die ERFORDERLICHE Einheit vor dieser Einheit ausfüllen.', 'brainpress' );
					} else {
						$unit_availability_date = BrainPress_Data_Course::strtotime( $unit_availability_date );
						$year_now = date( 'Y', BrainPress_Data_Course::time_now() );
						$unit_year = date( 'Y', $unit_availability_date );
						$format = $year_now !== $unit_year ? _x( 'M d, Y', 'Verfügbares Datum der Einheit mit Jahr für die zukünftige Einheit.', 'brainpress' ) : _x( 'M d', 'Verfügbares Datum der Einheit ohne Jahr für zukünftige Einheit.', 'brainpress' );
						// Requires custom hook to attached
						$when = date_i18n( $format, $unit_availability_date );

						$delay_date = sprintf(
							'<span class="unit-delay-date">%s</span>',
							sprintf( __( 'Beginnt %s', 'brainpress' ), $when )
						);
						$unit_status = __( 'Diese Einheit ist zum geplanten Startdatum verfügbar.', 'brainpress' );
						/**
						 ...
						 * @return $date or null
						 **/
						$delay_date = apply_filters( 'brainpress_unit_delay_markup', $delay_date, $unit_availability_date );
			...
			if ( $is_unit_available || $can_update_course ) {
				$unit_url = BrainPress_Data_Unit::get_unit_url( $unit_id );
			} else {
			...
			if ( ( $is_unit_available && $with_modules ) || $can_update_course ) {
				...
				foreach ( $unit['pages'] as $page_number => $page ) {
					...
					$is_page_structure_visible = BrainPress_Data_Unit::is_page_structure_visible( $course_id, $unit_id, $page_number, $student_id );
					if ( ! $is_page_structure_visible ) { continue; }

					if ( ! BrainPress_Data_Course::can_view_page( $course_id, $unit_id, $page_number ) ) {
						continue;
					}
					...
					if ( $heading_visible ) {
						...
						$is_section_seen = BrainPress_Data_Student::is_section_seen( $course_id, $unit_id, $page_number );
						$section_data    = '';
						...
						if ( ! $clickable ) {
							...
							$section_data  = sprintf( ' data-title="%s"', esc_attr__( 'Du musst alle ERFORDERLICHEN Module vor diesem Abschnitt abschließen.', 'brainpress' ) );
						}
						...
						if ( $clickable || $can_update_course ) {
							$module_table .= '<a href="' . $section_link . '">' . ( ! empty( $page['title'] ) ? esc_html( $page['title'] ) : esc_html__( 'Ohne Titel', 'brainpress' ) ) . '</a>';
						} else {
							$module_table .= sprintf( '<span>%s</span>', ! empty( $page['title'] ) ? esc_html( $page['title'] ) : esc_html__( 'Ohne Titel', 'brainpress' ) );
						}
					...
					foreach ( $page['modules'] as $module ) {
						// Hide hidden modules
						$is_module_structure_visible = BrainPress_Data_Unit::is_module_structure_visible( $course_id, $unit_id, $module->ID, $student_id );
						if ( ! $is_module_structure_visible ) { continue; }

						$attributes = BrainPress_Data_Module::attributes( $module->ID );
						$is_required = isset( $attributes['mandatory'] ) && cp_is_true( $attributes['mandatory'] );

						if ( ! BrainPress_Data_Course::can_view_module( $course_id, $unit_id, $module->ID, $page_number ) ) {
							continue;
						}

						// Get completion states.
						$module_seen = BrainPress_Helper_Utility::get_array_val( $student_progress, 'completion/' . $unit_id . '/modules_seen/' . $module->ID );
						$module_passed = BrainPress_Helper_Utility::get_array_val( $student_progress, 'completion/' . $unit_id . '/passed/' . $module->ID );
						$module_answered = BrainPress_Helper_Utility::get_array_val( $student_progress, 'completion/' . $unit_id . '/answered/' . $module->ID );
						...
						//$completed_class = empty( $completed_class ) && isset( $module_passed ) && ! empty( $module_answered ) && ! $attributes['assessable'] && $attributes['mandatory'] ? 'module-completed' : '';
						$completed_class = '';
						$is_module_completed = BrainPress_Data_Student::is_module_completed( $course_id, $unit_id, $module->ID, $student_id );
						...
						$info = __( 'Du musst alle ERFORDERLICHEN Module vor diesem Modul abgeschlossen haben.', 'brainpress' );
						$data_title = __( 'Vorschau', 'brainpress' );
						if ( ! $clickable ) {
						...
						$title = ! empty( $module->post_title ) ? esc_html( $module->post_title ) : esc_html__( 'Mod', 'brainpress' ) . '<br />';
						...
						$module_table .= '</li>';
						$is_previous_module_done = BrainPress_Data_Module::is_module_done_by_student( $module->ID, $student_id );
					}
		...
		if ( empty( $content_units ) ) {
			$content .= sprintf(
				...
				esc_html__( 'Derzeit keine sichtbaren Einheiten im Kurs. Bitte versuche es später erneut.', 'brainpress' )
			);
		} else {
		...
		if ( empty( $units ) ) {
			$content .= '<h3 class="zero-course-units">' . esc_html__( 'Derzeit keine Einheiten im Kurs. Bitte versuche es später erneut.', 'brainpress' ) . '</h3>';
		}
		$content .= '</div>';
	...
	public static function course_featured( $atts ) {
		extract( shortcode_atts(
			array(
				'course_id' => '',
				'featured_title' => __( 'Empfohlener Kurs', 'brainpress' ),
				'button_title' => __( 'Finde mehr heraus.', 'brainpress' ),
	...
	public static function course_calendar( $atts ) {
		...
		extract( shortcode_atts( array(
			...
			'month' => false,
			'next' => __( 'Weiter &raquo;', 'brainpress' ),
			'pre' => __( '&laquo; Zurück', 'brainpress' ),
			'year' => false,
		...
		if ( empty( $course_id ) ) {
			if ( $post && BrainPress_Data_Course::get_post_type_name() == $post->post_type ) {
				$course_id = $post->ID;
			} else {
		...
		$course_calendar = new BrainPress_Template_Calendar( $args );
	...
	public static function course_list( $atts ) {
		...
		$atts = BrainPress_Helper_Utility::sanitize_recursive(
			shortcode_atts(
				array(
					'completed_label' => __( 'Abgeschlossene Kurse', 'brainpress' ),
					'context' => 'all', // <blank>, enrolled, completed
					'current_label' => __( 'Aktuelle Kurse', 'brainpress' ),
					'dashboard' => false,
					'facilitator_label' => __( 'Moderierte Kurse', 'brainpress' ),
					'facilitator' => '',
					'future_label' => __( 'Beginnt bald', 'brainpress' ),
					'incomplete_label' => __( 'Unvollständige Kurse', 'brainpress' ),
					'instructor_msg' => __( 'Dem Kursleiter sind noch keine Kurse zugewiesen.', 'brainpress' ),
					'instructor' => '', // Note, one or the other
					'limit' => - 1,
					'manage_label' => __( 'Kurse verwalten', 'brainpress' ),
					'order' => 'ASC',
					'orderby' => 'meta', /// possible values: meta, title
					'past_label' => __( 'Vergangene Kurse', 'brainpress' ),
					'show_labels' => false,
					'status' => 'publish',
					'student_msg' => sprintf( __( 'Du bist in keinem Kurs eingeschrieben. <a href="%s">Siehe verfügbare Kurse.</a>', 'brainpress' ), BrainPress_Core::get_slug( 'course/', true ) ),
					'student' => '', // If both student and instructor is specified only student will be used
					'suggested_label' => __( 'Vorgeschlagene Kurse', 'brainpress' ),
					'suggested_msg' => __( 'Du bist in keinem Kurs eingeschrieben.<br />Hier sind einige, die Dir gefallen könnten, oder <a href="%s">alle verfügbaren Kurse anzeigen.</a>', 'brainpress' ),
		...
		if ( ! empty( $atts['instructor'] ) ) {
			...
			if ( ! empty( $instructors ) ) {
				foreach ( $instructors as $ins ) {
					$ins = (int) $ins;
					if ( $ins ) {
						$course_ids = BrainPress_Data_Instructor::get_assigned_courses_ids( $ins, $atts['status'] );
						if ( $course_ids ) {
				...
				if ( $instructor ) {
					$course_ids = BrainPress_Data_Instructor::get_assigned_courses_ids( $instructor, $atts['status'] );
					if ( $course_ids ) {
						$include_ids = array_unique( array_merge( $include_ids, $course_ids ) );
					}
				}
		...
		if ( ! empty( $atts['facilitator'] ) ) {
			...
			$include_ids = BrainPress_Data_Facilitator::get_facilitated_courses( $facilitator, 'publish', true );
		...
		if ( ! empty( $atts['student'] ) ) {
			...
			foreach ( $students as $student ) {
				...
				if ( $student ) {
					$courses_ids = array();
					$courses_to_add = BrainPress_Data_Student::get_enrolled_courses_ids( $student );
					if ( isset( $atts['status'] ) ) {
						foreach ( $courses_to_add as $course_id ) {
							...
							if ( 'publish' != $atts['status'] ) {
								$status = BrainPress_Data_Student::get_course_status( $course_id, $student, false );
		...
		if ( ! empty( $atts['categories'] ) ) {
			$post_args['tax_query'] = array(
				array(
					'taxonomy' => BrainPress_Data_Course::get_post_category_name(),
					'field' => 'slug',
		...
		switch ( $atts['context'] ) {
			case 'enrolled':
				...
				$user_id = get_current_user_id();
				$include_ids = BrainPress_Data_Student::get_enrolled_courses_ids( $user_id );
			break;
			case 'incomplete':
				$test_empty_courses_ids = true;
				$user_id = get_current_user_id();
				$ids = BrainPress_Data_Student::get_enrolled_courses_ids( $user_id );
				foreach ( $ids as $course_id ) {
					$status = BrainPress_Data_Student::get_course_status( $course_id, $user_id, false );
					if ( 'certified' != $status ) {
			...
			case 'manage':
				$user_id = get_current_user_id();
				$test_empty_courses_ids = true;
				if ( BrainPress_Data_Capabilities::can_manage_courses( $user_id ) ) {
					$local_args = array(
					'post_type' => BrainPress_Data_Course::get_post_type_name(),
					'nopaging' => true,
					'fields' => 'ids',
					...
					$include_ids = BrainPress_Data_Instructor::get_assigned_courses_ids( $user_id );
					if ( empty( $include_ids ) ) {
						$include_ids = BrainPress_Data_Facilitator::get_facilitated_courses( $user_id, array( 'all' ), true, 0, -1 );
					}
				}
			break;
			case 'all':
		...
		if ( ! $atts['dashboard'] ) {
			...
			if ( $student_list ) {
				$my_courses = BrainPress_Data_Student::my_courses( $student, $courses );
				...
				if ( empty( $courses ) ) {
					if ( $atts['dashboard'] ) {
						$content .= sprintf( '<p class="message">%s</p>', esc_html__( 'Du bist für keinen Kurs eingeschrieben.', 'brainpress' ) );
					}
				} else {
					$counter += count( $courses );
					$content .= BrainPress_Template_Course::course_list_table( $courses );
				}
			} else {
				foreach ( $courses as $course ) {
					$course_url = get_edit_post_link( $course->ID );
					$content .= do_shortcode( '[course_list_box course_id="' . $course->ID . '" override_button_text="' . esc_attr__( 'Kurs verwalten', 'brainpress' ) . '" override_button_link="' . esc_url( $course_url ) . '"]' );
					$counter += 1;
				}
			}
		}
		...
		if ( ( $atts['dashboard'] && ! empty( $counter ) ) || ! empty( $atts['show_labels'] ) ) {
			...
			$message = sprintf( $atts['suggested_msg'], esc_url( BrainPress_Core::get_slug( 'courses', true ) ) );
	...
	public static function course_social_links( $atts ) {
		$services = BrainPress_Helper_SocialMedia::get_social_sharing_keys();
		$atts = shortcode_atts(
			array(
				'course_id' => BrainPress_Helper_Utility::the_course( true ),
				'services' => implode( ',', $services ),
				'share_title' => __( 'Teilen', 'brainpress' ),
				'echo' => false,
			),
			$atts,
			'course_page'
		...
		foreach ( $services as $service ) {
			$is_on = cp_is_true( BrainPress_Core::get_setting( 'general/social_sharing/'.$service, 1 ) );
			if ( ! $is_on ) {
				continue;
			}
			switch ( $service ) {
				...
				case 'linkedin':
					$service_title = '<span class="fa fa-linkedin-square"></span>';
					$services_content .= '<a href="https://www.linkedin.com/sharing/share-offsite/?url=' . $course_url . '" class="linkedin-share" target="_blank"><span class="service-title">' . $service_title . '</span></a>';
					$services_content .= ' ';
					break;
				...
				default:
					$services_content .= apply_filters( 'brainpress_social_link_' . $service, '', $course_id );
					$services_content .= ' ';
					break;
			}
		}
	...
	public static function course_discussion( $atts ) {
		$course_id = BrainPress_Data_Course::get_current_course_id();

		$allow_discussion = BrainPress_Data_Course::get_setting( $course_id, 'allow_discussion', false );

		if ( ! cp_is_true( $allow_discussion ) ) { return false; }

		$comments_args = array(
			// Change the title of send button.
			'label_submit' => __( 'Senden', 'brainpress' ),
			// Change the title of the reply section.
			'title_reply' => __( 'Schreibe eine Antwort oder einen Kommentar', 'brainpress' ),
		...
		$wp_list_comments_args = array(
			...
			'end-callback' => null,
			'type' => 'all',
			'reply_text' => __( 'Antworten', 'brainpress' ),
			'page' => '',
			'per_page' => '',
			'avatar_size' => 32,
	...
	public static function units_dropdown( $atts ) {
		...
		if ( $include_general ) {
			if ( ! $general_title ) {
				$general_title = __( '-- Allgemein --', 'brainpress' );
			}
	...
	public static function course_units( $atts ) {
		...
		$units = BrainPress_Data_Course::get_units( $course_id, 'publish' );
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			...
			if ( get_current_user_id() != $course->details->post_author ) {
				...
				if ( ! current_user_can( 'brainpress_view_all_units_cap' ) ) {
					 ...
					 */
					if ( ! BrainPress_Data_Student::is_enrolled_in_course( $user_id, $course_id ) ) {
		...
		foreach ( $units as $unit ) {
			$unit_url = BrainPress_Data_Unit::get_url( $unit->ID );
			$content .= sprintf(
				'<li><a href="%s">%s</a></li>',
				esc_url( $unit_url ),
				esc_html( $unit->post_title )
		...
		if ( ! count( $units ) ) {
			$content = __( '0 Kurseinheiten noch vorbereitet. Bitte versuche es später erneut.', 'brainpress' );
		}
	...
	public static function course_breadcrumbs( $atts ) {
		...
		$course_url = BrainPress_Data_Course::get_course_url( $course_id ); //get_permalink( $course_id );
		$course_home = BrainPress_Core::get_slug( 'course', true );
		$units_slug = BrainPress_Core::get_slug( 'unit/' );
		...
		switch ( $type ) {
			...
			case 'unit_single':
				$units_breadcrumbs = sprintf(
					...
					esc_html__( 'Einheiten', 'brainpress' )
				);
				break;
		}
